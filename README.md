...
Its memorable visual is a restrained **three-stroke golden signature framing the display name**. Two offset overlines occupy `y=27–35` and `y=34–41`; they use `2.25` and `1.5` pt strokes and remain above the rendered capital glyphs. A stronger `4` pt underline occupies `y=88–97`, below the 31 pt display name and above the role line at `y=100`. The curves therefore surround the typography without crossing any glyph. Their widths scale from an estimated displayed-name width, clamped to `180–435` px: the lead uses 45%, the staggered upper tail 36%, and the underline 86%. Name text remains at `zIndex: 4`, while artwork remains at `zIndex: 3`. The `curves` coordinates stay in the 0–1 element box, making the React SVG preview and ReportLab `curveTo` export share the same geometry.
...
Zapamiętywalnym motywem jest oszczędna **trzyczęściowa złota sygnatura kadrująca imię i nazwisko**. Dwie przesunięte linie nad tekstem zajmują zakresy `y=27–35` i `y=34–41`; mają obrysy `2.25` i `1.5` pt oraz pozostają nad renderowanymi wersalikami. Mocniejsze podkreślenie `4` pt zajmuje `y=88–97`, pod 31-punktowym nazwiskiem i nad stanowiskiem na `y=100`. Krzywe otaczają więc typografię bez przecinania żadnego glifu. Ich szerokości wynikają z szacowanej szerokości wyświetlanego nazwiska ograniczonej do `180–435` px: początkowa linia używa 45%, przesunięta górna końcówka 36%, a podkreślenie 86%. Tekst nazwiska pozostaje na `zIndex: 4`, natomiast grafika na `zIndex: 3`. Współrzędne `curves` pozostają w zakresie 0–1 ramki elementu, więc SVG Reacta i eksport ReportLab `curveTo` dzielą jedną geometrię.
...
Deterministyczny generator Python używa standardowych mechanizmów `Builder`, `need_section`, `keep_together` / `flowGroup` oraz helperów doświadczenia, edukacji, umiejętności i sekcji dodatkowych. Wszystkie trzy elementy sygnatury zależne od szerokości nazwiska mają `flowRole: "masthead"`, a belki i obliczane linie nagłówków `flowRole: "section-chrome"`, więc zmiany odstępów i paginacja przesuwają ornament razem z należącą do niego treścią. Belki stopki oraz asymetryczne szaro-złote szyny są `fixedToPage` i powtarzają się na kolejnych stronach.
