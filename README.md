...
│   │   │   ├── canvas/RecordBlockAdd/    # Record hover/pin adapter: add/reorder entry, toggle description, recoverable deletion
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, component `SectionsPanel` and functions `handleAppearancePalette` / `handleAppearanceTextSize` — strict Atrium/Sterling/Linden/Monument/Slate/Meridian/Cadenza/Vellum gate, template-specific accessible radio groups, and S–XL layout transaction; `SectionsPanel.module.css`, Atrium classes `palettePaperAtrium`, `paletteAtriumPortrait`, `paletteAtriumMastheadRule`, `paletteAtriumSectionRule`, and `paletteAtriumFolio` plus the established template miniatures; tests: `SectionsPanel.test.js`
...
- `scripts/generate_iconic_icons.py`, `_save_png`, `_ATRIUM_GLYPHS`, and palette glyph inventories in `SUBSET_THEMES`; `backend/template_assets/iconic/{atrium-*,sterling-*,linden-*,monument-*,slate-*-accent,meridian-*,cadenza-*,vellum-*}` — matching real icon themes for every appearance-enabled template; asset-colour tests: `backend/tests/test_{atrium,sterling,linden,monument,slate,meridian,cadenza,vellum}_appearance_assets.py`
...
- `frontend/src/components/canvas/RecordBlockAdd/RecordBlockAdd.jsx`, lines 37–167, component `RecordBlockAdd` — record-specific **Wpis**, reorder, state-dependent **Dodaj opis** / recoverable **Usuń opis**, recoverable record deletion, and spread-side gutter actions on the shared toolbar
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, lines 215–222, 331–345, and 386–400, `recordBlockAnchorsById` — one affordance and full-record highlight per record, with `descriptionAction` forwarded to both text render branches
...
The landing page leads with the job seeker's desired outcome rather than the editor mechanism: **“Pokaż, dlaczego warto zaprosić Cię na rozmowę.”** The supporting copy explains that CV Studio turns experience into a professional CV for a specific application, helps sharpen wording without inventing facts, checks general ATS readability, and removes manual formatting work. The structured A4 engine remains the proof behind that promise: content stays independent from the template while deterministic reflow handles rhythm, spacing, record integrity, and pagination. Section order is: header → hero → before/after → three-step content flow → document engine → templates → direct A4 editor → exact-preview/PDF explanation → task-focused AI → privacy strip → pricing → FAQ → final CTA → footer.

The copy does not promise employment or a guaranteed interview: it promises a stronger presentation of the candidate's real experience. ATS guidance still includes the explicit caveat that systems may produce different results, palette/type-scale editing is labelled as available only in selected templates, and Pro remains one payment for 30 days without automatic renewal. The hero's **“PDF bez znaku wodnego w planie Free”** message records the approved upcoming Free-plan policy; the entitlement and renderer implementation are intentionally unchanged in this landing-only change and still need a separate update before the behavior matches the message. The visual hierarchy remains restrained and editorial: sharp rules, warm paper, near-black typography, one muted earth accent, compact mono labels, and real product mockups instead of generic SaaS illustrations.

Every product visual is a **real template mockup** from `frontend/public/template-mockups/`. The hero replaces the former static Linden/Monument overlap and count badge with a calm 15-second proof-sheet cycle through Linden, Sterling, and Vellum. Each A4 mockup stays large while the old-CV transformation annotation remains fixed and readable. The loop animates only `transform` and `opacity`, has a 44 px **Pauza / Wznów** control, and becomes a static three-document fan under `prefers-reduced-motion`. The templates section remains an endless right-to-left marquee of **every** registry mockup (hover/focus pauses the strip and scales the card; reduced motion falls back to a static wrap), the editor section shows Meridian, and the final CTA overlaps three mockups (Regent, Cadenza, Slate) on a near-black panel.
...
**Consistent CTA hierarchy.** The hero leads with the recruitment outcome above. Its primary action remains **“Stwórz CV za darmo”** (→ wizard), the shorter secondary action is **“Wgraj swoje CV”** (→ import), and **“Najpierw zobacz przykładowe CV”** opens the demo. The benefit rail states **“Zacznij za darmo”**, **“AI pomaga dopasować CV do oferty”**, and **“PDF bez znaku wodnego w planie Free”**. The fixed product annotation over the looping three-template proof sheet explains the old-to-new transformation: imported content moves into a premium template and the intelligent layout places it automatically. The heading uses three independent line boxes instead of one wrapping highlighted phrase, and the hero collapses to one column at 1024 px, preventing highlight collision on compact laptops and at 200% zoom. Each call-to-action still fires the same per-source event through `queueGuestEvent`: `hero_wizard`, `hero_import`, `hero_demo`, `before_after_import`, `templates_wizard`, `pricing_free`, `pricing_pro`, `final_wizard`, `final_import`.
...
- `frontend/src/pages/Hero/Hero.jsx`, lines 140–705, function `buildStartUrl`, component `CtaLink`, and component `Hero` — preserves directed starts and event attribution while rendering the outcome-led hero, the controllable Linden/Sterling/Vellum proof sheet, exact A4/PDF explanation, bounded AI tasks, privacy/pricing caveats, real before/after comparison, and registry-driven template marquee
- `frontend/src/pages/Hero/Hero.module.css`, lines 222–2276, selectors `.hero`, `.heroDocumentDeck`, `.heroDocument`, `.heroMotionToggle`, `.documentEngineSection`, `.templatesSection`, `.editorSection`, `.wysiwygSection`, `.aiSection`, `.trustStrip`, `.pricingSection`, `.faqSection`, and `.finalCta` — responsive Swiss/editorial composition, non-overlapping block headline, transform/opacity-only infinite hero cycle, static reduced-motion fan, real-mockup layouts, warm-paper surfaces, sharp rules, restrained earth accents, and mobile collapse for the added sections
- `frontend/src/pages/Hero/Hero.test.js`, lines 1–77, suite `landing product positioning` — guards the outcome positioning, non-overlap and 1024 px responsive contracts, Free/AI benefit copy, three-template hero sequence, retired stack/count selectors, pause and reduced-motion contracts, honest capability caveats, dynamic template count, anchors, directed-start builders, and all existing CTA analytics events
...
- `frontend/src/pages/Hero/Hero.module.css`, lines 1–2276; `frontend/src/pages/Login/Login.module.css`, lines 1–476; `frontend/src/pages/Register/Register.module.css`, lines 1–511 — spacious narrative and authentication surfaces on the shared tokens
...
The portrait area is a 112×126 px rectangle with an offset "shadow" frame, two accent corner registration squares, a solid accent base bar, a light tint fill, and a generated `portrait.png` glyph tagged `photoSlot: "frame"` / `"glyph"`. Gallery upload in template mode fits the user raster inside the drafting outline — see [Profile photo slot](#profile-photo-slot-template-mode). Only that decorative photo cluster plus the page rails/footer are `fixedToPage`/`locked`; fitted sidebar sections remain selectable and editable. With the photo visible, every contact stays in the wrapping `contact-main` masthead band under the name/role pill. Hiding the slot moves those same elements — never copies them — into the existing stacked sidebar band at x=33/y=42 and adds three first-page-only chrome elements above them: a palette-matched 12×12 pt `contact.png` at x=27/y=20, the tracked Montserrat label **DANE KONTAKTOWE** at x=49/y=21, and a 46×1 pt accent rule at x=49/y=34. The first contact retains its authored hidden-state position 7 pt below the rule, and the first ordinary sidebar section remains exactly 40 pt below the lowest contact. Showing the photo removes that alternate heading and restores the original wrapping descriptor, contact coordinates, sidebar positions, and the same placeholder or uploaded raster. Education, skills, languages, and supported extra sections are packed as complete blocks in the left rail under the photo; anything that does not fit before the footer falls through to the main flow instead of being clipped. Summary, experience, fallback education/skills, and custom sections use `Builder`, `need_section`, and record `flowGroup` tags. Continuation pages retain the slate rail, accent hairline divider, footer tab, and page number without duplicating personal sidebar data or the photo-less contact heading.
...
Slate keeps white section glyphs on the shared `slate` theme because every palette places them inside a filled accent badge. Contacts, the portrait placeholder, and the photo-less contact heading use the accent themes `slate-accent`, `slate-monochrome-accent`, `slate-copper-accent`, `slate-forest-accent`, `slate-plum-accent`, and `slate-teal-accent`. Each theme owns the same eight required real transparent 128×128 PNGs: six channel glyphs, `portrait.png`, and `contact.png` — 48 common required palette assets across the six themes. The default steel accent folder also retains ten additional section and legacy glyphs, so the six physical folders contain 58 PNGs in total. An uploaded raster is never recoloured; palette changes update its nested placeholder, both the active and parked contact descriptors, and any visible photo-less heading. The browser preview and ReportLab PDF therefore use the same ink without a CSS filter, even across hide → palette/type change → show. Main section badges use `flowRole: "section-chrome"`; ordinary records use `content`.
...
- `scripts/generate_iconic_icons.py`, function `draw_contact`, constants `_SLATE_PALETTE_GLYPHS` and `_SLATE_ACCENT_GLYPHS`, and the six Slate accent themes
...
- `frontend/src/utils/profilePhotoVisibility.test.js` — three-element header contract, unchanged hidden contact geometry, full hide→show round trip, idempotent legacy hydration, and non-repeating locked chrome
...
- `frontend/src/templates/linden.js`, lines 1–1156, export `lindenTemplate`; `frontend/src/templates/index.js`, lines 20–30 — generated starter with `appearanceTemplateId: "linden"`, the main-flow anchor, uniform sidebar hairlines, the original sand job band, and no redundant upper body rule, plus the paid picker entry.
...
- All nine generator call sites: `atrium.py`, function `_gen_atrium`; `cadenza.py`, lines 198–268; `linden.py`, lines 245–284; `meridian.py`, lines 324–384; `monument.py`, lines 110–141; `regent.py`, lines 80–140; `slate.py`, lines 255–275 and 445–464; `sterling.py`, lines 172–235; `vellum.py`, function `_gen_vellum`.
...
- `backend/app/services/cv_source_layout.py`, lines 164–787, functions `_heading_kind`, `_page_lines`, `_assign_lanes`, `extract_pdf_source_pages`, `source_sections_prompt`, `_prose`, `_middle_dot_items`, `_looks_like_plain_skill_group_label`, `_skill_group_label_indices`, `_nested_skill_groups`, `_nested_language_skill_group`, and `ground_cv_data_from_source` — exact heading recognition, span/font geometry, separate column lanes, complete wrapped summaries, styled or bullet-inferred skill groups, visually nested languages, heading-title rejection, and visual-gap grouping of references
...
Tests: `backend/tests/test_cloudflare_cv_extraction.py`, lines 175–709, class `CloudflareCvExtractionTests`, covers thinking-disabled native-text Gemma, the explicit thinking opt-in, scan-page Qwen, independent text/JSON/vision budgets, zero SDK retries, Cloudflare `3036` daily-limit handling, `3040` capacity fallback, fenced/typed content, one-shot Gemma-to-Llama recovery with aggregated response-presence telemetry, OpenAI rollback JSON Mode, final invalid model JSON, and missing credentials without a network call. Its two-column regression at lines 501–558 deliberately returns a wrong summary and unsupported skill from the mocked model, then verifies source-grounded summary, specialisations, both references, prompt isolation, safe telemetry, and final Atrium rendering. The styled-groups regression at lines 561–616 verifies the complete wrapped summary, hyphen continuation, sentence fragments beginning with `education` and `skills` remaining body text, two bold groups with middle-dot items, rejection of a copied WORK EXPERIENCE title, and final Atrium content. The same-weight Monument regression at lines 619–673 reproduces Gemma's flat result and verifies four restored categories, wrapped bullet continuity, same-style `Języki obce` nesting, removal of model duplicates, and final Monument rendering. `backend/tests/test_extract_cv_rejection.py` covers monthly quota and failure-not-consuming behaviour. `frontend/src/utils/cvImportRequest.test.js`, lines 1–21, locks the four-minute no-retry contract and all import-history status labels. `frontend/src/utils/aiCvPanelScroll.test.js`, lines 8–35, protects the fixed-controls/scrolling-list split, stable scrollbar gutter, accessible region, and short-viewport upload overflow. `backend/tests/test_cv_data.py` covers nested skills normalization.
...
- `frontend/src/utils/sectionRecord.js`, lines 621–718 and 1216–1283, functions `buildRecordClone` and private `buildRecordDescription` — semantic field roles for generic records, including period/location overlays and optional descriptions
- `frontend/src/utils/syncCvDataFromCanvas.js`, lines 120–382 and 558–611, private `profileRecordFromCanvas` / `syncEditorStructures` and function `syncCvDataFromCanvas` — immutable structural upsert, canvas-order insertion, exact semantic deletion, and conservative text synchronization
...
- `frontend/src/utils/syncCvDataFromCanvas.test.js`, lines 48–265 and 267–462 — manual/AI edit guards plus added Experience, overlay Education, insertion-order, custom-section, template-replacement, and duplicate-placeholder deletion regressions
...
│   │   │   ├── canvas/RecordBlockAdd/    # Adapter hover/pin wpisu: dodawanie/kolejność, przełączanie opisu i odwracalne usunięcie
...
- `frontend/src/utils/atriumTypographyLayout.js`, funkcje `applyAtriumTextSizeLayout` i `applyAtriumRenderedHeightsLayout`; `frontend/src/utils/sterlingTypographyLayout.js`, funkcje `applySterlingTextSizeLayout` i `applySterlingRenderedHeightsLayout`; `frontend/src/utils/monumentTypographyLayout.js`, funkcje `applyMonumentTextSizeLayout` i `applyMonumentRenderedHeightsLayout`; `frontend/src/utils/slateTypographyLayout.js`, linie 23–41 i 54–82, funkcje `applySlateTextSizeLayout` i `applySlateRenderedHeightsLayout` — konserwatywne transakcje presetów uwzględniające glify, jeden zbiorczy pomiar textarea po renderze i końcowy pack właściwy dla liczby kolumn; regresje Atrium i Slate: `atriumTypographyLayout.test.js` oraz `slateTypographyLayout.test.js`
- `scripts/generate_iconic_icons.py`, `_save_png`, `_ATRIUM_GLYPHS` oraz zestawy glifów palet w `SUBSET_THEMES`; `backend/template_assets/iconic/{atrium-*,sterling-*,linden-*,monument-*,slate-*-accent,meridian-*,cadenza-*,vellum-*}` — prawdziwe dopasowane motywy ikon dla każdego szablonu z Wyglądem; testy kolorów: `backend/tests/test_{atrium,sterling,linden,monument,slate,meridian,cadenza,vellum}_appearance_assets.py`
...
- `frontend/src/hooks/useA4Elements.js`, linie 1552–1643, funkcja `handleEditElementValues`; `backend/app/services/pdf_generator.py`, linie 613–679 (`renderText`) i 1446–1484 (`render_elements`) — normalizuje legacy nagłówek wypełnionego pasa przy każdej zmianie treści i stosuje ten sam kontrakt ramki/fontu/trackingu w PDF
...
- `frontend/src/components/canvas/RecordBlockAdd/RecordBlockAdd.jsx`, linie 37–167, komponent `RecordBlockAdd` — akcje **Wpis**, kolejności, zależnego od stanu **Dodaj opis** / odwracalnego **Usuń opis**, odwracalnego usunięcia rekordu i wyboru guttera spreadu na wspólnym pasku
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, linie 215–222, 331–345 i 386–400, `recordBlockAnchorsById` — jeden affordance i obrys całego wpisu na rekord, z przekazaniem `descriptionAction` do obu gałęzi renderujących tekst
...
Landing prowadzi od oczekiwanego rezultatu osoby szukającej pracy, a nie od mechaniki edytora: **„Pokaż, dlaczego warto zaprosić Cię na rozmowę.”** Opis wyjaśnia, że CV Studio zamienia doświadczenie w profesjonalne CV na konkretną rekrutację, pomaga wyostrzyć treść bez wymyślania faktów, sprawdza ogólną czytelność dla ATS i usuwa ręczną walkę z formatowaniem. Strukturalny silnik A4 pozostaje dowodem tej obietnicy: treść jest niezależna od szablonu, a deterministyczny reflow pilnuje rytmu, odstępów, integralności wpisów i podziału na strony. Kolejność sekcji: header → hero → przed/po → trzy kroki pracy z treścią → silnik dokumentu → szablony → bezpośredni edytor A4 → wyjaśnienie zgodności podglądu z PDF → AI do konkretnych zadań → prywatność → cennik → FAQ → końcowe CTA → stopka.

Copy nie obiecuje zatrudnienia ani gwarantowanej rozmowy: obiecuje mocniejszą prezentację prawdziwego doświadczenia kandydata. Opis ATS nadal zawiera jasne zastrzeżenie, że poszczególne systemy mogą zwrócić różne wyniki, zmiana palety/skali typografii jest oznaczona jako dostępna tylko w wybranych szablonach, a Pro pozostaje jednorazową płatnością za 30 dni bez automatycznego odnowienia. Komunikat hero **„PDF bez znaku wodnego w planie Free”** zapisuje zatwierdzoną, nadchodzącą politykę planu Free; implementacja uprawnień i renderera celowo nie zmienia się w tej zmianie ograniczonej do landingu i nadal wymaga osobnego wdrożenia, zanim zachowanie zrówna się z komunikatem. Hierarchia wizualna pozostaje spokojna i redakcyjna: ostre linie, ciepły papier, niemal czarna typografia, jeden przygaszony ziemisty akcent, zwarte etykiety mono i prawdziwe mockupy produktu zamiast ogólnych ilustracji SaaS.

Każdy wizual produktu to **realny mockup szablonu** z `frontend/public/template-mockups/`. Hero zastępuje wcześniejsze statyczne nałożenie Lindena i Monumenta oraz badge licznika spokojnym, 15-sekundowym cyklem próby drukarskiej przez Linden, Sterling i Vellum. Każdy mockup A4 pozostaje duży, a adnotacja o transformacji starego CV jest nieruchoma i czytelna. Pętla animuje wyłącznie `transform` i `opacity`, ma kontrolkę **Pauza / Wznów** o wysokości 44 px, a przy `prefers-reduced-motion` zmienia się w statyczną talię trzech dokumentów. Sekcja szablonów pozostaje nieskończonym marquee w prawo→lewo ze **wszystkimi** mockupami z rejestru (hover/focus pauzuje taśmę i powiększa kartę; ograniczenie ruchu przełącza na statyczną siatkę), sekcja edytora pokazuje Meridian, a końcowe CTA nakłada trzy mockupy (Regent, Cadenza, Slate) na niemal czarnym panelu.
...
**Spójna hierarchia CTA.** Hero prowadzi powyższym rezultatem rekrutacyjnym. Główne działanie nadal brzmi **„Stwórz CV za darmo”** (→ kreator), krótsze drugorzędne **„Wgraj swoje CV”** prowadzi do importu, a **„Najpierw zobacz przykładowe CV”** otwiera demo. Pasek korzyści komunikuje **„Zacznij za darmo”**, **„AI pomaga dopasować CV do oferty”** i **„PDF bez znaku wodnego w planie Free”**. Nieruchoma adnotacja na zapętlonej prezentacji trzech mockupów wyjaśnia transformację starego dokumentu: zaimportowana treść trafia do szablonu premium, a inteligentny układ rozmieszcza ją automatycznie. Nagłówek korzysta z trzech niezależnych bloków wiersza zamiast jednej zawijanej frazy na czarnym tle, a hero przechodzi do jednej kolumny przy 1024 px, dzięki czemu podświetlenia nie nachodzą na siebie na kompaktowych laptopach ani przy powiększeniu 200%. Każde CTA nadal wysyła przez `queueGuestEvent` te same zdarzenia źródłowe: `hero_wizard`, `hero_import`, `hero_demo`, `before_after_import`, `templates_wizard`, `pricing_free`, `pricing_pro`, `final_wizard`, `final_import`.
...
- `frontend/src/pages/Hero/Hero.jsx`, linie 140–705, funkcja `buildStartUrl`, komponent `CtaLink` i komponent `Hero` — zachowuje skierowane starty i atrybucję zdarzeń, renderując hero skupione na rezultacie, sterowaną próbę Linden/Sterling/Vellum, wyjaśnienie zgodności A4/PDF, ograniczone zadania AI, zastrzeżenia prywatności/cennika, prawdziwe porównanie przed/po i marquee z rejestru szablonów
- `frontend/src/pages/Hero/Hero.module.css`, linie 222–2276, selektory `.hero`, `.heroDocumentDeck`, `.heroDocument`, `.heroMotionToggle`, `.documentEngineSection`, `.templatesSection`, `.editorSection`, `.wysiwygSection`, `.aiSection`, `.trustStrip`, `.pricingSection`, `.faqSection` i `.finalCta` — responsywna kompozycja Swiss/editorial, nienachodzący blokowy nagłówek, nieskończony cykl hero oparty wyłącznie na `transform`/`opacity`, statyczna talia reduced-motion, układy z prawdziwych mockupów, powierzchnie ciepłego papieru, ostre linie, oszczędne ziemiste akcenty i mobilne składanie dodanych sekcji
- `frontend/src/pages/Hero/Hero.test.js`, linie 1–77, suite `landing product positioning` — chroni pozycjonowanie rezultatu, kontrakty braku nakładania i breakpointu 1024 px, komunikaty Free/AI, sekwencję trzech szablonów hero, usunięte selektory starego stosu/licznika, kontrakty pauzy i reduced-motion, uczciwe zastrzeżenia funkcji, dynamiczną liczbę szablonów, anchory, buildery skierowanych startów i wszystkie istniejące zdarzenia analityczne CTA
...
- `frontend/src/pages/Hero/Hero.module.css`, linie 1–2276; `frontend/src/pages/Login/Login.module.css`, linie 1–476; `frontend/src/pages/Register/Register.module.css`, linie 1–511 — przestronne powierzchnie narracyjne/auth na wspólnych tokenach
...
Slate zachowuje białe glify sekcji we wspólnym motywie `slate`, ponieważ każda paleta umieszcza je wewnątrz wypełnionego badge'a w akcencie. Kontakty, placeholder portretu i nagłówek wariantu bez zdjęcia korzystają z motywów `slate-accent`, `slate-monochrome-accent`, `slate-copper-accent`, `slate-forest-accent`, `slate-plum-accent` i `slate-teal-accent`. Każdy motyw ma ten sam zestaw ośmiu wymaganych prawdziwych przezroczystych PNG 128×128: sześć glifów kanałów, `portrait.png` i `contact.png` — łącznie 48 wspólnych wymaganych assetów paletowych w sześciu motywach. Domyślny folder stalowego akcentu zachowuje dodatkowo dziesięć glifów sekcji i starszych dokumentów, dlatego sześć fizycznych folderów zawiera razem 58 PNG. Wgrany raster nigdy nie jest przebarwiany; zmiana palety aktualizuje jego zagnieżdżony placeholder, aktywny i odłożony deskryptor kontaktów oraz widoczny nagłówek wariantu bez zdjęcia. Podgląd w przeglądarce i PDF ReportLab używają dzięki temu identycznego tuszu bez filtra CSS także w sekwencji hide → zmiana palety/tekstu → show. Badge sekcji głównej mają `flowRole: "section-chrome"`; zwykłe rekordy mają rolę `content`.
...
- `frontend/src/utils/profilePhotoVisibility.js`, funkcje `normalizeProfilePhotoVisibilityPersistence`, `hideProfilePhoto` i `showProfilePhoto` — materializacja/usunięcie trzyczęściowego nagłówka kontaktów, zachowanie stosu x=33/y=42, dokładne odtworzenie geometrii i aktualizacja już ukrytych starszych dokumentów Slate
...
- `scripts/generate_iconic_icons.py`, funkcja `draw_contact`, stałe `_SLATE_PALETTE_GLYPHS` i `_SLATE_ACCENT_GLYPHS` oraz sześć motywów akcentowych Slate
...
- `frontend/src/utils/profilePhotoVisibility.test.js` — kontrakt trzech elementów nagłówka, niezmieniona geometria ukrytych kontaktów, pełny round trip hide→show, idempotentna hydratacja starszego zapisu i niepowtarzane zablokowane chrome
...
- `frontend/src/templates/linden.js`, linie 1–1156, eksport `lindenTemplate`; `frontend/src/templates/index.js`, linie 20–30 — wygenerowany starter z `appearanceTemplateId: "linden"`, anchorem głównego flow, jednolitymi hairlines sidebara, oryginalnym piaskowym pasem stanowiska i bez zbędnej górnej linii treści oraz płatny wpis w pickerze.
...
- **Wygląd ukrytego stanu.** Wszystkie osiem szablonów z zakładką Wygląd stosuje zmianę palety oraz typografii S/M/L/XL także do latentnej specyfikacji stanowiska i blueprintów dekoracji, nie tylko do żywych elementów. Pole dodane po zmianie wyglądu używa więc bieżącej palety i rozmiaru fontu, a nie starych wartości generatora.
...
- Miejsca wywołań wszystkich dziewięciu generatorów: `atrium.py`, funkcja `_gen_atrium`; `cadenza.py`, linie 198–268; `linden.py`, linie 245–284; `meridian.py`, linie 324–384; `monument.py`, linie 110–141; `regent.py`, linie 80–140; `slate.py`, linie 255–275 i 445–464; `sterling.py`, linie 172–235; `vellum.py`, funkcja `_gen_vellum`.
...
- `backend/app/services/cv_source_layout.py`, linie 164–787, funkcje `_heading_kind`, `_page_lines`, `_assign_lanes`, `extract_pdf_source_pages`, `source_sections_prompt`, `_prose`, `_middle_dot_items`, `_looks_like_plain_skill_group_label`, `_skill_group_label_indices`, `_nested_skill_groups`, `_nested_language_skill_group` i `ground_cv_data_from_source` — dokładne rozpoznawanie nagłówków, geometria spanów/fontów, osobne kolumny, pełne zawijane podsumowania, stylowane lub wywnioskowane z punktów grupy skills, wizualnie zagnieżdżone języki, odrzucanie nagłówka jako stanowiska i grupowanie referencji według odstępów
...
Testy: `backend/tests/test_cloudflare_cv_extraction.py`, linie 175–709, klasa `CloudflareCvExtractionTests`, obejmuje Gemmę z wyłączonym thinkingiem, jawny opt-in, Qwen dla skanów, niezależne budżety tekst/JSON/vision, brak retry SDK, dzienny limit `3036`, fallback po capacity `3040`, fenced/typed content, odzyskiwanie Gemma→Llama z telemetrią obecności odpowiedzi, JSON Mode rollbacku OpenAI, końcowy błędny JSON i brak danych logowania bez sieci. Regresja dwukolumnowa w liniach 501–558 celowo zwraca z mocka błędne podsumowanie i nieobecną umiejętność, a następnie sprawdza ugruntowane źródłem podsumowanie, specjalizacje, obie referencje, izolację promptu, bezpieczną telemetrię i końcowy render Atrium. Regresja stylowanych grup w liniach 561–616 sprawdza pełne zawijane podsumowanie, łączniki, fragmenty zdań zaczynające się od `education` i `skills`, dwie pogrubione grupy z elementami rozdzielanymi kropką środkową, odrzucenie tytułu WORK EXPERIENCE i render Atrium. Regresja Monument o jednakowej wadze fontu w liniach 619–673 odtwarza płaski wynik Gemmy i sprawdza cztery odzyskane kategorie, ciągłość zawiniętych punktów, zagnieżdżenie `Języki obce` o zgodnym stylu, usunięcie duplikatów modelu i końcowy render Monument. `backend/tests/test_extract_cv_rejection.py` sprawdza miesięczny limit oraz brak naliczenia po błędzie. `frontend/src/utils/cvImportRequest.test.js`, linie 1–21, utrwala kontrakt czterech minut bez retry i wszystkie etykiety historii importów. `frontend/src/utils/aiCvPanelScroll.test.js`, linie 8–35, chroni podział na nieruchome kontrolki i przewijaną listę, stabilny gutter scrollbara, dostępny region oraz overflow panelu wysyłania w niskim oknie. `backend/tests/test_cv_data.py` obejmuje normalizację zagnieżdżonych skills.
...
- `frontend/src/utils/sectionRecord.js`, linie 621–718 i 1216–1283, funkcja `buildRecordClone` i prywatna `buildRecordDescription` — role semantyczne pól wpisów generycznych, w tym overlayów okresu/lokalizacji oraz opcjonalnych opisów
- `frontend/src/utils/syncCvDataFromCanvas.js`, linie 120–382 i 558–611, prywatne `profileRecordFromCanvas` / `syncEditorStructures` i funkcja `syncCvDataFromCanvas` — niemutujący upsert struktur, wstawianie w kolejności płótna, precyzyjne usunięcia semantyczne i konserwatywna synchronizacja tekstu
...
- `frontend/src/utils/syncCvDataFromCanvas.test.js`, linie 48–265 i 267–462 — zabezpieczenia zmian ręcznych/AI oraz regresje dodanego Doświadczenia, Edukacji z overlayami, kolejności wstawienia, sekcji własnej, pełnej podmiany szablonu i usunięcia przy zduplikowanych placeholderach
...
- `backend/tests/test_cloudflare_cv_extraction.py`, linie 175–709, klasa `CloudflareCvExtractionTests` — Gemma z wyłączonym thinkingiem i opt-in reasoningu, routing Qwen vision, niezależne budżety tekst/JSON/vision, wyłączone retry SDK, fallback Llama po pustym/błędnym wyniku lub Cloudflare `3040`, nieponawiany `3036`, fenced/typed JSON, JSON Mode rollbacku, server-only credentials, dokładne nagłówki źródłowe, stylowane i spłaszczone taksonomie Monument, ochrona zawiniętych punktów i końcowe rendery szablonów
