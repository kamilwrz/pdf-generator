...
The **Dostosuj CV** flyout is a layout-and-appearance panel rather than a technical spacing console. It uses two accessible tabs. **Układ** contains document structure, density, and rhythm controls. For the Sterling template, **Wygląd** contains six curated, restrained palettes shown as miniature CV sheets: **Błękit Północy**, **Grafitowe Atelier**, **Szałwiowa Rezerwa**, **Burgundowy List**, **Bursztynowa Księga**, and **Nocny Fiord**. Selecting a sheet changes every recognised semantic Sterling colour role — paper, rail, body and display text, muted metadata, accents, dividers, and rules — while preserving manually assigned colours. Each palette also switches to a real matching contact-icon asset set rather than using a browser-only CSS filter, so the editor canvas and ReportLab export stay identical. Other templates show an honest Sterling-only availability state until their own curated systems are implemented.
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, lines 97–598, component `SectionsPanel` — **Dostosuj CV** tabs, document status, grouped section structure, density controls, Sterling palette cards, and S–XL typography controls; `frontend/src/components/editor/SectionsPanel/SectionsPanel.module.css`, lines 1–130 — responsive drawer, miniature sheets, segmented type scale, focus states, and mobile overlay; tests: `SectionsPanel.test.js`, lines 1–89
...
- `frontend/src/utils/porticoMastheadReflow.js`, lines 1–20, function `reflowPorticoAfterMastheadChange` — full cross-page Portico section pack after photo/title height changes, followed by trailing-page chrome reconciliation
...
- `frontend/src/hooks/useA4Elements.js`, lines 2318–2386, callbacks `removeContactChannel`, `addContactChannel`, `toggleTitle`, `setProfilePhotoVisible`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto` — history-aware commits, post-layout sidebar alignment, and the Portico-only cross-page masthead reflow
- `frontend/src/components/gallery/GalleryItem/GalleryItem.jsx`, lines 32–50 — template-mode click → `applyProfilePhoto` (no prompt)
- `frontend/src/utils/sectionStructure.js`, lines 675–834, private `healSplitFlowGroupMemberships` and function `sectionElementIds` — restores atomic ownership when one record's `flowGroup` members straddle the next section heading; lines 1026–1043 and 1200–1281, private `resolveSidebarPhotoFloor` and function `packSidebarLane` — ignore hidden photo chrome and reapply the shared contact-derived rail floor during full structural packs
...
- `frontend/src/utils/porticoMastheadReflow.js`, lines 1–20, `reflowPorticoAfterMastheadChange` — Portico-only cross-page repack after the local title/photo transform.
...
Flyout **Dostosuj CV** to panel zarządzania układem i wyglądem, a nie techniczna konsola odstępów. Ma dwie dostępne zakładki. **Układ** zawiera strukturę dokumentu, gęstość i rytm. Dla szablonu Sterling zakładka **Wygląd** udostępnia sześć kuratorowanych, stonowanych palet pokazanych jako miniaturowe kartki CV: **Błękit Północy**, **Grafitowe Atelier**, **Szałwiowa Rezerwa**, **Burgundowy List**, **Bursztynowa Księga** i **Nocny Fiord**. Wybór kartki zmienia wszystkie rozpoznane semantyczne role kolorystyczne Sterlinga — papier, szynę, tekst główny i display, wyciszone metadane, akcenty, podziały i linie — pozostawiając ręcznie nadane kolory. Każda paleta przełącza także prawdziwy, dopasowany zestaw ikon kontaktowych zamiast filtra CSS działającego tylko w przeglądarce, dlatego canvas edytora i eksport ReportLab pozostają identyczne. Dla pozostałych szablonów panel uczciwie informuje, że opcje są na razie dostępne dla Sterlinga.
...
- `frontend/src/utils/sterlingTypographyLayout.js`, linie 25–93, funkcje `applySterlingTextSizeLayout` i `applySterlingRenderedHeightsLayout`; `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, linie 231–292, funkcja `handleSterlingTextSize` — konserwatywny pack presetu uwzględniający glify, a po renderze jeden zbiorczy pomiar wysokości textarea i końcowy pack main/sidebara; regresje: `frontend/src/utils/sterlingTypographyLayout.test.js`, linie 56–155
...
- `frontend/src/utils/porticoMastheadReflow.js`, linie 1–20, funkcja `reflowPorticoAfterMastheadChange` — pełne pakowanie sekcji Portico przez granice stron po zmianie wysokości przez zdjęcie/stanowisko, a następnie uzgodnienie chrome końcowych stron
...
- `frontend/src/hooks/useA4Elements.js`, linie 2318–2386, callbacki `removeContactChannel`, `addContactChannel`, `toggleTitle`, `setProfilePhotoVisible`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto` — operacje historii, wyrównanie sidebara oraz właściwy dla Portico reflow mastheadu przez granice stron
...
- `frontend/src/utils/sectionStructure.js`, linie 675–834, prywatna `healSplitFlowGroupMemberships` i funkcja `sectionElementIds` — przywracanie atomowej własności, gdy elementy `flowGroup` jednego rekordu leżą po obu stronach nagłówka następnej sekcji; linie 1026–1043 i 1200–1281, prywatna `resolveSidebarPhotoFloor` i funkcja `packSidebarLane` — ignorowanie ukrytego chrome zdjęcia i ponowne stosowanie wspólnego progu wyprowadzonego z kontaktów podczas pełnego pakowania strukturalnego
...
- `frontend/src/utils/porticoMastheadReflow.js`, linie 1–20, `reflowPorticoAfterMastheadChange` — właściwe dla Portico pakowanie przez granice stron po lokalnej transformacji tytułu/zdjęcia.
