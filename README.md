...
The opening hero is now a single white field: its 960 px content column is centred across the full section instead of sharing the viewport with a right-hand product animation. The kicker, heading, supporting copy, CTA group, demo link, and benefit rail share the centred axis; the paragraph keeps a narrower 680 px reading measure. The former Linden/Sterling/Vellum cycle, pause control, beige split field, and old-CV annotation have been removed from section 1; the annotation copy is intentionally reserved for a later product decision rather than moved elsewhere. Product visuals later on the page still use **real template mockups** from `frontend/public/template-mockups/`: the templates section keeps the right-to-left marquee of every registry mockup (hover/focus pauses the strip and scales the card; reduced motion falls back to a static wrap), the editor section shows Meridian, and the final CTA overlaps Regent, Cadenza, and Slate on a near-black panel.
...
**Consistent CTA hierarchy.** The hero leads with the recruitment outcome above. Its primary action remains **“Stwórz CV za darmo”** (→ wizard), the shorter secondary action is **“Wgraj swoje CV”** (→ import), and **“Najpierw zobacz przykładowe CV”** opens the demo. The benefit rail states **“Zacznij za darmo”**, **“AI pomaga dopasować CV do oferty”**, and **“PDF bez znaku wodnego w planie Free”**. The heading uses three independent line boxes instead of one wrapping highlighted phrase, while the complete copy block remains centred as one readable column across wide, compact, and zoomed layouts. Each call-to-action still fires the same per-source event through `queueGuestEvent`: `hero_wizard`, `hero_import`, `hero_demo`, `before_after_import`, `templates_wizard`, `pricing_free`, `pricing_pro`, `final_wizard`, `final_import`.
...
- `frontend/src/pages/Hero/Hero.jsx`, lines 132–640, function `buildStartUrl`, component `CtaLink`, and component `Hero` — preserves directed starts and event attribution while rendering the centred outcome-led hero, exact A4/PDF explanation, bounded AI tasks, privacy/pricing caveats, real before/after comparison, and registry-driven template marquee
- `frontend/src/pages/Hero/Hero.module.css`, lines 222–1979, selectors `.hero`, `.heroCopy`, `.heroActions`, `.heroTrust`, `.documentEngineSection`, `.templatesSection`, `.editorSection`, `.wysiwygSection`, `.aiSection`, `.trustStrip`, `.pricingSection`, `.faqSection`, and `.finalCta` — centres the 960 px white hero column and its complete content axis, and provides responsive Swiss/editorial composition, non-overlapping block headlines, real-mockup layouts, warm-paper surfaces, sharp rules, restrained earth accents, and mobile collapse for later sections
- `frontend/src/pages/Hero/Hero.test.js`, lines 1–66, suite `landing product positioning` — guards the outcome positioning, expanded and centred hero content/CTA axis, removal of the animated showcase and deferred copy, Free/AI benefit copy, honest capability caveats, dynamic template count, anchors, directed-start builders, and all existing CTA analytics events
...
- `frontend/src/pages/Hero/Hero.module.css`, lines 1–1979; `frontend/src/pages/Login/Login.module.css`, lines 1–476; `frontend/src/pages/Register/Register.module.css`, lines 1–511 — spacious narrative and authentication surfaces on the shared tokens
...
Otwierające hero jest teraz jednym białym polem: kolumna treści o szerokości 960 px jest wyśrodkowana na całej powierzchni sekcji, zamiast dzielić viewport z prawą animacją produktu. Kicker, heading, opis, grupa CTA, link demo i pasek korzyści korzystają ze wspólnej osi centralnej; akapit zachowuje węższą miarę 680 px dla czytelności. Dawny cykl Linden/Sterling/Vellum, kontrolka pauzy, beżowy podział i adnotacja o starym CV zostały usunięte z sekcji 1; tekst adnotacji świadomie pozostaje do późniejszej decyzji produktowej i nie został przeniesiony w inne miejsce. Wizuale w dalszych sekcjach nadal są **realnymi mockupami szablonów** z `frontend/public/template-mockups/`: sekcja szablonów zachowuje marquee w prawo→lewo ze wszystkimi mockupami z rejestru (hover/focus pauzuje taśmę i powiększa kartę; ograniczenie ruchu przełącza na statyczną siatkę), sekcja edytora pokazuje Meridian, a końcowe CTA nakłada Regent, Cadenza i Slate na niemal czarnym panelu.
...
**Spójna hierarchia CTA.** Hero prowadzi powyższym rezultatem rekrutacyjnym. Główne działanie nadal brzmi **„Stwórz CV za darmo”** (→ kreator), krótsze drugorzędne **„Wgraj swoje CV”** prowadzi do importu, a **„Najpierw zobacz przykładowe CV”** otwiera demo. Pasek korzyści komunikuje **„Zacznij za darmo”**, **„AI pomaga dopasować CV do oferty”** i **„PDF bez znaku wodnego w planie Free”**. Nagłówek korzysta z trzech niezależnych bloków wiersza zamiast jednej zawijanej frazy na czarnym tle, a kompletny blok treści pozostaje wyśrodkowany jako jedna czytelna kolumna na szerokich, kompaktowych i powiększonych układach. Każde CTA nadal wysyła przez `queueGuestEvent` te same zdarzenia źródłowe: `hero_wizard`, `hero_import`, `hero_demo`, `before_after_import`, `templates_wizard`, `pricing_free`, `pricing_pro`, `final_wizard`, `final_import`.
...
- `frontend/src/pages/Hero/Hero.jsx`, linie 132–640, funkcja `buildStartUrl`, komponent `CtaLink` i komponent `Hero` — zachowuje skierowane starty i atrybucję zdarzeń, renderując wyśrodkowane hero skupione na rezultacie, wyjaśnienie zgodności A4/PDF, ograniczone zadania AI, zastrzeżenia prywatności/cennika, prawdziwe porównanie przed/po i marquee z rejestru szablonów
- `frontend/src/pages/Hero/Hero.module.css`, linie 222–1979, selektory `.hero`, `.heroCopy`, `.heroActions`, `.heroTrust`, `.documentEngineSection`, `.templatesSection`, `.editorSection`, `.wysiwygSection`, `.aiSection`, `.trustStrip`, `.pricingSection`, `.faqSection` i `.finalCta` — centruje białą kolumnę hero o szerokości 960 px i kompletną oś jej treści oraz zapewnia responsywną kompozycję Swiss/editorial, nienachodzące blokowe nagłówki, układy z prawdziwych mockupów, powierzchnie ciepłego papieru, ostre linie, oszczędne ziemiste akcenty i mobilne składanie dalszych sekcji
- `frontend/src/pages/Hero/Hero.test.js`, linie 1–66, suite `landing product positioning` — chroni pozycjonowanie rezultatu, rozszerzoną i wyśrodkowaną oś treści/CTA hero, usunięcie animowanego showcase'u i odłożonego tekstu, komunikaty Free/AI, uczciwe zastrzeżenia funkcji, dynamiczną liczbę szablonów, anchory, buildery skierowanych startów i wszystkie istniejące zdarzenia analityczne CTA
...
- `frontend/src/pages/Hero/Hero.module.css`, linie 1–1979; `frontend/src/pages/Login/Login.module.css`, linie 1–476; `frontend/src/pages/Register/Register.module.css`, linie 1–511 — przestronne powierzchnie narracyjne/auth na wspólnych tokenach
