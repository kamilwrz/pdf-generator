...
- **Masthead identity (show/hide job title, name upper/lowercase toggle).** Sterling never calls `tag_masthead_identity` at all (unlike Atrium, Meridian, Nova, Portico, Regent, Slate, Tessera, and Volt), so Vestige inherited that gap with nothing to fix by repositioning. Vestige now tags its own name/title elements and appends a `mastheadIdentity` anchor (`band_id: "vestige-masthead"`) directly, using the main column's first `section-chrome` top as `band_top`. The contact rail is intentionally **not** coupled to this anchor (`contact_band_id=None`): unlike templates whose contact row sits directly under the title, Vestige's rail is a parallel sidebar column pinned near the page top, so it must not shift when the title is hidden/shown.
...
- `backend/tests/test_vestige_template.py` — registry metadata, generated sidebar/main-column ownership, the 13 / 8.4 px heading type scale, the "stacked"-mode contact-band descriptor, the `mastheadIdentity` anchor (with a `null` `contactBandId`), and non-colliding `grid-member` languages cells when languages spill into the main column
...
- `backend/tests/test_vestige_template.py` — metadane rejestru, własność sidebara i głównej kolumny w wygenerowanym układzie, skala typograficzna nagłówków 13 / 8,4 px, deskryptor paska kontaktowego w trybie "stacked", anchor `mastheadIdentity` (z `null` w `contactBandId`) oraz brak kolizji komórek `grid-member` języków, gdy trafiają do kolumny głównej
