...
- `frontend/src/utils/sterlingAppearance.js`, lines 13–276, exports `STERLING_PALETTES`, `getSterlingAppearance`, `applySterlingPalette`, and `applySterlingTextSize` — semantic colour replacement, palette-specific icon paths, reversible role-aware type baselines, and persisted appearance intent; tests: `sterlingAppearance.test.js`, lines 1–51
...
Ten sam panel Sterlinga ma zależne od roli presety tekstu **S / M / L / XL**. **M** oznacza oryginalną skalę szablonu. Imię i nazwisko rośnie najmniej, treść główna najmocniej, a nagłówki, tytuły wpisów, metadane, stanowisko i kontakty korzystają z pośrednich mnożników; body i metadata zachowują twarde minima czytelności. Narzędzie zapisuje niezmienne bazowe wartości font-size i line-height przy każdym elemencie tekstowym, więc sekwencja XL → S → M odtwarza dokładne metryki autora zamiast mnożyć zaokrąglone wyniki. Textarea auto-height mierzą się ponownie, blok kontaktów jest jeszcze raz układany na podstawie przeskalowanego deskryptora, a istniejący mechanizm reflow/paginacji przesuwa dalszą treść lub dodaje strony bez ukrytego ściskania odstępów. Wybrany zamiar `{ palette, textSize }` oraz bazowe metryki są zapisywane w `PdfElements.extra_properties`, odtwarzane przy otwarciu dokumentu i bezpiecznie ignorowane przez ReportLab.
...
- `frontend/src/utils/sterlingAppearance.js`, linie 13–276, eksporty `STERLING_PALETTES`, `getSterlingAppearance`, `applySterlingPalette` i `applySterlingTextSize` — semantyczna zamiana kolorów, ścieżki ikon per paleta, odwracalne bazowe metryki typografii zależne od roli oraz zapis zamiaru wyglądu; testy: `sterlingAppearance.test.js`, linie 1–51
