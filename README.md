...
- **template** — structural editing: content/chrome positions are layout-owned (no free X/Y drag). `canFreePositionElement` also blocks template icons (`alignWithText` / `/template-assets/…`) and generator shapes (line/rectangle/circle/ellipse/polygon/path) even when a template omitted `flowRole` — this covers harbor/regent/portico/axis contact icons, masthead artwork, and generator frames, and similar. User gallery photos (`/images/…`) may still move, except a fitted profile-photo slot (`photoSlot: "image"` / glyph). **Układ CV** flyout (sidebar label + panel title; formerly “Sekcje”) docked beside the 72px tool rail (reorder + density presets + advanced vertical rhythm `stack` / `record` / `section` / `after_rule`, defaults 4 / 10 / 21 / 8), gallery photo-slot targets (`applyProfilePhoto`), auto-height reflow with reclaim. Topbar **Odblokuj edycję** (icon + tooltip) copies the document into freeform.
...
`spacing_px` is persisted on the Pdf row and applied live via `applyFlowSpacing`. Initial fill flows (import / bio wizard) send the live Sections-panel knobs to `POST /ai/fill_template`. **Zmień szablon** regenerates with generator defaults (`DEFAULT_FLOW_SPACING`) and calls `adoptDocumentFlowSpacing` so the previous template’s custom rhythm does not leak into the new layout (`use_spacing` + `get_spacing()` in the Python generators). Icon masthead contact labels (Regent / Portico) are tagged `flowRole: "masthead"` with their icons so a short phone line above the header rule is not mistaken for a section heading when rhythm knobs run; `isSectionHeading` also rejects phone-only labels, labels beside masthead icons, and untagged period lines such as `2011 – 2016`. `resolveFlowStart` keeps authored masthead→section clearances in the 6–56 px window (Regent often sit at 8–18 px) and only substitutes the 36 px default masthead fallback when a prior pack left a huge white band or an overlap. Tight left-aligned iconic mastheads Regent that were previously force-packed to that 36 px band heal back to a tight ~10 px clearance on the next spacing/reorder pack; this heal-back is gated on `hasCenteredMasthead`, so Portico's centered "Ivy League" masthead — which authors a deliberate ~36 px clearance — is exempt and keeps it (otherwise a reorder would yank every section ~26 px up). `sectionElementIds` keeps classic Y-interval membership (so Monument chips above a title stay with that section) and only heals the stacked continuation-page case where Obsługa chrome → Języki chrome → Obsługa body would otherwise leave the earlier section chrome-only.

The **Dostosuj CV** flyout is a layout-and-appearance panel rather than a technical spacing console. **Układ** is always available and contains document structure, density, and rhythm controls. The second accessible tab, **Wygląd**, is currently rendered only when `activeTemplateId === "sterling"`; it is absent for every other template, including sidebar templates that happen to share Sterling-like metadata or structure. This explicit product gate prevents incomplete palette wiring from exposing controls that can resize text but cannot recolour the selected template. The shared appearance infrastructure remains in the codebase so each additional template can opt in later after receiving its own reviewed semantic colour and typography contract. For Sterling, **Wygląd** contains six curated, restrained palettes shown as miniature CV sheets: **Błękit Północy**, **Grafitowe Atelier**, **Szałwiowa Rezerwa**, **Burgundowy List**, **Bursztynowa Księga**, and **Nocny Fiord**. Selecting a sheet changes every recognised semantic Sterling colour role — paper, rail, body and display text, muted metadata, accents, dividers, and rules — while preserving manually assigned colours. Each palette also switches to a real matching contact-icon asset set rather than using a browser-only CSS filter, so the editor canvas and ReportLab export stay identical.

The same Sterling appearance panel exposes role-aware **S / M / L / XL** text presets. **M** is the original template scale. Display names grow least, body copy grows most, and headings, record titles, metadata, job position, and contacts use intermediate factors; body and metadata retain hard readability floors. The utility stores immutable baseline font size and line height per text element, so switching XL → S → M restores the exact authored metrics instead of multiplying rounded values. A preset starts as one layout transaction: a reusable browser-canvas measurer reads the active font's real glyph widths (including weight, italic style, and letter spacing), the wrap estimator respects word boundaries and the bullet marker's separate grid column, and every flow textarea receives a conservative height before `applyFlowSpacing` packs the main and sidebar lanes. After the new typography has painted and `document.fonts.ready` has settled, `handleSterlingTextSize` waits two animation frames, reads the intrinsic `scrollHeight` of every mounted non-masthead textarea, and passes the complete height map to `applySterlingRenderedHeightsLayout`. That helper commits all browser heights first and performs one final document-wide pack; it never applies the measurements as competing per-field reflows. Consequently a long final Experience description moves the following Education or Languages section instead of painting underneath it after any S/M/L/XL change. A request token discards a stale settle when the user selects another size quickly. Textareas on unmounted continuation pages keep the conservative pre-paint estimate until rendered. The contact band is laid out from its scaled descriptor and page chrome is reconciled after each pack, so genuine overflow creates a continuation while returning to M can collapse it again. Tests without a DOM keep the deterministic character-count fallback. The selected `{ palette, textSize }` intent and typography baselines are persisted through `PdfElements.extra_properties`, restored when a saved document opens, and ignored safely by ReportLab.

The document card groups page-count status (`formatPageCountLabel`) with the separate **Zmieść na …** page-reduction goal. Structure is split into counted **Kolumna główna** / **Jedna kolumna** and **Sidebar** groups; each group has compact title-cased rows, ↑↓ reorder, a restrained grip cue, and its own contextual **+ Dodaj sekcję** action. **Gęstość** offers **Kompaktowa / Standardowa / Przestronna** relative to `baselineFlowSpacing`; **Zoptymalizuj układ** runs offline spacing trials (see below); and the collapsed **Precyzyjne odstępy** area uses minus/value/plus steppers for the four px knobs (Wewnątrz wpisu / Między wpisami / Między sekcjami / Pod nagłówkiem) plus **Przywróć ustawienia szablonu**. The drawer is 380 px on desktop, 360 px on laptop, 340 px at narrower widths, and becomes a fixed overlay below 720 px. Reset restores knobs captured when the CV was rendered or loaded (`baselineFlowSpacing` in `useA4Elements`, set via `pinFlowSpacingBaseline` / `adoptDocumentFlowSpacing`). If the live knobs already match that baseline, reset does **not** call `applyFlowSpacing`: a force-pack to exact `SPACE_*` is not identical to generator geometry (ReportLab cursor advance, masthead clearance, under-rule gaps) and was pulling later sections onto page 1 on every shared-packer template (Monument, Tessera/Slate when packed, …). Changing a knob away from baseline and then resetting still retargets the canvas to the baseline rhythm.
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, lines 96–595, component `SectionsPanel` — **Dostosuj CV** tabs, the strict `activeTemplateId === "sterling"` Appearance gate, document status, grouped section structure, density controls, Sterling palette cards, and S–XL typography controls; `frontend/src/components/editor/SectionsPanel/SectionsPanel.module.css`, lines 1–130 — responsive drawer, miniature sheets, segmented type scale, focus states, and mobile overlay; tests: `SectionsPanel.test.js`, lines 1–93
- `frontend/src/utils/sterlingAppearance.js`, lines 1–310, exports `STERLING_PALETTES`, `getSterlingAppearance`, `applySterlingPalette`, and `applySterlingTextSize` — semantic colour replacement, palette-specific icon paths, reversible role-aware type baselines, glyph-aware flow-textarea height seeding, and persisted appearance intent; tests: `sterlingAppearance.test.js`, lines 1–53
...
- `frontend/src/utils/sterlingTypographyLayout.js`, lines 25–93, functions `applySterlingTextSizeLayout` and `applySterlingRenderedHeightsLayout`; `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, lines 236–299, function `handleSterlingTextSize` — Sterling-gated, conservative glyph-aware preset packing followed by one post-paint batch of browser-measured textarea heights and final main/sidebar repack; regressions: `frontend/src/utils/sterlingTypographyLayout.test.js`, lines 56–155
- `backend/app/schemas/pdf_schema.py`, lines 186–190, model `PdfElement`; `shared/pdf-element.schema.json`, lines 865–924; `backend/app/crud/pdfs.py`, lines 116–120, 211–215, 364–368, and 434–438; `frontend/src/components/modals/ModalPdfs/ModalPdfs.jsx`, lines 106–111 — validate, pack, update, unpack, and hydrate Sterling appearance metadata through `extra_properties`; persistence tests: `test_contact_channel_roundtrip.py` and `test_pdf_element_updates.py`
- `scripts/generate_iconic_icons.py`, lines 323–350, `SUBSET_THEMES`; `backend/template_assets/iconic/sterling-*` — six matching contact-icon themes; asset-colour test: `backend/tests/test_sterling_appearance_assets.py`, lines 1–35
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, lines 67–105, `LANE_TRANSFER_TEMPLATE_IDS` and `sectionAnchorsById` — mounts the affordance on every template-mode **main and sidebar** heading with lane-local `canMoveUp` / `canMoveDown` (and `laneTransfer` for compatible sidebar templates) from the **full** `A4_Elements` document (not the per-page filtered list)
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, `sectionAnchorsById` (lines 67–130) — passes lane-local `canMoveUp` / `canMoveDown` and `laneTransfer` on Sterling/Tessera/Slate/Vestige/Cadenza/Linden from full-document main + sidebar order (each page mounts its own `CanvasElements` with a page filter; flags must not use that filtered list or cross-page moves stay disabled)
...
- `frontend/src/utils/sectionStructure.js`, lines 1026–1043 and 1200–1281, private `resolveSidebarPhotoFloor` and function `packSidebarLane` — visible-photo and hidden-contact rail floors used by every spacing, fit, transfer, and reorder pack; also function `deriveSectionStyle` (lines 2814–3067), `sectionChromeRuleRelTop` (lines 3101–3105), `healSimpleChromeRuleGaps` (lines 285–359), and private `pickLinearBodySample` (lines 2731–2752)
- `frontend/src/utils/languagesLayout.js`, functions `isLanguagesSectionTitle` (lines 26–28), `buildLanguagesMainGrid` (lines 130–, defaults to 3 columns below `NARROW_MAIN_COLUMN_MAX_WIDTH`, else `LANGUAGES_GRID_COLUMNS = 4`), `restyleLanguagesMembersAsSidebar` (lines 269–)
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, `LANE_TRANSFER_TEMPLATE_IDS` (lines 67–74) + `sectionAnchorsById`
...
- `frontend/src/utils/languagesLayout.test.js` — grid cells + CEFR runs; sidebar collapse; (regression) `buildLanguagesMainGrid` defaults to 3 columns below `NARROW_MAIN_COLUMN_MAX_WIDTH` (a 4th of 4 entries wraps onto a new row) and to 4 at a single-column-template width (all 4 fit one row)
...
- `frontend/src/utils/sectionStructure.js`, function `healSkillChipLabelBaselines` (lines 220–256); called from `applyFlowSpacing` (lines 2648–2675)
...
Every product visual is a **real template mockup** from `frontend/public/template-mockups/`: the hero shows two overlapping A4 mockups (Linden over Monument) with a live template count, the templates section is an endless right-to-left marquee of **every** registry mockup (hover/focus pauses the strip and scales the card; `prefers-reduced-motion` falls back to a static wrap), the editor section shows Meridian, and the final CTA overlaps three mockups (Regent, Vestige, Slate) on a near-black panel.
...
In the normal editor, Topbar entry points are **Importuj CV**, **Utwórz CV krok po kroku**, and **Szablony** (enabled after a successful fill). The templates control is absolutely positioned over the live A4 left edge; the grid button opens the change-template modal, and the arrows cycle allowed templates in place. The `?start=demo` product-demo mode uses a separate, reduced chrome: it keeps undo/redo, **Układ CV**, zoom, and page navigation, while hiding template selection, project naming, import, save, download, clear, and account actions. A guest **Stwórz moje CV** flow opens four data steps and then authentication; an authenticated user opening the wizard gets all five steps, including template selection.
...
**Masthead photo slot.** Vestige also emits an empty-state profile-photo well (`_gen_vestige`, near the end, before the final `return`) — the same three-element pattern Regent uses: a filled `#F4F4F2` rectangle (`photoSlot: "ornament"`, id `vestige-photo-well`), an unfilled `#D7D7D4`-stroke outline (`photoSlot: "frame"`, `photoShape: "rect"`, id `vestige-photo-frame`), and a centred graphite portrait glyph (`photoSlot: "glyph"`, id `vestige-photo-glyph`, drawn from `backend/template_assets/iconic/vestige/portrait.png`). The slot sits at `left=505, top=25, width=60, height=74.4` — flush against the page's right margin above the masthead rule, with the 60×74.4 box matching Regent's own portrait aspect ratio (100×124). The client recognises the frame/glyph generically off their `photoSlot` values (`frontend/src/utils/profilePhoto.js`'s `isProfilePhotoFrame`/`isPortraitGlyph`), not a hardcoded template id, so clicking either one opens the gallery and the chosen photo is inset with `objectFit: "cover"`; `vestige-photo-frame` was added alongside Regent's and Portico's ids to that module's zero-inset ("fills the well edge-to-edge") branches so the applied photo covers the well exactly like theirs.
...
- `backend/app/services/cv_templates/templates/vestige.py`, lines 61–549, function `_gen_vestige` — semantic geometry transformation over the shared deterministic sidebar planner; emits the static `DANE KONTAKTOWE` heading/rule outside the managed contact band; normalises every sidebar hairline to `_RULE_HEIGHT = 1.0`; rebuilds the contact rail (`_place_stacked_icon_contacts`) and masthead identity (`tag_masthead_identity`, `band_top` pinned to the title's own `top` so `blockPt` is `0`) directly from `cv`; excludes and proportionally re-translates `grid-member` cells; aligns the sidebar's first section with the main column's first section by computing their top delta instead of a fixed offset; recomputes both columns' textarea `height` at a uniform 12 px line height and carries the resulting per-page cumulative shift into every later element's `top` in BOTH columns; appends the empty-state photo-slot well/frame/glyph triplet
- `frontend/src/templates/vestige.js`, exported `vestigeTemplate` — editable A4 starter regenerated from the backend output
- `frontend/src/utils/transferSectionLane.js`, lines 221–268, private `restyleLanguagesMembersAsMain` — rebuilds Languages chrome and its grid for the main column, including the destination lane's rule colour
...
- `frontend/src/templates/vestige.test.js`, lines 10–57 — rail width, main-column masthead, static contact heading, contact icon placement, and uniform 1 pt sidebar/main rule geometry; the following test covers the photo-slot triplet's geometry/tags
- `frontend/src/utils/transferSectionLane.test.js`, lines 376–414 — Languages expands into a main-column grid and adopts the destination rule colour
...
The coral role bar is linked to the editable job-position text: its width is recalculated from the rendered title plus its horizontal padding, constrained to the masthead column. Hiding the job position hides that linked bar as well; restoring the position restores both elements. The name begins at y=60, while the role bar and its text begin at y=92/y=98. This 32 pt name-to-bar rhythm lowers the compact identity cluster toward the contacts and document body without crowding either element.
...
- `frontend/src/templates/index.js`, line 26 — paid `tessera` registry entry
- `backend/app/services/cv_templates/registry.py`, `_GENERATORS["tessera"]` and `TEMPLATE_LAYOUTS["tessera"]`
- `scripts/generate_iconic_icons.py`, lines 245–266 and 291–306 — `draw_portrait` and curated `tessera` icon theme
...
- `backend/tests/test_contact_band_templates.py`, lines 75–105, test `test_centered_masthead_reserves_two_contact_rows_without_moving_body`
...
- `backend/app/services/cv_templates/templates/portico.py`, function `_gen_portico` — centered masthead with photo slot + `_place_centered_icon_contacts`; body reuses the left-aligned `section()`/`Builder` pattern as `regent.py`
...
- `frontend/src/templates/portico.js` — static starter spec generated from the backend generator's own output coordinates, including the Montserrat/Inter split and 12 px content line height
...
- `frontend/src/templates/sterling.js` — static starter emitted directly from the generator's own demo output (icon `src` values are stored relative and get `API_BASE_URL` prepended at load time, same as Regent/Portico); exported array `sterlingTemplate`
...
Cadenza is a paid `["sidebar", "icons"]` template. It uses an original, restrained Swiss editorial composition: warm `#F5F1E8` paper, a white information rail, black reading text, taupe `#B38B6D` rules, and muted `#808080` metadata. Lora establishes the display hierarchy while Inter keeps editable copy compact and legible. Its contact glyphs come from the dedicated taupe `backend/template_assets/iconic/cadenza/` theme, including the contact-band descriptor used when a channel is added later. Its fixed page furniture deliberately uses every supported geometric primitive—rectangle, circle, ellipse, and rules—only as non-editable decoration, so the sidebar and record lanes remain safe for reflow, density changes, pagination, and PDF export. Cadenza uses the same explicit sidebar metadata as the transferable sidebar family, so the editor exposes the move control for eligible sections and `transferSectionLane` restyles them when moving between the rail and main column.
...
- `frontend/src/templates/linden.js`, lines 1–1151, export `lindenTemplate`; `frontend/src/templates/index.js`, lines 20–35 — generated starter with the main-flow anchor and no redundant upper body rule, plus the paid picker entry.
- `frontend/src/utils/sectionStructure.js`, lines 914–993 and 2112–2162, functions `resolveFlowStart` and `packDocumentSections` — honours a generator-authored `mainFlowStart` before applying generic masthead-gap recovery, so reorder cannot pull Linden's first main section into the identity area.
...
- `backend/tests/test_linden_template.py`, lines 1–110 — registry tags, editorial/photo geometry, explicit main-flow start, absence of the redundant rule, full-name uppercase fit, semantic lanes, record groups, dedicated icons, and the contact/photo descriptor.
...
- `frontend/src/templates/linden.test.js`, lines 1–133 — starter geometry, main-section reorder anchor, removed-rule regression, contact band, title decoration toggle, fixed-width band behavior after title blur, stationary body/contact behavior, and a real sidebar → main → sidebar transfer through `transferSectionLane`.
- `frontend/src/utils/mastheadIdentityOps.test.js`, lines 78–97 — verifies that fixed Linden bands retain their authored width while explicitly content-sized Slate/Tessera-style bars still follow title text.
...
- `backend/app/services/cv_templates/templates/{regent,portico}.py` — per-template `_gen_*` entry points
...
`frontend/public/template-mockups/{monument,tessera,slate,portico,atrium,sterling,regent,vestige,meridian,cadenza,linden}.png` — the previews shown in the Hero template gallery (`frontend/src/pages/Hero/Hero.jsx`), the in-app template picker (`frontend/src/components/modals/TemplatesModal/TemplatesModal.jsx`), and the hover pane in **Wypełnij z mojego CV** (`frontend/src/components/ai/AiCvPanel/AiCvPanel.jsx`) — are rendered from those starter arrays, not hand-drawn mockups. After starter changes, regenerate the PNGs:
...
Portico's 100 pt reclaim applies only to document-flow elements. Page paper, the bottom decorative rule, and the page number are `fixedToPage` chrome and retain their authored coordinates while the photo is hidden; they receive no `photoLayoutHome` snapshot because showing the photo must not move them either. The photo transition retains restoration markers for persistence, but restoring Portico now reverses the photo's 100 pt delta from each element's **current** coordinate instead of replaying stale absolute tops. The current contact-band and masthead-identity descriptors are shifted by the same inverse delta, preserving any independent 22 pt job-title shift and the title's current `present` flag. Every managed `section-chrome`, `content`, and `grid-member` is shifted uniformly, including members that moved from a continuation page and therefore had no page-one home snapshot. This keeps reading order intact before the final pack instead of placing a restored Experience record behind Education. Consequently hiding/showing the job title and photo is commutative: all four hide/show orders return the title, masthead, and first body section to the same geometry. After either Portico photo or title toggle, `reflowPorticoAfterMastheadChange` runs the shared full-document section packer with the document's current spacing and the complete pre-toggle document as a semantic membership reference. Element IDs are stable across those masthead-only operations, so `packDocumentSections` reads section ownership from the trustworthy snapshot while placing the current transformed geometry. A degree that transiently lands at the end of Experience, or a Skills body that momentarily crosses above its heading, therefore remains owned by Education or Skills instead of being permanently attached to the previous section. The normal `sectionElementIds` split-`flowGroup` repair remains available for legacy geometry, but the Portico transition no longer asks already-shifted coordinates to rediscover ownership. Sections already placed on page 2 are reconsidered against the active masthead height, and `reconcileDocumentPages` removes continuation chrome when the page becomes empty. Restoration metadata is persisted so the same invariant survives save/reload.
...
- `frontend/src/utils/profilePhotoVisibility.js`, lines 32–451, private `isPorticoPackedFlowElement` and functions `hiddenProfileContactSectionFloor`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto`, `profilePhotoControlAnchor`, `alignSidebarAfterProfileContacts` — shared 40 pt contact floor, reversible visibility, live-delta Portico photo/title restoration that preserves section order in every toggle order, fixed footer chrome, complete Slate/Tessera cluster hiding, actual contact-stack measurement and contact/sidebar transfer, plus legacy document fallbacks
- `frontend/src/utils/porticoMastheadReflow.js`, lines 1–27, function `reflowPorticoAfterMastheadChange` — full cross-page Portico section pack after photo/title height changes, using the pre-toggle document as the membership reference before trailing-page chrome reconciliation
...
- `frontend/src/hooks/useA4Elements.js`, lines 2318–2385, callbacks `removeContactChannel`, `addContactChannel`, `toggleTitle`, `setProfilePhotoVisible`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto` — history-aware commits, post-layout sidebar alignment, and the Portico-only cross-page masthead reflow
- `frontend/src/components/gallery/GalleryItem/GalleryItem.jsx`, lines 32–50 — template-mode click → `applyProfilePhoto` (no prompt)
- `frontend/src/utils/sectionStructure.js`, lines 675–834, private `healSplitFlowGroupMemberships` and function `sectionElementIds` — restores atomic ownership when one record's `flowGroup` members straddle the next section heading; lines 2112–2174 and 2660–2694, functions `packDocumentSections` and `applyFlowSpacing` — optionally source section membership from a stable pre-transition snapshot while placing current geometry; lines 1026–1043 and 1200–1281, private `resolveSidebarPhotoFloor` and function `packSidebarLane` — ignore hidden photo chrome and reapply the shared contact-derived rail floor during full structural packs
...
- Generators: `regent`, `slate`, `tessera` (+ text templates via `_contact_line`)
...
- **All templates.** The manager now works on Atrium, Portico, Tessera and Slate (centered / wrapping masthead), plus **Regent** (a `stacked` layout mode, one channel per row). Each template passes a `band_id` to its contact placer and appends the band anchor after its masthead `flowRole` pass so the anchor keeps its own `masthead-anchor` role.
...
- Loading: PDF spinner minimum display time; `useToasts` / `ToastStack` renders smaller Swiss-style confirmation cards in the upper-right corner so notifications stay outside the A4 workspace. The neutral off-white card uses graphite type, fine grey-green borders, and only a restrained sage success accent (`#667A6C` / `#EEF3EF`); template badges use the same sage system instead of template-specific colours. Cards use enter/exit motion and keep a safe 12 px mobile inset. Every toast receives a replacement category from its variant and title, so a newer notification of the same kind immediately replaces the older one and clears its timer; explicit workflow keys such as `template-change` remain supported. Unrelated categories may coexist. Download actions carry a per-toast blob href, not a shared slot.
  - Implementation: `frontend/src/hooks/useToasts.js`, lines 7–79, exports `toastReplaceKey`, `mergeToastQueue`, and `useToasts`; `frontend/src/hooks/useApplyCvTemplate.js`, lines 29–88, hook `useApplyCvTemplate`; `frontend/src/components/common/ToastStack/ToastStack.jsx`, lines 7–69, component `ToastStack`; `frontend/src/components/common/ToastStack/ToastStack.module.css`, lines 1–135.
...
`spacing_px` jest zapisywane na dokumencie i od razu pakuje canvas (`applyFlowSpacing`). Pierwsze wypełnienie (import / kreator bio) wysyła bieżące knoby z panelu Układ CV do `POST /ai/fill_template`. **Zmień szablon** regeneruje z domyślnym rytmem generatora (`DEFAULT_FLOW_SPACING`) i woła `adoptDocumentFlowSpacing`, żeby niestandardowe odstępy poprzedniego szablonu nie przenosiły się na nowy layout (`use_spacing` + `get_spacing()` w generatorach Python). Etykiety kontaktów w mastheadzie ikonowym (Regent / Portico) mają `flowRole: "masthead"` razem z ikonami, żeby krótki numer telefonu nad linią nagłówka nie był brany za tytuł sekcji przy zmianie odstępów; `isSectionHeading` odrzuca też etykiety typu telefon, tekst obok ikony masthead oraz nieotagowane linie okresu w stylu `2011 – 2016`. `resolveFlowStart` zachowuje autorski odstęp masthead→sekcja w oknie 6–56 px (Regent często mają 8–18 px) i dopiero przy ogromnej białej dziurze albo nachodzeniu podstawia zapasowy odstęp 36 px. Ciasny, wyrównany do lewej masthead ikoniczny Regent, który wcześniejszy pack wcisnął do tych 36 px, wraca do ciasnego ~10 px przy kolejnej zmianie odstępów / kolejności sekcji; ten heal-back jest bramkowany przez `hasCenteredMasthead`, więc wycentrowany masthead „Ivy League” szablonu Portico — który celowo autoryzuje ~36 px prześwitu — jest z niego wyłączony i zachowuje go (inaczej zmiana kolejności podciągałaby wszystkie sekcje o ~26 px w górę). `sectionElementIds` zostaje przy klasycznym przedziale Y (żeby chipy Monument nad tytułem zostawały w swojej sekcji) i tylko leczy sklejoną kontynuację, gdzie chrome Obsługi → chrome Języków → treść Obsługi zostawiał wcześniejszą sekcję bez body.

Flyout **Dostosuj CV** to panel zarządzania układem i wyglądem, a nie techniczna konsola odstępów. **Układ** jest dostępny zawsze i zawiera strukturę dokumentu, gęstość oraz rytm. Druga dostępna zakładka, **Wygląd**, jest obecnie renderowana wyłącznie wtedy, gdy `activeTemplateId === "sterling"`; nie pojawia się dla żadnego innego szablonu, także sidebarowego, który przypadkiem współdzieli metadane lub strukturę podobną do Sterlinga. Ta jawna bramka produktowa nie pokazuje niedokończonych kontrolek, które potrafią zmienić skalę tekstu, ale nie kolorystykę wybranego szablonu. Wspólna infrastruktura wyglądu pozostaje w kodzie, aby kolejne szablony można było później włączać pojedynczo po przygotowaniu i weryfikacji ich semantycznego kontraktu kolorów i typografii. Dla Sterlinga zakładka **Wygląd** udostępnia sześć kuratorowanych, stonowanych palet pokazanych jako miniaturowe kartki CV: **Błękit Północy**, **Grafitowe Atelier**, **Szałwiowa Rezerwa**, **Burgundowy List**, **Bursztynowa Księga** i **Nocny Fiord**. Wybór kartki zmienia wszystkie rozpoznane semantyczne role kolorystyczne Sterlinga — papier, szynę, tekst główny i display, wyciszone metadane, akcenty, podziały i linie — pozostawiając ręcznie nadane kolory. Każda paleta przełącza także prawdziwy, dopasowany zestaw ikon kontaktowych zamiast filtra CSS działającego tylko w przeglądarce, dlatego canvas edytora i eksport ReportLab pozostają identyczne.

Ten sam panel Sterlinga ma zależne od roli presety tekstu **S / M / L / XL**. **M** oznacza oryginalną skalę szablonu. Imię i nazwisko rośnie najmniej, treść główna najmocniej, a nagłówki, tytuły wpisów, metadane, stanowisko i kontakty korzystają z pośrednich mnożników; body i metadata zachowują twarde minima czytelności. Narzędzie zapisuje niezmienne bazowe wartości font-size i line-height przy każdym elemencie tekstowym, więc sekwencja XL → S → M odtwarza dokładne metryki autora zamiast mnożyć zaokrąglone wyniki. Preset zaczyna się jako jedna transakcja layoutu: współdzielony pomiar canvasu przeglądarki odczytuje prawdziwe szerokości glifów aktywnego fontu (łącznie z grubością, kursywą i letter-spacing), estymator zawijania respektuje granice słów oraz osobną kolumnę znacznika punktora, a każde pole textarea przepływu dostaje konserwatywną wysokość przed spakowaniem obu kolumn przez `applyFlowSpacing`. Po narysowaniu nowej typografii i zakończeniu `document.fonts.ready` funkcja `handleSterlingTextSize` czeka dwie klatki animacji, odczytuje wewnętrzny `scrollHeight` każdego zamontowanego pola textarea spoza mastheadu i przekazuje pełną mapę wysokości do `applySterlingRenderedHeightsLayout`. Helper najpierw zatwierdza wszystkie wysokości przeglądarki, a potem wykonuje jeden końcowy pack całego dokumentu; pomiary nie konkurują już jako osobne reflow pól. Dzięki temu długi ostatni opis Doświadczenia przesuwa następną sekcję Wykształcenie lub Języki zamiast rysować się pod nią po dowolnej zmianie S/M/L/XL. Token żądania odrzuca spóźniony settle, gdy użytkownik szybko wybierze kolejny rozmiar. Textarea na niezamontowanych stronach kontynuacji zachowują konserwatywną estymację sprzed renderu do chwili wyświetlenia. Blok kontaktów jest układany z przeskalowanego deskryptora, a chrome stron jest uzgadniane po każdym pakowaniu, więc realny overflow tworzy kontynuację, a powrót do M może ją ponownie zwinąć. Testy bez DOM zachowują deterministyczny fallback liczony ze znaków. Wybrany zamiar `{ palette, textSize }` oraz bazowe metryki są zapisywane w `PdfElements.extra_properties`, odtwarzane przy otwarciu dokumentu i bezpiecznie ignorowane przez ReportLab.

Karta dokumentu łączy status liczby stron (`formatPageCountLabel`) z osobnym celem redukcji **Zmieść na …**. Struktura jest podzielona na policzone grupy **Kolumna główna** / **Jedna kolumna** i **Sidebar**; każda ma zwarte wiersze title-case, strzałki ↑↓, dyskretny uchwyt wizualny i kontekstową akcję **+ Dodaj sekcję**. **Gęstość** oferuje **Kompaktowa / Standardowa / Przestronna** względem `baselineFlowSpacing`; **Zoptymalizuj układ** uruchamia offline próby odstępów (poniżej); zwinięte **Precyzyjne odstępy** używają stepperów minus/wartość/plus dla czterech wartości px (Wewnątrz wpisu / Między wpisami / Między sekcjami / Pod nagłówkiem) oraz akcji **Przywróć ustawienia szablonu**. Panel ma 380 px na desktopie, 360 px na laptopie, 340 px na węższych ekranach i poniżej 720 px staje się stałą nakładką. Reset przywraca wartości zapisane przy renderze / wczytaniu CV (`baselineFlowSpacing` w `useA4Elements`, ustawiane przez `pinFlowSpacingBaseline` / `adoptDocumentFlowSpacing`). Gdy żywe wartości już są równe temu baseline, reset **nie** woła `applyFlowSpacing`: force-pack do dokładnych `SPACE_*` nie jest tożsamy z geometrią generatora (kursor ReportLab, odstęp pod mastheadem, luki pod linią nagłówka) i wciągał późniejsze sekcje na stronę 1 we wszystkich szablonach na wspólnym pakerze (Monument, Tessera/Slate przy pakowaniu, …). Zmiana wartości poza baseline i potem reset nadal przepakowuje canvas do rytmu baseline.
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, linie 96–595, komponent `SectionsPanel` — zakładki **Dostosuj CV**, ścisła bramka Wyglądu `activeTemplateId === "sterling"`, status dokumentu, grupy struktury, kontrolki gęstości, kartki palet Sterlinga oraz typografia S–XL; `frontend/src/components/editor/SectionsPanel/SectionsPanel.module.css`, linie 1–130 — responsywny panel, miniaturowe kartki, segmentowana skala tekstu, stany focus i nakładka mobilna; testy: `SectionsPanel.test.js`, linie 1–93
- `frontend/src/utils/sterlingAppearance.js`, linie 1–310, eksporty `STERLING_PALETTES`, `getSterlingAppearance`, `applySterlingPalette` i `applySterlingTextSize` — semantyczna zamiana kolorów, ścieżki ikon per paleta, odwracalne bazowe metryki typografii zależne od roli, startowe wysokości przepływu mierzone z uwzględnieniem glifów oraz zapis zamiaru wyglądu; testy: `sterlingAppearance.test.js`, linie 1–53
...
- `frontend/src/utils/sterlingTypographyLayout.js`, linie 25–93, funkcje `applySterlingTextSizeLayout` i `applySterlingRenderedHeightsLayout`; `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, linie 236–299, funkcja `handleSterlingTextSize` — dostępny wyłącznie dla Sterlinga konserwatywny pack presetu uwzględniający glify, a po renderze jeden zbiorczy pomiar wysokości textarea i końcowy pack main/sidebara; regresje: `frontend/src/utils/sterlingTypographyLayout.test.js`, linie 56–155
- `backend/app/schemas/pdf_schema.py`, linie 186–190, model `PdfElement`; `shared/pdf-element.schema.json`, linie 865–924; `backend/app/crud/pdfs.py`, linie 116–120, 211–215, 364–368 i 434–438; `frontend/src/components/modals/ModalPdfs/ModalPdfs.jsx`, linie 106–111 — walidacja, pakowanie, aktualizacja, odpakowanie i hydratacja metadanych wyglądu Sterlinga przez `extra_properties`; testy trwałości: `test_contact_channel_roundtrip.py` i `test_pdf_element_updates.py`
- `scripts/generate_iconic_icons.py`, linie 323–350, `SUBSET_THEMES`; `backend/template_assets/iconic/sterling-*` — sześć pasujących motywów ikon kontaktowych; test koloru assetów: `backend/tests/test_sterling_appearance_assets.py`, linie 1–35
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, linie 67–105, `LANE_TRANSFER_TEMPLATE_IDS` oraz `sectionAnchorsById` — montaż affordance przy każdym nagłówku **głównym i sidebarowym** z lokalnymi dla toru `canMoveUp` / `canMoveDown` (oraz `laneTransfer` dla kompatybilnych szablonów sidebarowych) z **pełnego** dokumentu `A4_Elements`
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, `sectionAnchorsById` (linie 67–130) — przekazuje lokalne dla toru `canMoveUp` / `canMoveDown` oraz `laneTransfer` na Sterling/Tessera/Slate/Vestige/Cadenza/Linden z kolejności main + sidebar w całym dokumencie (każda strona montuje własny `CanvasElements` z filtrem strony; flagi nie mogą używać tej przefiltrowanej listy, bo wtedy przesunięcia między stronami zostają wyłączone)
...
- `frontend/src/utils/sectionStructure.js`, linie 1026–1043 i 1200–1281, prywatne `resolveSidebarPhotoFloor` oraz funkcja `packSidebarLane` — progi widocznego zdjęcia i ukrytego stosu kontaktów używane przy każdym pakowaniu odstępów, fit, transferze i reorderze; ponadto funkcja `deriveSectionStyle` (linie 2814–3067), `sectionChromeRuleRelTop` (linie 3101–3105), `healSimpleChromeRuleGaps` (linie 285–359) i prywatne `pickLinearBodySample` (linie 2731–2752)
- `frontend/src/utils/languagesLayout.js`, funkcje `isLanguagesSectionTitle` (linie 26–28), `buildLanguagesMainGrid` (linie 130–, domyślnie 3 kolumny poniżej `NARROW_MAIN_COLUMN_MAX_WIDTH`, inaczej `LANGUAGES_GRID_COLUMNS = 4`), `restyleLanguagesMembersAsSidebar` (linie 269–)
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, `LANE_TRANSFER_TEMPLATE_IDS` (linie 67–74) + `sectionAnchorsById`
...
- `frontend/src/utils/languagesLayout.test.js` — komórki siatki + CEFR; zwijanie do sidebara; (regresja) `buildLanguagesMainGrid` domyślnie używa 3 kolumn poniżej `NARROW_MAIN_COLUMN_MAX_WIDTH` (4. z 4 wpisów zawija się na nowy wiersz) oraz 4 przy szerokości szablonu jednokolumnowego (wszystkie 4 mieszczą się w jednym wierszu)
...
- `frontend/src/utils/sectionStructure.js`, funkcja `healSkillChipLabelBaselines` (linie 220–256); wywoływana z `applyFlowSpacing` (linie 2648–2675)
...
W trybie szablonu dokument przekraczający docelową liczbę stron pokazuje małą plakietkę przy kafelku **Układ CV**, akcję dopasowania do jednej strony w Topbarze tylko wtedy, gdy bezpieczna drabina odstępów potrafi osiągnąć jedną stronę, oraz jeden delikatny toast na dokument; blokujący modal nie otwiera się automatycznie. Celem jest jedna strona dla szablonów z sidebarem i o jedną stronę mniej dla pozostałych układów. Akcja w Topbarze pojawia się reaktywnie po przekroczeniu limitu, ma tooltip **„Zmieść CV na 1 stronę…”** i korzysta z tego samego progresywnego handlera dopasowania co panel. Wyniki emergency lub impossible pozostają w panelu, ponieważ wymagają decyzji użytkownika albo skracania AI. Po otwarciu **Układu CV** uruchamiane jest progowe sprawdzenie dopasowania tylko wtedy, gdy panel jest widoczny; pokazuje ono uczciwą podpowiedź z akcją **Zmieść na …**.
...
W zwykłym edytorze wejścia w topbarze to **Importuj CV**, **Utwórz CV krok po kroku** oraz **Szablony** (aktywne po udanym fillu). Kontrolka szablonów stoi nad żywą lewą krawędzią A4; ikona otwiera modal zmiany szablonu, a strzałki przełączają dozwolone szablony w miejscu. Tryb demo produktu `?start=demo` ma osobny, ograniczony chrome: zachowuje undo/redo, **Układ CV**, zoom i nawigację stron, a ukrywa wybór szablonu, nazwę projektu, import, zapis, pobieranie, czyszczenie i akcje konta. Gość przez **Stwórz moje CV** przechodzi cztery kroki danych i autoryzację; zalogowany użytkownik ma pełne pięć kroków, w tym wybór szablonu.
...
Vestige to płatny, monochromatyczny szablon sidebarowy (`layouts: ["sidebar", "icons"]`) dla osób na stanowiskach executive, consultant, lawyer, analyst i manager. Jasnoszary lewy panel o szerokości 174 pt zawiera niezależne wiersze telefonu, e-maila, LinkedIn, lokalizacji, umiejętności, języków oraz wykształcenia. Statyczny nagłówek `DANE KONTAKTOWE` z krótką grafitową linią opisuje panel kontaktowy nad jego zarządzanymi wierszami; pozostaje widoczny nawet po usunięciu wszystkich opcjonalnych kanałów, ponieważ jest elementem mastheadu szablonu, a nie członkiem `contactChannel`. Wszystkie dekoracyjne hairline'y w sidebarze i kolumnie głównej mają dokładnie 1 pkt wysokości, dzięki czemu pierwsza linia sidebara pod Podsumowaniem zawodowym, ustawiona na ułamkowej współrzędnej Y, nie jest rasteryzowana grubiej od sąsiednich. Pierwszy wiersz kontaktu jest wyrównany do nazwiska Cormorant Garamond 34 px w prawej kolumnie o szerokości 335 pt, dzięki czemu oba panele tworzą jeden skomponowany masthead; dalej prawa kolumna przechodzi w zwartą, rozstrzeloną linię stanowiska w Montserrat, spokojne podsumowanie zawodowe oraz rekordy doświadczenia. Paleta jest celowo ograniczona do bieli, grafitu i neutralnej szarości (`#F4F4F2`, `#1B1B1A`, `#D7D7D4`). Nagłówki sekcji mają własną, jeszcze bardziej stonowaną skalę typograficzną — 13 px w kolumnie głównej, 8,4 px w panelu sidebara — mniejszą niż 14 / 9,4 px Sterlinga, dzięki czemu wąski dwukolumnowy układ czyta się jako powściągliwy edytorialny panel, a nie przebarwiony Sterling.
...
**Slot na zdjęcie w mastheadzie.** Vestige generuje też pusty, gotowy do wypełnienia slot na zdjęcie profilowe (`_gen_vestige`, tuż przed końcowym `return`) — ten sam trzyelementowy wzorzec, którego używa Regent: wypełniony prostokąt `#F4F4F2` (`photoSlot: "ornament"`, id `vestige-photo-well`), niewypełniony kontur `#D7D7D4` (`photoSlot: "frame"`, `photoShape: "rect"`, id `vestige-photo-frame`) oraz wyśrodkowany, grafitowy glif portretu (`photoSlot: "glyph"`, id `vestige-photo-glyph`, rysowany z `backend/template_assets/iconic/vestige/portrait.png`). Slot znajduje się na `left=505, top=25, width=60, height=74,4` — przy prawym marginesie strony, nad linią mastheadu, a proporcje boksu 60×74,4 odpowiadają własnym proporcjom portretu Regent (100×124). Klient rozpoznaje ramkę/glif ogólnie, na podstawie ich wartości `photoSlot` (`frontend/src/utils/profilePhoto.js`, `isProfilePhotoFrame`/`isPortraitGlyph`), a nie zakodowanego na sztywno id szablonu, więc kliknięcie w którykolwiek z nich otwiera galerię, a wybrane zdjęcie jest wstawiane z `objectFit: "cover"`; `vestige-photo-frame` dodano obok id Regent i Portico do gałęzi tego modułu odpowiedzialnych za zerowy margines („zdjęcie wypełnia slot od krawędzi do krawędzi”), więc wstawione zdjęcie pokrywa slot dokładnie tak samo jak w tamtych szablonach.
...
- `frontend/src/templates/vestige.test.js`, linie 10–57 — szerokość panelu, masthead w głównej kolumnie, statyczny nagłówek kontaktów, położenie ikon kontaktu i jednolita geometria linii sidebara/main o wysokości 1 pkt; kolejny test obejmuje geometrię/tagi trójki slotu na zdjęcie
- `frontend/src/utils/transferSectionLane.test.js`, linie 376–414 — Języki rozwijają się do siatki w kolumnie głównej i przyjmują kolor linii pasa docelowego
...
- `backend/tests/test_vestige_template.py`, linie 15–85 i 230–248 — własność sidebara i kolumny głównej, jednolita grubość linii 1 pkt, położenie statycznego nagłówka kontaktów oraz jego obecność bez kanałów; pozostałe testy obejmują metadane rejestru, skalę typograficzną nagłówków 13 / 8,4 px, deskryptor paska kontaktowego w trybie "stacked", anchor `mastheadIdentity` z `blockPt == 0` i `null` w `contactBandId`, brak kolizji komórek `grid-member` języków, gdy trafiają do kolumny głównej, brak nakładania się elementów sidebara, gdy zawężenie zawija długie podsumowanie na więcej linii, niż pierwotnie zakładał Sterling, wyrównanie pierwszej sekcji sidebara z pierwszą sekcją kolumny głównej, jednolitą wysokość linii 12 px w treści kolumny głównej, geometrię i tagi trójki well/frame/glif slotu na zdjęcie, linię mastheadu podążającą za rzeczywistą (przeliczoną) wysokością stosu imię/stanowisko oraz odstępy między rekordami w kolumnie głównej zbiegające się do co najwyżej dwóch rozmiarów (w obrębie rekordu, między rekordami) na liście czterech stanowisk
...
Koralowy pasek stanowiska jest powiązany z edytowalnym tekstem: jego szerokość jest obliczana z szerokości wyrenderowanego tytułu oraz paddingu poziomego i ograniczana do kolumny mastheadu. Ukrycie stanowiska ukrywa również ten pasek, a ponowne dodanie przywraca oba elementy. Imię i nazwisko zaczyna się na y=60, a pasek stanowiska i jego tekst na y=92/y=98. Rytm 32 pt między imieniem a paskiem obniża zwarty blok identyfikacyjny w stronę kontaktów i treści dokumentu bez ściskania elementów.
...
- `frontend/src/templates/index.js`, linia 26 — płatny wpis rejestru `tessera`
- `backend/app/services/cv_templates/registry.py`, `_GENERATORS["tessera"]` i `TEMPLATE_LAYOUTS["tessera"]`
- `scripts/generate_iconic_icons.py`, linie 245–266 i 291–306 — `draw_portrait` i dedykowany motyw ikon `tessera`
...
- `backend/tests/test_contact_band_templates.py`, linie 75–105, test `test_centered_masthead_reserves_two_contact_rows_without_moving_body`
...
- `backend/app/services/cv_templates/templates/portico.py`, funkcja `_gen_portico` — wycentrowany masthead ze slotem zdjęcia + `_place_centered_icon_contacts`; korpus jak w `regent.py`
...
- `frontend/src/templates/sterling.js` — statyczny starter emitowany bezpośrednio z wyjścia demo generatora (wartości `src` ikon przechowywane względnie, baza API dodawana przy ładowaniu, tak jak w Regent/Portico); eksportowana tablica `sterlingTemplate`
...
- `backend/tests/test_cv_template_layouts.py`, `test_sterling_places_education_on_page_two_sidebar_when_page_one_rail_is_full`, linie 1463–1541 — end-to-end: gdy Doświadczenie paginuje, a szyna strony 1 jest już pełna, Wykształcenie renderuje się jako kicker sidebara na stronie 2 (`left == 34`), a nie w kolumnie głównej obok pustej szyny
...
Cadenza to płatny szablon `["sidebar", "icons"]`. Korzysta z autorskiej, spokojnej kompozycji editorialnej w duchu szwajcarskim: ciepły papier `#F5F1E8`, biała szyna informacji, czarny tekst do czytania, taupe `#B38B6D` dla reguł oraz stonowane metadane `#808080`. `Lora` buduje hierarchię ekspozycyjną, a `Inter` zachowuje zwartą i czytelną edytowalną treść. Glify kontaktowe pochodzą z dedykowanego, taupe motywu `backend/template_assets/iconic/cadenza/`, także w deskryptorze paska kontaktów używanym po późniejszym dodaniu kanału. Jego nieruchome wyposażenie strony celowo wykorzystuje wszystkie obsługiwane prymitywy geometryczne — prostokąt, koło, elipsę i linie — wyłącznie jako dekorację, dzięki czemu sidebar i tory rekordów pozostają bezpieczne przy reflow, zmianie gęstości, paginacji oraz eksporcie PDF. Cadenza korzysta z tych samych jawnych metadanych sidebara co rodzina szablonów z transferem, więc edytor pokazuje kontrolkę przenoszenia dla kwalifikujących się sekcji, a `transferSectionLane` zmienia ich styl przy przejściu między szyną a kolumną główną.
...
- `frontend/src/templates/linden.js`, linie 1–1151, eksport `lindenTemplate`; `frontend/src/templates/index.js`, linie 20–35 — wygenerowany starter z anchorem głównego flow i bez zbędnej górnej linii treści oraz płatny wpis w pickerze.
- `frontend/src/utils/sectionStructure.js`, linie 914–993 i 2112–2162, funkcje `resolveFlowStart` i `packDocumentSections` — respektuje wygenerowany `mainFlowStart` przed ogólnym leczeniem odstępu mastheadu, więc reorder nie podciąga pierwszej głównej sekcji pod obszar tożsamości.
...
- `frontend/src/utils/mastheadIdentityOps.js`, linie 37–91, funkcja `resizeContentSizedTitleDecorations`; `frontend/src/hooks/useA4Elements.js`, linie 1485–1532, funkcja `handleEditElementValues` — rozróżnia stałe semantyczne pasy tożsamości od jawnie dynamicznych pasków stanowiska przy zatwierdzaniu edycji tekstu, dzięki czemu pas Linden nie zwija się po blur, a Slate/Tessera nadal zmieniają szerokość.
...
- `backend/tests/test_linden_template.py`, linie 1–110 — tagi rejestru, geometria editorialna/zdjęcia, jawny początek głównego flow, brak zbędnej linii, pełne imię i nazwisko uppercase, tory semantyczne, grupy rekordów, dedykowane ikony i deskryptor kontakt/zdjęcie.
...
- `frontend/src/templates/linden.test.js`, linie 1–133 — geometria startera, anchor reorderu sekcji głównych, regresja usuniętej linii, pasek kontaktów, przełączanie dekoracji stanowiska, stała szerokość pasa po blur stanowiska, nieruchoma treść/kontakty oraz prawdziwy transfer sidebar → main → sidebar przez `transferSectionLane`.
- `frontend/src/utils/mastheadIdentityOps.test.js`, linie 78–97 — sprawdza, że stały pas Linden zachowuje autorską szerokość, a jawnie dynamiczne paski w stylu Slate/Tessera nadal dopasowują się do tekstu stanowiska.
...
- `backend/app/services/cv_templates/templates/{regent,portico}.py` — osobne wejścia `_gen_*`
...
Pliki `frontend/public/template-mockups/{monument,tessera,slate,portico,atrium,sterling,regent,vestige,meridian,cadenza,linden}.png` — podglądy w galerii Hero, pickerze i panelu **Wypełnij z mojego CV** — pochodzą z tych tablic starterów, nie z ręcznych grafik. Po zmianie starterów odtwórz PNG:
...
Odzyskanie 100 pt w Portico obejmuje wyłącznie elementy przepływu dokumentu. Papier strony, dolna linia dekoracyjna i numer strony są chrome `fixedToPage` i zachowują autorskie współrzędne po ukryciu zdjęcia; nie otrzymują migawki `photoLayoutHome`, ponieważ ponowne pokazanie zdjęcia również nie może ich przesunąć. Przejście zdjęcia zachowuje metadane odtwarzania, ale podczas przywracania Portico odwraca deltę zdjęcia 100 pt od **bieżącej** współrzędnej każdego elementu zamiast ponownie nakładać nieaktualne pozycje absolutne. Bieżące deskryptory paska kontaktów i tożsamości mastheada są przesuwane o tę samą odwrotną deltę, dzięki czemu zachowują niezależne przesunięcie stanowiska 22 pt oraz aktualną flagę `present`. Wszystkie zarządzane elementy `section-chrome`, `content` i `grid-member` przesuwają się jednakowo, także te, które przeszły ze strony kontynuacji i nie miały migawki strony 1. Przed końcowym pakowaniem zachowuje to kolejność czytania zamiast przenosić przywrócony rekord Doświadczenia za Wykształcenie. Ukrywanie i pokazywanie stanowiska oraz zdjęcia jest więc przemienne: wszystkie cztery kolejności akcji kończą się identyczną geometrią tytułu, mastheada i pierwszej sekcji treści. Po każdym przełączeniu zdjęcia albo stanowiska `reflowPorticoAfterMastheadChange` uruchamia wspólny packer wszystkich sekcji z bieżącymi odstępami dokumentu i pełnym dokumentem sprzed przełączenia jako semantyczną referencją przynależności. Identyfikatory elementów są stabilne podczas tych operacji dotyczących wyłącznie mastheada, dlatego `packDocumentSections` odczytuje własność sekcji z wiarygodnej migawki, ale rozmieszcza bieżącą, przekształconą geometrię. Dyplom, który przejściowo trafia na koniec Doświadczenia, lub treść Umiejętności, która chwilowo przechodzi nad swój nagłówek, pozostaje więc własnością Wykształcenia albo Umiejętności zamiast zostać trwale przypisana do poprzedniej sekcji. Zwykła naprawa rozdzielonego `flowGroup` w `sectionElementIds` pozostaje dostępna dla starszej geometrii, lecz przejście Portico nie próbuje już odtwarzać własności z już przesuniętych współrzędnych. Sekcje umieszczone wcześniej na stronie 2 są ponownie sprawdzane względem aktywnej wysokości mastheada, a `reconcileDocumentPages` usuwa chrome pustej strony kontynuacji. Metadane odtwarzania są zapisywane, więc zasada działa także po zapisie i ponownym otwarciu dokumentu.
...
- `frontend/src/utils/profilePhotoVisibility.js`, linie 32–451, prywatna `isPorticoPackedFlowElement` oraz funkcje `hiddenProfileContactSectionFloor`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto`, `profilePhotoControlAnchor`, `alignSidebarAfterProfileContacts` — wspólny próg kontaktów 40 pt, odwracalna widoczność, odtwarzanie zdjęcia/stanowiska Portico z bieżącej delty i zachowaniem kolejności sekcji w każdej kolejności akcji, stałe chrome stopki, ukrycie kompletnego klastra, pomiar rzeczywistego stosu kontaktów i transfer kontaktów/sidebara Slate/Tessera oraz fallbacki starszych dokumentów
- `frontend/src/utils/porticoMastheadReflow.js`, linie 1–27, funkcja `reflowPorticoAfterMastheadChange` — pełne pakowanie sekcji Portico przez granice stron po zmianie wysokości przez zdjęcie/stanowisko z dokumentem sprzed przełączenia jako referencją przynależności, a następnie uzgodnienie chrome końcowych stron
...
- `frontend/src/hooks/useA4Elements.js`, linie 2318–2385, callbacki `removeContactChannel`, `addContactChannel`, `toggleTitle`, `setProfilePhotoVisible`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto` — operacje historii, wyrównanie sidebara oraz właściwy dla Portico reflow mastheadu przez granice stron
...
- `frontend/src/utils/sectionStructure.js`, linie 675–834, prywatna `healSplitFlowGroupMemberships` i funkcja `sectionElementIds` — przywracanie atomowej własności, gdy elementy `flowGroup` jednego rekordu leżą po obu stronach nagłówka następnej sekcji; linie 2112–2174 i 2660–2694, funkcje `packDocumentSections` i `applyFlowSpacing` — opcjonalne odczytanie przynależności sekcji ze stabilnej migawki sprzed transformacji przy rozmieszczaniu bieżącej geometrii; linie 1026–1043 i 1200–1281, prywatna `resolveSidebarPhotoFloor` i funkcja `packSidebarLane` — ignorowanie ukrytego chrome zdjęcia i ponowne stosowanie wspólnego progu wyprowadzonego z kontaktów podczas pełnego pakowania strukturalnego
...
- `backend/app/services/cv_templates/shared/text.py` — `_contact_line` / `_contact_line_core`
- Generatory: `regent`, `slate`, `tessera` (+ szablony tekstowe przez `_contact_line`)
...
- **Wszystkie szablony.** Menedżer działa teraz w Atrium, Portico, Tessera i Slate (masthead wycentrowany / zawijany), a także w **Regent** (tryb układu `stacked`, jeden kanał na wiersz). Każdy szablon przekazuje `band_id` do swojego placera kontaktów i dopina anchor paska **po** przejściu ustawiającym `flowRole` masthead, aby anchor zachował własną rolę `masthead-anchor`.
...
- Miejsca wywołań szablonów: `backend/app/services/cv_templates/templates/{atrium,portico,tessera,slate,regent}.py`.
...
Przy bezpośrednim przejściu z jednego elementu tekstowego do drugiego podczas edit-zoomu w widoku dwóch stron nowa edycja jest aktywowana synchronicznie po kliknięciu. Poprzednia edycja nie pozostaje więc w przejściowym stanie, który mógłby przywrócić rozkładówkę przed zaznaczeniem drugiego elementu. Dzięki temu strona i zoom 200% pozostają zachowane do czasu rzeczywistego wyjścia z edycji.
