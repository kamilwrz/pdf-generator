...
- `backend/tests/test_vestige_template.py` — registry metadata, generated sidebar/main-column ownership, the 13 / 8.4 px heading type scale, the "stacked"-mode contact-band descriptor, the `mastheadIdentity` anchor with `blockPt == 0` and a `null` `contactBandId`, non-colliding `grid-member` languages cells when languages spill into the main column, and non-overlapping sidebar elements after narrowing rewraps a long summary onto more lines than Sterling originally planned for
