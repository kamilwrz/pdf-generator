...
│   │       └── canvasHighlightBounds(.test).js # Render-aware semantic highlight bounds and Portico regression
│   ├── package.json
│   └── .env.example
├── shared/
...
In **template mode**, hovering a detected **main or sidebar** section heading reveals one grouped toolbar outside the authored A4 content. A single page follows the section lane (sidebar on the left, main column on the right). In two-page view, the first sheet always uses its outer-left gutter and the second its outer-right gutter; the 18 px centre gap is intentionally never used because the grouped toolbar would render beneath or over the neighbouring page. The spread reserves 220 px on each outside edge inside its horizontal scroll extent, so neither toolbar is clipped on narrower editor windows. The whole section strip on the heading's page receives a subtle pointer-inert outline, so the user sees the scope before acting. That outline is calculated from **painted bounds**, not only stored flow coordinates: single-line `text` headings contribute their live DOM `Range` (important because `Text.module.css` uses `line-height: 1` and glyph ink can extend above the line box), and text-aligned section icons contribute the same optical top shift used by their canvas image. When hover or pin makes the toolbar visible, the heading is measured once more and unioned with the precomputed full-section rectangle; late font layout therefore cannot leave the top border crossing the heading, while textarea headings keep their ordinary box geometry. In both lanes, the next lane-local section start is also a hard lower boundary for the outline. A stale or intentionally generous textarea box may still own the current section, but it cannot visually absorb the next one; this covers main-column Experience → Summary as well as sidebar Skills → Languages without changing document data or layout. The toolbar exposes the labelled **Sekcja** add action and **↑/↓** directly (disabled at lane boundaries); lane transfer, Skills layout, and the destructive action live under **More**. A single click on the heading pins the toolbar across pointer leave, a click elsewhere dismisses it, and a double click on text hides structural chrome and opens text editing. Transient hover remains visible for 1,000 ms after pointer leave, giving the user enough time to reach an outer gutter. Controls retain a compact 36 px on-screen target at every canvas zoom, with 15 px icons, delayed Polish tooltips, neutral Swiss-style chrome, and single-toolbar exclusivity. The first template-editor visit also shows a one-time hint explaining hover, pin, and double-click editing. Adding still inserts immediately **under that section** in the same lane; reorder and deletion still re-pack under the active rhythm.

Implementation:

- `frontend/src/components/canvas/CanvasHoverToolbar/CanvasHoverToolbar.jsx`, lines 35–174, component `CanvasHoverToolbar`, and `CanvasHoverToolbar.module.css`, lines 1–217 — grouped gutter surface, screen-stable controls, delayed tooltips, semantic highlight, and overflow menu; `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, lines 40–178, component `SectionRecordAdd` — section-specific add/reorder/transfer/layout/delete actions, spread-side override, and a visible-state live heading measurement that expands (never shrinks) the section outline
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, lines 88–176 and 310–354, functions `fillSectionAnchors`, `sectionAnchorsById`, and prop `spreadSide` — computes lane-local movement/transfer flags, supplies the next main/sidebar heading as the page-local lower highlight boundary, and forwards the physical spread edge; `frontend/src/utils/canvasHighlightBounds.js`, lines 1–102, functions `unionCanvasBounds`, `elementBoundsOnPage`, and `includeRenderedElementBounds`, together with `frontend/src/utils/elementBounds.js`, lines 125–139, function `getVisualBounds` — unions real text ink, optically shifted icons, and ordinary element boxes, then clips an oversized box at the next semantic section; `frontend/src/pages/PdfCanvas.jsx`, lines 1927–1946, two-page `visiblePages.map` — assigns outer-left / outer-right toolbar gutters while publishing deletion snapshots and structural handlers through `PdfContext`; `frontend/src/App.css`, lines 121–133, `.canvas-spread` — reserves both outside gutters inside the horizontal scroll extent
...
- `frontend/src/utils/canvasHighlightBounds.test.js`, lines 1–142 — exact Portico regression proving the 14 px optically aligned icon moves the semantic border from stored `top: 259` to painted `top: 253`, live-heading union and missing-DOM fallback coverage, plus a main/sidebar-neutral regression where an oversized Skills body is clipped at the following Languages boundary
...
- `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, lines 24–165, props `laneTransfer` / `gutterSide`
...
│   │       └── canvasHighlightBounds(.test).js # Obrys semantyczny z żywego renderu i regresja Portico
│   ├── package.json
│   └── .env.example
├── shared/
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, linie 88–176 i 310–354, funkcja `fillSectionAnchors`, `sectionAnchorsById` i prop `spreadSide` — wylicza lokalne dla toru flagi kolejności/transferu, przekazuje następny nagłówek main/sidebara jako dolną granicę obrysu na stronie i przekazuje fizyczną krawędź spreadu; `frontend/src/utils/canvasHighlightBounds.js`, linie 1–102, funkcje `unionCanvasBounds`, `elementBoundsOnPage` i `includeRenderedElementBounds`, wraz z `frontend/src/utils/elementBounds.js`, linie 125–139, funkcja `getVisualBounds` — łączy rzeczywisty tusz tekstu, optycznie przesunięte ikony i zwykłe pudełka elementów, a potem przycina zbyt wysokie pudełko na następnej sekcji semantycznej; `frontend/src/pages/PdfCanvas.jsx`, linie 1927–1946, mapowanie `visiblePages` widoku dwóch stron — przypisuje zewnętrzny lewy/prawy gutter i publikuje snapshoty usuwania oraz handlery strukturalne przez `PdfContext`; `frontend/src/App.css`, linie 121–133, `.canvas-spread` — rezerwuje oba zewnętrzne guttery w poziomym obszarze przewijania
...
- `frontend/src/utils/canvasHighlightBounds.test.js`, linie 1–142 — dokładna regresja Portico potwierdzająca, że 14-pikselowa ikona wyrównana optycznie przesuwa krawędź semantyczną z zapisanego `top: 259` do narysowanego `top: 253`, testy sumy z żywym nagłówkiem i fallbacku bez DOM oraz neutralna wobec main/sidebara regresja, w której zbyt wysokie body Umiejętności jest przycięte na granicy kolejnych Języków
...
- `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, linie 24–165, propsy `laneTransfer` / `gutterSide`
