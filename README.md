...
│   │       ├── regentAppearance(.test).js # Two classic and two creative semantic Regent editions with real icons
...
- `frontend/src/utils/regentAppearance.js`, exports `REGENT_PALETTES`, `REGENT_TEXT_SIZES`, `getRegentAppearance`, `applyRegentPalette`, and `applyRegentTextSize` — two classic and two creative editions, semantic paper/text/rule/folio recolouring, latent-title and inline-run updates, true-colour icon switching, manual-colour protection, persisted intent, and reversible role-aware typography; tests: `regentAppearance.test.js`
...
- `scripts/generate_iconic_icons.py`, `_save_png`, `_ATRIUM_GLYPHS`, Regent entries in `THEME_VARIANTS`, and palette glyph inventories in `SUBSET_THEMES`; `backend/template_assets/iconic/{atrium-*,regent-*,sterling-*,linden-*,monument-*,slate-*-accent,meridian-*,cadenza-*,vellum-*}` — matching real icon themes for every appearance-enabled template; asset-colour tests: `backend/tests/test_{atrium,regent,sterling,linden,monument,slate,meridian,cadenza,vellum}_appearance_assets.py`
...
The landing page leads with the job seeker's desired outcome rather than the editor mechanism: **“Stwórz CV, które prowadzi do rozmowy.”** The supporting copy invites the user to upload an existing CV or start from scratch, then explains that CV Studio helps tailor the content to an offer and arranges it into a professional document ready to send. The intentionally compact landing flow retains the original numbered sections **01, 05, 09, 10, and 11**: hero → template gallery → privacy strip → pricing → FAQ → footer. Removed section numbers remain visible as intentional gaps in the Swiss poster index. Authentication pages are separate routes and are not affected by this landing composition.

The copy does not promise employment or a guaranteed interview: it promises a stronger presentation of the candidate's real experience. ATS guidance still includes the explicit caveat that systems may produce different results, palette/type-scale editing is labelled as available only in selected templates, and Pro remains one payment for 30 days without automatic renewal. The hero benefit rail uses three concise capability labels—**“ZA DARMO”**, **“CZYSTY PDF”**, and **“INTELIGENTNY LAYOUT”**. The visual hierarchy remains restrained and editorial: sharp rules, warm paper, near-black typography, one muted earth accent, compact mono labels, and real product mockups instead of generic SaaS illustrations.

The opening hero keeps its 960 px content column centred across the full section while `frontend/public/women-job-call.png` fills a non-interactive background layer. A flat paper-coloured overlay and restrained desaturation keep the photograph subordinate to the message; the overlay becomes stronger on compact screens. The kicker, heading, supporting copy, CTA group, demo link, and benefit rail remain on the centred axis, and the paragraph keeps a narrower 680 px reading measure. The template section uses **real template mockups** from `frontend/public/template-mockups/` in a right-to-left marquee of every registry entry; hover or keyboard focus pauses the strip and scales the card, while reduced motion falls back to a static wrap.
...
**Consistent CTA hierarchy.** The hero leads with the recruitment outcome above. Its primary action remains **“Stwórz CV za darmo”** (→ wizard), the shorter secondary action is **“Wgraj swoje CV”** (→ import), and **“Najpierw zobacz przykładowe CV”** opens the demo. The benefit rail states **“ZA DARMO”**, **“CZYSTY PDF”**, and **“INTELIGENTNY LAYOUT”**. The heading uses three independent line boxes instead of one wrapping highlighted phrase, while the complete copy block remains centred as one readable column across wide, compact, and zoomed layouts. The surviving calls to action emit `hero_wizard`, `hero_import`, `hero_demo`, `templates_wizard`, `pricing_free`, and `pricing_pro` through `queueGuestEvent`.
...
- `frontend/src/pages/Hero/Hero.jsx`, lines 88–398, function `buildStartUrl`, component `CtaLink`, and component `Hero` — preserves directed starts and retained event attribution while rendering the centred outcome-led hero, registry-driven template marquee, privacy strip, pricing, FAQ, and footer
- `frontend/src/pages/Hero/Hero.module.css`, lines 190–1265, selectors `.hero`, `.heroMedia`, `.heroCopy`, `.heroActions`, `.heroTrust`, `.templatesSection`, `.trustStrip`, `.pricingSection`, `.faqSection`, and `.footer` — centres the 960 px hero column and provides the responsive Swiss/editorial composition for the five retained sections and footer
- `frontend/src/pages/Hero/Hero.test.js`, lines 1–78, suite `landing product positioning` — guards the outcome positioning, centred hero, retained section indices, exact five-section composition, footer, template count, surviving anchors, directed-start builders, and retained CTA analytics events
...
- `frontend/src/pages/Hero/Hero.module.css`, lines 1–2017; `frontend/src/pages/Login/Login.module.css`, lines 1–476; `frontend/src/pages/Register/Register.module.css`, lines 1–511 — spacious narrative and authentication surfaces on the shared tokens
...
- `frontend/src/utils/regentAppearance.js`, lines 15–430, exports `REGENT_PALETTES`, `REGENT_TEXT_SIZES`, `getRegentAppearance`, `applyRegentPalette`, and `applyRegentTextSize` — four semantic editions, true-colour icon switching, reversible type, and persisted intent
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, lines 238–259 and 569–618, component `SectionsPanel`; `SectionsPanel.module.css`, lines 46 and 59–68, Regent palette miniature classes — accessible two-by-two appearance selector and S–XL controls
...
- `scripts/generate_iconic_icons.py`, lines 20–27 and 511, `THEME_VARIANTS`; `backend/template_assets/iconic/regent-{ivory,sapphire,burgundy}/` — 45 palette-matched raster glyphs
...
- `backend/tests/test_regent_template.py`, lines 15–125; `test_regent_appearance_assets.py`, lines 9–34 — registry and generated appearance metadata, Montserrat summary at body scale, contact-icon generation, complete 15-glyph inventories, and exact visible RGB for every edition
...
- `backend/app/services/cv_data.py`, functions `normalize_cv_data`, `_split_language_rows`, `_normalize_languages`, `skill_groups`, `is_distinct_skill_family_title`, `_expand_skill_category_lines`, `_absorb_skills_alias_sections`, and `extract_contact_fields_from_raw`
...
Language regressions additionally cover a three-cell horizontal grid through extraction, grounding, normalization, and Meridian rendering; rejection of a model-only language on a fully native PDF with no Languages section; and safe vision passthrough that ignores nameless/bare-level provider rows (`backend/tests/test_cloudflare_cv_extraction.py`, lines 1049–1158, tests `test_horizontal_language_grid_keeps_all_source_languages`, `test_native_pdf_without_languages_rejects_model_invention`, and `test_vision_language_grounding_keeps_named_provider_rows_only`). Pipe-delimited multi-language normalization plus preservation of semicolon-bearing structured level notes are covered in `backend/tests/test_cv_data.py`, lines 471–513, tests `test_pipe_delimited_language_row_becomes_separate_entries` and `test_structured_language_levels_keep_detail_punctuation`.
...
- `frontend/src/pages/Hero/Hero.jsx`, lines 264–398, component `Hero` — pricing + FAQ for Darmowy/Pro and the landing footer
...
│   │       ├── regentAppearance(.test).js # Dwie klasyczne i dwie kreatywne edycje Regenta z prawdziwymi ikonami
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, komponent `SectionsPanel` oraz funkcje `handleAppearancePalette` / `handleAppearanceTextSize` — ścisła bramka Atrium/Regent/Sterling/Linden/Monument/Slate/Meridian/Cadenza/Vellum, dostępne radiogrupy właściwe dla szablonu i transakcja S–XL; `SectionsPanel.module.css`, klasy Regenta `paletteGridRegent`, `palettePaperRegent`, `paletteRegentName`, `paletteRegentContacts`, `paletteRegentSectionRule` i `paletteRegentFolio` oraz dotychczasowe miniatury szablonów; testy: `SectionsPanel.test.js`
...
- `frontend/src/utils/regentAppearance.js`, eksporty `REGENT_PALETTES`, `REGENT_TEXT_SIZES`, `getRegentAppearance`, `applyRegentPalette` i `applyRegentTextSize` — dwie klasyczne i dwie kreatywne edycje, semantyczne kolory papieru/tekstu/linii/folio, latentne stanowisko i runs inline, prawdziwe ikony, ochrona ręcznych kolorów, zapis zamiaru oraz odwracalna typografia zależna od roli; testy: `regentAppearance.test.js`
...
Landing prowadzi od oczekiwanego rezultatu osoby szukającej pracy, a nie od mechaniki edytora: **„Stwórz CV, które prowadzi do rozmowy.”** Opis zaprasza do wgrania starego CV albo rozpoczęcia od zera, a następnie wyjaśnia, że CV Studio pomaga dopasować treść do oferty i układa ją w profesjonalny dokument gotowy do wysłania. Celowo skrócony przepływ landingu zachowuje oryginalne numery sekcji **01, 05, 09, 10 i 11**: hero → galeria szablonów → prywatność → cennik → FAQ → stopka. Brakujące numery pozostają widoczne jako zamierzone przerwy w szwajcarskim indeksie plakatowym. Ekrany uwierzytelniania są osobnymi trasami i ta kompozycja landingu ich nie zmienia.

Copy nie obiecuje zatrudnienia ani gwarantowanej rozmowy: obiecuje mocniejszą prezentację prawdziwego doświadczenia kandydata. Opis ATS nadal zawiera jasne zastrzeżenie, że poszczególne systemy mogą zwrócić różne wyniki, zmiana palety/skali typografii jest oznaczona jako dostępna tylko w wybranych szablonach, a Pro pozostaje jednorazową płatnością za 30 dni bez automatycznego odnowienia. Pasek korzyści hero używa trzech zwięzłych etykiet możliwości — **„ZA DARMO”**, **„CZYSTY PDF”** i **„INTELIGENTNY LAYOUT”**. Hierarchia wizualna pozostaje spokojna i redakcyjna: ostre linie, ciepły papier, niemal czarna typografia, jeden przygaszony ziemisty akcent, zwarte etykiety mono i prawdziwe mockupy produktu zamiast ogólnych ilustracji SaaS.

Otwierające hero zachowuje kolumnę treści o szerokości 960 px wyśrodkowaną na całej powierzchni sekcji, a `frontend/public/women-job-call.png` wypełnia nieinteraktywną warstwę tła. Jednolity overlay w kolorze papieru i ograniczone nasycenie podporządkowują zdjęcie komunikatowi; na kompaktowych ekranach overlay jest mocniejszy. Kicker, heading, opis, grupa CTA, link demo i pasek korzyści pozostają na wspólnej osi centralnej, a akapit zachowuje węższą miarę 680 px dla czytelności. Sekcja szablonów używa **realnych mockupów** z `frontend/public/template-mockups/` w marquee w prawo→lewo ze wszystkimi wpisami rejestru; hover lub focus klawiatury pauzuje taśmę i powiększa kartę, a ograniczenie ruchu przełącza ją na statyczną siatkę.
...
- `frontend/src/pages/Hero/Hero.jsx`, linie 88–398, funkcja `buildStartUrl`, komponent `CtaLink` i komponent `Hero` — zachowuje skierowane starty i pozostałą atrybucję zdarzeń, renderując wyśrodkowane hero, marquee z rejestru szablonów, prywatność, cennik, FAQ i stopkę
- `frontend/src/pages/Hero/Hero.module.css`, linie 190–1265, selektory `.hero`, `.heroMedia`, `.heroCopy`, `.heroActions`, `.heroTrust`, `.templatesSection`, `.trustStrip`, `.pricingSection`, `.faqSection` i `.footer` — centruje kolumnę hero o szerokości 960 px i zapewnia responsywną kompozycję Swiss/editorial dla pięciu zachowanych sekcji oraz stopki
- `frontend/src/pages/Hero/Hero.test.js`, linie 1–78, suite `landing product positioning` — chroni pozycjonowanie rezultatu, wyśrodkowane hero, zachowane indeksy sekcji, dokładny układ pięciu sekcji, stopkę, dynamiczną liczbę szablonów, zachowane anchory, buildery skierowanych startów i pozostałe zdarzenia analityczne CTA
...
- `frontend/src/pages/Hero/Hero.module.css`, linie 1–2017; `frontend/src/pages/Login/Login.module.css`, linie 1–476; `frontend/src/pages/Register/Register.module.css`, linie 1–511 — przestronne powierzchnie narracyjne/auth na wspólnych tokenach
...
- `frontend/src/utils/regentAppearance.js`, linie 15–430, eksporty `REGENT_PALETTES`, `REGENT_TEXT_SIZES`, `getRegentAppearance`, `applyRegentPalette` i `applyRegentTextSize` — cztery edycje semantyczne, prawdziwe ikony, odwracalna typografia i zapis zamiaru
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, linie 238–259 i 569–618, komponent `SectionsPanel`; `SectionsPanel.module.css`, linie 46 i 59–68, klasy miniatur Regenta — dostępny wybór wyglądu 2×2 oraz kontrolki S–XL
...
- `scripts/generate_iconic_icons.py`, linie 20–27 i 511, `THEME_VARIANTS`; `backend/template_assets/iconic/regent-{ivory,sapphire,burgundy}/` — 45 glifów rastrowych dopasowanych do palet
...
- `backend/tests/test_regent_template.py`, linie 15–125; `test_regent_appearance_assets.py`, linie 9–34 — metadane rejestru i wygenerowanego wyglądu, podsumowanie Montserrat w skali treści, generowanie ikon kontaktowych, pełne 15-elementowe zestawy glifów oraz dokładny widoczny RGB każdej edycji
...
- **Wygląd ukrytego stanu.** Wszystkie dziewięć szablonów z zakładką Wygląd stosuje zmianę palety oraz typografii S/M/L/XL także do latentnej specyfikacji stanowiska i blueprintów dekoracji, nie tylko do żywych elementów. Pole dodane po zmianie wyglądu używa więc bieżącej palety i rozmiaru fontu, a nie starych wartości generatora.
...
- `backend/app/services/cv_data.py` — funkcje `normalize_cv_data`, `_split_language_rows`, `_normalize_languages`, `skill_groups`, `is_distinct_skill_family_title`, `_expand_skill_category_lines`, `_absorb_skills_alias_sections` i `extract_contact_fields_from_raw`
- `backend/app/services/cv_templates/shared/text.py` — `_place_skills_section`
- `backend/app/services/contact_links.py`
...
Regresje językowe obejmują dodatkowo trzykomórkową poziomą siatkę na całej ścieżce ekstrakcja → grounding → normalizacja → render Meridian, odrzucenie języka istniejącego wyłącznie w odpowiedzi modelu dla natywnego PDF-u bez sekcji Języki oraz bezpieczny passthrough vision ignorujący rekordy bez nazwy i sam poziom (`backend/tests/test_cloudflare_cv_extraction.py`, linie 1049–1158, testy `test_horizontal_language_grid_keeps_all_source_languages`, `test_native_pdf_without_languages_rejects_model_invention` i `test_vision_language_grounding_keeps_named_provider_rows_only`). Normalizację wielu języków rozdzielonych pionową kreską oraz zachowanie średników w opisowym poziomie mapowania sprawdza `backend/tests/test_cv_data.py`, linie 471–513, testy `test_pipe_delimited_language_row_becomes_separate_entries` i `test_structured_language_levels_keep_detail_punctuation`.
...
- `frontend/src/pages/Hero/Hero.jsx`, linie 264–398, komponent `Hero` — cennik i FAQ planów Darmowy/Pro oraz stopka landingu
