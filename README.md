...
- `backend/app/services/pdf_generator.py`, lines 151–315 and 1401–1519, methods `renderImage`, `_clip_image_box`, `_draw_image_cover`, and `render_elements` — bounded 2× cover crop and exact ellipse/rounded clipping for canvas image radii
...
- `backend/app/services/pdf_generator.py`, lines 151–315, methods `PDF_Generator.renderImage`, `_clip_image_box`, and `_draw_image_cover` — shared EXIF-aware, bounded 2× cover crop plus rectangular, rounded, or exact circular clipping
...
- `backend/app/services/pdf_generator.py`, lines 151–315 and 1401–1519, methods `PDF_Generator._draw_image_cover` and `render_elements`
- `backend/app/crud/pdfs.py`, `create_new_pdf`, `update_pdf_elements`

Tests: `backend/tests/test_render_on_demand.py`, lines 108–154 — authenticated `/images/{id}/content` export with a high-resolution circular JPEG and a bounded PDF response; `backend/tests/test_pdf_shapes.py`, lines 153–216 — exact circle clip and rasterised output.
...
- `backend/app/services/pdf_generator.py`, lines 151–315 and 1401–1519, methods `renderImage`, `_clip_image_box`, `_draw_image_cover`, and `render_elements`; EXIF-aware bounded cover crop, rounded/circular clipping, and hidden-slot filtering
...
- `backend/app/services/pdf_generator.py`, lines 1375–1399, method `_draw_watermark` (diagonal overlay, isolated via `saveState`/`restoreState` so it cannot leak fill/alpha/font state); lines 1401–1519, `render_elements(..., watermark=False)`, with the per-page call at 1512–1514
...
- `backend/app/services/pdf_generator.py`, linie 151–315 i 1401–1519, metody `renderImage`, `_clip_image_box`, `_draw_image_cover` i `render_elements` — ograniczony crop `cover` 2× oraz dokładny clipping elipsy/zaokrągleń dla promienia obrazu z canvasu
...
- `backend/app/services/pdf_generator.py`, linie 151–315, metody `PDF_Generator.renderImage`, `_clip_image_box` i `_draw_image_cover` — ten sam offset `+1.0` co kanwa, crop `cover` 2× uwzględniający EXIF oraz clipping prostokątny, zaokrąglony lub dokładnie okrągły
...
- `backend/app/main.py` — CORS `expose_headers=["Content-Disposition"]`
- `backend/app/services/pdf_generator.py`, linie 151–315 i 1401–1519 — metody `PDF_Generator._draw_image_cover` i `render_elements`
- `backend/app/crud/pdfs.py` — `create_new_pdf`, `update_pdf_elements`

Testy: `backend/tests/test_render_on_demand.py`, linie 108–154 — eksport autoryzowanego `/images/{id}/content` z wysokorozdzielczym okrągłym JPEG-iem i ograniczonym rozmiarem odpowiedzi PDF; `backend/tests/test_pdf_shapes.py`, linie 153–216 — dokładna maska koła i rasteryzowany wynik.
...
- `backend/app/services/pdf_generator.py`, linie 151–315 i 1401–1519, metody `renderImage`, `_clip_image_box`, `_draw_image_cover` i `render_elements`; ograniczony crop `cover` z EXIF, clipping zaokrąglony/okrągły i filtrowanie ukrytego slotu
...
- `backend/app/services/pdf_generator.py`, linie 1375–1399, metoda `_draw_watermark` (ukośna nakładka, izolowana przez `saveState`/`restoreState`, więc nie może wyciec kolor wypełnienia/przezroczystości/fontu); linie 1401–1519, `render_elements(..., watermark=False)`, z wywołaniem per strona w liniach 1512–1514
