...
1. **Choose a landing-page start** → the primary CTA “Stwórz CV za darmo” (`start=wizard`) opens the four-step data wizard. After the wizard, guests save the profile locally and continue to registration/login; authentication adopts the draft, generates the Free Meridian CV, and opens the full editor. The secondary “Wgraj swoje CV” (`start=import`) still uses the import flow. The tertiary demo link (`start=demo`) opens the limited Linden starter with the exact Julia Bernat data used by the Linden picker mockup. The editor topbar’s labelled **Zmień szablon** control opens the change-template gallery after the CV exists.
...
Section 02 stays on the white `var(--paper)` surface and presents CV Studio as a five-step path from source information to a ready PDF: start from an existing CV or from scratch, refine and tailor the content, edit the A4 document, switch its template without rewriting, and export a clean PDF. The asymmetric desktop composition places **“Od pierwszych informacji do gotowego PDF.”** beside one semantic ordered list; it collapses into the same logical reading order on compact and zoomed layouts. The heading **“Ta sama treść. Inny charakter.”** then introduces **real template mockups** from `frontend/public/template-mockups/` in a right-to-left marquee of every registry entry and the CTA **“Stwórz CV w wybranym szablonie”**. Hover or keyboard focus pauses the strip and scales the card, while reduced motion falls back to a static wrap.
...
- `frontend/src/pages/Hero/Hero.jsx`, lines 88–443, function `buildStartUrl`, component `CtaLink`, and component `Hero`; Section 02 is lines 186–291 — preserves directed starts and event attribution while rendering the centred outcome-led hero, semantic five-step offer, labelled registry-driven template marquee, privacy strip, pricing, FAQ, and footer
- `frontend/src/pages/Hero/Hero.module.css`, lines 190–1358, including `.templatesSection`, `.offerIntro`, `.offerStatement`, `.offerLead`, `.offerSteps`, `.offerStepIndex`, `.templateGalleryHeader`, `.templateGalleryLabel`, and `.templateMarquee` at lines 454–663 — keeps Section 02 on the white paper token and provides its asymmetric wide layout, compact single-column flow, full-bleed gallery, and reduced-motion fallback
- `frontend/src/pages/Hero/Hero.test.js`, lines 1–108, suite `landing product positioning`; Section 02 regression is lines 37–63 — guards the outcome positioning, white offer surface, exact ordered five-step flow before the gallery, responsive collapse, section indices, footer, template count, surviving anchors, directed-start builders, and retained CTA analytics events
...
1. **Wybór startu na stronie głównej** → główne CTA „Stwórz CV za darmo” (`start=wizard`) otwiera czterostopniowy kreator danych. Po jego zakończeniu gość zapisuje profil lokalnie i przechodzi do rejestracji/logowania; po uwierzytelnieniu szkic jest przejmowany, generowany jest darmowy Meridian i otwiera się pełny edytor. Drugorzędne „Wgraj swoje CV” (`start=import`) nadal używa przepływu importu, a link demo (`start=demo`) otwiera ograniczony starter Linden z dokładnie tymi samymi danymi Julii Bernat co mockup Linden w pickerze. Podpisana kontrolka **Zmień szablon** w edytorze służy do zmiany wyglądu dopiero po wygenerowaniu CV.
...
Landing prowadzi od oczekiwanego rezultatu osoby szukającej pracy, a nie od mechaniki edytora: **„Stwórz CV, które prowadzi do rozmowy.”** Opis zaprasza do wgrania starego CV albo rozpoczęcia od zera, a następnie wyjaśnia, że CV Studio pomaga dopasować treść do oferty i układa ją w profesjonalny dokument gotowy do wysłania. Celowo skrócony przepływ landingu używa numerów sekcji **01, 02, 09, 10 i 11**: hero → pięciokrokowa oferta CV Studio z galerią szablonów → prywatność → cennik → FAQ → stopka. Późniejsze brakujące numery pozostają widoczne jako zamierzone przerwy w szwajcarskim indeksie plakatowym. Ekrany uwierzytelniania są osobnymi trasami i ta kompozycja landingu ich nie zmienia.
...
Sekcja 02 pozostaje na białym tle `var(--paper)` i przedstawia CV Studio jako pięć kroków od danych wejściowych do gotowego PDF: start z obecnego CV lub od zera, dopracowanie i dopasowanie treści, edycja dokumentu A4, zmiana szablonu bez przepisywania oraz eksport czystego PDF. Asymetryczna kompozycja desktopowa zestawia hasło **„Od pierwszych informacji do gotowego PDF.”** z jedną semantyczną listą uporządkowaną; na układach kompaktowych i przy powiększeniu składa się w tę samą logiczną kolejność. Nagłówek **„Ta sama treść. Inny charakter.”** wprowadza zachowane marquee **realnych mockupów** z `frontend/public/template-mockups/` oraz CTA **„Stwórz CV w wybranym szablonie”**. Hover lub focus klawiatury pauzuje taśmę i powiększa kartę, a ograniczenie ruchu przełącza ją na statyczną siatkę.
...
- `frontend/src/pages/Hero/Hero.jsx`, linie 88–443, funkcja `buildStartUrl`, komponent `CtaLink` i komponent `Hero`; sekcja 02 zajmuje linie 186–291 — zachowuje skierowane starty i atrybucję zdarzeń, renderując wyśrodkowane hero, semantyczną pięciokrokową ofertę, opisane marquee z rejestru szablonów, prywatność, cennik, FAQ i stopkę
- `frontend/src/pages/Hero/Hero.module.css`, linie 190–1358, w tym `.templatesSection`, `.offerIntro`, `.offerStatement`, `.offerLead`, `.offerSteps`, `.offerStepIndex`, `.templateGalleryHeader`, `.templateGalleryLabel` i `.templateMarquee` w liniach 454–663 — utrzymuje sekcję 02 na białym tokenie papieru oraz zapewnia asymetryczny szeroki układ, jednokolumnowy przepływ kompaktowy, galerię full-bleed i fallback ograniczonego ruchu
- `frontend/src/pages/Hero/Hero.test.js`, linie 1–108, suite `landing product positioning`; regresja sekcji 02 zajmuje linie 37–63 — chroni pozycjonowanie rezultatu, białe tło oferty, dokładnie pięć uporządkowanych kroków przed galerią, składanie responsywne, indeksy sekcji, stopkę, liczbę szablonów, anchory, buildery startów i zachowane zdarzenia analityczne CTA
