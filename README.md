...
- `backend/app/services/cv_templates/templates/vestige.py`, lines 57–544, function `_gen_vestige` — semantic geometry transformation over the shared deterministic sidebar planner; emits the static `DANE KONTAKTOWE` heading/rule outside the managed contact band; rebuilds the contact rail (`_place_stacked_icon_contacts`) and masthead identity (`tag_masthead_identity`, `band_top` pinned to the title's own `top` so `blockPt` is `0`) directly from `cv`; excludes and proportionally re-translates `grid-member` cells; aligns the sidebar's first section with the main column's first section by computing their top delta instead of a fixed offset; recomputes both columns' textarea `height` at a uniform 12 px line height and carries the resulting per-page cumulative shift into every later element's `top` in BOTH columns; appends the empty-state photo-slot well/frame/glyph triplet
...
- `frontend/src/templates/vestige.test.js`, lines 10–50 — rail width, main-column masthead, static contact heading, contact icon placement, and section-rule geometry; the following test covers the photo-slot triplet's geometry/tags
...
- `frontend/src/templates/vestige.test.js`, linie 10–50 — szerokość panelu, masthead w głównej kolumnie, statyczny nagłówek kontaktów, położenie ikon kontaktu i geometria linii sekcji; kolejny test obejmuje geometrię/tagi trójki slotu na zdjęcie
