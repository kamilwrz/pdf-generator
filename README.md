...
- `backend/tests/test_vellum_template.py`, lines 26–149 — registry metadata, asymmetric identity, circular photo slot, dedicated summary background, skills-first order, exact experience/education period and city anchors, and continuation pages without repeated identity/contact data
- `frontend/src/templates/vellum.test.js`, lines 21–147 — starter hierarchy plus continuous-summary invariants after spacing, repeated packing, section reorder, and live textarea reflow
...
- Editing surfaces + selection row:
  ...
  `frontend/src/components/canvas/Textarea/Textarea.jsx`, lines 453–576,
  component `Textarea` (`commitEditable` and the deterministic Enter handler),
  `frontend/src/utils/editableSerialize.js`, lines 91–198
  (`bulletRunsToEditableHtml`, `createTextareaEnterEdit`), lines 228–307
  (`flatten`, `serializeEditable`), and lines 317–570 (selection offset mapping),
  ...
  marker column and the height mirror, Enter preserves explicit blank
  paragraphs, and only bare trailing bullet placeholders are removed. Any content change that
...
- `frontend/src/utils/textRuns.test.js`,
  `frontend/src/utils/editableSerialize.test.js`, lines 55–279 — normalization,
  mark toggling, run slicing, DOM serialization round-trip, explicit consecutive
  blank paragraphs, filled/empty bullet Enter behavior, caret restoration, the
  Monument training paragraph, and production wiring to the shared grid.
...
- `backend/tests/test_vellum_template.py`, linie 26–149 — metadane rejestru, asymetryczna tożsamość, okrągły slot zdjęcia, osobne tło podsumowania, kolejność umiejętności przed doświadczeniem, dokładne anchory okresu i miasta doświadczenia/wykształcenia oraz strony kontynuacji bez powtórzonych danych tożsamości i kontaktu
- `frontend/src/templates/vellum.test.js`, linie 21–147 — hierarchia startera oraz inwarianty ciągłego tła podsumowania po zmianie odstępów, ponownym pakowaniu, zmianie kolejności i live reflow textarea
...
- Powierzchnie edycji + rząd zaznaczenia:
  ...
  `frontend/src/utils/textRuns.js` (`rangeColor`),
  `frontend/src/components/canvas/Text/Text.jsx`,
  `frontend/src/components/canvas/Textarea/Textarea.jsx`, linie 453–576,
  komponent `Textarea` (`commitEditable` i deterministyczna obsługa Enter),
  `frontend/src/utils/editableSerialize.js`, linie 91–198
  (`bulletRunsToEditableHtml`, `createTextareaEnterEdit`), linie 228–307
  (`flatten`, `serializeEditable`) oraz linie 317–570 (mapowanie offsetów
  zaznaczenia), a także `frontend/src/utils/textareaHeight.js`, linie 12–84
  ...
  wyświetlaniem/PDF i lustrem wysokości, Enter zachowuje jawne puste akapity, a
...
- `backend/tests/test_pdf_inline_runs.py` — tożsamość bez runów, parytet
  ...
  offsety rysowania.
- `frontend/src/utils/textRuns.test.js`,
  `frontend/src/utils/editableSerialize.test.js`, linie 55–279 — normalizacja,
  przełączanie marek, cięcie runów, round-trip serializacji DOM, jawne kolejne
  puste akapity, Enter na pełnym/pustym punkcie, odtwarzanie kursora, akapit
  szkolenia z Monument oraz podłączenie produkcyjne do wspólnej siatki.
