...
│   │   ├── templates/        # 19 template specs + helpers + demoCv.js (guest-mode demo content)
...
Sterling is a paid, two-column template (`layouts: ["sidebar"]`) with a quiet blue-gray (`#4A6FA5` accent, `#26313F` ink, `#F7F8FA` paper) palette. Its brief is a **centered letterhead masthead** — serif (`CormorantGaramond`) display name, tracked uppercase title, mid-dot contact line — sitting on a **full-width tinted "letterhead band"** (a `595`-pt-wide filled rectangle in the rail tint `#EDF1F6`, from the top of the page down to the masthead rule) and closed by a **horizontal rule spanning both columns**. The band exists so the vertical sidebar divider and the rail fill both begin at the band's bottom edge (`rule_y`) instead of at `y = 0`: because the masthead is centered on the page, its title and contact lines cross the `x = 210` column boundary, and a full-height divider would run straight up through them, visually "cutting" the centered letterhead. Reusing the rail tint (`#EDF1F6`) makes the top band and the left rail read as one continuous field, while the main column below the band stays on paper. Below that rule the page splits into a wide sidebar (`210` pt) and the main column; **which section lands in which column is decided by a balance-driven planner** (described next), not a fixed rule. One rule color (`#C7CFDA`) is reused for the masthead underline, the sidebar divider, and every main-column section rule, so the page reads as one coherent system rather than several separately-styled dividers — the "harmonijny" (harmonious) brief was explicit about this. Continuation pages carry no masthead, so their rail and divider run the full page height.

**Section placement is balance-driven.** Rather than filling the sidebar first, Sterling measures every section's height in both column widths and calls the shared `plan_columns` planner (`backend/app/services/cv_templates/shared/column_planner.py`) to partition sections between the columns, minimising page-1 imbalance. Experience is anchored to the main column; every other section is movable, and may render in either column. Because the sidebar cannot paginate, the sidebar assignment is a hard page-1 fit while the main column may overflow onto later pages (its overflow is therefore not counted as wasted space). In practice Education follows Experience in the main column, and moves into the sidebar only when a long Experience block already fills page 1. The planner is a small greedy local search: seed each section by its natural column affinity, force the sidebar under budget, then repeatedly apply the single move that most reduces `max(empty_sidebar, empty_main)` until the columns are balanced or no move clears a minimum-improvement threshold.
...
- `backend/app/services/cv_templates/shared/column_planner.py`, `plan_columns` / `PlaceableSection` / `ColumnPlan` — the pure, balance-driven two-column partitioner shared by sidebar templates
- `backend/app/services/cv_templates/templates/sterling.py`, function `_gen_sterling` — centered letterhead masthead + closing rule, `sidebar_kicker`, per-section descriptor building (measures each section in both column widths), `plan_columns` call, then per-column rendering through the shared sidebar (`_fit_sidebar_sections` / `_fitted_sidebar_body_elements`) and main (`_place_experience_record` / `_place_education_record` / `_place_skills_section` / `_extra_sections`) helpers
...
Atrium, Axis, and Blueprint are already Jan Kowalski dumps and are left alone by that script (edit or re-dump them separately if their generators change).

`frontend/public/template-mockups/{nova,volt,monument,words,cardinal,harbor,tessera,slate,portico,axis,atrium,regent,aurelia,blueprint,sterling,ledger,nimbus,cinder,kernel,aldine}.png` — the previews shown in the Hero template gallery (`frontend/src/pages/Hero/Hero.jsx`), the in-app template picker (`frontend/src/components/modals/TemplatesModal/TemplatesModal.jsx`), and the hover pane in **Wypełnij z mojego CV** (`frontend/src/components/ai/AiCvPanel/AiCvPanel.jsx`) — are rendered from those starter arrays, not hand-drawn mockups. After starter changes, regenerate the PNGs:
...
│   │   ├── templates/        # 19 specyfikacji szablonów + helpery + demoCv.js (treść demo w trybie gościa)
│   │   └── utils/            # a4ElementFactories, freeformShapes, canvasFont, canvasElementSchema, geometry, reflow, sectionBuilder, sectionRecord, sectionIcons, guestDocument, guestWizardDraft, claimGuestWizardDraft, resolveActiveCvData, guestEvents
│   ├── package.json
│   └── .env.example
├── shared/
...
**Zasady:** 19 id szablonów frontu muszą odpowiadać `_GENERATORS` w `cv_templates/registry.py` (re-eksport z `cv_generator.py`). Każdy `cv_templates/templates/<id>.py` zawiera wyłącznie żywy generator tego szablonu — bez wspólnego silnika multi-theme i martwych gałęzi siblingów. Sekrety tylko w env. `uploads/` i `static/generated/` to dane runtime. Bajty obrazów użytkownika nie są publicznie montowane — tylko przez `GET /images/{id}/content`.
...
Sterling to płatny, dwukolumnowy szablon (`layouts: ["sidebar"]`) w stonowanej palecie niebiesko-szarej (`#4A6FA5` akcent, `#26313F` tusz, `#F7F8FA` papier). Brief to **wycentrowany masthead w stylu papieru firmowego** — szeryfowe (`CormorantGaramond`) imię i nazwisko, rozstrzelony wielkoliterowy tytuł, linia kontaktu z kropkami pośrodku — osadzony na **pełnoszerokościowym, przyciemnionym „pasie papieru firmowego”** (wypełniony prostokąt o szerokości `595` pt w kolorze szyny `#EDF1F6`, od góry strony do linii mastheadu) i zamknięty **poziomą linią rozciągającą się na obie kolumny**. Pas istnieje po to, aby pionowy divider sidebaru oraz wypełnienie szyny zaczynały się dopiero przy dolnej krawędzi pasa (`rule_y`), a nie przy `y = 0`: ponieważ masthead jest wycentrowany na stronie, jego tytuł i linia kontaktu przecinają granicę kolumn `x = 210`, a divider biegnący od `y = 0` przechodziłby przez nie w pionie, wizualnie „przycinając” wycentrowany nagłówek. Reużycie koloru szyny (`#EDF1F6`) sprawia, że górny pas i lewa szyna czytają się jako jedno ciągłe pole, podczas gdy kolumna główna poniżej pasa pozostaje na papierze. Poniżej tej linii strona dzieli się na szeroki sidebar (`210` pt) i kolumnę główną; **o tym, która sekcja trafia do której kolumny, decyduje planer sterowany balansem** (opisany dalej), a nie stała reguła. Jeden kolor linii (`#C7CFDA`) jest reużywany dla podkreślenia mastheadu, dividera sidebaru i każdej linii sekcji w kolumnie głównej, dzięki czemu strona czyta się jako jeden spójny system, a nie zbiór osobno stylizowanych linii — brief „harmonijny” był w tej kwestii wyraźny. Strony kontynuacyjne nie mają mastheadu, więc ich szyna i divider biegną na całą wysokość strony.

**Rozmieszczanie sekcji jest sterowane balansem.** Zamiast najpierw wypełniać sidebar, Sterling mierzy wysokość każdej sekcji w obu szerokościach kolumn i wywołuje wspólny planer `plan_columns` (`backend/app/services/cv_templates/shared/column_planner.py`), który dzieli sekcje między kolumny, minimalizując nierównowagę na stronie 1. Doświadczenie jest zakotwiczone w kolumnie głównej; każda inna sekcja jest ruchoma i może wyrenderować się w dowolnej kolumnie. Ponieważ sidebar nie może dzielić się na strony, jego przydział to twarde dopasowanie do strony 1, podczas gdy kolumna główna może przechodzić na kolejne strony (jej nadmiar nie jest więc liczony jako zmarnowane miejsce). W praktyce Wykształcenie następuje po Doświadczeniu w kolumnie głównej i przechodzi do sidebara tylko wtedy, gdy długie Doświadczenie zapełnia już stronę 1. Planer to małe zachłanne przeszukiwanie lokalne: zasiej każdą sekcję według jej naturalnej przynależności do kolumny, wepchnij sidebar poniżej budżetu, a następnie wielokrotnie zastosuj pojedynczy ruch najbardziej redukujący `max(puste_sidebar, puste_główna)`, aż kolumny się zrównoważą lub żaden ruch nie przekroczy progu minimalnej poprawy.
...
- `backend/app/services/cv_templates/shared/column_planner.py`, `plan_columns` / `PlaceableSection` / `ColumnPlan` — czysty, sterowany balansem partycjoner dwukolumnowy współdzielony przez szablony sidebarowe
- `backend/app/services/cv_templates/templates/sterling.py`, funkcja `_gen_sterling` — wycentrowany masthead w stylu papieru firmowego + zamykająca linia, `sidebar_kicker`, budowa deskryptorów sekcji (mierzy każdą sekcję w obu szerokościach kolumn), wywołanie `plan_columns`, a następnie renderowanie każdej kolumny przez wspólne helpery sidebaru (`_fit_sidebar_sections` / `_fitted_sidebar_body_elements`) i kolumny głównej (`_place_experience_record` / `_place_education_record` / `_place_skills_section` / `_extra_sections`)
...
- `frontend/src/templates/sterling.test.js` — dekoracje strony/sidebaru/dividera (szerszy pas, cienka linia w kolorze reguły), pełnoszerokościowy pas papieru firmowego, którego wypełnienie szyny i divider zaczynają się przy dolnej krawędzi pasa (nigdy nie przecinając wycentrowanego mastheadu), wycentrowana treść mastheadu zamknięta poziomą linią, kickery sidebaru otagowane `flowRole: "sidebar-chrome"` + `flowLane: "sidebar"`, podsumowanie/umiejętności/języki w sidebarze, strukturalne (nie zlepione) wykształcenie w sidebarze współdzielące jeden `flowGroup`, dokładnie jedna sekcja w kolumnie głównej (Doświadczenie), warstwowy (nie jednowierszowy) tytuł/organizacja/okres rekordu, zero elementów `rectangle`
...
Atrium, Axis i Blueprint są już zrzutami Jana Kowalskiego i ten skrypt ich nie rusza (edytuj lub zrzucaj osobno po zmianie generatora).

Pliki `frontend/public/template-mockups/{nova,volt,monument,words,cardinal,harbor,tessera,slate,portico,axis,atrium,regent,aurelia,blueprint,sterling,ledger,nimbus,cinder,kernel,aldine}.png` — podglądy w galerii Hero, pickerze i panelu **Wypełnij z mojego CV** — pochodzą z tych tablic starterów, nie z ręcznych grafik. Po zmianie starterów odtwórz PNG:
