...
The landing page welcomes the user with the job seeker's desired outcome rather than the editor mechanism: **“Stwórz CV, które prowadzi do rozmowy.”** The hero intentionally has no supporting subheading: the heading leads directly to the two start actions, while the three-capability offer below explains what CV Studio does. The intentionally compact landing flow uses numbered sections **01, 02, 09, 10, and 11**: hero → concise CV Studio capability offer and template gallery → privacy strip → pricing → FAQ → footer. The later missing numbers remain visible as intentional gaps in the Swiss poster index. Authentication pages are separate routes and are not affected by this landing composition.
...
The opening hero keeps its 960 px content column centred across the full section while `frontend/public/women-job-call.png` fills a non-interactive background layer. A flat paper-coloured overlay and restrained desaturation keep the photograph subordinate to the message; the overlay becomes stronger on compact screens. The kicker, welcome heading, CTA group, demo link, and benefit rail remain on the centred axis. A 48 px desktop system gap (32 px on compact screens) connects the heading directly to the actions after removal of the former supporting paragraph.

Section 02 stays on the white `var(--paper)` surface and states the offer without presenting one mandatory workflow. **“Jedno CV. Wiele wersji. Bez pisania od nowa.”** sits beside a semantic list of three concrete capabilities: tailoring content to an offer, editing directly on the A4 page, and switching templates without rewriting. The heading is built from independent, undecorated block lines with explicit leading and spacing, so wrapping cannot make neighbouring lines overlap; no individual word is underlined or highlighted. Removing two redundant rows and tightening the section rhythm reduces the empty vertical field before the gallery. The layout collapses into the same logical reading order on compact and zoomed views. The heading **“Ta sama treść. Inny charakter.”** then introduces **real template mockups** from `frontend/public/template-mockups/` in a right-to-left marquee of every registry entry and the CTA **“Stwórz CV w wybranym szablonie”**. Hover or keyboard focus pauses the strip and scales the card, while reduced motion falls back to a static wrap.
...
- `frontend/src/pages/Hero/Hero.jsx`, function `buildStartUrl`, component `CtaLink`, and component `Hero` — preserves directed starts and event attribution while rendering the centred outcome-led hero, semantic three-capability offer, labelled registry-driven template marquee, privacy strip, pricing, FAQ, and footer
- `frontend/src/pages/Hero/Hero.module.css`, lines 189–1359, including `.hero`, `.heroActions`, `.templatesSection`, `.offerIntro`, `.offerStatement`, `.offerLead`, `.offerSteps`, `.offerStepIndex`, `.templateGalleryHeader`, `.templateGalleryLabel`, and `.templateMarquee` at lines 448–647 — keeps the welcome composition and Section 02 on the white paper token while providing their wide, compact, and reduced-motion layouts
- `frontend/src/pages/Hero/Hero.test.js`, suite `landing product positioning` — guards the outcome positioning, white offer surface, exactly three capabilities before the gallery, undecorated non-overlapping heading blocks, responsive collapse, section indices, footer, template count, surviving anchors, directed-start builders, and retained CTA analytics events
...
- `frontend/src/pages/Hero/Hero.jsx`, lines 296–430, component `Hero` — pricing + FAQ for Darmowy/Pro and the landing footer
...
Landing wita użytkownika oczekiwanym rezultatem osoby szukającej pracy, a nie mechaniką edytora: **„Stwórz CV, które prowadzi do rozmowy.”** Hero celowo nie ma podtytułu pomocniczego: po nagłówku od razu pojawiają się dwie akcje startowe, a oferta trzech funkcji poniżej wyjaśnia, co robi CV Studio. Celowo skrócony przepływ landingu używa numerów sekcji **01, 02, 09, 10 i 11**: hero → zwięzła oferta funkcji CV Studio z galerią szablonów → prywatność → cennik → FAQ → stopka. Późniejsze brakujące numery pozostają widoczne jako zamierzone przerwy w szwajcarskim indeksie plakatowym. Ekrany uwierzytelniania są osobnymi trasami i ta kompozycja landingu ich nie zmienia.
...
- `frontend/src/pages/Hero/Hero.jsx`, funkcja `buildStartUrl`, komponent `CtaLink` i komponent `Hero` — zachowuje skierowane starty i atrybucję zdarzeń, renderując wyśrodkowane hero, semantyczną ofertę trzech funkcji, opisane marquee z rejestru szablonów, prywatność, cennik, FAQ i stopkę
- `frontend/src/pages/Hero/Hero.module.css`, linie 189–1359, w tym `.hero`, `.heroActions`, `.templatesSection`, `.offerIntro`, `.offerStatement`, `.offerLead`, `.offerSteps`, `.offerStepIndex`, `.templateGalleryHeader`, `.templateGalleryLabel` i `.templateMarquee` w liniach 448–647 — utrzymuje kompozycję powitalną i sekcję 02 na białym tokenie papieru oraz zapewnia ich szeroki, kompaktowy i ograniczony ruchowo układ
- `frontend/src/pages/Hero/Hero.test.js`, suite `landing product positioning` — chroni pozycjonowanie rezultatu, białe tło oferty, dokładnie trzy funkcje przed galerią, niedekorowane i nienachodzące bloki nagłówka, składanie responsywne, indeksy sekcji, stopkę, liczbę szablonów, anchory, buildery startów i zachowane zdarzenia analityczne CTA
...
- `frontend/src/pages/Hero/Hero.jsx`, linie 296–430, komponent `Hero` — cennik i FAQ planów Darmowy/Pro oraz stopka landingu
