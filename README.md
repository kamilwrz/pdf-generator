...
- **template** — structural editing: content/chrome positions are layout-owned (no free X/Y drag). `canFreePositionElement` also blocks template icons (`alignWithText` / `/template-assets/…`) and generator shapes (line/rectangle/circle/ellipse/polygon/path) even when a template omitted `flowRole` — this covers harbor/regent/volt/portico/axis contact icons, masthead artwork, and generator frames, and similar. User gallery photos (`/images/…`) may still move, except a fitted profile-photo slot (`photoSlot: "image"` / glyph). **Układ CV** flyout (sidebar label + panel title; formerly “Sekcje”) docked beside the 72px tool rail (reorder + density presets + advanced vertical rhythm `stack` / `record` / `section` / `after_rule`, defaults 4 / 10 / 21 / 8), gallery photo-slot targets (`applyProfilePhoto`), auto-height reflow with reclaim. Topbar **Odblokuj edycję** (icon + tooltip) copies the document into freeform.
...
- `scripts/generate_iconic_icons.py`, lines 325–352, `SUBSET_THEMES`; `backend/template_assets/iconic/sterling-*` — six matching contact-icon themes; asset-colour test: `backend/tests/test_sterling_appearance_assets.py`, lines 1–35
...
- `frontend/src/templates/iconic.js`, export `regentTemplate` — guest demo document, identical to the Regent picker starter
...
- `frontend/src/templates/iconic.js`, export `regentTemplate` — regenerated starter (photo slot with portrait glyph, no user raster)
...
- `frontend/src/templates/linden.js`, lines 1–1151, export `lindenTemplate`; `frontend/src/templates/index.js`, lines 22–39 — generated starter with the main-flow anchor and no redundant upper body rule, plus the paid picker entry.
...
- `frontend/src/templates/iconic.js`, exports `regentTemplate` and `voltTemplate`
...
- `backend/app/services/cv_templates/templates/{regent,volt,portico}.py` — per-template `_gen_*` entry points
...
```bash
python scripts/regenerate_template_starters.py   # rewrites remaining starters (+ Regent/Volt in iconic.js)
```
...
- Generators: `regent`, `volt`, `slate`, `tessera` (+ text templates via `_contact_line`)
...
- Template call sites: `backend/app/services/cv_templates/templates/{atrium,portico,tessera,slate,regent,volt}.py`.
...
- **All six contact-band templates.** Atrium, Portico, Tessera, Slate, Regent and Volt each tag their name/title via `tag_masthead_identity` and append a zero-footprint identity anchor (`flowRole: "masthead-anchor"`, `mastheadIdentity.id == "masthead-main"`).
...
- `scripts/generate_iconic_icons.py`, linie 325–352, `SUBSET_THEMES`; `backend/template_assets/iconic/sterling-*` — sześć pasujących motywów ikon kontaktowych; test koloru assetów: `backend/tests/test_sterling_appearance_assets.py`, linie 1–35
...
- `frontend/src/templates/iconic.js`, eksport `regentTemplate` — starter ze slotem i ikoną portretu, bez rastra użytkownika
...
- `frontend/src/templates/linden.js`, linie 1–1151, eksport `lindenTemplate`; `frontend/src/templates/index.js`, linie 22–39 — wygenerowany starter z anchorem głównego flow i bez zbędnej górnej linii treści oraz płatny wpis w pickerze.
...
- `frontend/src/templates/iconic.js`, eksporty `regentTemplate` i `voltTemplate`
...
- `backend/app/services/cv_templates/templates/{regent,volt,portico}.py` — osobne wejścia `_gen_*`
...
```bash
python scripts/regenerate_template_starters.py   # przepisuje pozostałe startery (+ Regent/Volt w iconic.js)
```
...
- Testy: `frontend/src/utils/profilePhotoVisibility.test.js`, linie 115–146 (dwustronicowy reflow w obu kolejnościach przełączania zdjęcia/stanowiska Portico); `frontend/src/utils/sectionStructure.test.js`, linie 1891–1953; `frontend/src/templates/slate.test.js`, linie 6–78; `frontend/src/templates/tessera.test.js`, linie 6–81; `backend/tests/test_cv_template_layouts.py`, linie 103–215 i 258–378; `backend/tests/test_contact_channel_roundtrip.py`, linie 54–76; `backend/tests/test_pdf_watermark.py`, linie 119–137
...
- `backend/app/services/cv_templates/shared/text.py` — `_contact_line` / `_contact_line_core`
- Generatory: `regent`, `volt`, `slate`, `tessera` (+ szablony tekstowe przez `_contact_line`)
...
- Miejsca wywołań szablonów: `backend/app/services/cv_templates/templates/{atrium,portico,tessera,slate,regent,volt}.py`.
...
- **Wszystkie sześć szablonów z paskiem kontaktu.** Atrium, Portico, Tessera, Slate, Regent i Volt tagują swoje imię/tytuł przez `tag_masthead_identity` i dopinają zerowej powierzchni anchor tożsamości (`flowRole: "masthead-anchor"`, `mastheadIdentity.id == "masthead-main"`).
...
- Miejsca wywołań szablonów: `backend/app/services/cv_templates/templates/{harbor,atrium,portico,tessera,slate,regent,volt}.py`.
