...
| `pdf_elements` | Canvas elements; geometry + style columns; extras in `extra_properties` JSON (`fixedToPage`, `repeatOnContinuation`, `locked`, `flowRole`, `flowGroup`, `preserveInitialLayout`, Sterling/Linden/Monument/Slate/Meridian/Cadenza `appearanceSettings` + reversible type baselines, bold, `runs` inline-decoration overlay, connectors, …) |
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, lines 186–270, component `SectionsPanel` appearance definition; lines 345–419, functions `handleAppearancePalette` / `handleAppearanceTextSize`; lines 493–643, Appearance tab and controls — strict Sterling/Linden/Monument/Slate/Meridian/Cadenza gate, template-specific accessible radio groups, and S–XL layout transaction; `SectionsPanel.module.css`, lines 105–128 — white-paper Meridian and Cadenza miniatures; tests: `SectionsPanel.test.js`, lines 9–71
...
- `scripts/generate_iconic_icons.py`, lines 61–76, `_save_png`, and lines 291–350, palette glyph inventories / `SUBSET_THEMES`; `backend/template_assets/iconic/{sterling-*,linden-*,monument-*,slate-*-accent,meridian-*,cadenza-*}` — matching real icon themes for every appearance-enabled template; asset-colour tests: `backend/tests/test_{sterling,linden,monument,slate,meridian,cadenza}_appearance_assets.py`
...
| `pdf_elements` | Elementy kanwy (+ `extra_properties`, m.in. `fixedToPage`, `repeatOnContinuation`, `locked`, `flowRole`, `flowGroup`, `preserveInitialLayout`, `appearanceSettings` Sterling/Linden/Monument/Slate/Meridian/Cadenza i odwracalne bazowe metryki tekstu, `runs` — nakładka dekoracji inline) |
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, linie 186–270, definicja wyglądu w komponencie `SectionsPanel`; linie 345–419, funkcje `handleAppearancePalette` / `handleAppearanceTextSize`; linie 493–643, zakładka i kontrolki Wyglądu — ścisła bramka Sterling/Linden/Monument/Slate/Meridian/Cadenza, dostępne radiogrupy właściwe dla szablonu i transakcja S–XL; `SectionsPanel.module.css`, linie 105–128 — białe miniatury Meridian i Cadenzy; testy: `SectionsPanel.test.js`, linie 9–71
...
- `scripts/generate_iconic_icons.py`, linie 61–76, `_save_png`, oraz linie 291–350, zestawy glifów palet / `SUBSET_THEMES`; `backend/template_assets/iconic/{sterling-*,linden-*,monument-*,slate-*-accent,meridian-*,cadenza-*}` — prawdziwe dopasowane motywy ikon dla każdego szablonu z Wyglądem; testy kolorów: `backend/tests/test_{sterling,linden,monument,slate,meridian,cadenza}_appearance_assets.py`
