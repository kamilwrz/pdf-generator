...
- `frontend/src/utils/canvasPageWheel.js` / `frontend/src/hooks/useCanvasPageWheel.js` — wheel at scroll edge → `goToPage` (PageControls label sync)
...
Tessera and Slate fit complete compact sections via `_fit_sidebar_sections`; anything that does not fit spills into the main column instead of being truncated. Harbor instead uses its own page-aware right-column `Builder`, because each education line retains a separate icon and typography role. Harbor marks date, city and icon elements that share a horizontal row as `record-overlay`: they follow the owning textarea's page and Y coordinate but do not add another row to the vertical `flowGroup` height. Reflow also recognises legacy saved Harbor overlays from their shared group and exact Y coordinate, so reopening an older document repairs separated icons and falsely inflated records. The shared Iconic cap offset remains unchanged for Nova, Volt, Cardinal and other templates. Harbor opts its mixed rows out of that shared offset: 11 px sidebar icons are geometrically centred 0.25 px inside 11.5 px rows. In experience records, employer, date and location are all rendered as 12 px line-height textareas at the same `top`; metadata textareas are fixed-height overlays. The right metadata strip reserves 150 px, uses a 7.2 px base font (down to 6.4 px only for unusually long values), 9 px icons and compact 2/6 px internal gaps. Width estimates include a 4 px safety allowance per label so complete date and city values remain visible without crossing into the employer field. Using one DOM text model removes the browser-dependent baseline mismatch between `Text` and `Textarea` without shifting icons in other templates. Harbor's 28 px inter-column gutter is an explicit reflow boundary: a rule or marker lying entirely to the right of a main-column textarea belongs to the sidebar even when it falls inside the generic 32 px decoration tolerance. This prevents deep sidebar chrome from blocking reclaim of an experience record that still fits on page 1. For an already saved document, an equal-height page-2 measurement is allowed to run this reclaim once, so stale pagination repairs itself after the document is reopened without creating a render loop. Iconic experience entries use the same textarea-block stack as project records (`SPACE_STACK` inside a job, `SPACE_RECORD` / 10 px between jobs) so canvas spacing matches exported PDF rhythm.
...
Płótno **A4 pion** z dwoma trwałymi trybami na rekordzie `Pdf` (`editor_mode`, `template_id`, opcjonalne `spacing_px`). Pionowe kółko nad `.canvas-area` najpierw przewija overflow; na górnej/dolnej krawędzi (albo gdy strona mieści się bez overflow) wywołuje `goToPage`, więc **PageControls** (`Strona N / M`) pokazuje nowy numer. Gestów poziomych, Ctrl/Meta+wheel oraz pól edycyjnych nie przejmuje (`frontend/src/utils/canvasPageWheel.js`, `frontend/src/hooks/useCanvasPageWheel.js`, testy w `canvasPageWheel.test.js`). Szyna scrolla canvasu jest ostylowana w `App.css` (navy thumb + złota krawędź na chłodnym torze; Firefox przez `scrollbar-color`).
