...
The assistant opens as a responsive panel up to 430 px wide, with slightly enlarged interface text for readability. Its composer starts at two lines, grows with the prompt up to 136 px, and then scrolls internally so long commands do not push the conversation out of view. Grammar, style, and improve **correction cards** stay compact in the chat; on hover they animate open, stack the full **Przed** / **Po** texts vertically, and peek slightly left out of the panel. Leaving the card restores the previous size and position.
...
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, lines 63–114, `CorrectionCard` — hover-expand Przed/Po correction review
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, lines 490–1097, component `AiAssistant` — layout-session history boundary, request flow, review cards, and composer
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, lines 528–536 and 1070–1091 — auto-growing two-line chat composer
- `frontend/src/components/ai/AiAssistant/AiAssistant.module.css`, lines 42–62, 314–376, 430–671, and 838–885 — panel width, correction hover expand, and composer sizing
...
- `backend/app/services/layout_gpt.py`, lines 32–520 (`SECTION_HEADER_GAP_*`, `_can_share_text_row`, `_build_text_rows`, `_build_layout_snapshot_data`, `build_layout_snapshot`, `build_layout_user_prompt`), 554–622 (`_resolve_model_references`), 716–791 (`_parse_section_inventory`, `_affected_text_ids`), 792–900 (`_changes_to_findings`, `_collapses_below_min_section_gap`), and 968–1211 (`compile_layout_gpt_response`)
...
Tests: `backend/tests/test_layout_gpt.py`, lines 103–116 (`test_user_prompt_standardizes_positive_section_header_gaps`), 132–193 (row grouping and text-height regressions), 297–330 (`test_compile_rejects_collapsing_section_header_gap_to_zero`), and 332–366 (`test_compile_allows_standardizing_section_header_gap_to_target`), plus the inventory and compiler tests in the same module; also `test_openai_pricing.py`, `test_ai_credits.py`, `test_ai_chat_command.py`, and `test_layout_analysis.py`.
...
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, linie 63–114, `CorrectionCard` — rozwijanie poprawek Przed/Po po najechaniu
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, linie 490–1097, komponent `AiAssistant` — granica historii sesji Układu, wysyłanie żądań, karty zmian i pole wpisywania
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, linie 528–536 i 1070–1091 — automatycznie rosnące, dwuwierszowe pole czatu
- `frontend/src/components/ai/AiAssistant/AiAssistant.module.css`, linie 42–62, 314–376, 430–671 i 838–885 — szerokość panelu, hover poprawek i pole wpisywania
...
- `backend/app/services/layout_gpt.py`, linie 32–520 (`SECTION_HEADER_GAP_*`, `_can_share_text_row`, `_build_text_rows`, `_build_layout_snapshot_data`, `build_layout_snapshot`, `build_layout_user_prompt`), 554–622 (`_resolve_model_references`), 716–791 (`_parse_section_inventory`, `_affected_text_ids`), 792–900 (`_changes_to_findings`, `_collapses_below_min_section_gap`) i 968–1211 (`compile_layout_gpt_response`)
- `backend/app/services/layout_analysis.py` — `resolve_directed_operation`, `summarize_geometry_issues`
- `backend/app/services/openai_pricing.py` — `usage_from_response`, `estimate_cost_usd`

Testy: `backend/tests/test_layout_gpt.py`, linie 103–116 (`test_user_prompt_standardizes_positive_section_header_gaps`), 132–193 (regresje grupowania wierszy oraz wysokości tekstu), 297–330 (`test_compile_rejects_collapsing_section_header_gap_to_zero`) i 332–366 (`test_compile_allows_standardizing_section_header_gap_to_target`), a także testy inwentarza i kompilatora w tym samym module; ponadto `test_openai_pricing.py`, `test_ai_credits.py`, `test_ai_chat_command.py` i `test_layout_analysis.py`.
