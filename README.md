...
6. **Bio wizard** → five-step fullscreen creator (`BioCvModal`) for the normal entry, or a four-step data-only variant for demo conversion. Authenticated users use draft CRUD on `/ai/bio_cv_draft`; guests autosave the wizard profile to `localStorage` (`cvstudio.guest.wizardDraft` via `guestWizardDraft.js`, separate from the canvas key `cvstudio.guest.doc`). After demo registration/login, the guest snapshot is adopted and `POST /ai/fill_template` generates the Regent before the full editor opens.
