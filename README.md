...
Element properties open as a **floating inspector** anchored above the selection on the canvas (`Editor` via `createPortal`), not as a slide-out beside the tool rail. The panel keeps the full field set from the former docked inspector; **Text** and **TextArea** still differ (TextArea adds bullets, text align, line height / letter spacing, width / height). Placement uses selection DOM bboxes (`floatingPanelPosition.js`: prefer above, flip below, clamp to the viewport). The 68px tool rail remains for tools / documents / logout; only **Sekcje** still docks as a flyout next to that rail.
...
- `frontend/src/components/editor/Editor/Editor.jsx` — floating property inspector (portal); Text vs TextArea field sets; multi-select bulk edits
...
Właściwości elementu otwierają się jako **pływający inspector** zakotwiczony nad zaznaczeniem na canvasie (`Editor` przez `createPortal`), a nie jako wysuwany panel obok szyny narzędzi. Panel zachowuje pełny zestaw pól z dawnego dockowanego inspectora; **Text** i **TextArea** nadal się różnią (TextArea: punktory, wyrównanie tekstu, wysokość linii / tracking, szerokość / wysokość). Pozycja liczy bbox DOM zaznaczenia (`floatingPanelPosition.js`: preferuj nad, flip pod, clamp do viewport). Szyna 68px zostaje dla narzędzi / dokumentów / wylogowania; tylko **Sekcje** nadal dokują się jako flyout obok tej szyny.
...
- `frontend/src/components/editor/Editor/Editor.jsx` — pływający inspector właściwości (portal); zestawy pól Text vs TextArea; edycja zbiorcza
