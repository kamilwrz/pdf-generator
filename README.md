...
- `frontend/src/pages/Hero/Hero.jsx`, function `buildStartUrl` and component `CtaLink` — only `start=import` conditionally returns a `/register` URL; every other intent uses `getEditorPath({ start })`; `CtaLink` renders the primary / secondary / text-link chrome and queues the per-source funnel event on click; before/after `afterMock` is `{ name: "Sterling", image: "/template-mockups/sterling-showcase.png" }`, a dedicated same-content render, not `previewById(...)`; the "before" card (`.oldDocument`) is real Jan Kowalski CV text (name, address, phone, email, first education entry, first job), not the earlier abstract CSS-bar wireframe; templates section renders a duplicated `TEMPLATE_PREVIEWS` marquee (all mockups) under `#szablony`
- `frontend/src/pages/Hero/Hero.module.css` — quiet-luxury white/ivory + `#171717` + gold `#B8954A` system with hard gold offset shadows; real-mockup compositions (`.heroStack` / `.templateMarquee` + `@keyframes templateMarquee` / `.editorMock` + floating `.aiCard`s / `.finalStack`) and the compact `.trustStrip`; `.oldDocument`/`.oldName`/`.oldContactLine`/`.oldSectionBar`/`.oldEntryTitle`/`.oldEntryMeta` recreate a dated Word-CV look (Georgia serif, navy `#1F2B52`, grey `#D6D6D6` section bars) — the one deliberate exception to the site's own Swiss/sans design system, since the whole point is to look outdated
- `scripts/render_sterling_showcase.py` — generates `frontend/public/template-mockups/sterling-showcase.png` from the Jan Kowalski CV data (kept in sync with the literal text in `Hero.jsx`'s `.oldDocument` block) via `generate_resume("sterling", cv)` and the same ReportLab/PyMuPDF pipeline as `scripts/render_iconic_mockups.py`; re-run manually if the Jan Kowalski content or Sterling's layout ever changes
...
Main-column skills render via `_place_skills_section` (one UMIEJĘTNOŚCI chrome, then optional bold category labels + mid-dot chip rows from `_skills_inline_content`). Single-column / icons languages use `_place_languages_grid` — four equal-width textarea cells across the content column (`Name — Level`, level in italic accent colour via textarea `runs`, `flowRole: "grid-member"`). Sidebar languages (Tessera, Slate, Sterling) are plain `Name - Level` lines (hyphen, no bullets). Vertical bullet lists (`_bullet_list_content`, `bulletList: true`) remain for sidebar skills and other flat chip sections (interests, certifications). Sidebar education in Tessera, Slate and Sterling uses the same structured stack as the main column — separate diploma / school / meta / description elements via `_build_sidebar_education_elements` / `_fitted_sidebar_body_elements` (not one mashed plaintext textarea). Description lines carry `bulletList: true`.
...
Sekcja przed/po to prawdziwa transformacja tej samej treści, nie dwa niepowiązane dokumenty: karta „PRZED” (blok `.oldDocument` w `Hero.jsx`) odtwarza prawdziwe, przestarzałe CV w stylu Worda (Jan Kowalski — krój szeryfowy, granatowe nagłówki, szare belki sekcji, celowo poza własnym systemem projektowym Swiss strony, bo ma wyglądać na przestarzałe) za pomocą prawdziwego tekstu HTML/CSS, a nie abstrakcyjnego szkieletu ani zrzutu ekranu; karta „PO” pokazuje dedykowany render Sterlinga z DOKŁADNIE tą samą treścią CV (`frontend/public/template-mockups/sterling-showcase.png`, wygenerowany z tych samych danych Jana Kowalskiego, a nie standardowy mockup pickera szablonów z ogólną personą demo). `afterMock` w `Hero.jsx` to mały obiekt inline (`{ name: "Sterling", image: "/template-mockups/sterling-showcase.png" }`), a nie `previewById(...)`, właśnie dlatego, że ten obraz jest unikalny dla tej sekcji.
...
- `frontend/src/pages/Hero/Hero.jsx`, funkcja `buildStartUrl` i komponent `CtaLink` — tylko `start=import` warunkowo zwraca URL `/register`; każda inna intencja używa `getEditorPath({ start })`; `CtaLink` renderuje wariant primary / secondary / link i kolejkuje zdarzenie lejka z danym źródłem; karta przed/po `afterMock` to `{ name: "Sterling", image: "/template-mockups/sterling-showcase.png" }` — dedykowany render tej samej treści, a nie `previewById(...)`; karta „przed” (`.oldDocument`) to prawdziwy tekst CV Jana Kowalskiego (imię, adres, telefon, e-mail, pierwszy wpis wykształcenia, pierwsza praca), a nie wcześniejszy abstrakcyjny szkielet CSS; sekcja szablonów to zduplikowany marquee `TEMPLATE_PREVIEWS` (wszystkie mockupy) pod `#szablony`
- `frontend/src/pages/Hero/Hero.module.css` — system quiet-luxury biel/kość słoniowa + `#171717` + złoto `#B8954A` z twardymi złotymi cieniami offset; kompozycje z realnych mockupów (`.heroStack` / `.templateMarquee` + `@keyframes templateMarquee` / `.editorMock` + pływające `.aiCard` / `.finalStack`) i zwarty `.trustStrip`; `.oldDocument`/`.oldName`/`.oldContactLine`/`.oldSectionBar`/`.oldEntryTitle`/`.oldEntryMeta` odtwarzają przestarzały wygląd CV w stylu Worda (szeryf Georgia, granat `#1F2B52`, szare `#D6D6D6` belki sekcji) — jedyny celowy wyjątek od własnego systemu projektowego Swiss/sans strony, bo cała idea polega na tym, żeby wyglądać przestarzale
- `scripts/render_sterling_showcase.py` — generuje `frontend/public/template-mockups/sterling-showcase.png` z danych CV Jana Kowalskiego (zsynchronizowanych z dosłownym tekstem w bloku `.oldDocument` w `Hero.jsx`) przez `generate_resume("sterling", cv)` i ten sam potok ReportLab/PyMuPDF co `scripts/render_iconic_mockups.py`; uruchom ponownie ręcznie, jeśli treść Jana Kowalskiego albo układ Sterlinga kiedykolwiek się zmienią
...
Umiejętności w kolumnie głównej renderuje `_place_skills_section` (jeden chrome UMIEJĘTNOŚCI, potem opcjonalne pogrubione etykiety kategorii + wiersze chipów ze środkowymi kropkami z `_skills_inline_content`). Języki w układzie jednokolumnowym / icons używają `_place_languages_grid` — cztery równe komórki textarea na szerokość kolumny treści (`Nazwa — Poziom`, poziom kursywą w kolorze akcentu przez `runs`, `flowRole: "grid-member"`). Języki w sidebarze (Tessera, Slate, Sterling) to zwykłe linie `Nazwa - Poziom` (hyphen, bez punktorów). Pionowe listy punktów (`_bullet_list_content`, `bulletList: true`) pozostają dla skills w sidebarze oraz innych płaskich sekcji (zainteresowania, certyfikaty). Wykształcenie w sidebarze Tessera / Slate / Sterling ma tę samą strukturę co kolumna główna — osobne elementy dyplom / uczelnia / meta / opis przez `_build_sidebar_education_elements` / `_fitted_sidebar_body_elements` (nie jeden zbity textarea). Opis ma `bulletList: true`.
