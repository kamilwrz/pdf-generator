...
- `backend/tests/test_cloudflare_cv_extraction.py`, linie 121–506, klasa `CloudflareCvExtractionTests` — wybór dostawcy, payloady Gemma text/Qwen vision, niezależne budżety reasoning/JSON/vision, fallback Llama JSON po pustym albo błędnym wyniku, fenced/typed JSON, JSON Mode rollbacku, server-only credentials, dokładne nagłówki źródłowe, zagnieżdżone skills w stylu Monument i końcowy render Atrium
