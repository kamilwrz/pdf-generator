...
- `frontend/src/pages/Hero/Hero.jsx`, lines 132–672, function `buildStartUrl`, component `CtaLink`, and component `Hero` — preserves directed starts and event attribution while rendering the outcome-led hero, exact A4/PDF explanation, bounded AI tasks, privacy/pricing caveats, real before/after comparison, and registry-driven template marquee
- `frontend/src/pages/Hero/Hero.module.css`, lines 222–2188, selectors `.hero`, `.documentEngineSection`, `.templatesSection`, `.editorSection`, `.wysiwygSection`, `.aiSection`, `.trustStrip`, `.pricingSection`, `.faqSection`, and `.finalCta` — responsive Swiss/editorial composition, non-overlapping block headline, real-mockup layouts, warm-paper surfaces, sharp rules, restrained earth accents, and mobile collapse for the added sections
- `frontend/src/pages/Hero/Hero.test.js`, lines 1–64, suite `landing product positioning` — guards the outcome positioning, non-overlap and 1024 px responsive contracts, Free/AI benefit copy, honest capability caveats, dynamic template count, Linden's primary hero placement, anchors, directed-start builders, and all existing CTA analytics events
...
- `frontend/src/pages/Hero/Hero.module.css`, lines 1–2188; `frontend/src/pages/Login/Login.module.css`, lines 1–476; `frontend/src/pages/Register/Register.module.css`, lines 1–511 — spacious narrative and authentication surfaces on the shared tokens
...
- `frontend/src/pages/Hero/Hero.jsx`, linie 132–672, funkcja `buildStartUrl`, komponent `CtaLink` i komponent `Hero` — zachowuje skierowane starty i atrybucję zdarzeń, renderując hero skupione na rezultacie, wyjaśnienie zgodności A4/PDF, ograniczone zadania AI, zastrzeżenia prywatności/cennika, prawdziwe porównanie przed/po i marquee z rejestru szablonów
- `frontend/src/pages/Hero/Hero.module.css`, linie 222–2188, selektory `.hero`, `.documentEngineSection`, `.templatesSection`, `.editorSection`, `.wysiwygSection`, `.aiSection`, `.trustStrip`, `.pricingSection`, `.faqSection` i `.finalCta` — responsywna kompozycja Swiss/editorial, nienachodzący blokowy nagłówek, układy z prawdziwych mockupów, powierzchnie ciepłego papieru, ostre linie, oszczędne ziemiste akcenty i mobilne składanie dodanych sekcji
- `frontend/src/pages/Hero/Hero.test.js`, linie 1–64, suite `landing product positioning` — chroni pozycjonowanie rezultatu, kontrakty braku nakładania i breakpointu 1024 px, komunikaty Free/AI, uczciwe zastrzeżenia funkcji, dynamiczną liczbę szablonów, pierwszoplanową pozycję Linden w hero, anchory, buildery skierowanych startów i wszystkie istniejące zdarzenia analityczne CTA
...
- `frontend/src/pages/Hero/Hero.module.css`, linie 1–2188; `frontend/src/pages/Login/Login.module.css`, linie 1–476; `frontend/src/pages/Register/Register.module.css`, linie 1–511 — przestronne powierzchnie narracyjne/auth na wspólnych tokenach
