...
- Loading: PDF spinner minimum display time; `useToasts` / `ToastStack` renders compact, high-contrast confirmation cards in the upper-left corner so notifications do not cover the A4 canvas. Cards use enter/exit motion and collapse to a safe 12 px mobile inset. A template-change toast shows the selected template as an accent-colour badge instead of redundant body copy and uses the `template-change` replacement key, so a newer selection immediately removes an older template notification while unrelated toasts remain visible. Download actions carry a per-toast blob href, not a shared slot.
  - Implementation: `frontend/src/hooks/useToasts.js`, lines 8–70, exports `mergeToastQueue` and `useToasts`; `frontend/src/hooks/useApplyCvTemplate.js`, lines 37–98, hook `useApplyCvTemplate`; `frontend/src/components/common/ToastStack/ToastStack.jsx`, lines 25–75, component `ToastStack`; `frontend/src/components/common/ToastStack/ToastStack.module.css`, lines 1–135.
  - Tests: `frontend/src/hooks/useToasts.test.js`, lines 1–21.
...
- Toasty i spinner PDF z minimalnym czasem widoczności; `useToasts` / `ToastStack` pokazuje kompaktowe, kontrastowe karty potwierdzeń w lewym górnym rogu, dzięki czemu komunikaty nie zasłaniają canvasa A4. Karty mają animację wejścia/wyjścia, a na urządzeniach mobilnych zachowują bezpieczny margines 12 px. Toast zmiany szablonu pokazuje wybrany szablon jako plakietkę w jego kolorze zamiast zbędnej treści opisowej i używa klucza zastępowania `template-change`, więc nowszy wybór natychmiast usuwa starszy komunikat o szablonie bez usuwania niezależnych toastów. Akcje pobierania niosą blob href w toastcie (nie we wspólnym slocie).
  - Implementacja: `frontend/src/hooks/useToasts.js`, linie 8–70, eksporty `mergeToastQueue` i `useToasts`; `frontend/src/hooks/useApplyCvTemplate.js`, linie 37–98, hook `useApplyCvTemplate`; `frontend/src/components/common/ToastStack/ToastStack.jsx`, linie 25–75, komponent `ToastStack`; `frontend/src/components/common/ToastStack/ToastStack.module.css`, linie 1–135.
  - Testy: `frontend/src/hooks/useToasts.test.js`, linie 1–21.
