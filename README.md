...
- **Wizard closed from its direct landing entry returns to the landing page** — `?start=wizard` (“Stwórz CV od początku”) opens the guided wizard as the very first thing a guest sees, with nothing yet on the canvas. Previously, closing it without filling anything just cleared the dialog and left the visitor on an empty freeform canvas with no explanation. It now navigates back to `/` the first time that specific entry wizard is closed with the canvas still empty; reopening the wizard later from the Topbar, or closing it after content already exists, behaves exactly as before (just closes).
...
- `frontend/src/pages/PdfCanvas.jsx`, lines 657–682, function `handleShowBioCvModal` (`wizardEntryNavigatedRef`) — redirects to `/` on the first empty-canvas close of a `?start=wizard` entry
...
- **Zamknięcie kreatora otwartego z bezpośredniego wejścia na landingu wraca na stronę główną** — `?start=wizard` („Stwórz CV od początku”) otwiera kreator jako pierwszą rzecz, jaką widzi gość, z niczym jeszcze na płótnie. Wcześniej zamknięcie go bez wypełnienia niczego po prostu czyściło dialog i zostawiało odwiedzającego na pustym płótnie freeform bez żadnego wyjaśnienia. Teraz nawiguje z powrotem na `/` przy pierwszym zamknięciu tego konkretnego kreatora wejściowego, gdy płótno jest wciąż puste; ponowne otwarcie kreatora później z Topbara albo zamknięcie go, gdy treść już istnieje, zachowuje się dokładnie jak wcześniej (po prostu zamyka).
...
- `frontend/src/pages/PdfCanvas.jsx`, linie 657–682, funkcja `handleShowBioCvModal` (`wizardEntryNavigatedRef`) — przekierowanie na `/` przy pierwszym zamknięciu pustego płótna z wejścia `?start=wizard`
