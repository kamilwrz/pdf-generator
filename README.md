...
**Consistent CTA hierarchy.** The hero leads with the recruitment outcome above. Its primary action remains **“Stwórz CV za darmo”** (→ wizard), the shorter secondary action is **“Wgraj swoje CV”** (→ import), and **“Najpierw zobacz przykładowe CV”** opens the demo. The benefit rail states **“ZA DARMO”**, **“POMOC AI”**, and **“INTELIGENTNY LAYOUT”**. The heading uses three independent line boxes instead of one wrapping highlighted phrase, while the complete copy block remains centred as one readable column across wide, compact, and zoomed layouts. Each call-to-action still fires the same per-source event through `queueGuestEvent`: `hero_wizard`, `hero_import`, `hero_demo`, `before_after_import`, `templates_wizard`, `pricing_free`, `pricing_pro`, `final_wizard`, `final_import`.
...
- `frontend/src/pages/Hero/Hero.test.js`, lines 1–68, suite `landing product positioning` — guards the outcome positioning, expanded and centred hero content/CTA axis, removal of the animated showcase and deferred copy, concise capability labels, honest capability caveats, dynamic template count, anchors, directed-start builders, and all existing CTA analytics events
...
- `frontend/src/pages/Hero/Hero.test.js`, linie 1–68, suite `landing product positioning` — chroni pozycjonowanie rezultatu, rozszerzoną i wyśrodkowaną oś treści/CTA hero, usunięcie animowanego showcase'u i odłożonego tekstu, zwięzłe etykiety możliwości, uczciwe zastrzeżenia funkcji, dynamiczną liczbę szablonów, anchory, buildery skierowanych startów i wszystkie istniejące zdarzenia analityczne CTA
