...
During the canvas enter hold, auto-height reflow is suppressed and resumes after fonts are ready. Every textarea emitted by the Python generators carries `preserveInitialLayout: true` (via `_block` in `cv_generator_primitives.py`). On first mount the canvas may **shrink** a box to browser `scrollHeight` when ReportLab overshoots (so empty slack cannot inflate visual section gaps), but it will not **grow** — independent growth races still stretch gaps. Editing content or later changing typography/width still triggers normal auto-height reflow. Trailing blank lines and bare bullet markers (`•`) are trimmed from textarea content on blur / display / document scrub (`trimTrailingEmptyTextareaLines` / `trimTrailingEmptyTextareaPayload` in `textareaHeight.js`) so measured height matches visible copy and bullet lists do not leave a tall empty outline that pushes the next record. **While editing, trailing empties are kept** so Enter can open a new paragraph or a blank gap before another bullet group (e.g. a heading line + blank + listed points in a regular section). Blank lines *between* real content are never stripped. In bullet mode, Enter after a filled item continues with `• `; Enter on a bare `•` clears the marker into a blank paragraph. Display rendering keeps a line box for empty rows so those gaps do not collapse. See `textareaHeight.test.js` (`shouldShrinkPreservedLayout`, trailing-empty / mid-gap cases) and `textareaReflow.test.js` packing cases.
...
W czasie enter-hold reflow auto-height jest wstrzymany i wraca po gotowości fontów. Każda textarea z generatorów Pythona ma `preserveInitialLayout: true` (przez `_block` w `cv_generator_primitives.py`). Przy pierwszym montażu canvas może **zmniejszyć** box do `scrollHeight` przeglądarki, gdy ReportLab zawyży wysokość (żeby pusta przestrzeń nie psuła wizualnych odstępów sekcji), ale nie **powiększa** go — niezależny growth nadal psuje rytm. Edycja treści lub późniejsza zmiana typografii/szerokości nadal uruchamia normalny auto-height reflow. Końcowe puste linie i same markery bullet (`•`) są przycinane z treści textarea przy blur / wyświetlaniu / scrubie dokumentu (`trimTrailingEmptyTextareaLines` / `trimTrailingEmptyTextareaPayload` w `textareaHeight.js`), żeby zmierzona wysokość odpowiadała widocznej treści i listy punktowane nie zostawiały wysokiego pustego obramowania, które spycha kolejny rekord. **W trakcie edycji końcowe puste wiersze są zachowane**, żeby Enter mógł otworzyć nowy akapit albo lukę przed kolejną grupą punktów (np. linia nagłówka + pusta + wymienione punkty w zwykłej sekcji). Puste linie *między* realną treścią nigdy nie są usuwane. W trybie listy Enter po wypełnionym punkcie kontynuuje `• `; Enter na samym `•` czyści marker do pustego akapitu. Render wyświetlania utrzymuje wysokość wiersza dla pustych linii, żeby te luki się nie zapadały. Zobacz `textareaHeight.test.js` (`shouldShrinkPreservedLayout`, przypadki trailing-empty / mid-gap) oraz packing w `textareaReflow.test.js`.
