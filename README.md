...
**Implemented today:** editor (including guest mode without an account), templates, extract/fill, bio draft, AI assistant (goal-oriented actions, rating dashboard, translate, layout review cards), entitlements (Darmowy / Pro — 59 zł / 30 days), autosave, local or S3 storage, JWT auth.
...
Section decorations explicitly tagged with `flowRole: "section-chrome"` are treated as a rigid visual composition by `compactChromeCluster`: spacing changes move the complete heading/icon/frame/rule group but preserve every authored mutual Y offset. Only two recognized legacy-corruption signatures are rebuilt (the old sequential `SPACE_STACK` marker layout and a flattened Monument accent rule). This keeps template-specific section rhythm stable across Nova, Volt, Monument, Cinder, and other templates while still repairing already damaged documents.
...
- tests: `editorMode.test.js`, `sectionStructure.test.js` (including chrome + full first `flowGroup` orphan reservation and later experience-record keep-together under pack), `flowSpacing.test.js`, `layoutDensity.test.js`, `SectionsPanel.test.js`, `floatingPanelPosition.test.js`, `test_flow_spacing.py`
...
- `frontend/src/utils/sectionBuilder.js`, `SECTION_LAYOUTS`; function `buildSectionElements` — layout constructors for "aa", "cc-sub", "cc-edu", and "cc-exp" (record field-line specs in private `recordLineSpecs`; heights via private `measureGeneratorBlockHeight`; content uses `bodyLeft`; image markers keep `src` / `alignWithText`)
...
- `frontend/src/hooks/useA4Elements.js`, function `handleAddSection` — optional `afterHeadingId`, style sampling, optional `iconName`, construction, placement, post-add selection; exposed through `PdfContext` as `addSection`
...
Applying a layout change calls the same `editElementValues` commit path as any manual content edit (just like `SectionRecordAdd` / `RecordBlockAdd` reuse existing structural-edit plumbing), so undo/redo, autosave, and the normal auto-height reflow (which already shifts later content when a textarea's measured height changes) all work with no new plumbing — switching to a taller bullet list pushes following sections down exactly as if the user had typed the extra lines by hand.
...
- `frontend/src/components/canvas/FlatSectionLayoutToggle/FlatSectionLayoutToggle.jsx` — hover affordance, structurally mirroring `SectionRecordAdd` / `RecordBlockAdd` (hover timing, exclusive visible slot via `useHoverPlusExclusive`, zoom-aware sizing via `recordPlusLayoutSize`) but rendering one bare icon instead of a two-cluster set
...
- `frontend/src/pages/Hero/Hero.jsx`, function `buildStartUrl` and component `CtaLink` — only `start=import` conditionally returns a `/register` URL; every other intent uses `getEditorPath({ start })`; `CtaLink` renders the primary / secondary / text-link chrome and queues the per-source funnel event on click
- `frontend/src/pages/Hero/Hero.module.css` — quiet-luxury white/ivory + `#171717` + gold `#B8954A` system with hard gold offset shadows; real-mockup compositions (`.heroStack` / `.editorMock` + floating `.aiCard`s / `.finalStack`) and the compact `.trustStrip`
...
- `backend/app/services/cv_templates/templates/axis.py`, function `_gen_axis` — masthead, `_timeline_record` (content column + `_place_gutter` overlays), `_place_skill_chips`, `_place_languages`, `_place_diamond_list`
...
### Aurelia contrasting Bézier name/title template

Aurelia is a paid, one-column template (`layouts: ["single"]`) with a quiet-luxury palette: warm white `#FEFDF9`, charcoal `#272724`, body grey `#464540`, muted grey `#77736B`, mist grey `#D6D6D3`, slate `#3A3A36`, stone `#5A5A54`, type white `#FFFFFF`, and antique gold `#B3924F` / `#8B713A` for the name accent, section bars, and page rails. It deliberately keeps body copy modest (`9.3` pt with `13.6` pt leading) and uses generous negative space instead of panels, cards, or a sidebar.

Its memorable visual is an abstract **thick contrasting Bézier name and title composition**. A `22` pt mist-grey vertical gesture on the right sits at `zIndex: 1`; a broad `44` pt slate stroke forms the organic nameplate at `zIndex: 2`; a `30` pt stone title plate sits under the job title at the same layer; and a `6` pt gold accent stroke adds a handmade top edge at `zIndex: 3`. Name and title stay at `zIndex: 4` in white, so glyphs read clearly over the dark fields. The plates use large bounding boxes (`52` / `36` pt tall) because their job is background mass and contrast, not hairline decoration. The element array is also emitted from back to front because PDF export paints in array order rather than relying only on CSS stacking.

The composition reacts to the displayed name and title. Name width is clamped to `180–435` px; title width to `140–360` px. The slate nameplate adds `56` px of breathing room, the stone title plate adds `48` px, the gold accent uses about half the name estimate, and the mist backdrop follows the right side while clamping its left edge to `x=425`. This keeps short strings visually substantial and prevents long ones from pushing artwork beyond the page. The `curves` coordinates stay in the 0–1 element box, making the React SVG preview and ReportLab `curveTo` export share the same geometry.
...
The deterministic Python generator uses the standard `Builder`, `need_section`, `keep_together` / `flowGroup`, experience, education, skills, and extra-section helpers. All four contrasting masthead brushstrokes carry `flowRole: "masthead"`; heading bars and calculated rules carry `flowRole: "section-chrome"`, so spacing changes and pagination move each ornament with the content it belongs to. The footer bars and asymmetric grey/gold rails are `fixedToPage` and repeat on continuation pages.

Implementation:

- `frontend/src/templates/aurelia.js`, export `aureliaTemplate` and function `sectionHeading` — thick contrasting name/title Bézier stack, white type, gold accent, back-to-front paint order, label-aware rules, restrained section bars, one-column sample content
- `frontend/src/templates/helpers.js`, lines 43–65, function `bezierPath` — template-authoring helper for normalized cubic paths
- `backend/app/services/cv_templates/templates/aurelia.py`, function `_gen_aurelia` and nested function `section` — dynamic slate/stone/gold masthead artwork, PDF-safe paint order, label-aware section chrome, compact shared records, repeating page decorations
...
Records, skills and languages deliberately reuse the same shared, stacked layout every other single-column template uses (`_place_experience_record`, `_place_education_record`, `_place_skills_section`, and the generic `_extra_sections` languages fallback) — title, then a muted meta line, then bullets; skills as one mid-dot inline line; languages as one bulleted "Name — Level" block. An earlier revision instead put the record date on the same row as the title (right-aligned, the source design's `.role` pattern) and drew skills/languages as individually positioned outline tags and badge rectangles. **Both were reverted after real breakage**: `sectionStructure.js`'s structural packer (`packDocumentSections` / `applyFlowSpacing`, which runs on every Add Section, section reorder, and Sections-panel rhythm change) always re-stacks a section's body elements sequentially by reading order — it has no concept of two elements sharing one visual row, only the exact-offset-preserving `flowRole: "section-chrome"` path (heading + rule) and the fully-exempt `flowRole: "masthead"` path (which is why the framed masthead below is safe). A direct repro confirmed it: after `appendSectionAtEnd`, a same-row date drifted 18px below its title and the tag-tray text drifted hundreds of pixels down the page. The fix is architectural, not a template-specific patch — stay inside the "one plain element per row" shape the packer already understands, matching every other working template.
...
### Manifest architectural sidebar template

Manifest is a paid, two-column template (`layouts: ["sidebar"]`) adapted from a Claude Design "Modernist" design-system project: near-mono red (`#EC3013`) on ink/paper, zero corner radius, strong 2px rules, a dark inverted header band (ink background, paper/accent type), and a light sidebar rail separated from the main column by a solid divider. Proportions carry the source design's own A4 page (authored in CSS px) over to this app's A4 canvas (points) via the standard 0.75 (72/96 dpi) ratio, since both are literally "A4" — e.g. the source's 240px sidebar becomes 180pt.

The header band spans the full page width with the name (large, paper-colored), an accent eyebrow ("CURRICULUM VITAE"), and an accent uppercase title on the left, and right-aligned contact lines on the right — every element tagged `flowRole: "masthead"`, so this two-column arrangement is exempt from section packing entirely (see the packer-safety note below). The sidebar (page 1 only) opens with an unconditionally placed Summary block, then fits Education / Languages / any other eligible extras into the remaining budget via the shared `_fit_sidebar_sections` / `_sidebar_candidates` mechanism Tessera already uses — Skills is explicitly excluded from that candidate list so it always renders in the main column, matching the source design. The main column reuses the same left-anchored heading + full-width rule shape every single-column template uses, with experience records numbered "01 · Role title" and a distinct org line (deep accent) plus an uppercase, letter-spaced period line.

**Packer-safety note** (a direct continuation of the discovery documented in `blueprint.py`): the source design's per-record oversized ordinal number sitting beside the role (a `grid-template-columns: 52px 1fr` row) and its per-language five-segment proficiency bar are both same-row, multi-element patterns — exactly the shape that corrupted under `sectionStructure.js`'s structural packer when Blueprint first shipped with a same-row date and individually positioned tag/badge elements. Both were designed out from the start here: the ordinal is folded into the title text ("01 · Senior AML Analyst…", one textarea) and language proficiency into the sidebar's plain bulleted "Name — Level" line — no new same-row risk was introduced. The **sidebar itself** is a different, already-proven-safe case: because it sits well to the left of the main column's headings (`sectionStructure.js`'s `sameColumnAsHeading`, `SIDEBAR_LEFT_GAP = 150` — sidebar content starts at `left: 42`, the main heading at `left: 212`, a 170px gap), it is excluded from the main-column packer's membership sweeps entirely, the same guarantee that makes the masthead safe. This was verified directly: a repro script ran `appendSectionAtEnd`, `applyFlowSpacing` (both default and compact rhythm), and `reorderSection` against a real two-page Manifest document, and the sidebar's summary/education/languages content was byte-identical before and after every operation. One more sidebar-specific pitfall: `sidebar_kicker()`'s heading labels are explicitly tagged `flowRole: "content"` — without it, a kicker's 2px rule directly beneath a short bold label would match `sectionStructure.js`'s untagged "heading + wide rule below" phantom-section heuristic (`isSectionHeading`), and because the false match's own `left` is far to the LEFT of the real main-column heading, `sameColumnAsHeading` evaluated from *that* phantom heading would treat main-column content as "the same column" and could pull it into a bogus sidebar section — a more dangerous failure mode than the phantom sections the same heuristic produced for Blueprint's language rows.

Paint-order note: this app's PDF export paints elements in **array order**, not by their `zIndex` (a canvas-editor-only stacking hint) — see `aurelia.py`'s equivalent note. The header band's ink background is therefore appended to the `header` array *before* the name/title/contact text drawn on top of it; getting this backwards silently paints the band over the text in the exported PDF while still looking correct in editors that do respect `zIndex`.

Font note: the source design specifies Archivo throughout. It is not among this app's 8 registered PDF/canvas font families (see Blueprint's font-note above for the full rationale behind substituting a registered grotesk). Roboto stands in here — a different registered grotesk than Blueprint's Inter, so the two templates read as visually distinct despite both substituting for an unavailable display sans.

Implementation:

- `backend/app/services/cv_templates/templates/manifest.py`, function `_gen_manifest` — inverted header band, `sidebar_kicker`, sidebar summary + `_fit_sidebar_sections` fitting, main-column `section()`, local `_place_record` for numbered experience rows (see the module docstring for the same-row-layout adaptations and why)
- `backend/app/services/cv_templates/registry.py`, `_GENERATORS["manifest"]` and `TEMPLATE_LAYOUTS["manifest"]` (`frozenset({"sidebar"})`)
- `frontend/src/templates/manifest.js` — static starter emitted directly from the generator's own demo output (no images, so no API_BASE_URL rewrite is needed); exported array `manifestTemplate`
- `frontend/src/templates/index.js`, registry entry `manifest` (`tier: "paid"`, `layouts: ["sidebar"]`, `accent: "#EC3013"`)
- `frontend/scripts/dump-iconic-templates.mjs`, `frontend/public/template-mockups/manifest.png` — source-driven A4 preview

Tests:

- `frontend/src/templates/manifest.test.js` — page/sidebar/divider decorations, header band painted before its own text (the array-order paint-order regression guard), masthead-exempt header content, sidebar kickers tagged `flowRole: "content"`, plain bulleted languages (not a segmented bar), left-anchored main headings on a full-width rule, numbered records with the ordinal folded into the title, one mid-dot skills line, zero `rectangle` elements anywhere
- `frontend/src/templates/manifest.pack.test.js` (with `manifest.multipage.fixture.json`) — a real two-page, four-job Manifest document: main-column headings stay glued to their bodies through `listDocumentSections` / `sectionElementIds` after `applyFlowSpacing` (default and compact rhythm) and after `reorderSection`, and the sidebar snapshot is asserted byte-identical across all three operations
...
- `frontend/src/utils/textareaReflow.test.js` — Iconic grouping, explicit `flowRole` values, keep-heading-with-body, stale-page gaps, chrome rhythm, non-collapsing record spacing, page-2 section reclaim that reserves chrome + `SPACE_SECTION` when the body grows, Harbor's narrow-gutter column isolation, and current/legacy horizontal overlay alignment
...
- `backend/app/services/cv_templates/shared/text.py`, `_skills_inline_content`; `_skills_sidebar_content`; `_place_skills_section` — parent heading + nested category labels; each category+chips pair uses `keep_together` / `flowGroup` so canvas rhythm keeps the inner gap as `stack` (4 px), not `record` (10 px)
...
- `backend/app/api/routes/ai_assistant.py` — `VALID_ACTIONS` (+ `translate`), `target_language`, `AssistantResponse.categories` / `strengths` / `priorities`; passes `db` into `analyze_action` for ATS image resolution
- `backend/app/services/ats_readability.py` — PDF render + PyMuPDF extract + deterministic ATS categories / weighted overall
- `backend/app/services/ai_assistant_service.py` — `_annotate_employment_tense` / `employment_tense` on structured elements; `_detect_language_mix` / `_ensure_language_mix_feedback` for bilingual header/body consistency; `_safe_result` structured scores; `_rate_cv` / `_rate_design` / `_rate_position` / `_ats_score` hybrid ATS merge; `_check_style` / `_improve_content` tense + language-mix rules; `_translate_cv`; `_layout_session`; `_chat`
...
- `backend/app/services/entitlements.py`, lines 33–68 (`PRO_PASS_DAYS`, `PLAN_SEEDS`, `CREDIT_PLN`), 75–78 (`normalize_plan_slug`), 139–164 (`migrate_legacy_plans_to_pro`), 232–263 (`PLAN_DISPLAY`), 266–298 (`list_selectable_plans`), 301–319 (`set_user_plan` — 30-day pass + credit reset), 490–511 (`assert_can_use_ai_action`), `get_entitlements`, `assert_can_export`, `charge_ai_credits`
...
**Zaimplementowane:** edytor (w tym tryb gościa bez konta), szablony, extract/fill, szkic bio, asystent AI (cele użytkownika, dashboard oceny, tłumaczenie, karty układu), entitlements (Darmowy / Pro — 59 zł / 30 dni), autozapis, dysk lokalny lub S3, JWT.
...
- `frontend/src/utils/sectionBuilder.js`, `SECTION_LAYOUTS`; funkcja `buildSectionElements` — konstruktory układów „aa”, „cc-sub”, „cc-edu” i „cc-exp” (specyfikacje linii rekordu w prywatnym `recordLineSpecs`; wysokości przez prywatne `measureGeneratorBlockHeight`; treść na `bodyLeft`; markery obrazu zachowują `src` / `alignWithText`)
...
- `frontend/src/hooks/useA4Elements.js`, funkcja `handleAddSection` — opcjonalne `afterHeadingId`, próbkowanie stylu, opcjonalne `iconName`, budowa, umieszczenie, zaznaczenie; wystawiana przez `PdfContext` jako `addSection`
...
Strona główna jest nastawiona na konwersję i jeden rezultat — edytowalne CV do PDF. Kolejność sekcji: header → hero → przed/po → jak to działa (trzy kroki) → szablony → edytor + AI → pasek zaufania (prywatność) → cennik → FAQ → końcowe CTA → stopka. Każdy wizual produktu to **realny mockup szablonu** z `frontend/public/template-mockups/` (bez zdjęć stockowych i bez placeholderów): hero pokazuje dwa nachodzące mockupy A4 (Portico na Monument) z dynamiczną liczbą szablonów, karta „PO” w sekcji przed/po oraz sekcja edytora renderują prawdziwe mockupy, a końcowe CTA nakłada trzy mockupy na niemal czarnym panelu. Dawna sekcja trzech kart „Zacznij tak, jak Ci wygodnie”, pełnoekranowa sekcja prywatności oraz stockowe końcowe CTA (`women-job-call.png`) zostały usunięte; ich istotne informacje trafiły odpowiednio do hero, zwartego paska zaufania i nowego końcowego CTA.
...
- `frontend/src/pages/Hero/Hero.jsx`, funkcja `buildStartUrl` i komponent `CtaLink` — tylko `start=import` warunkowo zwraca URL `/register`; każda inna intencja używa `getEditorPath({ start })`; `CtaLink` renderuje wariant primary / secondary / link i kolejkuje zdarzenie lejka z danym źródłem
- `frontend/src/pages/Hero/Hero.module.css` — system quiet-luxury biel/kość słoniowa + `#171717` + złoto `#B8954A` z twardymi złotymi cieniami offset; kompozycje z realnych mockupów (`.heroStack` / `.editorMock` + pływające `.aiCard` / `.finalStack`) i zwarty `.trustStrip`
...
- `backend/app/services/cv_templates/templates/axis.py`, funkcja `_gen_axis` — masthead, `_timeline_record` (kolumna treści + nakładki `_place_gutter`), `_place_skill_chips`, `_place_languages`, `_place_diamond_list`
...
### Szablon Aurelia z kontrastowym motywem Béziera (imię + stanowisko)

Aurelia to płatny, jednokolumnowy szablon (`layouts: ["single"]`) w estetyce quiet luxury: ciepła biel `#FEFDF9`, grafit `#272724`, szarość treści `#464540`, muted grey `#77736B`, mglista szarość `#D6D6D3`, slate `#3A3A36`, stone `#5A5A54`, biel typografii `#FFFFFF` oraz antyczne złoto `#B3924F` / `#8B713A` dla akcentu nazwiska, belek sekcji i szyn strony. Tekst body jest celowo skromny (`9.3` pt z interlinią `13.6` pt), a hierarchię buduje oddech zamiast paneli, kart czy sidebaru.

Zapamiętywalnym motywem jest abstrakcyjna **gruba, kontrastowa kompozycja nazwiska i stanowiska z Bézierów**. Pionowy gest w mglistej szarości po prawej ma obrys `22` pt i `zIndex: 1`; szeroka slate’owa kreska `44` pt tworzy organiczny nameplate na `zIndex: 2`; stone title plate `30` pt leży pod stanowiskiem na tej samej warstwie; a złoty ślad `6` pt dodaje ręcznie rysowaną górną krawędź na `zIndex: 3`. Nazwisko i stanowisko pozostają na `zIndex: 4` w bieli, dlatego glify są czytelne na ciemnych polach. Płyty mają duże ramki (`52` / `36` pt wysokości), bo ich rolą jest masa tła i kontrast, a nie cienka dekoracja. Tablica elementów jest również emitowana od tła do tekstu, ponieważ eksport PDF maluje według kolejności tablicy, a nie wyłącznie według warstw CSS.

Kompozycja reaguje na wyświetlane nazwisko i stanowisko. Szerokość nazwiska jest ograniczona do `180–435` px, stanowiska do `140–360` px. Slate nameplate dodaje `56` px oddechu, stone title plate `48` px, złoty akcent używa około połowy oszacowania nazwiska, a szare tło podąża za prawą częścią i ogranicza początek do `x=425`. Dzięki temu krótkie napisy zachowują obecność, a długie nie wypychają grafiki poza stronę. Współrzędne `curves` pozostają w zakresie 0–1 ramki elementu, więc SVG Reacta i eksport ReportLab `curveTo` dzielą jedną geometrię.
...
Deterministyczny generator Python używa standardowych mechanizmów `Builder`, `need_section`, `keep_together` / `flowGroup` oraz helperów doświadczenia, edukacji, umiejętności i sekcji dodatkowych. Wszystkie cztery kontrastowe pociągnięcia mastheadu mają `flowRole: "masthead"`, a belki i obliczane linie nagłówków `flowRole: "section-chrome"`, więc zmiany odstępów i paginacja przesuwają ornament razem z należącą do niego treścią. Belki stopki oraz asymetryczne szaro-złote szyny są `fixedToPage` i powtarzają się na kolejnych stronach.

Implementacja:

- `frontend/src/templates/aurelia.js`, eksport `aureliaTemplate` i funkcja `sectionHeading` — gruby kontrastowy stos Bézierów imię/stanowisko, biały tekst, złoty akcent, kolejność malowania od tła, linie zależne od etykiety, oszczędne belki sekcji, przykładowa treść jednokolumnowa
...
- `backend/app/services/cv_templates/templates/aurelia.py`, funkcja `_gen_aurelia` i zagnieżdżona funkcja `section` — dynamiczna grafika slate/stone/gold, kolejność malowania bezpieczna dla PDF, chrome sekcji zależny od etykiety, zwarte wspólne rekordy, powtarzalne dekoracje strony
...
Rekordy, umiejętności i języki celowo reużywają ten sam współdzielony, warstwowy układ, jakiego używa każdy inny jednokolumnowy szablon (`_place_experience_record`, `_place_education_record`, `_place_skills_section` oraz generyczny fallback języków w `_extra_sections`): tytuł, potem stonowana linia meta, potem punkty; umiejętności jako jedna linia tekstu z kropkami; języki jako jeden blok wypunktowany „Nazwa — Poziom”. Wcześniejsza wersja umieszczała datę rekordu w tym samym wierszu co tytuł (wyrównaną do prawej, wzorzec `.role` z projektu źródłowego), a umiejętności/języki rysowała jako indywidualnie pozycjonowane tagi konturowe i prostokąty odznak. **Obie zostały wycofane po realnym uszkodzeniu układu**: strukturalny packer w `sectionStructure.js` (`packDocumentSections` / `applyFlowSpacing`, uruchamiany przy każdym dodaniu sekcji, zmianie kolejności sekcji i zmianie rytmu w panelu Sekcje) zawsze przepakowuje elementy treści sekcji SEKWENCYJNIE według kolejności czytania — nie ma pojęcia o dwóch elementach dzielących jeden wiersz wizualny, poza ścieżką zachowującą dokładne przesunięcia dla `flowRole: "section-chrome"` (nagłówek + linia) oraz w pełni zwolnioną ścieżką `flowRole: "masthead"` (dlatego oprawiony masthead poniżej jest bezpieczny). Bezpośrednia reprodukcja to potwierdziła: po `appendSectionAtEnd` data w tym samym wierszu przesunęła się 18px poniżej tytułu, a tekst tacki tagów przesunął się o setki pikseli w dół strony. Poprawka jest architektoniczna, nie łatką specyficzną dla szablonu — pozostać w kształcie „jeden zwykły element na wiersz”, który packer już rozumie, tak jak każdy inny działający szablon.
...
### Szablon Manifest (architektoniczny sidebar)

Manifest to płatny, dwukolumnowy szablon (`layouts: ["sidebar"]`) zaadaptowany z projektu Claude Design opartego na systemie projektowym „Modernist”: prawie-mono czerwień (`#EC3013`) na tuszu/papierze, zerowy promień narożnika, mocne linie 2px, ciemny odwrócony pasek nagłówka (tło w kolorze tuszu, typografia w kolorze papieru/akcentu) oraz jasny pasek boczny oddzielony od kolumny głównej pełną linią. Proporcje przenoszą własną stronę A4 projektu źródłowego (autorowaną w CSS px) na kanwę A4 tej aplikacji (punkty) przez standardowy współczynnik 0.75 (72/96 dpi), skoro obie strony są dosłownie „A4” — np. sidebar o szerokości 240px z projektu źródłowego staje się 180pt.

Pasek nagłówka rozciąga się na pełną szerokość strony: imię i nazwisko (duże, w kolorze papieru), akcentowy eyebrow („CURRICULUM VITAE”) i akcentowy tytuł wielkimi literami po lewej, a wyrównane do prawej linie kontaktu po prawej — każdy element oznaczony `flowRole: "masthead"`, więc ten dwukolumnowy układ jest całkowicie zwolniony z packingu sekcji (patrz uwaga o bezpieczeństwie packera poniżej). Sidebar (tylko strona 1) zaczyna się bezwarunkowo umieszczonym blokiem Podsumowania, a następnie dopasowuje Wykształcenie / Języki / dowolne inne kwalifikujące się sekcje dodatkowe do pozostałego budżetu przez współdzielony mechanizm `_fit_sidebar_sections` / `_sidebar_candidates`, którego już używa Tessera — Umiejętności są jawnie wykluczone z tej listy kandydatów, więc zawsze renderują się w kolumnie głównej, zgodnie z projektem źródłowym. Kolumna główna reużywa ten sam zakotwiczony do lewej kształt nagłówka + linia na pełną szerokość, jakiego używa każdy jednokolumnowy szablon, z numerowanymi rekordami doświadczenia „01 · Tytuł stanowiska” i osobną linią organizacji (głęboki akcent) oraz wielkoliterową, rozstrzeloną linią okresu.

**Uwaga o bezpieczeństwie packera** (bezpośrednia kontynuacja odkrycia opisanego w `blueprint.py`): oversized numer porządkowy obok stanowiska w projekcie źródłowym (wiersz `grid-template-columns: 52px 1fr`) oraz pięciosegmentowy pasek biegłości językowej to oba wzorce jednowierszowe, wieloelementowe — dokładnie ten kształt, który uszkodził się pod strukturalnym packerem `sectionStructure.js`, gdy Blueprint po raz pierwszy wystartował z datą w tym samym wierszu i indywidualnie pozycjonowanymi elementami tagów/odznak. Oba zostały zaprojektowane bezpiecznie od samego początku: numer porządkowy jest wtopiony w tekst tytułu („01 · Senior AML Analyst…”, jedna textarea), a poziom języka w zwykłą wypunktowaną linię sidebaru „Nazwa — Poziom” — nie wprowadzono żadnego nowego ryzyka jednowierszowego. Sam **sidebar** to inny, już sprawdzony bezpieczny przypadek: ponieważ leży wystarczająco daleko na lewo od nagłówków kolumny głównej (`sameColumnAsHeading` w `sectionStructure.js`, `SIDEBAR_LEFT_GAP = 150` — treść sidebaru zaczyna się na `left: 42`, nagłówek główny na `left: 212`, odstęp 170px), jest całkowicie wykluczony z przemiatań przynależności packera kolumny głównej — ta sama gwarancja, która czyni masthead bezpiecznym. Zostało to zweryfikowane bezpośrednio: skrypt reprodukcyjny uruchomił `appendSectionAtEnd`, `applyFlowSpacing` (rytm domyślny i kompaktowy) oraz `reorderSection` na realnym dwustronicowym dokumencie Manifest, a treść podsumowania/wykształcenia/języków w sidebarze była identyczna co do bajta przed i po każdej operacji. Jeszcze jedna pułapka specyficzna dla sidebaru: etykiety nagłówków w `sidebar_kicker()` są jawnie otagowane `flowRole: "content"` — bez tego, linia 2px bezpośrednio pod krótką pogrubioną etykietą pasowałaby do nieoznaczonej heurystyki fantomowej sekcji „nagłówek + szeroka linia poniżej” w `sectionStructure.js` (`isSectionHeading`), a ponieważ `left` fałszywego dopasowania leży daleko na LEWO od prawdziwego nagłówka kolumny głównej, `sameColumnAsHeading` liczone od TEGO fantomowego nagłówka potraktowałoby treść kolumny głównej jako „tę samą kolumnę” i mogłoby wciągnąć ją do fałszywej sekcji sidebaru — bardziej niebezpieczny tryb awarii niż fantomowe sekcje, które ta sama heurystyka wytworzyła dla wierszy języków w Blueprint.

Uwaga o kolejności malowania: eksport PDF tej aplikacji maluje elementy w **kolejności tablicy**, a nie według `zIndex` (wskazówki dotyczącej warstwowania tylko w edytorze canvas) — patrz odpowiednia uwaga w `aurelia.py`. Tło paska nagłówka jest więc dodawane do tablicy `header` *przed* tekstem imienia/tytułu/kontaktu malowanym na nim; odwrócenie tej kolejności po cichu maluje pasek nad tekstem w wyeksportowanym PDF, mimo że nadal wygląda poprawnie w edytorach respektujących `zIndex`.

Uwaga o czcionce: projekt źródłowy wskazuje Archivo wszędzie. Nie należy ona do 8 zarejestrowanych rodzin czcionek PDF/canvas tej aplikacji (pełne uzasadnienie substytucji zarejestrowanym grotesk — patrz uwaga o czcionce przy Blueprint). Tutaj zastępuje ją Roboto — inny zarejestrowany grotesk niż Inter w Blueprint, dzięki czemu oba szablony czytają się jako wizualnie odrębne, mimo że oba zastępują niedostępny display sans.

Implementacja:

- `backend/app/services/cv_templates/templates/manifest.py`, funkcja `_gen_manifest` — odwrócony pasek nagłówka, `sidebar_kicker`, podsumowanie w sidebarze + dopasowanie `_fit_sidebar_sections`, `section()` kolumny głównej, lokalne `_place_record` dla numerowanych wierszy doświadczenia (przyczyny adaptacji układu jednowierszowego opisane w docstringu modułu)
- `backend/app/services/cv_templates/registry.py`, `_GENERATORS["manifest"]` i `TEMPLATE_LAYOUTS["manifest"]` (`frozenset({"sidebar"})`)
- `frontend/src/templates/manifest.js` — statyczny starter emitowany bezpośrednio z wyjścia demo generatora (bez obrazów, więc bez przepisywania API_BASE_URL); eksportowana tablica `manifestTemplate`
- `frontend/src/templates/index.js`, wpis rejestru `manifest` (`tier: "paid"`, `layouts: ["sidebar"]`, `accent: "#EC3013"`)
- `frontend/scripts/dump-iconic-templates.mjs`, `frontend/public/template-mockups/manifest.png` — podgląd A4 generowany ze źródła

Testy:

- `frontend/src/templates/manifest.test.js` — dekoracje strony/sidebaru/dividera, pasek nagłówka namalowany przed swoim tekstem (guard regresji kolejności malowania w tablicy), treść nagłówka zwolniona z packingu jako masthead, kickery sidebaru otagowane `flowRole: "content"`, zwykłe wypunktowane języki (nie segmentowany pasek), lewostronnie zakotwiczone nagłówki główne nad linią na pełną szerokość, numerowane rekordy z numerem porządkowym wtopionym w tytuł, jedna linia umiejętności z kropkami, zero elementów `rectangle` w całym dokumencie
- `frontend/src/templates/manifest.pack.test.js` (z `manifest.multipage.fixture.json`) — realny dwustronicowy dokument Manifest z czterema stanowiskami: nagłówki kolumny głównej pozostają przyklejone do swojego body w `listDocumentSections` / `sectionElementIds` po `applyFlowSpacing` (rytm domyślny i kompaktowy) oraz po `reorderSection`, a migawka sidebaru jest identyczna co do bajta we wszystkich trzech operacjach
...
- `frontend/src/utils/textareaReflow.test.js` — grupowanie Iconic, jawne wartości `flowRole`, keep-heading-with-body, stale-page gaps, rytm chrome, niekolidujące odstępy rekordów, reclaim sekcji ze strony 2 z rezerwacją chrome + `SPACE_SECTION` przy uroście body, izolacja kolumn Harbor przy wąskim gutterze oraz wyrównanie nowych i starszych nakładek poziomych
...
- `backend/app/services/cv_templates/shared/text.py` — `_skills_inline_content`; `_skills_sidebar_content`; `_place_skills_section` (nagłówek nadrzędny + etykiety kategorii; każda para kategoria+chipy w `keep_together` / `flowGroup`, żeby rytm kanwy trzymał wewnętrzny odstęp jako `stack` 4 px, nie `record` 10 px)
...
- `backend/app/api/routes/ai_assistant.py` — `translate`, `target_language`, `categories` / `strengths` / `priorities`
- `backend/app/services/ai_assistant_service.py` — `_annotate_employment_tense` / `employment_tense`; `_detect_language_mix` / `_ensure_language_mix_feedback` dla spójności PL/EN nagłówków i treści; `_ats_score` hybrydowy merge ATS; strukturalne wyniki ocen; reguły czasu i języka w `_check_style` / `_improve_content`; `_translate_cv`, `_layout_session`, `_chat`
- `backend/app/services/entitlements.py`, `PRO_ONLY_AI_ACTIONS` = `{design_rating, layout}` — **Sprawdź wygląd**
...
- `backend/app/services/entitlements.py`, linie 33–68 (`PRO_PASS_DAYS`, `PLAN_SEEDS`, `CREDIT_PLN`), 75–78 (`normalize_plan_slug`), 139–164 (`migrate_legacy_plans_to_pro`), 232–263 (`PLAN_DISPLAY`), 266–298 (`list_selectable_plans`), 301–319 (`set_user_plan`), 490–511 (`assert_can_use_ai_action`)
