...
- `backend/app/services/cv_templates/templates/nimbus.py`, `_gen_nimbus` — Lora masthead (name/photo/contact), 3 px rules, 56 px clearance under the header divider, heading+underline chrome only (no rail/chip markers); `test_nimbus_keeps_education_record_with_heading_near_page_break`
...
- `backend/app/services/cv_templates/templates/nimbus.py`, `_gen_nimbus` — masthead Lora (imię/zdjęcie/kontakt), linie 3 px, 56 px odstępu pod dzielnikiem, chrome nagłówka = etykieta + podkreślenie (bez rail/chip); `test_nimbus_keeps_education_record_with_heading_near_page_break`
