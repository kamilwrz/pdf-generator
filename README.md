...
- `frontend/src/components/editor/StartChooser/StartChooser.module.css`, lines 9–422 — Swiss/grid styling with an absolute canvas-only overlay, visible CV Studio brand, rectilinear axis rules, a two-card primary grid, secondary action row, mobile collapse, and bottom-left logout control; Topbar and the 56/72 px tool rail remain available
...
- `frontend/src/App.css`, lines 1–120 — viewport shell, canvas gutters, neutral scrollbars, and compact breakpoint; lines 123–351 keep document typography isolated inside `.page-canvas`
...
- `frontend/src/components/editor/StartChooser/StartChooser.module.css`, lines 9–422; `frontend/src/components/editor/SectionsPanel/SectionsPanel.module.css`, lines 3–161; `frontend/src/components/gallery/Gallery/Gallery.module.css`, lines 1–142; `frontend/src/components/ai/AiAssistant/AiAssistant.module.css`, lines 2–1319 — compact desktop chrome and mobile drawers/sheets
- `frontend/src/hooks/usePdfExport.js`, lines 24–253, hook `usePdfExport`; `frontend/src/pages/PdfCanvas.jsx`, lines 1878–1970, component `PdfCanvas` — document-only serialization and separation of editor overlays from export data
...
- Empty document/template galleries and StartChooser provide explicit next actions without replacing editor chrome. The chooser is covered by `frontend/src/utils/startChooser.test.js`, lines 13–68, and `frontend/src/utils/demoModeChrome.test.js`, lines 46–64.
...
- `frontend/src/components/editor/StartChooser/StartChooser.module.css`, linie 9–422 — styl Swiss/grid z absolutnym overlayem ograniczonym do canvasa, znakiem CV Studio, prostokreślnymi osiami, gridem dwóch głównych kart, wierszem akcji drugorzędnych, układem mobilnym i ikoną wylogowania w lewym dolnym rogu; Topbar oraz szyna 56/72 px pozostają dostępne
...
- `frontend/src/App.css`, linie 1–120 — shell viewportu, rynny canvasa, neutralne scrollbary i breakpoint compact; linie 123–351 izolują typografię dokumentu wewnątrz `.page-canvas`
...
- `frontend/src/components/editor/StartChooser/StartChooser.module.css`, linie 9–422; `frontend/src/components/editor/SectionsPanel/SectionsPanel.module.css`, linie 3–161; `frontend/src/components/gallery/Gallery/Gallery.module.css`, linie 1–142; `frontend/src/components/ai/AiAssistant/AiAssistant.module.css`, linie 2–1319 — zwarte chrome desktopowe i mobilne drawery/sheets
- `frontend/src/hooks/usePdfExport.js`, linie 24–253, hook `usePdfExport`; `frontend/src/pages/PdfCanvas.jsx`, linie 1878–1970, komponent `PdfCanvas` — serializacja wyłącznie dokumentu oraz separacja overlayów edytora od danych eksportu
...
- Puste galerie dokumentów/szablonów i StartChooser pokazują jawny następny krok bez zastępowania chrome edytora. Zachowanie wyboru obejmują `frontend/src/utils/startChooser.test.js`, linie 13–68, oraz `frontend/src/utils/demoModeChrome.test.js`, linie 46–64.
