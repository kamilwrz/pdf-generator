...
6. **Bio wizard** → the four-step fullscreen data creator (`BioCvModal`) is shared by the landing CTA and demo conversion. It has no template-selection step. Authenticated users use draft CRUD on `/ai/bio_cv_draft`; guests autosave the wizard profile to `localStorage` (`cvstudio.guest.wizardDraft` via `guestWizardDraft.js`). After either registration flow, the guest snapshot is adopted and `POST /ai/fill_template` generates the Regent before the full editor opens.
...
6. **Kreator bio** → wspólny, czterostopniowy kreator danych (`BioCvModal`) bez wyboru szablonu. Zalogowani używają CRUD `/ai/bio_cv_draft`; goście zapisują profil do `localStorage` (`cvstudio.guest.wizardDraft` przez `guestWizardDraft.js`). Po rejestracji/logowaniu snapshot jest przejmowany, a `POST /ai/fill_template` generuje Regenta przed otwarciem pełnego edytora.
...
W zwykłym edytorze wejścia w topbarze to **Importuj CV**, **Utwórz CV krok po kroku** oraz **Szablony** (aktywne po udanym fillu). Kontrolka szablonów stoi nad żywą lewą krawędzią A4; ikona otwiera modal zmiany szablonu, a strzałki przełączają dozwolone szablony w miejscu. Tryb demo produktu `?start=demo` ma osobny, ograniczony chrome: zachowuje undo/redo, zmianę szablonu, **Układ CV**, zoom i nawigację stron, a ukrywa nazwę projektu, import, zapis, pobieranie, czyszczenie i akcje konta. **Stwórz moje CV** otwiera czterostopniowy kreator danych; wybór szablonu i generowanie układu czekają do rejestracji lub logowania.
