...
- `frontend/src/templates/cadenza.test.js`, lines 133–224; `backend/tests/test_text_transform.py`, lines 33–61 — complete band/accent construction plus short, medium, and long heading values re-centred after consecutive input updates in the canvas model and PDF renderer
...
- `frontend/src/utils/sectionStructure.js`, lines 475–655, private functions `resolveSectionChromeBandStart`, `isFilledSectionBand`, and `isMatchingSectionBandAccent`; lines 831–918, function `sectionElementIds`; lines 940–984, function `normalizeFilledBandSectionHeading`; lines 1588–1714, private `compactChromeCluster`; lines 3030–3317, function `deriveSectionStyle` — explicit main-chrome ownership, filled-band boundary recovery, persisted accent/heading repair, rigid cluster packing, and full-band alignment sampling used by Add Section
...
- `frontend/src/templates/cadenza.test.js`, lines 76–131 — starter palette, masthead, full-width centred section labels, copper markers, date rail, and icon paths; lines 133–178 — Add Section reproduces the complete band/accent/alignment cluster; lines 180–224 — legacy headings re-centre after every input value; lines 226–308 — idempotent repeated spacing, repair of persisted displaced accents, blank-page prevention, record reorder, and section reorder
...
- `frontend/src/templates/linden.js`, lines 1–1151, export `lindenTemplate`; `frontend/src/templates/index.js`, lines 20–30 — generated starter with the main-flow anchor, uniform sidebar hairlines, and no redundant upper body rule, plus the paid picker entry.
...
- `frontend/src/templates/cadenza.test.js`, linie 133–224; `backend/tests/test_text_transform.py`, linie 33–61 — budowa kompletnego pasa/akcentu oraz krótkie, średnie i długie wartości nagłówka centrowane ponownie po kolejnych zmianach inputu w modelu canvasu i rendererze PDF
...
- `frontend/src/utils/sectionStructure.js`, linie 475–655, prywatne funkcje `resolveSectionChromeBandStart`, `isFilledSectionBand` i `isMatchingSectionBandAccent`; linie 831–918, funkcja `sectionElementIds`; linie 940–984, funkcja `normalizeFilledBandSectionHeading`; linie 1588–1714, prywatna `compactChromeCluster`; linie 3030–3317, funkcja `deriveSectionStyle` — jawna własność chrome kolumny głównej, odzyskiwanie granicy wypełnionego pasa, naprawa zapisanego przesunięcia akcentu/nagłówka, sztywne pakowanie klastra oraz próbkowanie pełnoszerokiej ramki używane przez Dodaj sekcję
...
- `frontend/src/templates/cadenza.test.js`, linie 76–131 — paleta startera, masthead, pełnoszeroko centrowane etykiety sekcji, miedziane znaczniki, pas dat i ścieżki ikon; linie 133–178 — Dodaj sekcję odtwarza kompletny klaster pasa/akcentu/wyrównania; linie 180–224 — legacy nagłówki centrują się ponownie po każdej wartości inputu; linie 226–308 — idempotentne powtarzanie odstępów, naprawa zapisanych przesuniętych akcentów, zapobieganie pustym stronom oraz zmiana kolejności rekordu i sekcji
...
- `frontend/src/templates/linden.js`, linie 1–1151, eksport `lindenTemplate`; `frontend/src/templates/index.js`, linie 20–30 — wygenerowany starter z anchorem głównego flow, jednolitymi hairlines sidebara i bez zbędnej górnej linii treści oraz płatny wpis w pickerze.
