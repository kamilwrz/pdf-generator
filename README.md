...
Language regressions additionally cover a three-cell horizontal grid through extraction, grounding, normalization, and Meridian rendering; a CV30-style centred Languages heading followed by **Computer skills**, where a deliberately wrong provider response must be replaced with the three source languages; rejection of a model-only language on a fully native PDF with no Languages section; and safe vision passthrough that ignores nameless/bare-level provider rows (`backend/tests/test_cloudflare_cv_extraction.py`, lines 1076–1241, tests `test_horizontal_language_grid_keeps_all_source_languages`, `test_centred_languages_stop_before_computer_skills_heading`, `test_native_pdf_without_languages_rejects_model_invention`, and `test_vision_language_grounding_keeps_named_provider_rows_only`). Pipe-delimited multi-language normalization plus preservation of semicolon-bearing structured level notes are covered in `backend/tests/test_cv_data.py`, lines 471–514, tests `test_pipe_delimited_language_row_becomes_separate_entries` and `test_structured_language_levels_keep_detail_punctuation`.
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, komponent `SectionsPanel` oraz funkcje `handleAppearancePalette` / `handleAppearanceTextSize` — ścisła bramka Atrium/Regent/Sterling/Linden/Monument/Slate/Meridian/Cadenza/Vellum, dostępne radiogrupy właściwe dla szablonu i transakcja S–XL; `SectionsPanel.module.css`, wspólna `paletteGrid` oraz klasy Regenta `palettePaperRegent`, `paletteRegentName`, `paletteRegentContacts`, `paletteRegentSectionRule` i `paletteRegentFolio` oraz dotychczasowe miniatury szablonów; testy: `SectionsPanel.test.js`
