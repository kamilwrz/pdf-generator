...
The provider is called through Cloudflare's OpenAI-compatible base URL, using the existing `openai` Python SDK. Gemma 4 and Qwen 3.8 are reasoning models, so Cloudflare requests use the current `max_completion_tokens` parameter and `reasoning_effort=low`; this preserves the configured 8,000-token completion budget for visible CV JSON instead of allowing hidden reasoning to exhaust it. Cloudflare's published JSON Mode allowlist does not currently include either selected model, so their request deliberately omits `response_format` and enforces JSON through the extraction prompt plus a strict parser. The parser accepts one plain or Markdown-fenced JSON object and typed text parts, but rejects arbitrary/non-object output. The explicit OpenAI rollback keeps `response_format=json_object`.

Client construction is lazy: missing Cloudflare credentials do not prevent `/health`, authentication, editing, or PDF export from starting. `CV_EXTRACT_PROVIDER=openai` and `CV_EXTRACT_OPENAI_MODEL=gpt-4o` are an explicit rollback path, not an automatic fallback; a Cloudflare outage therefore cannot silently send a user's CV to another processor. Provider rate limits become a retryable 429, temporary/provider failures become 503, an empty visible completion becomes a retryable 502, and malformed/non-object CV JSON becomes 422. Empty-response diagnostics log only provider, model, extraction mode, finish reason, whether reasoning existed, and token count—never CV text, reasoning content, raw responses, or credentials. Raw provider exceptions are never returned to the browser.
...
Tests: `backend/tests/test_cloudflare_cv_extraction.py`, lines 43–201, class `CloudflareCvExtractionTests`, covers text-only requests, scan-page vision fallback, Cloudflare request parameters, fenced/typed content, safe reasoning-only failures, OpenAI rollback JSON Mode, invalid model JSON, and missing credentials without a network call. `backend/tests/test_extract_cv_rejection.py` covers monthly quota and failure-not-consuming behaviour. `frontend/src/utils/cvImportRequest.test.js`, lines 1–21, locks the four-minute no-retry contract and all import-history status labels. `backend/tests/test_cv_data.py` covers nested skills normalization.

Official references: [Workers AI REST setup](https://developers.cloudflare.com/workers-ai/get-started/rest-api/) explains token permissions; [OpenAI compatibility](https://developers.cloudflare.com/workers-ai/configuration/open-ai-compatibility/) documents the `/ai/v1` endpoint; [Gemma 4](https://developers.cloudflare.com/workers-ai/models/gemma-4-26b-a4b-it/) and [Qwen 3.8](https://developers.cloudflare.com/workers-ai/models/qwen3.8-27b/) document reasoning and `max_completion_tokens`; [JSON Mode](https://developers.cloudflare.com/workers-ai/features/json-mode/) provides the current supported-model list; [Workers AI pricing](https://developers.cloudflare.com/workers-ai/platform/pricing/) explains the account-wide daily Free neuron allocation.
...
- `backend/tests/test_cloudflare_cv_extraction.py`, lines 43–201, class `CloudflareCvExtractionTests` — provider selection, text/vision payloads, reasoning budget, prompt-only/fenced JSON, typed content, safe empty responses, rollback JSON Mode, and server-side credential gating
...
- **CV import returns `extract_provider_empty_response` (502):** deploy the current backend, which uses `max_completion_tokens`, low reasoning, and omits unsupported JSON Mode for Gemma/Qwen. If it persists, inspect the sanitized `cv_extraction` warning for model/mode, `finish_reason`, reasoning presence, and completion-token count; the log intentionally contains no CV content.
...
Dostawca jest wywoływany przez kompatybilny z OpenAI adres bazowy Cloudflare przy użyciu istniejącego SDK `openai`. Gemma 4 i Qwen 3.8 są modelami reasoningowymi, dlatego żądania Cloudflare używają aktualnego parametru `max_completion_tokens` oraz `reasoning_effort=low`; dzięki temu większość skonfigurowanego budżetu 8000 tokenów zostaje na widoczny JSON CV zamiast ukrytego rozumowania. Oficjalna allowlista Cloudflare JSON Mode nie zawiera obecnie tych dwóch modeli, więc żądanie celowo pomija `response_format`, a JSON jest wymuszany ścisłym promptem i parserem. Parser przyjmuje jeden zwykły albo opakowany fence'em obiekt JSON oraz typowane fragmenty tekstowe, lecz odrzuca dowolny tekst i JSON niebędący obiektem. Jawny rollback OpenAI zachowuje `response_format=json_object`.

Klient powstaje leniwie: brak danych Cloudflare nie blokuje startu `/health`, logowania, edycji ani eksportu PDF. `CV_EXTRACT_PROVIDER=openai` wraz z `CV_EXTRACT_OPENAI_MODEL=gpt-4o` to jawna ścieżka rollbacku, nie automatyczny fallback; awaria Cloudflare nie może więc po cichu wysłać CV do innego procesora. Limit dostawcy daje ponawialny 429, przejściowa awaria lub konfiguracja — 503, pusta widoczna odpowiedź — ponawialny 502, a niepoprawny lub nieobiektowy JSON — 422. Diagnostyka pustej odpowiedzi zapisuje wyłącznie dostawcę, model, tryb ekstrakcji, `finish_reason`, obecność reasoningu i liczbę tokenów — nigdy treść CV, treść rozumowania, surową odpowiedź ani dane dostępowe. Surowe wyjątki dostawcy nie trafiają do przeglądarki.
...
Testy: `backend/tests/test_cloudflare_cv_extraction.py`, linie 43–201, klasa `CloudflareCvExtractionTests`, obejmuje żądania text-only, vision dla skanu, parametry Cloudflare, fenced/typed content, bezpieczny błąd przy samym reasoningu, JSON Mode rollbacku OpenAI, błędny JSON i brak danych logowania bez sieci. `backend/tests/test_extract_cv_rejection.py` sprawdza miesięczny limit oraz brak naliczenia po błędzie. `frontend/src/utils/cvImportRequest.test.js`, linie 1–21, utrwala kontrakt czterech minut bez retry i wszystkie etykiety historii importów. `backend/tests/test_cv_data.py` obejmuje normalizację zagnieżdżonych skills.

Oficjalne źródła: [REST Workers AI](https://developers.cloudflare.com/workers-ai/get-started/rest-api/) opisuje uprawnienia tokenu; [zgodność z OpenAI](https://developers.cloudflare.com/workers-ai/configuration/open-ai-compatibility/) — endpoint `/ai/v1`; [Gemma 4](https://developers.cloudflare.com/workers-ai/models/gemma-4-26b-a4b-it/) i [Qwen 3.8](https://developers.cloudflare.com/workers-ai/models/qwen3.8-27b/) — reasoning i `max_completion_tokens`; [JSON Mode](https://developers.cloudflare.com/workers-ai/features/json-mode/) — aktualną listę wspieranych modeli; [cennik Workers AI](https://developers.cloudflare.com/workers-ai/platform/pricing/) — wspólną dla konta dzienną pulę neuronów Free.
...
- `backend/tests/test_cloudflare_cv_extraction.py`, linie 43–201, klasa `CloudflareCvExtractionTests` — wybór dostawcy, payloady text/vision, budżet reasoningu, prompt-only/fenced JSON, typed content, bezpieczne puste odpowiedzi, JSON Mode rollbacku i server-only credentials
...
- Import CV zwraca `extract_provider_empty_response` (502): wdróż aktualny backend używający `max_completion_tokens`, niskiego reasoningu i bez niewspieranego JSON Mode dla Gemmy/Qwena. Jeśli problem zostanie, sprawdź bezpieczny warning loggera `cv_extraction`: model/tryb, `finish_reason`, obecność reasoningu i liczba tokenów; log celowo nie zawiera treści CV.
