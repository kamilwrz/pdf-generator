...
- `backend/app/services/cv_templates/templates/portico.py`, function `_gen_portico` — centered `_block` masthead, `_place_centered_icon_contacts` for the contact row, `align="center"` on the summary's body block only; Experience/Education/Skills/Extras reuse the same `section()`/`Builder` pattern as `nova.py`
...
Known limitation: because the summary's paragraph is centered while its heading stays left-aligned in the icon gutter (a deliberate consistency choice, not an oversight — see above), an unusually short summary can read visually asymmetric against its own heading; this is expected for the design and does not affect Experience/Education/Skills, which are fully left-aligned.
...
Pro provides CV and design ratings, role fit, grammar, style, improvements, ATS guidance, ordinary chat, and **Układ** (the full-canvas geometry session). Free has no AI assistant (except the one lifetime CV import).
...
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, lines 661–1301, component `AiAssistant` — Pro upgrade path for Układ, local greeting with suggestion chips, deferred request, optional `template_id`, layout-session history boundary, review cards, and composer
...
- `backend/app/services/cv_templates/templates/portico.py`, funkcja `_gen_portico` — wycentrowany masthead przez `_block`, `_place_centered_icon_contacts` dla wiersza kontaktu, `align="center"` tylko na bloku treści podsumowania; Doświadczenie/Wykształcenie/Umiejętności/Sekcje dodatkowe używają tego samego wzorca `section()`/`Builder` co `nova.py`
...
- `frontend/src/templates/helpers.js` — `block()` zyskał opcjonalny parametr `align` (domyślnie `"left"`), żeby statyczne szablony startowe mogły autoryzować wycentrowane textarea
- `scripts/generate_iconic_icons.py`, `THEMES["portico"]`
...
Znane ograniczenie: ponieważ akapit podsumowania jest wycentrowany, a jego nagłówek pozostaje wyrównany do lewej w rynnie ikon (celowy wybór na rzecz spójności, a nie przeoczenie — zob. wyżej), niezwykle krótkie podsumowanie może wizualnie wyglądać asymetrycznie względem własnego nagłówka; to zamierzone zachowanie projektowe i nie dotyczy Doświadczenia/Wykształcenia/Umiejętności, które są w pełni wyrównane do lewej.
...
Pro obejmuje oceny CV i projektu, dopasowanie do oferty, gramatykę, styl, ulepszanie opisów, wskazówki ATS, zwykły czat oraz **Układ** (pełnopłótnowa sesja geometrii). Darmowy nie ma asystenta AI (poza jednym importem CV).
...
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, linie 661–1301, komponent `AiAssistant` — ścieżka ulepszenia do Pro dla Układu, lokalne powitanie z chipami propozycji, odroczony request, opcjonalne `template_id`, granica historii sesji, karty zmian i pole wpisywania
