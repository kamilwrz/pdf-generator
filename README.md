...
- `frontend/src/components/canvas/Textarea/Textarea.jsx` — auto-height effect now re-measures on `textTransform` changes, not only `content`/`width`/`fontSize`; shared by every masthead-identity template
...
- `backend/tests/test_vestige_template.py` — registry metadata, generated sidebar/main-column ownership, the 13 / 8.4 px heading type scale, the "stacked"-mode contact-band descriptor, the `mastheadIdentity` anchor with `blockPt == 0` and a `null` `contactBandId`, and non-colliding `grid-member` languages cells when languages spill into the main column
...
- `frontend/src/components/canvas/Textarea/Textarea.jsx` — efekt auto-wysokości przelicza się teraz również po zmianie `textTransform`, nie tylko `content`/`width`/`fontSize`; komponent współdzielony przez każdy szablon z tożsamością mastheadu
...
- `backend/tests/test_vestige_template.py` — metadane rejestru, własność sidebara i głównej kolumny w wygenerowanym układzie, skala typograficzna nagłówków 13 / 8,4 px, deskryptor paska kontaktowego w trybie "stacked", anchor `mastheadIdentity` z `blockPt == 0` i `null` w `contactBandId`, oraz brak kolizji komórek `grid-member` języków, gdy trafiają do kolumny głównej
