...
**Consistent CTA hierarchy.** The hero leads with **"CV, które dopasowuje się do Twojej treści."**, making its real A4 layout engine the primary promise. Its primary action is **"Stwórz CV za darmo"** (→ wizard); the secondary is **"Mam już CV — wgraj PDF"** (→ import); a tertiary text link offers the demo ("Najpierw chcesz zobaczyć produkt? Otwórz szablon Regent w edytorze"). A compact trust row then states **"Bez konta na start"**, **"Pierwszy import CV gratis"**, and **"PDF zgodny z podglądem"**. The header CTA is **"Stwórz CV"** (→ wizard, no longer the import CTA). Each call-to-action fires a per-source funnel event through `queueGuestEvent` so analytics can attribute the click to its surface: `hero_wizard`, `hero_import`, `hero_demo`, `before_after_import`, `templates_wizard`, `pricing_free`, `pricing_pro`, `final_wizard`, `final_import` (added to the fixed `event_type` vocabulary in `events.py`; buffered while anonymous, flushed after auth — see [Guest mode](#guest-mode-editor-without-an-account)).
...
- `backend/tests/test_linden_template.py`, lines 1–97 — registry tags, editorial/photo geometry, full-name uppercase fit, semantic main/sidebar lanes, record groups, dedicated icons, and the contact/photo descriptor.
...
**Spójna hierarchia CTA.** Hero prowadzi hasłem **„CV, które dopasowuje się do Twojej treści.”**, więc realny silnik układu A4 jest główną obietnicą produktu. Główne działanie to **„Stwórz CV za darmo”** (→ kreator), drugorzędne **„Mam już CV — wgraj PDF”** (→ import), a trzeciorzędny link prowadzi do demo („Najpierw chcesz zobaczyć produkt? Otwórz przykładowe CV w edytorze”). Niżej zwarty trust row komunikuje **„Bez konta na start”**, **„Pierwszy import CV gratis”** i **„PDF zgodny z podglądem”**. CTA w headerze to **„Stwórz CV”** (→ kreator, już nie import). Każde CTA wysyła przez `queueGuestEvent` zdarzenie lejka z konkretnym źródłem, żeby analityka mogła przypisać kliknięcie do powierzchni: `hero_wizard`, `hero_import`, `hero_demo`, `before_after_import`, `templates_wizard`, `pricing_free`, `pricing_pro`, `final_wizard`, `final_import` (dodane do stałego słownika `event_type` w `events.py`; buforowane anonimowo, wysyłane po zalogowaniu — zob. [Tryb gościa](#tryb-gościa-edytor-bez-konta)).
...
- `backend/tests/test_linden_template.py`, linie 1–97 — tagi rejestru, geometria editorialna/zdjęcia, pełne imię i nazwisko uppercase, semantyczne tory main/sidebar, grupy rekordów, dedykowane ikony oraz deskryptor kontakt/zdjęcie.
