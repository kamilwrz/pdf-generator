...
The same Sterling appearance panel exposes role-aware **S / M / L / XL** text presets. **M** is the original template scale. Display names grow least, body copy grows most, and headings, record titles, metadata, job position, and contacts use intermediate factors; body and metadata retain hard readability floors. The utility stores immutable baseline font size and line height per text element, so switching XL → S → M restores the exact authored metrics instead of multiplying rounded values. A preset is applied as one layout transaction: a reusable browser-canvas measurer reads the active font's real glyph widths (including weight, italic style, and letter spacing), the wrap estimator respects word boundaries and the bullet marker's separate grid column, and every flow textarea receives its new rendered height before `applyFlowSpacing` packs the main and sidebar lanes. This closes the one-line underestimation that could leave the last experience bullet underneath the following Languages heading after S → M or another scale change. The contact band is laid out from its scaled descriptor, browser auto-height still verifies the result, and page chrome is reconciled only after packing, so genuine overflow creates a continuation while returning to M can collapse it again. Tests without a DOM keep the deterministic character-count fallback. The selected `{ palette, textSize }` intent and typography baselines are persisted through `PdfElements.extra_properties`, restored when a saved document opens, and ignored safely by ReportLab.
...
- `frontend/src/utils/sterlingTypographyLayout.js`, lines 1–39, function `applySterlingTextSizeLayout`; `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, lines 219–232, function `handleSterlingTextSize` — atomic glyph measurement, contact relayout, main/sidebar packing, and page-chrome reconciliation for each type preset; regressions: `frontend/src/utils/sterlingTypographyLayout.test.js`, lines 53–100
...
Ten sam panel Sterlinga ma zależne od roli presety tekstu **S / M / L / XL**. **M** oznacza oryginalną skalę szablonu. Imię i nazwisko rośnie najmniej, treść główna najmocniej, a nagłówki, tytuły wpisów, metadane, stanowisko i kontakty korzystają z pośrednich mnożników; body i metadata zachowują twarde minima czytelności. Narzędzie zapisuje niezmienne bazowe wartości font-size i line-height przy każdym elemencie tekstowym, więc sekwencja XL → S → M odtwarza dokładne metryki autora zamiast mnożyć zaokrąglone wyniki. Preset jest stosowany jako jedna transakcja layoutu: współdzielony pomiar canvasu przeglądarki odczytuje prawdziwe szerokości glifów aktywnego fontu (łącznie z grubością, kursywą i letter-spacing), estymator zawijania respektuje granice słów oraz osobną kolumnę znacznika punktora, a każde pole textarea przepływu dostaje nową wysokość renderowania przed spakowaniem obu kolumn przez `applyFlowSpacing`. Usuwa to zaniżenie o jedną linię, przez które ostatni punkt doświadczenia mógł wchodzić pod następny nagłówek Języki po przejściu S → M lub innej zmianie skali. Blok kontaktów jest układany z przeskalowanego deskryptora, auto-height przeglądarki nadal weryfikuje wynik, a chrome stron jest uzgadniane dopiero po pakowaniu, więc realny overflow tworzy kontynuację, a powrót do M może ją ponownie zwinąć. Testy bez DOM zachowują deterministyczny fallback liczony ze znaków. Wybrany zamiar `{ palette, textSize }` oraz bazowe metryki są zapisywane w `PdfElements.extra_properties`, odtwarzane przy otwarciu dokumentu i bezpiecznie ignorowane przez ReportLab.
...
- `frontend/src/utils/sterlingTypographyLayout.js`, linie 1–39, funkcja `applySterlingTextSizeLayout`; `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, linie 219–232, funkcja `handleSterlingTextSize` — atomowy pomiar glifów, przeliczenie kontaktów, pakowanie main/sidebara i uzgodnienie chrome stron dla każdego presetu; regresje: `frontend/src/utils/sterlingTypographyLayout.test.js`, linie 53–100
