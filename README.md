...
| Cloudflare Workers AI | hosted API | Native-text CV extraction (reasoning Gemma 4, with one JSON-mode Llama fallback) and scan extraction (Qwen 3.8 Vision) | `ai_service.py`, `cloudflare_pricing.py` |
...
The provider is called through Cloudflare's OpenAI-compatible base URL, using the existing `openai` Python SDK. Native-text Gemma is a reasoning model and receives `max_completion_tokens=32000` plus `reasoning_effort=low` by default; it omits JSON Mode because Gemma is not on Cloudflare's published JSON Mode allowlist. The Llama fallback is on that allowlist, so it receives `max_tokens=8000` and `response_format=json_object` without reasoning parameters. Qwen 3.8 Vision is also a reasoning model: scanned-document requests use `max_completion_tokens=8000` plus `reasoning_effort=low`, omit JSON Mode, and rely on the same strict prompt/parser boundary. The parser accepts one plain or Markdown-fenced JSON object and typed text parts, but rejects arbitrary/non-object output. The explicit OpenAI rollback keeps `response_format=json_object` and uses the separate 8,000-token JSON budget.

Client construction is lazy: missing Cloudflare credentials do not prevent `/health`, authentication, editing, or PDF export from starting. `CV_EXTRACT_PROVIDER=openai` and `CV_EXTRACT_OPENAI_MODEL=gpt-4o` are an explicit rollback path, not an automatic cross-provider fallback; a Cloudflare outage therefore cannot silently send a user's CV to another processor. The default native-text attempt uses reasoning-based Gemma. It gets one same-provider retry on `CLOUDFLARE_TEXT_FALLBACK_MODEL` when the result is empty, is not a valid JSON object, or fails CV normalisation. The default fallback is JSON-mode Llama 3.1 8B Fast. Telemetry sums tokens and estimated cost from every attempted model and records `fallback_used` / `model_attempts`. Provider rate limits become a retryable 429, temporary/provider failures become 503, both text attempts returning empty becomes a retryable 502, and a final malformed/non-object CV result becomes 422. Diagnostics log only provider, model, extraction mode, failure category, finish reason, whether reasoning existed, and token count—never CV text, reasoning content, raw responses, or credentials.
...
- `backend/app/core/config.py`, lines 65–111, Cloudflare and `CV_EXTRACT_*` settings — server-only credentials, primary/fallback models, reasoning effort, and independent reasoning/JSON/vision limits
...
Tests: `backend/tests/test_cloudflare_cv_extraction.py`, lines 121–506, class `CloudflareCvExtractionTests`, covers native-text reasoning Gemma, scan-page vision fallback, independent reasoning/JSON/vision budgets, model-specific Cloudflare parameters, fenced/typed content, safe reasoning-only failures, one-shot Gemma-to-Llama recovery after either empty or invalid output with aggregated telemetry, OpenAI rollback JSON Mode, final invalid model JSON, and missing credentials without a network call. Its two-column regression at lines 378–437 deliberately returns a wrong summary and unsupported skill from the mocked model, then verifies source-grounded summary, specialisations, both references, prompt isolation, safe telemetry, and final Atrium rendering. The Portico/Monument-style regression at lines 438–495 verifies the complete wrapped summary, hyphen continuation, sentence fragments beginning with `education` and `skills` remaining body text, two bold skill groups with middle-dot items, rejection of a copied WORK EXPERIENCE title, and final Atrium content. `backend/tests/test_extract_cv_rejection.py` covers monthly quota and failure-not-consuming behaviour. `frontend/src/utils/cvImportRequest.test.js`, lines 1–21, locks the four-minute no-retry contract and all import-history status labels. `frontend/src/utils/aiCvPanelScroll.test.js`, lines 8–35, protects the fixed-controls/scrolling-list split, stable scrollbar gutter, accessible region, and short-viewport upload overflow. `backend/tests/test_cv_data.py` covers nested skills normalization.

Official references: [PyMuPDF text-extraction details](https://pymupdf.readthedocs.io/en/latest/app1.html) documents `dict` output and line/span bounding boxes; [Workers AI REST setup](https://developers.cloudflare.com/workers-ai/get-started/rest-api/) explains token permissions; [OpenAI compatibility](https://developers.cloudflare.com/workers-ai/configuration/open-ai-compatibility/) documents the `/ai/v1` endpoint; [Gemma 4 26B](https://developers.cloudflare.com/workers-ai/models/gemma-4-26b-a4b-it/) documents the primary reasoning model; [Llama 3.1 8B Fast](https://developers.cloudflare.com/ai/models/%40cf/meta/llama-3.1-8b-instruct-fast/) documents the JSON fallback; [Qwen 3.8](https://developers.cloudflare.com/workers-ai/models/qwen3.8-27b/) documents vision, reasoning, and `max_completion_tokens`; [JSON Mode](https://developers.cloudflare.com/workers-ai/features/json-mode/) lists Llama Fast as supported; [Workers AI pricing](https://developers.cloudflare.com/workers-ai/platform/pricing/) explains the account-wide daily Free neuron allocation.
...
- `backend/tests/test_cloudflare_cv_extraction.py`, lines 121–506, class `CloudflareCvExtractionTests` — provider selection, Gemma text/Qwen vision payloads, independent reasoning/JSON/vision budgets, Llama JSON fallback after empty or invalid output, fenced/typed JSON, rollback JSON Mode, server-side credential gating, exact source headings, Portico/Monument-style nested skills, and final Atrium rendering
...
| `CLOUDFLARE_TEXT_MODEL` | no | Primary reasoning model for native-text CVs | `@cf/google/gemma-4-26b-a4b-it` |
| `CLOUDFLARE_TEXT_FALLBACK_MODEL` | no | One-shot same-provider JSON fallback after an empty, malformed, or non-normalisable primary result | `@cf/meta/llama-3.1-8b-instruct-fast` |
| `CLOUDFLARE_TEXT_REASONING_EFFORT` | no | Gemma text reasoning effort: `low`, `medium`, or `high` | `low` |
...
| `CV_EXTRACT_TEXT_MAX_COMPLETION_TOKENS` | no | Native-text completion budget, including model reasoning | `32000` |
...
- **CV import returns `extract_provider_empty_response` (502):** the default Gemma primary now gets one automatic JSON-mode Llama fallback after empty or invalid output. A remaining 502 means both attempts returned empty. On Render, set `CLOUDFLARE_TEXT_MODEL=@cf/google/gemma-4-26b-a4b-it` and `CV_EXTRACT_TEXT_MAX_COMPLETION_TOKENS=32000`; remove an old `CLOUDFLARE_TEXT_MODEL` override pinning Llama and remove the legacy shared `CV_EXTRACT_MAX_COMPLETION_TOKENS=8000` unless it is intentional. Inspect sanitized `cv_extraction` warnings for attempted model, failure category, `finish_reason`, reasoning presence, and completion-token count; logs intentionally contain no CV content.
...
| Cloudflare Workers AI | hostowane API | Import natywnego tekstu CV (reasoningowa Gemma 4 z jednorazowym fallbackiem Llama JSON Mode) oraz ekstrakcja skanów (Qwen 3.8 Vision) | `ai_service.py`, `cloudflare_pricing.py` |
...
Dostawca jest wywoływany przez kompatybilny z OpenAI adres bazowy Cloudflare przy użyciu istniejącego SDK `openai`. Tekstowa Gemma jest modelem reasoningowym i domyślnie dostaje `max_completion_tokens=32000` oraz `reasoning_effort=low`; pomija JSON Mode, ponieważ Gemmy nie ma na oficjalnej allowliście Cloudflare. Awaryjna Llama znajduje się na tej liście, dlatego dostaje `max_tokens=8000` oraz `response_format=json_object`, bez parametrów reasoningu. Qwen 3.8 Vision również jest reasoningowy: żądania skanów używają `max_completion_tokens=8000` i `reasoning_effort=low`, pomijają JSON Mode i polegają na tym samym ścisłym prompcie/parserze. Parser przyjmuje jeden zwykły albo opakowany fence'em obiekt JSON oraz typowane fragmenty tekstowe, lecz odrzuca dowolny tekst i JSON niebędący obiektem. Jawny rollback OpenAI zachowuje `response_format=json_object` i używa osobnego budżetu JSON 8000 tokenów.

Klient powstaje leniwie: brak danych Cloudflare nie blokuje startu `/health`, logowania, edycji ani eksportu PDF. `CV_EXTRACT_PROVIDER=openai` wraz z `CV_EXTRACT_OPENAI_MODEL=gpt-4o` to jawna ścieżka rollbacku, nie automatyczny fallback między dostawcami; awaria Cloudflare nie może więc po cichu wysłać CV do innego procesora. Domyślna próba dla natywnego tekstu używa reasoningowej Gemmy. Dostaje ona jedno ponowienie u tego samego dostawcy na `CLOUDFLARE_TEXT_FALLBACK_MODEL`, gdy wynik jest pusty, nie jest poprawnym obiektem JSON albo nie przechodzi normalizacji CV. Domyślnym fallbackiem jest Llama 3.1 8B Fast w JSON Mode. Telemetria sumuje tokeny i koszt wszystkich prób oraz zapisuje `fallback_used` / `model_attempts`. Limit dostawcy daje ponawialny 429, przejściowa awaria lub konfiguracja — 503, puste obie próby tekstowe — ponawialny 502, a niepoprawny końcowy wynik CV — 422. Diagnostyka zapisuje jedynie dostawcę, model, tryb ekstrakcji, kategorię błędu, `finish_reason`, obecność reasoningu i liczbę tokenów — nigdy treść CV, reasoning, surową odpowiedź ani dane dostępowe.
...
- `backend/app/core/config.py`, linie 65–111, ustawienia Cloudflare i `CV_EXTRACT_*` — sekrety serwerowe, modele główny/awaryjny, poziom reasoningu i niezależne limity reasoning/JSON/vision
...
Testy: `backend/tests/test_cloudflare_cv_extraction.py`, linie 121–506, klasa `CloudflareCvExtractionTests`, obejmuje reasoningową Gemmę dla natywnego tekstu, vision dla skanu, niezależne budżety reasoning/JSON/vision, parametry zależne od modelu, fenced/typed content, bezpieczny błąd przy samym reasoningu, jednorazowe odzyskanie Gemma→Llama po pustym albo błędnym wyniku z agregacją telemetrii, JSON Mode rollbacku OpenAI, końcowy błędny JSON i brak danych logowania bez sieci. Regresja dwukolumnowa w liniach 378–437 celowo zwraca z mocka modelu błędne podsumowanie i nieobecną umiejętność, a następnie sprawdza ugruntowane źródłem podsumowanie, specjalizacje, obie referencje, izolację promptu, bezpieczną telemetrię i końcowy render Atrium. Regresja w stylu Portico/Monument w liniach 438–495 sprawdza pełne zawijane podsumowanie, kontynuację słowa z łącznikiem, zachowanie fragmentów zdań zaczynających się od `education` i `skills` jako treści, dwie pogrubione grupy skills z elementami rozdzielanymi kropką środkową, odrzucenie skopiowanego tytułu WORK EXPERIENCE i końcową treść Atrium. `backend/tests/test_extract_cv_rejection.py` sprawdza miesięczny limit oraz brak naliczenia po błędzie. `frontend/src/utils/cvImportRequest.test.js`, linie 1–21, utrwala kontrakt czterech minut bez retry i wszystkie etykiety historii importów. `frontend/src/utils/aiCvPanelScroll.test.js`, linie 8–35, chroni podział na nieruchome kontrolki i przewijaną listę, stabilny gutter scrollbara, dostępny region oraz overflow panelu wysyłania w niskim oknie. `backend/tests/test_cv_data.py` obejmuje normalizację zagnieżdżonych skills.

Oficjalne źródła: [szczegóły ekstrakcji tekstu PyMuPDF](https://pymupdf.readthedocs.io/en/latest/app1.html) opisują format `dict` i prostokąty linii/spanów; [REST Workers AI](https://developers.cloudflare.com/workers-ai/get-started/rest-api/) opisuje uprawnienia tokenu; [zgodność z OpenAI](https://developers.cloudflare.com/workers-ai/configuration/open-ai-compatibility/) — endpoint `/ai/v1`; [Gemma 4 26B](https://developers.cloudflare.com/workers-ai/models/gemma-4-26b-a4b-it/) — główny model reasoningowy; [Llama 3.1 8B Fast](https://developers.cloudflare.com/ai/models/%40cf/meta/llama-3.1-8b-instruct-fast/) — fallback JSON; [Qwen 3.8](https://developers.cloudflare.com/workers-ai/models/qwen3.8-27b/) — vision, reasoning i `max_completion_tokens`; [JSON Mode](https://developers.cloudflare.com/workers-ai/features/json-mode/) wymienia Llamę Fast jako wspieraną; [cennik Workers AI](https://developers.cloudflare.com/workers-ai/platform/pricing/) opisuje wspólną dzienną pulę neuronów Free.
...
- `backend/tests/test_cloudflare_cv_extraction.py`, linie 121–506, klasa `CloudflareCvExtractionTests` — wybór dostawcy, payloady Gemma text/Qwen vision, niezależne budżety reasoning/JSON/vision, fallback Llama JSON po pustym albo błędnym wyniku, fenced/typed JSON, JSON Mode rollbacku, server-only credentials, dokładne nagłówki źródłowe, zagnieżdżone skills w stylu Portico/Monument i końcowy render Atrium
...
| `CLOUDFLARE_TEXT_MODEL` | nie | Główny model reasoningowy dla natywnego tekstu CV | `@cf/google/gemma-4-26b-a4b-it` |
| `CLOUDFLARE_TEXT_FALLBACK_MODEL` | nie | Jednorazowy fallback JSON u tego samego dostawcy po pustym, błędnym lub nienormalizowalnym wyniku primary | `@cf/meta/llama-3.1-8b-instruct-fast` |
| `CLOUDFLARE_TEXT_REASONING_EFFORT` | nie | Poziom reasoningu tekstowej Gemmy: `low`, `medium` albo `high` | `low` |
...
| `CV_EXTRACT_TEXT_MAX_COMPLETION_TOKENS` | nie | Budżet odpowiedzi natywnego tekstu, łącznie z reasoningiem modelu | `32000` |
...
- Import CV zwraca `extract_provider_empty_response` (502): domyślna Gemma dostaje teraz jedno automatyczne ponowienie Llamą w JSON Mode po pustym albo błędnym wyniku. Dalszy 502 oznacza puste obie próby. Na Render ustaw `CLOUDFLARE_TEXT_MODEL=@cf/google/gemma-4-26b-a4b-it` i `CV_EXTRACT_TEXT_MAX_COMPLETION_TOKENS=32000`; usuń stary override `CLOUDFLARE_TEXT_MODEL` przypinający Llamę i starszy wspólny `CV_EXTRACT_MAX_COMPLETION_TOKENS=8000`, o ile nie jest zamierzony. Sprawdź bezpieczne warningi `cv_extraction`: model, kategorię błędu, `finish_reason`, obecność reasoningu i liczbę tokenów; logi nie zawierają treści CV.
