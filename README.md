...
- `frontend/src/utils/sectionRecord.js`, lines 737–792, functions `findRecordDescription` / `getRecordDescriptionAction`; lines 830–885, function `listRecordBlockAddAnchors`; lines 1088–1177 and 1189–1212, functions `addRecordDescription` / `removeRecordDescription`; plus `listUpperRecordMembers`, `inferRecordLayout`, `pickRecordTemplateGroup`, `ensureCanonicalRecordTemplate`, `insertRecordBlockAfterRecord`, `removeRecordBlock`, and `reorderRecordBlock` — resolve the state-dependent menu action, preserve sibling/body style and record membership, open the lane-scoped description hole, and rhythm-pack every structural change
...
- `frontend/src/utils/sectionRecord.test.js`, lines 632–757, `describe("optional record descriptions")`, plus the remaining `sectionRecord` suites — education remove/add round-trip, single-record Experience fallback, sibling style/group preservation, gap closing, dynamic anchor state, Skills exclusion, upper-band-only eligibility, placeholder insertion, record deletion, and reorder
...
- `frontend/src/utils/sectionRecord.js`, linie 737–792, funkcje `findRecordDescription` / `getRecordDescriptionAction`; linie 830–885, funkcja `listRecordBlockAddAnchors`; linie 1088–1177 i 1189–1212, funkcje `addRecordDescription` / `removeRecordDescription`; dodatkowo `listUpperRecordMembers`, `inferRecordLayout`, `pickRecordTemplateGroup`, `ensureCanonicalRecordTemplate`, `insertRecordBlockAfterRecord`, `removeRecordBlock` i `reorderRecordBlock` — rozstrzyga stan akcji menu, zachowuje styl rodzeństwa/ciała i przynależność wpisu, otwiera lane'ową dziurę na opis i przepakowuje rytm po każdej zmianie strukturalnej
...
- `frontend/src/utils/sectionRecord.test.js`, linie 632–757, `describe("optional record descriptions")`, oraz pozostałe suite’y `sectionRecord` — round-trip usuń/dodaj dla edukacji, fallback pojedynczego wpisu Doświadczenia, zachowanie stylu/grupy rodzeństwa, zamknięcie odstępu, dynamiczny stan kotwicy, wykluczenie Umiejętności, kwalifikacja tylko górnego pasa, wstawianie placeholderów, usunięcie wpisu i reorder
