...
**Projekt** assesses typography, hierarchy, colour consistency, emphasis, and text alignment. It does not send or display a geometry report, and intentional small template labels are not a penalty. The backend still checks for unreadable structural faults privately; any collision, clipped textarea, line through text, or element outside the page caps the displayed score at **5/10**, without exposing diagnostic counts in this assessment.
...
- `backend/app/services/ai_assistant_service.py`, lines 354–441, `_rate_design` — template-respecting visual rating and private 5/10 safety cap; lines 1044–1139, `_layout_session` — UI-ready plain-language summary and tips; plus `_model_for_action`, `_chat`
...
**Projekt** ocenia typografię, hierarchię, spójność kolorów, wyróżnienia i wyrównanie tekstu. Nie wysyła ani nie pokazuje raportu geometrii, a celowo małe etykiety szablonu nie obniżają wyniku. Backend nadal prywatnie sprawdza nieczytelne błędy konstrukcyjne; kolizja, ucięte pole tekstowe, linia przechodząca przez tekst lub element poza stroną ogranicza widoczną ocenę do **5/10**, bez pokazywania liczników diagnostycznych w tej ocenie.
...
- `backend/app/services/ai_assistant_service.py`, linie 354–441 — `_rate_design`, ocena zgodna z szablonem i prywatny limit 5/10; linie 1044–1139 — `_layout_session`, gotowe dla UI proste podsumowanie i wskazówki; ponadto `_model_for_action`, `_chat`
