...
Portico's 100 pt reclaim applies only to document-flow elements. Page paper, the bottom decorative rule, and the page number are `fixedToPage` chrome and retain their authored coordinates while the photo is hidden; they receive no `photoLayoutHome` snapshot because showing the photo must not move them either. The photo transition also snapshots the complete masthead-identity descriptor and shifts the stored title reconstruction `top` into the current photo-less coordinate system. Consequently hiding/showing the job title while the photo remains hidden reconstructs it in the compressed masthead, independent of action order; restoring the photo returns both the visible title and its descriptor to the authored coordinate. After either Portico photo or title toggle, `reflowPorticoAfterMastheadChange` runs the shared full-document section packer with the document's current spacing. Before compacting those sections, `sectionElementIds` repairs any `flowGroup` whose members straddle a following section heading: every title/company/description mate is reassigned to the section containing the record's earliest member. A last job title can therefore no longer remain under Experience while its company and bullets are packed into Education after the photo is hidden. Sections already placed on page 2 are then reconsidered for the reclaimed page-1 space, and `reconcileDocumentPages` removes continuation chrome when the page becomes empty. The nested restoration snapshot is persisted so the same invariant survives save/reload.
...
- `frontend/src/utils/profilePhotoVisibility.js`, lines 78–410, functions `hiddenProfileContactSectionFloor`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto`, `profilePhotoControlAnchor`, `alignSidebarAfterProfileContacts` — shared 40 pt contact floor, reversible visibility, order-independent Portico photo/title reflow with fixed footer chrome, complete Slate/Tessera cluster hiding, actual contact-stack measurement and contact/sidebar transfer, plus legacy document fallbacks
...
Odzyskanie 100 pt w Portico obejmuje wyłącznie elementy przepływu dokumentu. Papier strony, dolna linia dekoracyjna i numer strony są chrome `fixedToPage` i zachowują autorskie współrzędne po ukryciu zdjęcia; nie otrzymują migawki `photoLayoutHome`, ponieważ ponowne pokazanie zdjęcia również nie może ich przesunąć. Przejście zdjęcia zapisuje też pełny deskryptor tożsamości mastheadu i przesuwa zapisane `top` rekonstrukcji stanowiska do bieżącego układu bez zdjęcia. Ukrycie i pokazanie stanowiska przy nadal schowanym zdjęciu odtwarza je więc w skompresowanym mastheadzie niezależnie od kolejności akcji; pokazanie zdjęcia przywraca widoczny tytuł i jego deskryptor do autorskiej pozycji. Po każdym przełączeniu zdjęcia albo stanowiska Portico funkcja `reflowPorticoAfterMastheadChange` uruchamia wspólny packer wszystkich sekcji z bieżącymi odstępami dokumentu. Przed kompaktowaniem tych sekcji `sectionElementIds` naprawia każdy `flowGroup`, którego elementy leżą po obu stronach nagłówka następnej sekcji: wszystkie części tytuł/firma/opis wracają do sekcji zawierającej najwcześniejszy element rekordu. Ostatnie stanowisko nie może więc pozostać w Doświadczeniu, gdy jego firma i punkty zostałyby po ukryciu zdjęcia zapakowane do Wykształcenia. Sekcje umieszczone wcześniej na stronie 2 są następnie ponownie sprawdzane względem odzyskanego miejsca na stronie 1, a `reconcileDocumentPages` usuwa chrome pustej strony kontynuacji. Zagnieżdżona migawka odtwarzania jest zapisywana, więc zasada działa także po zapisie i ponownym otwarciu dokumentu.
...
- `frontend/src/utils/profilePhotoVisibility.js`, linie 78–410, funkcje `hiddenProfileContactSectionFloor`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto`, `profilePhotoControlAnchor`, `alignSidebarAfterProfileContacts` — wspólny próg kontaktów 40 pt, odwracalna widoczność, niezależny od kolejności reflow zdjęcia/stanowiska Portico ze stałym chrome stopki, ukrycie kompletnego klastra, pomiar rzeczywistego stosu kontaktów i transfer kontaktów/sidebara Slate/Tessera oraz fallbacki starszych dokumentów
...
- Testy: `frontend/src/utils/profilePhotoVisibility.test.js`, linie 187–248 (dwustronicowy reflow w obu kolejnościach przełączania zdjęcia/stanowiska Portico oraz regresja rekordu stanowiska rozciętego nagłówkiem następnej sekcji); `frontend/src/utils/sectionStructure.test.js`, linie 1887–1950; `frontend/src/templates/slate.test.js`, linie 6–78; `frontend/src/templates/tessera.test.js`, linie 6–81; `backend/tests/test_cv_template_layouts.py`, linie 103–215 i 258–378; `backend/tests/test_contact_channel_roundtrip.py`, linie 54–76; `backend/tests/test_pdf_watermark.py`, linie 119–137
