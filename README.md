...
The provider is called through Cloudflare's OpenAI-compatible base URL, using the existing `openai` Python SDK. Native-text Gemma is a reasoning model and receives `max_completion_tokens=16000` plus `reasoning_effort=low` by default; it omits JSON Mode because Gemma is not on Cloudflare's published JSON Mode allowlist. The Llama fallback is on that allowlist, so it receives `max_tokens=16000` and `response_format=json_object` without reasoning parameters. Qwen 3.8 Vision is also a reasoning model: scanned-document requests use `max_completion_tokens=8000` plus `reasoning_effort=low`, omit JSON Mode, and rely on the same strict prompt/parser boundary. The parser accepts one plain or Markdown-fenced JSON object and typed text parts, but rejects arbitrary/non-object output. The explicit OpenAI rollback keeps `response_format=json_object` and uses the text completion budget.
...
- `backend/app/core/config.py`, lines 65–104, Cloudflare and `CV_EXTRACT_*` settings — server-only credentials, primary/fallback models, reasoning effort, and independent text/vision limits
...
Tests: `backend/tests/test_cloudflare_cv_extraction.py`, lines 121–502, class `CloudflareCvExtractionTests`, covers native-text reasoning Gemma, scan-page vision fallback, model-specific Cloudflare parameters, fenced/typed content, safe reasoning-only failures, one-shot Gemma-to-Llama recovery after either empty or invalid output with aggregated telemetry, OpenAI rollback JSON Mode, final invalid model JSON, and missing credentials without a network call. Its two-column regression at lines 374–433 deliberately returns a wrong summary and unsupported skill from the mocked model, then verifies source-grounded summary, specialisations, both references, prompt isolation, safe telemetry, and final Atrium rendering. The Portico/Monument-style regression at lines 434–491 verifies the complete wrapped summary, hyphen continuation, sentence fragments beginning with `education` and `skills` remaining body text, two bold skill groups with middle-dot items, rejection of a copied WORK EXPERIENCE title, and final Atrium content. `backend/tests/test_extract_cv_rejection.py` covers monthly quota and failure-not-consuming behaviour. `frontend/src/utils/cvImportRequest.test.js`, lines 1–21, locks the four-minute no-retry contract and all import-history status labels. `frontend/src/utils/aiCvPanelScroll.test.js`, lines 8–35, protects the fixed-controls/scrolling-list split, stable scrollbar gutter, accessible region, and short-viewport upload overflow. `backend/tests/test_cv_data.py` covers nested skills normalization.
...
- `backend/tests/test_cloudflare_cv_extraction.py`, lines 121–502, class `CloudflareCvExtractionTests` — provider selection, Gemma text/Qwen vision payloads, independent budgets, Llama JSON fallback after empty or invalid output, fenced/typed JSON, rollback JSON Mode, server-side credential gating, exact source headings, Portico/Monument-style nested skills, and final Atrium rendering
...
| `CV_EXTRACT_TEXT_MAX_COMPLETION_TOKENS` | no | Native-text completion budget, including model reasoning | `16000` |
...
- **CV import returns `extract_provider_empty_response` (502):** the default Gemma primary now gets one automatic JSON-mode Llama fallback after empty or invalid output. A remaining 502 means both attempts returned empty. On Render, set `CLOUDFLARE_TEXT_MODEL=@cf/google/gemma-4-26b-a4b-it` and `CV_EXTRACT_TEXT_MAX_COMPLETION_TOKENS=16000`; remove an old `CLOUDFLARE_TEXT_MODEL` override pinning Llama and remove the legacy shared `CV_EXTRACT_MAX_COMPLETION_TOKENS=8000` unless it is intentional. Inspect sanitized `cv_extraction` warnings for attempted model, failure category, `finish_reason`, reasoning presence, and completion-token count; logs intentionally contain no CV content.
...
Dostawca jest wywoływany przez kompatybilny z OpenAI adres bazowy Cloudflare przy użyciu istniejącego SDK `openai`. Tekstowa Gemma jest modelem reasoningowym i domyślnie dostaje `max_completion_tokens=16000` oraz `reasoning_effort=low`; pomija JSON Mode, ponieważ Gemmy nie ma na oficjalnej allowliście Cloudflare. Awaryjna Llama znajduje się na tej liście, dlatego dostaje `max_tokens=16000` oraz `response_format=json_object`, bez parametrów reasoningu. Qwen 3.8 Vision również jest reasoningowy: żądania skanów używają `max_completion_tokens=8000` i `reasoning_effort=low`, pomijają JSON Mode i polegają na tym samym ścisłym prompcie/parserze. Parser przyjmuje jeden zwykły albo opakowany fence'em obiekt JSON oraz typowane fragmenty tekstowe, lecz odrzuca dowolny tekst i JSON niebędący obiektem. Jawny rollback OpenAI zachowuje `response_format=json_object` i używa tekstowego budżetu odpowiedzi.
...
- `backend/app/core/config.py`, linie 65–104, ustawienia Cloudflare i `CV_EXTRACT_*` — sekrety serwerowe, modele główny/awaryjny, poziom reasoningu i niezależne limity tekst/vision
...
- `backend/tests/test_cloudflare_cv_extraction.py`, linie 121–502, klasa `CloudflareCvExtractionTests` — wybór dostawcy, payloady Gemma text/Qwen vision, niezależne budżety, fallback Llama JSON po pustym albo błędnym wyniku, fenced/typed JSON, JSON Mode rollbacku, server-only credentials, dokładne nagłówki źródłowe, zagnieżdżone skills w stylu Portico/Monument i końcowy render Atrium
...
| `CV_EXTRACT_TEXT_MAX_COMPLETION_TOKENS` | nie | Budżet odpowiedzi natywnego tekstu, łącznie z reasoningiem modelu | `16000` |
