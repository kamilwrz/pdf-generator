...
│   │   │   ├── canvas/CanvasHoverToolbar/ # Shared gutter toolbar, delayed tooltips, semantic-block highlight, direct actions/overflow menu
│   │   │   ├── canvas/GridEntryActions/ # Per-cell + / trash controls for repeatable short-entry grids
│   │   │   ├── canvas/SectionRecordAdd/  # Heading controls plus full-section context outline from plain content hover/focus
...
│   │   │   ├── editor/AddSectionModal/   # "+ Dodaj sekcję" modal (name + aa/grid/cc-sub/cc-edu/cc-exp layout picker)
...
Element properties open in a **measured top-left workspace inspector** (`Editor` via `createPortal`). Its left and top edges follow the live sidebar and topbar. The desktop inspector prefers a 304 px width and uses an intrinsic, content-driven height capped at 480 px, so a short form such as a selected **Text** element does not leave an empty fixed-height region. When a longer form reaches the cap, only the form body scrolls; the cap is reduced further when the remaining viewport height is smaller. The live A4 edge and available viewport width remain hard limits at every zoom: when 304 px would violate the 15 px page gap, the panel narrows to the usable dock width instead of covering the document. The compact layout still becomes a full-height drawer below 720 px. At a container width of 280 px or less, labelled fields take the complete card row rather than compressing. Icon controls and both sides of each numeric stepper retain 36 × 36 px targets, and the narrow form has no horizontal overflow. The inspector enters and exits with a 240 ms leftward fade/slide; reduced-motion mode removes the movement. A paper header with ink typography names the selected object in plain Polish, and a persistent help band explains **Ctrl + left mouse button = select multiple elements**. Every editable value has a visible layperson label (for example “Distance between rows”, “From the left edge”, and “Order on the page”). Numeric fields retain the minus/value/plus stepper contract, native number input, and keyboard editing. Page alignment keeps distinct object-alignment icons, and **Text** / **TextArea** retain their different field sets. Template mode continues to hide layout-owned position, size, layer, clone, and delete actions; freeform keeps the full set and resize chrome.
...
- `frontend/src/components/editor/Editor/Editor.jsx`, lines 150–1391, component `Editor` and helpers `Group`, `NumField`, `ColorField`, `FontField`, `BulkToolbar`; `Editor.module.css`, lines 1–346; `frontend/src/utils/editorInspectorWidth.js`, lines 1–34, exports `EDITOR_INSPECTOR_FIXED_WIDTH_PX` and `resolveEditorInspectorWidth`; `Sidebar/Sidebar.jsx`, lines 67 and 157; `Topbar/Topbar.jsx`, line 72 — measured top-left inspector, preferred 304 px desktop width with live page-edge narrowing at every zoom, intrinsic height capped at 480 px and by the viewport, AI Assistant-aligned paper/control/accent/focus/danger surfaces, live layout anchors, full-row compact fields, 36 px targets, plain-language visible labels, multi-select help, ± steppers, bulk edits (including persistent line-height and letter-spacing controls for compatible multi-selections, with a blank mixed-value state), robust pointer/keyboard inline-selection toolbar, a context-only mid-dot action highlighted during inline Skills editing, explicit editor-layer ordering, responsive container layout, and template-mode field gates. Regression coverage: `frontend/src/utils/editorInspectorWidth.test.js`, lines 9–39, three preferred-width and collision-boundary tests; `frontend/src/components/editor/Editor/Editor.test.js`, lines 5–34 and 44–68, content-driven height-cap and surface/control contracts; lines 94–107 and 125–140 retain the inline-Skills and bulk-spacing contracts.
...
Five layouts ship: **"aa"** — heading + rule + one auto-height content textarea (**Prosta treść**); **"grid"** — heading + rule + repeatable, equal-width short entries (**Krótkie wpisy w kolumnach**, described in the modal with **JĘZYKI** as the example); **"cc-sub"** — heading + rule + a category record (bold **Nazwa kategorii** + body **Treść…** — 2 lines; modal label **Prosta treść (kategorie)**), the same shape as nested skill groups under UMIEJĘTNOŚCI; **"cc-edu"** — a detailed record that follows the active template's Education structure; and **"cc-exp"** — a descriptive record that follows the active template's Experience structure. The two record choices remain semantically distinct because education has a school/organisation field while experience has an employer/organisation field, but both expose the same complete user-facing data set: name, organisation, period, location, and description. Category sections must not inflate to record placeholders when the user adds another block with **+** — `isSubcategorySectionTitle` / `ensureCanonicalRecordTemplate` keep the 2-line shape for non-education titles. Each record's fields share one `flowGroup` so they page-break as a unit. The fixed short-entry grid is supported; an unrelated free-form two-column prose layout ("bb") is still not offered.

The grid starts with one editable **Nazwa — wartość** cell. Hover or keyboard focus on any cell reveals exactly two application-only icons in the outside A4 gutter: **Dodaj wpis** (`+`) and **Usuń wpis** (trash). `+` inserts immediately after the active cell, keeps reading order, fills the current fixed-column row, and wraps to a new line when the row is full. Trash removes that cell and its content, then compacts every later cell left and upward so no empty slot remains; the action is recoverable through the standard **Cofnij** toast. The last cell's trash remains visible but disabled, preserving the only local `+` affordance. Pointer hover paints one cell frame only; selected or edited cells keep their existing single selection/edit frame instead of stacking structural and element outlines. `Shift+F10`/the Context Menu key focuses the actions, `Escape` returns focus to the cell, and all controls are portalled outside document layout and PDF export. Wide main columns use four tracks, narrower main columns three, and sidebars two. The heading and cell metadata are stored in `extra_properties`, while `custom_sections[].layout: "grid"` preserves the semantic shape; the same controls, item order, and fixed geometry therefore survive save/reopen, guest-profile recovery, and a later template change.

Generated **Languages** grids use the same controls. Adding or deleting a cell synchronises the canonical `cv_data.languages` array, so a later template change preserves the structural edit instead of regenerating the old language list. Immediately before a refill, the change-template action also snapshots the live grid in visual row/column order and carries that exact profile into the replacement document. This closes the React-effect timing window and preserves every cell even when several newly added rows still contain the identical **Język — poziom** seed. Backend normalization treats structured language mappings as ordered editor rows and therefore keeps their multiplicity, while legacy free-text imports continue to deduplicate repeated extraction noise. Language name, separator, and level (for example `Polski — C2`) use one body colour and normal type: the generators, static starters, lane-transfer rebuild, and new cells emit no automatic level `runs`, colour, or italic. On opening an older saved CV, only the exact legacy generator-owned suffix run is removed; unrelated user-authored inline formatting stays intact.
...
- `backend/app/schemas/pdf_schema.py`, lines 152–177; `backend/app/crud/pdfs.py`, lines 41–77 and 120–133; `shared/pdf-element.schema.json`, lines 421–594; and `frontend/src/utils/persistedCanvasElement.js`, lines 28–119, function `hydratePersistedCanvasElement` — persist and restore `flowLane` plus the grid identity/geometry fields (`editorSectionLayout`, `editorGrid*`, `gridSectionId`, `gridColumns`, `gridGutter`, `gridWidth`, `gridLeft`, `gridKind`) through `PdfElements.extra_properties`, without adding database columns.
- `frontend/src/utils/sectionBuilder.js`, lines 72–80 (`SECTION_LAYOUTS`), 142–196 (private `contentTextarea`), 211–228 (`resolveGridSectionColumns` / `gridSectionCellWidth`), and 355–581 (`buildSectionElements`) — layout constructors for "aa", "grid", "cc-sub", "cc-edu", and "cc-exp"; the grid branch stamps `gridKind: "entries"` on the heading and first `grid-member` textarea. Passing `lane: "sidebar"` stamps `flowLane: "sidebar"` + `flowRole: "sidebar-chrome"` (record field-line specs in private `recordLineSpecs`; heights via private `measureGeneratorBlockHeight`; content uses `bodyLeft`; image markers keep `src` / `alignWithText`; filled-band headings keep their sampled alignment frame).
- `frontend/src/utils/gridSection.js`, lines 453–483 (`listGridSectionEntryAnchors`), 499–556 (`insertGridSectionEntry`), and 575–617 (`removeGridSectionEntry`) — discovers generated Languages and explicit custom grids while rejecting Skills chips; inserts after the active cell, recalculates equal tracks/row heights, wraps, compacts after deletion, protects the last cell, and re-packs following sections with live spacing.
...
- `frontend/src/hooks/useA4Elements.js`, lines 1053–1081 (`handleAddGridSectionEntry`) and 1250–1265 (`handleRemoveGridSectionEntry`) — applies the grid operations, selects the inserted cell, queues deletion tombstones, and collapses empty pages. The existing `handleAddSection` owns optional `afterHeadingId` / `lane`, style sampling, record-template resolution/replacement, construction, placement, and post-add selection.
...
- `frontend/src/components/editor/AddSectionModal/AddSectionModal.jsx`, lines 15–47 (`LAYOUT_OPTIONS`) and 58–220 (`AddSectionModal`) — name + five-layout picker, including **Krótkie wpisy w kolumnach** / `grid` with the **JĘZYKI** example, dynamic placeholder, and optional icon gallery; subtitle differs for insert-under vs append-end.
- `frontend/src/components/editor/AddSectionModal/AddSectionModal.test.js`, lines 15–21 — regression for structure-first labels, the grid explanation/example, and the removal of blueprint-specific wording.
...
- `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, component `SectionRecordAdd` — adapts the shared gutter toolbar to section add/reorder and overflow transfer/layout/delete actions, and keeps the full semantic section outline active over plain body copy
- `frontend/src/components/canvas/GridEntryActions/GridEntryActions.jsx`, lines 35–200 (`GridEntryActions`, keyboard path at 83–111, gutter selection at 129–135, single hover frame at 137–147, actions at 148–174); `CanvasHoverToolbar.jsx`, lines 41–304 (`directActions` at 166 and 213–226); and `CanvasElements.jsx`, lines 205–212 and 409–421 (`gridEntryAnchorsById` and cell mount) — mount only `+` and trash for each eligible textarea cell, keep the toolbar on one outer gutter side, support hover/focus/`Shift+F10`/`Escape`, avoid stacking a toolbar frame over selection/edit chrome, and keep the final trash disabled.
...
- `frontend/src/utils/sectionBuilder.test.js`, lines 189–212 for the grid contract, plus `describe("buildSectionElements", …)` and `describe("build -> append -> reorder (composed production pipeline)", …)` — isolated layout construction and the composed production path used by `handleAddSection`.
- `frontend/src/utils/gridSection.test.js`, lines 103–267 — discovery without Skills-chip false positives, insertion after the hovered cell, fixed-column wrapping, semantic renamed Languages, uniform styling, deletion compaction, last-cell protection, and explicit custom-**JĘZYKI** precedence.
- `frontend/src/components/canvas/GridEntryActions/GridEntryActions.test.js`, lines 10–56; `CanvasElements.test.js`, lines 42–51; `CanvasHoverToolbar.test.js`, lines 27–37; and `frontend/src/pages/PdfCanvas.test.js`, lines 83–90 — exact two-action contract, keyboard path, fixed gutter side, one-frame hover regression, context wiring, direct-action rendering, and final-entry disablement.
...
Hovering or focusing plain section content now paints the same complete, pointer-inert boundary as hovering its heading. For example, pointing at the summary copy outlines **PODSUMOWANIE together with its body**, rather than framing only the textarea. Record fields and repeatable grid cells retain their more precise record/cell controls and are excluded from this section-content trigger, preventing stacked competing frames. Content hover exposes visual context only; the section action toolbar remains attached to the heading, so document editing and PDF output are unchanged.

In **template mode**, hovering a detected **main or sidebar** section heading reveals one grouped toolbar anchored to that heading: its left edge is 34 px to the right of the heading's rendered right edge, and its control row follows the heading's vertical centre. Record actions use the same portal surface but anchor to the opposite side of the first title element, with the toolbar's right edge 16 px left of that title. These anchors are page-local before portal measurement, so they continue to follow their elements through zoom, reorder, lane transfer, and two-page view without using an A4 edge as the spacing reference. The whole section strip on the heading's page receives a subtle pointer-inert outer outline, so the user sees the semantic scope before acting. While the pointer is over the exact heading, a second pointer-inert **brown inner outline** hugs that element's rendered glyph/box geometry. Selection and structural hover are independent: a selected or inline-edited heading keeps its persistent selection frame, while pointer hover still reveals the section outline and draws a separate offset hover ring around the selected heading so neither state overwrites the other. The hovered id remains transient hook state rather than entering `A4_Elements`, so revealing this inner frame does not select the element, open the inspector, or create history; keyboard focus reveals the structural toolbar while retaining the standard blue `:focus-visible` path instead of imitating pointer hover. During `CanvasElements` render, the base rectangle uses **persisted model geometry only**; it never reads a DOM Range before React commits a reorder or lane transfer, because that Range would still describe the section's previous position. When hover or keyboard focus makes the toolbar visible, `SectionRecordAdd` measures the current and next heading in `useLayoutEffect`, after the new coordinates are committed and before paint. The measurement is keyed to both headings' geometry, typography, content, and zoom, so a visible toolbar cannot reuse the previous slot. The toolbar exposes the labelled **Sekcja** add action and **↑/↓** directly (disabled at lane boundaries); lane transfer, Skills layout, and the destructive action live under **More**. An ordinary single click on template text selects it, starts inline editing, and opens the element-properties panel in the same action. `Ctrl`/`Cmd`-click remains additive multi-selection, and a click emitted after a drag is ignored rather than starting an edit. Opening **More** pins the toolbar only for the duration of menu interaction. Transient hover remains visible for 1,000 ms after pointer leave. Controls retain a compact 36 px on-screen target at every canvas zoom, with 15 px icons, delayed Polish tooltips, neutral Swiss-style chrome, and single-toolbar exclusivity. Adding still inserts immediately **under that section** in the same lane; reorder and deletion still re-pack under the active rhythm. Freeform mode intentionally keeps its existing selection and editing contract.

Implementation:

- `frontend/src/components/canvas/CanvasHoverToolbar/CanvasHoverToolbar.jsx`, lines 42–311, component `CanvasHoverToolbar` (`anchorX` at lines 48 and 150–153), and `CanvasHoverToolbar.module.css`, lines 1–286 — page-local semantic/element outlines, an optional element-relative horizontal anchor with an A4-edge fallback for existing callers, the selected-element hover ring, and a viewport-measured body portal with screen-stable controls. `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, lines 71–331, component `SectionRecordAdd` (rendered-heading anchor at lines 242–251 and toolbar mount at 301–310) — section actions, selected/editing hover eligibility, content-context listeners, geometry-keyed post-commit heading measurement, bounded ink-limit refinement, and exact 34 px right-edge / vertical-centre alignment to the rendered heading
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, lines 95–174 (`fillSectionAnchors`) and 193–264 (`nestedStructuralHoverIds` / `sectionAnchorsById`), and `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, lines 132–167 (`contentHoverKey` / `sectionContextVisible`) — collect same-page plain text members, exclude record/grid triggers, and keep the full section highlight visible during content pointer hover or keyboard focus without opening the heading toolbar
...
- `frontend/src/components/canvas/CanvasHoverToolbar/CanvasHoverToolbar.test.js`, lines 1–46 — body-portal, live page-scale measurement, element-anchor fallback, fixed positioning, the selected-element hover ring, and the context-layer contract above the inspector; `frontend/src/components/canvas/CanvasElements/CanvasElements.test.js`, lines 19–40 — selected/editing record and heading hover eligibility
- `frontend/src/components/canvas/CanvasElements/CanvasElements.test.js`, lines 42–61 and 75–88, tests `section and record toolbars use distinct element-relative gaps and centred anchors` and `plain section content reveals the complete section outline without stealing nested controls`, plus `frontend/src/components/canvas/CanvasHoverToolbar/CanvasHoverToolbar.test.js`, lines 18–25 and 48–55 — explicit element-anchor/page-edge-fallback coverage, exact left/right anchor direction, live title measurement, section-content trigger wiring, and highlight-only rendering regression coverage
...
- `frontend/src/components/canvas/recordPlusSize.js`, lines 20–91, constants `SECTION_TOOLBAR_OFFSET_SCREEN_PX` / `RECORD_TOOLBAR_OFFSET_SCREEN_PX` and functions `structuralToolbarLayoutSize` / `resolveStructuralToolbarSide` — exact zoom-stable 34 px section and 16 px record gaps, screen-stable 36 px targets, menu/label dimensions, and outer-gutter resolution in two-page view
- `frontend/src/components/canvas/RecordBlockAdd/RecordBlockAdd.jsx`, lines 46–234, component `RecordBlockAdd`, especially the live title measurement and anchor at lines 101–157 and toolbar mount at 205–214 — record-specific **Wpis**, exact 16 px left-edge / vertical-centre alignment to the first rendered title element, reorder, selected/editing hover eligibility, state-dependent **Dodaj opis** / recoverable **Usuń opis**, recoverable record deletion, and exact hovered-field bounds on the shared toolbar
...
The pure starter adapter converts the choices into an empty canonical `cv_data` profile plus temporary sentinel content for the existing `POST /ai/fill_template` layout generator. After generation, sentinels become empty canvas fields with persistent editor-only hints. `cvDataBindings` maps both simple and composite fields back to canonical data, `starterSectionKey` stores structural ownership, and `starter_structure` preserves chosen visibility and priority. A template change re-materializes both completed and still-empty fields. In a two-column template, the selected order is applied independently inside the main and sidebar lanes.

The first real keystroke clears `starterPlaceholder` and synchronizes the bound `cv_data` path. Save and export require a non-empty bound name and focus that field on failure. Output is prepared from a clone: empty starter sections, contact icon/label pairs, and an unused starter photo are removed and the remaining document is packed again. The full editor graph is persisted in `root`, while the compact output graph is sent as `render_root`; this keeps hints available after save/reload without printing them. The backend validates the name again and stores all starter metadata in existing `extra_properties`, so no SQL migration is required.
...
- `frontend/src/utils/cvStarter.js`, lines 7–259, exports `createDefaultStarterConfig`, `buildStarterDocument`, `prepareStarterProfileForTemplate`, and `finalizeStarterElements`
- `frontend/src/utils/starterElementStructure.js`, lines 61–182, functions `applyStarterElementStructure` and `prepareStarterElementsForRender`
...
- `frontend/src/hooks/usePdfExport.js`, lines 31–257, hook `usePdfExport` — persists `root` and sends the compact `render_root`
...
- `frontend/src/utils/cvStarter.test.js`
...
After a CV is generated, the masthead contact row is editable channel-by-channel, Enhancv-style. Hovering a contact label paints a 1 px earth-accent outline around its editable box and reveals a **trash** that removes that channel — its icon **and** its label as a unit; a **`+`** at the band end lists the channels not currently shown and inserts one (with its icon). The outline is application chrome outside the box model, so it does not change wrapping or exported geometry; selection/editing keeps its persistent frame and keyboard focus keeps the stronger blue ring. After either action only the contact members re-center / re-wrap inside their reserved masthead zone. The header rule and every section below retain their original Y coordinates, so repeated add/remove actions cannot distort the template rhythm. Editing a channel's text still works by clicking it (single-line `text` element).
...
- **Direct-edit outline.** Hovering the full name or a visible job-position field paints the same 1 px earth-accent outline used by contact labels. Single-line `Text` and wrapped `Textarea` mastheads share the contract; selected/edited fields do not stack a second frame, `focus-visible` remains blue, and the outline never participates in A4 measurement or PDF serialization.
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.test.js`, lines 68–89 — regression for contact/name/title semantic targeting, both text primitives, the zero-height text fallback, the shared accent outline, and focus-visible precedence.
...
It calls `/ai/fill_template` through `useApplyCvTemplate`. The profile starts from import or the A4 starter, then incorporates direct edits and live language-grid structure before the same snapshot is used to fill and replace the document. Empty starter fields are reintroduced as sentinels only for generation and remain empty after materialization.
...
│   │   │   ├── canvas/CanvasHoverToolbar/ # Wspólny pasek w gutterze, tooltipy, obrys bloku oraz akcje bezpośrednie/menu
│   │   │   ├── canvas/GridEntryActions/ # Kontrolki + / kosz dla każdej komórki powtarzalnej siatki
│   │   │   ├── canvas/SectionRecordAdd/  # Kontrolki nagłówka i pełny obrys sekcji z hoveru/fokusu zwykłej treści
...
│   │   │   ├── editor/AddSectionModal/   # modal „+ Dodaj sekcję” (nazwa + wybór układu aa/grid/cc-sub/cc-edu/cc-exp)
...
Właściwości elementu otwierają się w **mierzonym inspektorze lewego górnego obszaru roboczego** (`Editor` przez `createPortal`). Lewa i górna krawędź podążają za rzeczywistym sidebarem i topbarem. Desktopowy inspektor preferuje szerokość 304 px i ma naturalną wysokość wynikającą z zawartości, ograniczoną do 480 px, dlatego krótki formularz — na przykład po zaznaczeniu elementu **Text** — nie zostawia pustego obszaru o stałej wysokości. Gdy dłuższy formularz osiąga limit, przewija się tylko jego część robocza; limit jest dodatkowo zmniejszany, jeśli w viewporcie pozostało mniej miejsca. Żywa krawędź A4 i dostępna szerokość viewportu pozostają twardymi limitami przy każdym zoomie: gdy 304 px naruszyłoby odstęp 15 px, panel zwęża się do użytecznej szerokości doku zamiast zasłaniać dokument. Poniżej 720 px układ nadal przechodzi w pełnowysokościowy drawer. Przy szerokości kontenera 280 px lub mniejszej opisane pola zajmują cały wiersz karty zamiast się ściskać. Kontrolki ikon i obie strony steppera liczbowego zachowują cele 36 × 36 px, a wąski formularz nie tworzy poziomego overflow. Inspektor pojawia się i znika przez 240 ms fade/slide z lewej strony; tryb ograniczonego ruchu usuwa przesunięcie. Papierowy nagłówek z typografią `ink` nazywa edytowany obiekt prostym językiem, a stały pas pomocy wyjaśnia **Ctrl + lewy przycisk myszy = zaznacz wiele elementów**. Każda edytowana wartość ma widoczną etykietę zrozumiałą dla laika (np. „Odstęp między wierszami”, „Od lewej krawędzi”, „Kolejność na stronie”). Pola liczbowe zachowują kontrakt minus/wartość/plus, natywny input i obsługę klawiatury. Wyrównanie strony nadal ma osobne ikony, **Text** i **TextArea** zachowują różne zestawy pól, tryb szablonu ukrywa własności należące do layoutu, a freeform zachowuje pełny zestaw i resizer.
...
- `frontend/src/components/editor/Editor/Editor.jsx`, linie 150–1391, komponent `Editor` i helpery `Group`, `NumField`, `ColorField`, `FontField`, `BulkToolbar`; `Editor.module.css`, linie 1–346; `frontend/src/utils/editorInspectorWidth.js`, linie 1–34, eksporty `EDITOR_INSPECTOR_FIXED_WIDTH_PX` i `resolveEditorInspectorWidth`; `Sidebar/Sidebar.jsx`, linie 67 i 157; `Topbar/Topbar.jsx`, linia 72 — mierzony inspektor lewego górnego rogu, preferowana szerokość desktopowa 304 px ze zwężaniem według żywej krawędzi strony przy każdym zoomie, naturalna wysokość ograniczona do 480 px i dostępnego viewportu, powierzchnie `paper`/`control`, aktywne/fokusowe/destrukcyjne stany spójne z Asystentem AI, żywe kotwice layoutu, pełnowierszowe pola w wąskim wariancie, cele 36 px, widoczne etykiety prostym językiem, pomoc wielokrotnego zaznaczenia, steppery ±, edycja zbiorcza (w tym stale widoczne kontrolki interlinii i odstępu między znakami dla zgodnej multiselekcji oraz pusty stan wartości mieszanej), odporny na pointer/klawiaturę toolbar zaznaczenia inline, kontekstowa akcja kropki wyróżniona podczas edycji Umiejętności w linii, jawna kolejność warstw edytora, responsywny układ kontenerowy i bramki trybu szablonu. Pokrycie regresji: `frontend/src/utils/editorInspectorWidth.test.js`, linie 9–39, trzy testy preferowanej szerokości i granic kolizji; `frontend/src/components/editor/Editor/Editor.test.js`, linie 5–34 i 44–68, kontrakty wysokości zależnej od treści oraz powierzchni/kontrolek; linie 94–107 i 125–140 zachowują kontrakty Umiejętności w linii i zbiorczych odstępów.
...
- `backend/app/schemas/pdf_schema.py`, linie 152–177; `backend/app/crud/pdfs.py`, linie 41–77 i 120–133; `shared/pdf-element.schema.json`, linie 421–594; oraz `frontend/src/utils/persistedCanvasElement.js`, linie 28–119, funkcja `hydratePersistedCanvasElement` — utrwalają i odtwarzają `flowLane` oraz pola tożsamości/geometrii siatki (`editorSectionLayout`, `editorGrid*`, `gridSectionId`, `gridColumns`, `gridGutter`, `gridWidth`, `gridLeft`, `gridKind`) przez `PdfElements.extra_properties`, bez dodawania kolumn bazy.
- `frontend/src/utils/sectionBuilder.js`, linie 72–80 (`SECTION_LAYOUTS`), 142–196 (prywatna `contentTextarea`), 211–228 (`resolveGridSectionColumns` / `gridSectionCellWidth`) i 355–581 (`buildSectionElements`) — konstruktory „aa”, „grid”, „cc-sub”, „cc-edu” i „cc-exp”; gałąź siatki stempluje `gridKind: "entries"` na nagłówku i pierwszej textarea `grid-member`. `lane: "sidebar"` stempluje `flowLane: "sidebar"` + `flowRole: "sidebar-chrome"` (specyfikacje linii rekordu w prywatnym `recordLineSpecs`; wysokości przez prywatne `measureGeneratorBlockHeight`; treść na `bodyLeft`; markery obrazu zachowują `src` / `alignWithText`; nagłówki wypełnionych pasów zachowują próbkowaną ramkę wyrównania).
- `frontend/src/utils/gridSection.js`, linie 453–483 (`listGridSectionEntryAnchors`), 499–556 (`insertGridSectionEntry`) i 575–617 (`removeGridSectionEntry`) — wykrywa wygenerowane Języki i jawne siatki własne bez mylenia ich z chipami Umiejętności; wstawia za aktywną komórką, przelicza równe tory/wysokości wierszy, zawija, kompaktuje po usunięciu, chroni ostatnią komórkę i przepakowuje dalsze sekcje w bieżącym spacingu.
...
- `frontend/src/hooks/useA4Elements.js`, linie 1053–1081 (`handleAddGridSectionEntry`) i 1250–1265 (`handleRemoveGridSectionEntry`) — stosuje operacje siatki, zaznacza dodaną komórkę, kolejkuje tombstone usunięcia i zwija puste strony. Istniejący `handleAddSection` odpowiada za opcjonalne `afterHeadingId` / `lane`, próbkowanie stylu, wybór/podmianę wzorca rekordu, budowę, umieszczenie i zaznaczenie.
...
- `frontend/src/components/editor/AddSectionModal/AddSectionModal.jsx`, linie 15–47 (`LAYOUT_OPTIONS`) i 58–220 (`AddSectionModal`) — nazwa + wybór pięciu układów, w tym **Krótkie wpisy w kolumnach** / `grid` z przykładem **JĘZYKI**, dynamiczny placeholder i opcjonalna galeria ikon; inny podtytuł dla wstawienia pod sekcją i doklejenia na końcu.
- `frontend/src/components/editor/AddSectionModal/AddSectionModal.test.js`, linie 15–21 — regresja nazw opisujących strukturę, opisu/przykładu siatki i usunięcia domenowych określeń blueprintów.
...
- `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, komponent `SectionRecordAdd` — adaptuje wspólny pasek guttera do dodawania/kolejności sekcji oraz menu transferu/układu/usunięcia, a także utrzymuje pełny semantyczny obrys sekcji nad zwykłą treścią
- `frontend/src/components/canvas/GridEntryActions/GridEntryActions.jsx`, linie 35–200 (`GridEntryActions`, ścieżka klawiatury 83–111, wybór guttera 129–135, pojedyncza ramka hover 137–147, akcje 148–174); `CanvasHoverToolbar.jsx`, linie 41–304 (`directActions` w 166 i 213–226); oraz `CanvasElements.jsx`, linie 205–212 i 409–421 (`gridEntryAnchorsById` i montaż przy komórce) — montują wyłącznie `+` i kosz dla każdej kwalifikującej się textarea, utrzymują pasek po jednej zewnętrznej stronie guttera, obsługują hover/fokus/`Shift+F10`/`Escape`, nie nakładają ramki paska na chrome zaznaczenia/edycji i wyłączają kosz ostatniego wpisu.
...
- `frontend/src/utils/sectionBuilder.test.js`, linie 189–212 dla kontraktu siatki, a także `describe("buildSectionElements", …)` i `describe("build -> append -> reorder (composed production pipeline)", …)` — izolowana budowa układów i złożona ścieżka produkcyjna używana przez `handleAddSection`.
- `frontend/src/utils/gridSection.test.js`, linie 103–267 — wykrywanie bez false-positive dla chipów Umiejętności, wstawienie za komórką z hoveru, zawijanie stałej liczby kolumn, semantyczne Języki po zmianie nagłówka, jednolity styl, kompaktowanie po usunięciu, ochrona ostatniej komórki i pierwszeństwo jawnej własnej siatki **JĘZYKI**.
- `frontend/src/components/canvas/GridEntryActions/GridEntryActions.test.js`, linie 10–56; `CanvasElements.test.js`, linie 42–51; `CanvasHoverToolbar.test.js`, linie 27–37; oraz `frontend/src/pages/PdfCanvas.test.js`, linie 83–90 — kontrakt dokładnie dwóch akcji, ścieżka klawiatury, stała strona guttera, regresja pojedynczej ramki hover, wiring kontekstu, render akcji bezpośrednich i wyłączenie usuwania ostatniego wpisu.
...
Najechanie albo ustawienie fokusu na zwykłej treści sekcji rysuje teraz tę samą kompletną, nieprzechwytującą wskaźnika granicę co hover nagłówka. Przykładowo wskazanie tekstu podsumowania obejmuje obrysem **PODSUMOWANIE razem z jego treścią**, zamiast ramkować wyłącznie textarea. Pola rekordów i komórki powtarzalnej siatki zachowują dokładniejsze kontrolki wpisu/komórki i są wyłączone z tego wyzwalacza, dzięki czemu konkurujące ramki nie nakładają się. Hover treści pokazuje tylko kontekst wizualny; pasek akcji sekcji pozostaje przypięty do nagłówka, więc edycja dokumentu i eksport PDF nie zmieniają się.

W **trybie szablonu** najechanie na wykryty nagłówek sekcji **głównej lub sidebara** odsłania jeden pogrupowany pasek zakotwiczony do tego nagłówka: jego lewa krawędź znajduje się 34 px na prawo od wyrenderowanej prawej krawędzi nagłówka, a rząd kontrolek jest wyśrodkowany względem wysokości nagłówka. Akcje wpisu korzystają z tej samej powierzchni portalowej, lecz kotwiczą się po przeciwnej stronie pierwszego elementu tytułowego — prawa krawędź panelu znajduje się 16 px na lewo od tego elementu. Kotwice są lokalne dla strony przed pomiarem portalu, dlatego podążają za elementami przy zmianie zoomu, kolejności, toru i w widoku dwóch stron, bez używania krawędzi A4 jako punktu odniesienia dla odstępu. Cały pasek sekcji na stronie nagłówka dostaje subtelny zewnętrzny obrys bez obsługi pointera, więc semantyczny zakres operacji jest widoczny przed kliknięciem. Gdy wskaźnik znajduje się nad konkretnym nagłówkiem, drugi, **brązowy obrys wewnętrzny** bez obsługi pointera ściśle obejmuje wyrenderowane glify/pole tego elementu. Zaznaczenie i hover strukturalny są niezależne: zaznaczony lub edytowany inline nagłówek zachowuje trwałą ramkę zaznaczenia, a hover nadal odsłania obrys sekcji i rysuje osobny pierścień wokół zaznaczonego nagłówka. Id spod wskaźnika pozostaje przejściowym stanem hooka zamiast trafiać do `A4_Elements`, dlatego pokazanie obrysu nie zaznacza elementu, nie otwiera panelu i nie tworzy historii; fokus klawiatury zachowuje standardowy niebieski `:focus-visible`. Podczas renderowania `CanvasElements` bazowy prostokąt korzysta **wyłącznie z utrwalonej geometrii modelu** i nie czyta DOM Range przed commitem Reacta po zmianie kolejności albo transferze. Gdy hover lub fokus pokazuje pasek, `SectionRecordAdd` mierzy bieżący i następny nagłówek w `useLayoutEffect`, po zatwierdzeniu nowych współrzędnych, ale przed narysowaniem klatki. Pomiar jest powiązany kluczem z geometrią, typografią, treścią i zoomem obu nagłówków, dlatego widoczny pasek nie używa poprzedniej pozycji. Bezpośrednio dostępne są podpisana akcja **Sekcja** oraz **↑/↓**; transfer między kolumnami, układ Umiejętności i usunięcie trafiają do menu **Więcej**. Zwykły pojedynczy klik tekstu zaznacza go, rozpoczyna edycję inline i otwiera panel właściwości. `Ctrl`/`Cmd`-klik nadal dodaje do multiselekcji, a klik po przeciąganiu jest ignorowany. Otwarcie menu **Więcej** przypina pasek tylko na czas interakcji, a hover przejściowy pozostaje widoczny przez 1000 ms po zejściu kursora. Cele zachowują 36 px na ekranie przy każdym zoomie, ikony mają 15 px, tooltipy są opóźnione, a aktywny może być tylko jeden pasek. Dodawanie wstawia sekcję bezpośrednio **pod bieżącą** w tym samym torze; reorder i usunięcie przepakowują dokument zgodnie z aktywnym rytmem. Tryb freeform zachowuje dotychczasowy kontrakt zaznaczania i edycji.

Implementacja:

- `frontend/src/components/canvas/CanvasHoverToolbar/CanvasHoverToolbar.jsx`, linie 42–311, komponent `CanvasHoverToolbar` (`anchorX` w liniach 48 i 150–153), oraz `CanvasHoverToolbar.module.css`, linie 1–286 — lokalne dla strony obrysy semantyczny/elementu, opcjonalna kotwica pozioma względem elementu z fallbackiem do krawędzi A4 dla dotychczasowych wywołań, pierścień hoveru zaznaczonego pola i mierzony względem viewportu portal ze stałymi ekranowo kontrolkami. `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, linie 71–331, komponent `SectionRecordAdd` (kotwica nagłówka w liniach 242–251 i montaż paska w 301–310) — akcje sekcji, dostępność hoveru przy zaznaczeniu/edycji, listenery kontekstu treści, pomiar po commicie, ograniczone doprecyzowanie granicy tuszu oraz dokładne wyrównanie 34 px od prawej krawędzi i środka wysokości wyrenderowanego nagłówka
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, linie 95–174 (`fillSectionAnchors`) i 193–264 (`nestedStructuralHoverIds` / `sectionAnchorsById`), oraz `frontend/src/components/canvas/SectionRecordAdd/SectionRecordAdd.jsx`, linie 132–167 (`contentHoverKey` / `sectionContextVisible`) — zbierają zwykłe elementy tekstowe z tej samej strony, wykluczają wyzwalacze rekordów/siatek i utrzymują pełny obrys sekcji podczas hoveru albo fokusu treści bez otwierania paska nagłówka
...
- `frontend/src/components/canvas/CanvasHoverToolbar/CanvasHoverToolbar.test.js`, linie 1–46 — portal do `body`, żywy pomiar skali strony, fallback kotwicy elementu, pozycjonowanie fixed, pierścień hoveru zaznaczonego elementu i kontrakt warstwy kontekstowej nad inspektorem; `frontend/src/components/canvas/CanvasElements/CanvasElements.test.js`, linie 19–40 — dostępność hoveru zaznaczonych/edytowanych pól wpisu i nagłówka
- `frontend/src/components/canvas/CanvasElements/CanvasElements.test.js`, linie 42–61 i 75–88, testy `section and record toolbars use distinct element-relative gaps and centred anchors` oraz `plain section content reveals the complete section outline without stealing nested controls`, a także `frontend/src/components/canvas/CanvasHoverToolbar/CanvasHoverToolbar.test.js`, linie 18–25 i 48–55 — pokrycie jawnej kotwicy elementu i fallbacku krawędzi strony, kierunku lewej/prawej kotwicy, żywego pomiaru tytułu, połączenia hoveru treści sekcji i renderowania samego obrysu
...
- `frontend/src/components/canvas/recordPlusSize.js`, linie 20–91, stałe `SECTION_TOOLBAR_OFFSET_SCREEN_PX` / `RECORD_TOOLBAR_OFFSET_SCREEN_PX` i funkcje `structuralToolbarLayoutSize` / `resolveStructuralToolbarSide` — dokładne, stałe przy zoomie odstępy 34 px sekcji i 16 px wpisu, ekranowe cele 36 px, wymiary menu/etykiety i wybór zewnętrznego guttera w widoku dwóch stron
- `frontend/src/components/canvas/RecordBlockAdd/RecordBlockAdd.jsx`, linie 46–234, komponent `RecordBlockAdd`, szczególnie żywy pomiar i kotwica tytułu w liniach 101–157 oraz montaż paska w 205–214 — akcje **Wpis**, dokładne wyrównanie 16 px od lewej krawędzi i środka wysokości pierwszego wyrenderowanego elementu tytułowego, kolejność, dostępność hoveru przy zaznaczeniu/edycji, zależne od stanu **Dodaj opis** / odwracalne **Usuń opis**, odwracalne usunięcie rekordu i granice konkretnego pola pod wskaźnikiem
...
- `frontend/src/utils/cvStarter.js`, linie 7–259, eksporty `createDefaultStarterConfig`, `buildStarterDocument`, `prepareStarterProfileForTemplate` i `finalizeStarterElements`
- `frontend/src/utils/starterElementStructure.js`, linie 61–182, funkcje `applyStarterElementStructure` i `prepareStarterElementsForRender`
...
- `frontend/src/hooks/usePdfExport.js`, linie 31–257, hook `usePdfExport` — utrwala `root` i wysyła zwartą kopię `render_root`
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, linie 333–336 i 389–464; `Text/Text.jsx`, linie 54 i 204–215; `Text/Text.module.css`, linie 25–52 — rozpoznają semantyczne pola kontaktowe w trybie szablonu i rysują ich neutralną dla layoutu obwódkę hover, łącznie z fallbackiem tekstu canvasu o zerowej interlinii.
...
- **Obramowanie edycji bezpośredniej.** Najechanie na imię i nazwisko albo widoczne pole stanowiska rysuje to samo ziemiste obramowanie 1 px co przy etykietach kontaktowych. Jednowierszowy `Text` i zawijany `Textarea` współdzielą ten kontrakt; pola zaznaczone/edytowane nie nakładają drugiej ramki, `focus-visible` pozostaje niebieski, a obwódka nigdy nie uczestniczy w pomiarze A4 ani serializacji PDF.
...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.test.js`, linie 68–89 — regresja tagowania semantycznego kontaktu/imienia/stanowiska, obu prymitywów tekstowych, fallbacku tekstu o zerowej wysokości, wspólnej obwódki akcentowej i pierwszeństwa `focus-visible`.
