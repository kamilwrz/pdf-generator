...
- `backend/app/services/ai_service.py`, lines 70–518, functions `_completion_request_options`, `_request_completion`, `_visible_response_text`, `_pdf_text_pages`, `_pdf_pages_to_b64_images`, `_provider_settings`, `_message_text`, `_parse_model_json`, and `extract_cv_data` — model-specific parameters, provider selection, text/vision routing, one-shot text fallback, safe diagnostics, and JSON validation
...
- `backend/tests/test_cloudflare_cv_extraction.py`, lines 43–269, class `CloudflareCvExtractionTests` — provider selection, text/vision payloads, model-specific budgets, Llama JSON Mode, fenced/typed JSON, safe empty responses, the one-shot Gemma-to-Llama fallback, rollback JSON Mode, and server-side credential gating
...
- `backend/app/services/ai_service.py`, linie 70–518, funkcje `_completion_request_options`, `_request_completion`, `_visible_response_text`, `_pdf_text_pages`, `_pdf_pages_to_b64_images`, `_provider_settings`, `_message_text`, `_parse_model_json` i `extract_cv_data` — parametry zależne od modelu, wybór dostawcy, routing tekst/vision, jednorazowy fallback tekstowy, bezpieczna diagnostyka i walidacja JSON
...
- `backend/tests/test_cloudflare_cv_extraction.py`, linie 43–269, klasa `CloudflareCvExtractionTests` — wybór dostawcy, payloady text/vision, budżety zależne od modelu, Llama JSON Mode, fenced/typed JSON, bezpieczne puste odpowiedzi, jednorazowy fallback Gemma→Llama, JSON Mode rollbacku i server-only credentials
