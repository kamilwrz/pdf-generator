...
- `frontend/src/utils/sectionStructure.js`, lines 939–956 and 1113–1194, private `resolveSidebarPhotoFloor` and function `packSidebarLane` — visible-photo and hidden-contact rail floors used by every spacing, fit, transfer, and reorder pack; also functions `deriveSectionStyle` (line 2727), `sectionChromeRuleRelTop` (line 3014), `healSimpleChromeRuleGaps` (line 285), and private `pickLinearBodySample` (line 2644)
...
- `frontend/src/utils/sectionStructure.js`, lines 841–2089, functions `resolveFlowStart` and `packDocumentSections` — honours a generator-authored `mainFlowStart` before applying generic masthead-gap recovery, so reorder cannot pull Linden's first main section into the identity area.
...
- `frontend/src/utils/profilePhotoVisibility.js`, lines 71–384, functions `hiddenProfileContactSectionFloor`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto`, `profilePhotoControlAnchor`, `alignSidebarAfterProfileContacts` — shared 40 pt contact floor, reversible visibility, order-independent Portico photo/title reflow with fixed footer chrome, complete Slate/Tessera cluster hiding, actual contact-stack measurement and contact/sidebar transfer, plus legacy document fallbacks
...
- `frontend/src/hooks/useA4Elements.js`, lines 2338–2405, callbacks `removeContactChannel`, `addContactChannel`, `toggleTitle`, `setProfilePhotoVisible`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto` — history-aware commits, post-layout sidebar alignment, and the Portico-only cross-page masthead reflow
- `frontend/src/components/gallery/GalleryItem/GalleryItem.jsx`, lines 32–45 — template-mode click → `applyProfilePhoto` (no prompt)
- `frontend/src/utils/sectionStructure.js`, lines 939–956 and 1113–1194 — ignores hidden photo chrome and reapplies the shared contact-derived rail floor during full structural packs
...
- Tests: `frontend/src/utils/profilePhotoVisibility.test.js`, lines 115–146 (two-page reflow in both Portico photo/title toggle orders); `frontend/src/utils/sectionStructure.test.js`, lines 1887–1950; `frontend/src/templates/slate.test.js`, lines 6–78; `frontend/src/templates/tessera.test.js`, lines 6–81; `backend/tests/test_cv_template_layouts.py`, lines 103–215 and 258–378; `backend/tests/test_contact_channel_roundtrip.py`, lines 54–76; `backend/tests/test_pdf_watermark.py`, lines 119–137
...
- `frontend/src/utils/sectionStructure.js`, linie 939–956 i 1113–1194, prywatne `resolveSidebarPhotoFloor` oraz funkcja `packSidebarLane` — progi widocznego zdjęcia i ukrytego stosu kontaktów używane przy każdym pakowaniu odstępów, fit, transferze i reorderze; ponadto funkcje `deriveSectionStyle` (linia 2727), `sectionChromeRuleRelTop` (linia 3014), `healSimpleChromeRuleGaps` (linia 285) i prywatne `pickLinearBodySample` (linia 2644)
...
- `frontend/src/utils/sectionStructure.js`, linie 841–2089, funkcje `resolveFlowStart` i `packDocumentSections` — respektuje wygenerowany `mainFlowStart` przed ogólnym leczeniem odstępu mastheadu, więc reorder nie podciąga pierwszej głównej sekcji pod obszar tożsamości.
...
- `frontend/src/utils/profilePhotoVisibility.js`, linie 71–384, funkcje `hiddenProfileContactSectionFloor`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto`, `profilePhotoControlAnchor`, `alignSidebarAfterProfileContacts` — wspólny próg kontaktów 40 pt, odwracalna widoczność, niezależny od kolejności reflow zdjęcia/stanowiska Portico ze stałym chrome stopki, ukrycie kompletnego klastra, pomiar rzeczywistego stosu kontaktów i transfer kontaktów/sidebara Slate/Tessera oraz fallbacki starszych dokumentów
...
- `frontend/src/hooks/useA4Elements.js`, linie 2338–2405, callbacki `removeContactChannel`, `addContactChannel`, `toggleTitle`, `setProfilePhotoVisible`, `hideProfilePhoto`, `showProfilePhoto`, `removeProfilePhoto` — operacje historii, wyrównanie sidebara oraz właściwy dla Portico reflow mastheadu przez granice stron
...
- `frontend/src/utils/sectionStructure.js`, linie 939–956 i 1113–1194 — ignorowanie ukrytego chrome zdjęcia i ponowne stosowanie wspólnego progu wyprowadzonego z kontaktów podczas pełnego pakowania strukturalnego
...
- Testy: `frontend/src/utils/profilePhotoVisibility.test.js`, linie 115–146 (dwustronicowy reflow w obu kolejnościach przełączania zdjęcia/stanowiska Portico); `frontend/src/utils/sectionStructure.test.js`, linie 1887–1950; `frontend/src/templates/slate.test.js`, linie 6–78; `frontend/src/templates/tessera.test.js`, linie 6–81; `backend/tests/test_cv_template_layouts.py`, linie 103–215 i 258–378; `backend/tests/test_contact_channel_roundtrip.py`, linie 54–76; `backend/tests/test_pdf_watermark.py`, linie 119–137
