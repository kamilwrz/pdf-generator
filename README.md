...
- `backend/app/services/ai_assistant_service.py` — `_safe_result` structured scores; `_rate_cv` / `_rate_design` / `_rate_position` / `_ats_score` category JSON; `_translate_cv`; `_layout_session`; `_chat`
...
- `backend/app/services/ai_assistant_service.py` — strukturalne wyniki ocen, `_translate_cv`, `_layout_session`, `_chat`
- `backend/app/services/entitlements.py`, `PRO_ONLY_AI_ACTIONS` = `{design_rating, layout}` — **Sprawdź wygląd**
