...
**Section placement is balance-driven, and generalizes to every page the main column occupies.** Rather than filling the sidebar first, Sterling measures every section's height in both column widths and calls `plan_columns_multi_page` (`backend/app/services/cv_templates/shared/column_planner.py`), which partitions sections into the main column and one `SidebarBucket` per page the main column occupies. Page 1's rail is filled first; continuation-page rails are overflow catchers that receive sidebar-affinity sections that do not fit page 1, and also main-affinity leftovers (Education) that would start on page 2+ of the main column. Experience is anchored to the main column; every other section is movable, and may render in any column/bucket. Because a sidebar bucket cannot paginate, its assignment is a hard per-page fit, while the main column may overflow onto later pages (that overflow is therefore not counted as wasted space). Because which sections belong in the sidebar depends on how many pages the main column needs, and vice versa, `plan_columns_multi_page` resolves this with a bounded iteration (≤3 passes): partition with the pure `plan_columns` planner, measure the resulting main-column order's real page count by rendering it into a throwaway `Builder`, derive one `SidebarBucket` per page ≥ 2 that measurement found, and repeat until the derived bucket list stops changing. A CV whose main column fits on page 1 never derives a bucket beyond page 1, so this reduces to exactly the original single-page behavior. In practice Education follows Experience in the main column, and moves into the page-1 sidebar only when a long Experience block already fills page 1 *and* the rail still has room; a short extra section (e.g. Certifications) that doesn't fit page 1's rail can land on page 2's rail instead of the main column, once the main column actually spans 2 pages. If Education itself would start on page 2 of the main column because Experience already filled page 1 and page 1's rail is full, a fourth planner pass places the whole Education section on page 2's rail rather than leaving it in the main column beside an empty continuation rail. Education stays affinity `main` so a short Experience block still keeps it on page 1 even when a later extra paginates. The pure partitioner itself is a small greedy local search: seed each sidebar-affinity section by first-fit across buckets in ascending page order, force every bucket under budget (evicting the lowest-priority overflow back to main), then repeatedly apply the single move that most reduces `max(empty_main, empty_page1_sidebar)` until the columns are balanced or no move clears a minimum-improvement threshold. After that balance loop, leftover movable sections that would start on page 2+ of the main column are first-fit into continuation rails. Only page 1's rail (and the page-1 main column) enters that balance objective: the multi-page budgets are page-1-scoped, so the balancer fills page 1 as the single-page planner always did and never drains it to equalise fill across continuation rails or to fill the main column's multi-page capacity. See `docs/superpowers/specs/2026-08-12-multi-page-column-planner-design.md` for the full design, including the circular-dependency resolution and rejected alternatives (e.g. simulating pagination per candidate move, rejected for latency).
...
**Rozmieszczanie sekcji jest sterowane balansem i uogólnia się na każdą stronę, którą zajmuje kolumna główna.** Zamiast najpierw wypełniać sidebar, Sterling mierzy wysokość każdej sekcji w obu szerokościach kolumn i wywołuje `plan_columns_multi_page` (`backend/app/services/cv_templates/shared/column_planner.py`), który dzieli sekcje między kolumnę główną a po jednym `SidebarBucket` na każdą stronę zajmowaną przez kolumnę główną. Szyna strony 1 jest wypełniana jako pierwsza; szyny stron kontynuacyjnych są zbiornikami nadmiaru i przyjmują sekcje o przynależności do sidebara, które nie mieszczą się na stronie 1, a także resztki o przynależności do kolumny głównej (Wykształcenie), które zaczęłyby się na stronie 2+ kolumny głównej. Doświadczenie jest zakotwiczone w kolumnie głównej; każda inna sekcja jest ruchoma i może wyrenderować się w dowolnej kolumnie/kubełku. Ponieważ pojedynczy kubełek sidebara nie może dzielić się na strony, jego przydział to twarde dopasowanie na daną stronę, podczas gdy kolumna główna może przechodzić na kolejne strony (jej nadmiar nie jest więc liczony jako zmarnowane miejsce). Ponieważ to, które sekcje należą do sidebara, zależy od liczby stron potrzebnych kolumnie głównej — i odwrotnie — `plan_columns_multi_page` rozwiązuje to ograniczoną iteracją (≤3 przebiegi): partycjonuje czystym planerem `plan_columns`, mierzy rzeczywistą liczbę stron wynikowego porządku kolumny głównej, renderując go do jednorazowego `Builder`, wyprowadza po jednym `SidebarBucket` dla każdej strony ≥ 2, którą wykazał pomiar, i powtarza, aż wyprowadzona lista kubełków przestanie się zmieniać. CV, którego kolumna główna mieści się na stronie 1, nigdy nie wyprowadza kubełka poza stroną 1, więc sprowadza się to dokładnie do pierwotnego zachowania jednostronicowego. W praktyce Wykształcenie następuje po Doświadczeniu w kolumnie głównej i przechodzi do sidebara strony 1 tylko wtedy, gdy długie Doświadczenie zapełnia już stronę 1 *i* w szynie jest jeszcze miejsce; krótka sekcja dodatkowa (np. Certyfikaty), która nie mieści się w szynie strony 1, może trafić do szyny strony 2 zamiast do kolumny głównej, gdy kolumna główna faktycznie zajmuje 2 strony. Jeśli samo Wykształcenie zaczęłoby się na stronie 2 kolumny głównej, bo Doświadczenie zapełniło już stronę 1, a szyna strony 1 jest pełna, czwarty przebieg planera umieszcza całą sekcję Wykształcenie na szynie strony 2, zamiast zostawiać ją w kolumnie głównej obok pustej szyny kontynuacyjnej. Wykształcenie zachowuje przynależność `main`, więc krótkie Doświadczenie nadal trzyma je na stronie 1 nawet wtedy, gdy późniejsza sekcja dodatkowa paginuje. Sam czysty partycjoner to małe zachłanne przeszukiwanie lokalne: zasiej każdą sekcję o przynależności do sidebara metodą first-fit po kubełkach w rosnącej kolejności stron, wepchnij każdy kubełek poniżej budżetu (wypychając nadmiar o najniższym priorytecie z powrotem do głównej), a następnie wielokrotnie zastosuj pojedynczy ruch najbardziej redukujący `max(puste_główna, puste_sidebar_strony1)`, aż kolumny się zrównoważą lub żaden ruch nie przekroczy progu minimalnej poprawy. Po tej pętli balansu resztki ruchomych sekcji, które zaczęłyby się na stronie 2+ kolumny głównej, są dopasowywane first-fit do szyn kontynuacyjnych. Tylko szyna strony 1 (i kolumna główna strony 1) wchodzi do tej funkcji celu: budżety wielostronicowe są ograniczone do strony 1, więc balanser wypełnia stronę 1 tak jak zawsze robił to planer jednostronicowy i nigdy jej nie opróżnia, aby wyrównać wypełnienie szyn kontynuacyjnych ani aby zapełnić wielostronicową pojemność kolumny głównej. Pełny opis projektu, w tym rozwiązanie cyklicznej zależności i odrzucone alternatywy (np. symulacja paginacji dla każdego kandydującego ruchu, odrzucona ze względu na opóźnienia), znajduje się w `docs/superpowers/specs/2026-08-12-multi-page-column-planner-design.md`.
