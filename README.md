...
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, lines 468–1075, component `AiAssistant` — layout-session history boundary, request flow, review cards, and composer
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, lines 506–514 and 1048–1066 — auto-growing two-line chat composer
...
- `backend/app/services/ai_assistant_service.py`, lines 1044–1142, `_layout_session`; plus `_model_for_action`, `_chat`, `_rate_design`
- `backend/app/services/layout_gpt.py`, lines 24–500 (`SAFE_SECTION_GAP_*`, `_can_share_text_row`, `_build_text_rows`, `_build_layout_snapshot_data`, `build_layout_snapshot`, `build_layout_user_prompt`), 540–608 (`_resolve_model_references`), 702–777 (`_parse_section_inventory`, `_affected_text_ids`), 778–895 (`_changes_to_findings`, `_collapses_safe_section_gap`), and 963–1206 (`compile_layout_gpt_response`)
...
Tests: `backend/tests/test_layout_gpt.py`, lines 103–113 (`test_user_prompt_preserves_safe_section_header_whitespace`), 129–190 (row grouping and text-height regressions), and 294–334 (`test_compile_rejects_collapsing_a_safe_section_header_gap`), plus the inventory and compiler tests in the same module; also `test_openai_pricing.py`, `test_ai_credits.py`, `test_ai_chat_command.py`, and `test_layout_analysis.py`.
...
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, linie 468–1075, komponent `AiAssistant` — granica historii sesji Układu, wysyłanie żądań, karty zmian i pole wpisywania
- `frontend/src/components/ai/AiAssistant/AiAssistant.jsx`, linie 506–514 i 1048–1066 — automatycznie rosnące, dwuwierszowe pole czatu
...
- `backend/app/services/ai_assistant_service.py`, linie 1044–1142 — `_layout_session`; ponadto `_model_for_action`
- `backend/app/services/layout_gpt.py`, linie 24–500 (`SAFE_SECTION_GAP_*`, `_can_share_text_row`, `_build_text_rows`, `_build_layout_snapshot_data`, `build_layout_snapshot`, `build_layout_user_prompt`), 540–608 (`_resolve_model_references`), 702–777 (`_parse_section_inventory`, `_affected_text_ids`), 778–895 (`_changes_to_findings`, `_collapses_safe_section_gap`) i 963–1206 (`compile_layout_gpt_response`)
- `backend/app/services/layout_analysis.py` — `resolve_directed_operation`, `summarize_geometry_issues`
- `backend/app/services/openai_pricing.py` — `usage_from_response`, `estimate_cost_usd`

Testy: `backend/tests/test_layout_gpt.py`, linie 103–113 (`test_user_prompt_preserves_safe_section_header_whitespace`), 129–190 (regresje grupowania wierszy oraz wysokości tekstu) i 294–334 (`test_compile_rejects_collapsing_a_safe_section_header_gap`), a także testy inwentarza i kompilatora w tym samym module; ponadto `test_openai_pricing.py`, `test_ai_credits.py`, `test_ai_chat_command.py` i `test_layout_analysis.py`.
