...
- `frontend/src/components/canvas/CanvasElements/CanvasElements.jsx`, lines 88–165 and 298–345, functions `fillSectionAnchors`, `sectionAnchorsById`, and prop `spreadSide` — computes lane-local movement/transfer flags plus a page-local semantic highlight and forwards the physical spread edge; `frontend/src/utils/canvasHighlightBounds.js`, lines 1–83, functions `unionCanvasBounds`, `elementBoundsOnPage`, and `includeRenderedElementBounds`, together with `frontend/src/utils/elementBounds.js`, lines 125–139, function `getVisualBounds` — unions real text ink, optically shifted icon, and ordinary element boxes; `frontend/src/pages/PdfCanvas.jsx`, lines 1927–1946, two-page `visiblePages.map` — assigns outer-left / outer-right toolbar gutters while publishing deletion snapshots and structural handlers through `PdfContext`; `frontend/src/App.css`, lines 121–133, `.canvas-spread` — reserves both outside gutters inside the horizontal scroll extent
...
- `frontend/src/utils/canvasHighlightBounds.test.js`, lines 1–87 — exact Portico regression proving the 14 px optically aligned icon moves the semantic border from stored `top: 259` to painted `top: 253`, plus live-heading union and missing-DOM fallback coverage
...
- `frontend/src/utils/canvasHighlightBounds.test.js`, linie 1–87 — dokładna regresja Portico potwierdzająca, że 14-pikselowa ikona wyrównana optycznie przesuwa krawędź semantyczną z zapisanego `top: 259` do narysowanego `top: 253`, a także testy sumy z żywym nagłówkiem i fallbacku bez DOM
