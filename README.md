...
- `frontend/src/utils/sectionStructure.js`, function `isDecorativeOrdinalChrome`; private `resolveSectionChromeBandStart`; function `sectionElementIds`; private `sameColumnAsHeading` (two-column sidebar exclusion, see above); functions `listSidebarSections` (lines 671–), `sidebarSectionElementIds`, `packSidebarLane` (lines 749–, optional `orderedHeadingIds`); function `applyFlowSpacing` (lines 1923–, main pack then sidebar lane); functions `appendSectionAtEnd` (lines 1512–, `lane: "sidebar"`), `insertSectionAfter` (lines 1672–, auto-detects sidebar anchors), `reorderSection` (lines 1780–), `removeSection` (lines 1844–), `deriveSectionStyle` (lines 2005–, optional `{ lane: "sidebar" }`) — ordinal safety net, chrome-band section boundaries, style sampling (`bodyLeft`, rule `relLeft`, optional `fromHeadingId` / sidebar rail defaults), end-of-document and after-section placement with full-document rhythm retarget
...
- `backend/app/services/cv_templates/templates/nimbus.py`, `_gen_nimbus` — Lora masthead with name accent bar, square `nimbus-photo-frame` slot (contact under the photo), 3 px rules, 56 px clearance under the header divider, heading+underline chrome only (no section chips); `test_nimbus_keeps_education_record_with_heading_near_page_break`
...
- `frontend/src/utils/sectionStructure.js`, funkcja `isDecorativeOrdinalChrome`; prywatne `resolveSectionChromeBandStart`; funkcja `sectionElementIds`; prywatne `sameColumnAsHeading` (wykluczanie dwukolumnowego sidebara, patrz wyżej); funkcje `listSidebarSections` (linie 671–), `sidebarSectionElementIds`, `packSidebarLane` (linie 749–, opcjonalne `orderedHeadingIds`); funkcja `applyFlowSpacing` (linie 1923–, najpierw kolumna główna, potem tor sidebara); funkcje `appendSectionAtEnd` (linie 1512–, `lane: "sidebar"`), `insertSectionAfter` (linie 1672–, auto-wykrywanie kotwic sidebara), `reorderSection` (linie 1780–), `removeSection` (linie 1844–), `deriveSectionStyle` (linie 2005–, opcjonalne `{ lane: "sidebar" }`) — siatka bezpieczeństwa dla odznak, granice pasa chrome, próbkowanie stylu (`bodyLeft`, `relLeft` linii, opcjonalne `fromHeadingId` / domyślne wartości wąskiej szyny) oraz umieszczanie na końcu / pod sekcją z ujednoliceniem rytmu całego dokumentu
...
- `backend/app/services/cv_templates/templates/nimbus.py`, `_gen_nimbus` — masthead Lora z paskiem przy imieniu, kwadratowym slotem `nimbus-photo-frame` (kontakt pod zdjęciem), linie 3 px, 56 px pod dzielnikiem, chrome sekcji = etykieta + podkreślenie (bez chipów); `test_nimbus_keeps_education_record_with_heading_near_page_break`
