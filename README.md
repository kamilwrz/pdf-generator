...
2. `recordStackGap` recognises consecutive textareas that look like title → meta/body (larger or bold line above a smaller line) and always packs them with `SPACE_STACK` from the previous placed bottom. That heals gaps already inflated to 14 px (for example after a title shrunk without moving its meta, or after meta was parked at the next page top while the title stayed on page 1). The heal also runs when the title's measured height matches the stored height, so a load/font remeasure without a height delta still restores 4 px. Meta → next title keeps `SPACE_RECORD` (14 px) because the smaller line comes first.
...
| `pdf_elements` | Canvas elements; geometry + style columns; extras in `extra_properties` JSON (`fixedToPage`, `locked`, bold, connectors, …) |
...
- Project: [`docs/cv-template-generation.md`](docs/cv-template-generation.md), [`docs/FEATURES.md`](docs/FEATURES.md), [`docs/designs/cv-only-ux-monetization.md`](docs/designs/cv-only-ux-monetization.md).
...
Ten README to wejście techniczne dla programistów. Opis produktowy funkcji: [`docs/FEATURES.md`](docs/FEATURES.md). Brief marketingowy pod sekcję „Dlaczego CV STUDIO” na stronie (funkcje + pozycjonowanie względem rynku, bez nazw marek konkurencji w copy publicznym): [`FEATURES_MARKETING.md`](FEATURES_MARKETING.md). Generowanie szablonów (AI extract vs layout w Pythonie): [`docs/cv-template-generation.md`](docs/cv-template-generation.md).
...
2. `recordStackGap` rozpoznaje kolejne textarea wyglądające jak tytuł → meta/treść (większa lub pogrubiona linia nad mniejszą) i zawsze pakuje je z `SPACE_STACK` od dolnej krawędzi poprzedniego elementu. To leczy odstępy już nadmuchane do 14 px (np. po skurczeniu tytułu bez przesunięcia meta albo po zaparkowaniu meta na górze następnej strony, gdy tytuł został na stronie 1). Leczenie działa też wtedy, gdy zmierzona wysokość tytułu jest już równa zapisanej — ponowny pomiar po załadowaniu fontów bez delty wysokości i tak przywraca 4 px. Meta → następny tytuł zachowuje `SPACE_RECORD` (14 px), bo mniejsza linia jest pierwsza.
...
| `pdfs` | Dokumenty CV |
| `pdf_elements` | Elementy kanwy (+ `extra_properties`) |
...
- Projekt: [`docs/cv-template-generation.md`](docs/cv-template-generation.md), [`docs/FEATURES.md`](docs/FEATURES.md), [`docs/designs/cv-only-ux-monetization.md`](docs/designs/cv-only-ux-monetization.md)
