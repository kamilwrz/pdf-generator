...
The same Sterling appearance panel exposes role-aware **S / M / L / XL** text presets. **M** is the original template scale. Display names grow least, body copy grows most, and headings, record titles, metadata, job position, and contacts use intermediate factors; body and metadata retain hard readability floors. The utility stores immutable baseline font size and line height per text element, so switching XL → S → M restores the exact authored metrics instead of multiplying rounded values. A preset is applied as one layout transaction: every flow textarea receives an immediate line-wrap height estimate (without the new-field caret allowance), the contact band is laid out from its scaled descriptor, and `applyFlowSpacing` packs the main and sidebar lanes before the first updated frame is painted. Browser auto-height measurements still refine glyph-specific wrapping afterwards, but they no longer operate on a mixture of stale and resized boxes that can stack later records or section headings on top of earlier text. Page chrome is reconciled only after the pack, so genuine overflow creates a continuation while returning to M can collapse it again. The selected `{ palette, textSize }` intent and typography baselines are persisted through `PdfElements.extra_properties`, restored when a saved document opens, and ignored safely by ReportLab.
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, lines 91–539, component `SectionsPanel` — **Dostosuj CV** tabs, document status, grouped section structure, density controls, Sterling palette cards, and S–XL typography controls; `frontend/src/components/editor/SectionsPanel/SectionsPanel.module.css`, lines 1–130 — responsive drawer, miniature sheets, segmented type scale, focus states, and mobile overlay; tests: `SectionsPanel.test.js`, lines 1–89
- `frontend/src/utils/sterlingAppearance.js`, lines 1–302, exports `STERLING_PALETTES`, `getSterlingAppearance`, `applySterlingPalette`, and `applySterlingTextSize` — semantic colour replacement, palette-specific icon paths, reversible role-aware type baselines, immediate flow-textarea height estimates, and persisted appearance intent; tests: `sterlingAppearance.test.js`, lines 1–53
- `frontend/src/utils/sterlingTypographyLayout.js`, lines 1–37, function `applySterlingTextSizeLayout`; `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, lines 218–228, function `handleSterlingTextSize` — atomic contact relayout, main/sidebar packing, and page-chrome reconciliation for each type preset; regression: `frontend/src/utils/sterlingTypographyLayout.test.js`, lines 53–70
...
Ten sam panel Sterlinga ma zależne od roli presety tekstu **S / M / L / XL**. **M** oznacza oryginalną skalę szablonu. Imię i nazwisko rośnie najmniej, treść główna najmocniej, a nagłówki, tytuły wpisów, metadane, stanowisko i kontakty korzystają z pośrednich mnożników; body i metadata zachowują twarde minima czytelności. Narzędzie zapisuje niezmienne bazowe wartości font-size i line-height przy każdym elemencie tekstowym, więc sekwencja XL → S → M odtwarza dokładne metryki autora zamiast mnożyć zaokrąglone wyniki. Preset jest stosowany jako jedna transakcja layoutu: każde pole textarea przepływu od razu dostaje oszacowaną wysokość po zawinięciu (bez zapasu na caret nowego pola), blok kontaktów jest układany z przeskalowanego deskryptora, a `applyFlowSpacing` pakuje main i sidebar przed narysowaniem pierwszej zaktualizowanej klatki. Pomiary auto-height w przeglądarce nadal doprecyzowują zawijanie dla rzeczywistych glifów, ale nie działają już na mieszaninie starych wysokości i nowych metryk, która mogła nakładać późniejsze wpisy lub nagłówki sekcji na wcześniejszy tekst. Chrome stron jest uzgadniane dopiero po pakowaniu, więc realny overflow tworzy kontynuację, a powrót do M może ją ponownie zwinąć. Wybrany zamiar `{ palette, textSize }` oraz bazowe metryki są zapisywane w `PdfElements.extra_properties`, odtwarzane przy otwarciu dokumentu i bezpiecznie ignorowane przez ReportLab.
...
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, linie 91–539, komponent `SectionsPanel` — zakładki **Dostosuj CV**, status dokumentu, grupy struktury, kontrolki gęstości, kartki palet Sterlinga oraz typografia S–XL; `frontend/src/components/editor/SectionsPanel/SectionsPanel.module.css`, linie 1–130 — responsywny panel, miniaturowe kartki, segmentowana skala tekstu, stany focus i nakładka mobilna; testy: `SectionsPanel.test.js`, linie 1–89
- `frontend/src/utils/sterlingAppearance.js`, linie 1–302, eksporty `STERLING_PALETTES`, `getSterlingAppearance`, `applySterlingPalette` i `applySterlingTextSize` — semantyczna zamiana kolorów, ścieżki ikon per paleta, odwracalne bazowe metryki typografii zależne od roli, natychmiastowe szacunki wysokości textarea przepływu oraz zapis zamiaru wyglądu; testy: `sterlingAppearance.test.js`, linie 1–53
- `frontend/src/utils/sterlingTypographyLayout.js`, linie 1–37, funkcja `applySterlingTextSizeLayout`; `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx`, linie 218–228, funkcja `handleSterlingTextSize` — atomowe przeliczenie kontaktów, pakowanie main/sidebara i uzgodnienie chrome stron dla każdego presetu; regresja: `frontend/src/utils/sterlingTypographyLayout.test.js`, linie 53–70
