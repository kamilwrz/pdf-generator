...
│   │   └── utils/            # a4ElementFactories, canvasElementSchema, geometry, reflow, sectionBuilder, sectionRecord, sectionIcons
│   ├── package.json
│   └── .env.example
├── shared/
...
- `frontend/src/utils/sectionStructure.js`, function `deriveSectionStyle` — samples image markers with `src` / `alignWithText`
- `frontend/src/hooks/useA4Elements.js`, function `handleAddSection` — style sampling, optional `iconName`, construction, placement, post-add selection; exposed through `PdfContext` as `addSection`
- `frontend/src/components/editor/AddSectionModal/AddSectionModal.jsx` — name + layout picker + optional icon gallery
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx` — "+ Dodaj sekcję" entry point, `iconOptions` from `listSectionIconOptions`, modal wiring; user-facing labels in `SPACING_FIELDS` / `displaySectionTitle`
...
- `frontend/src/utils/sectionStructure.js`, funkcja `deriveSectionStyle` — próbkuje markery obrazu z `src` / `alignWithText`
- `frontend/src/hooks/useA4Elements.js`, funkcja `handleAddSection` — próbkowanie stylu, opcjonalne `iconName`, budowa, umieszczenie, zaznaczenie; wystawiana przez `PdfContext` jako `addSection`
- `frontend/src/components/editor/AddSectionModal/AddSectionModal.jsx` — nazwa + wybór układu + opcjonalna galeria ikon
- `frontend/src/components/editor/SectionsPanel/SectionsPanel.jsx` — „+ Dodaj sekcję”, `iconOptions` z `listSectionIconOptions`, podpięcie modala; etykiety UI w `SPACING_FIELDS` / `displaySectionTitle`
