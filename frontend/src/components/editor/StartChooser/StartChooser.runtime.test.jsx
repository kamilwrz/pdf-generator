...
describe('interview onboarding entry', () => {
  it('puts the Pro interview after manual creation and import, with keyboard navigation', async () => {
    ...
    await user.tab();
    expect(screen.getByRole('link', { name: 'Asystent CV Rozpocznij rozmowę' })).toHaveFocus();
  ...
  it('explains Free availability and opens the account plan instead of AI', async () => {
    ...
    expect(screen.getByText('W pakiecie Free Asystent CV jest dostępny po przejściu na Pro.')).toBeVisible();
    expect(screen.queryByRole('link', { name: /Rozpocznij rozmowę/ })).toBeNull();
    await userEvent.setup().click(screen.getByRole('link', { name: 'Asystent CV Poznaj Pro' }));
  ...
  it('never grants unresolved or revoked access and updates when permissions arrive', () => {
    ...
    expect(screen.getByRole('link', { name: 'Asystent CV Sprawdź dostęp' })).toHaveAttribute('href', '/app/account');
    expect(screen.queryByText('W Twoim Pro')).toBeNull();
    rerender(view({ ai_assistant: true }));
    expect(screen.getByRole('link', { name: /Rozpocznij rozmowę/ })).toHaveAttribute('href', '/app/interview');
