...
const operations = {
  load: ['Otwieramy Twoją historię', 'Pobieramy profil zawodowy i zapisany etap rozmowy.', 'Odczyt danych konta'],
  ...
  confirm: ['Porządkujemy Twój profil', 'Zatwierdzone informacje staną się podstawą kolejnych wersji CV.', 'Zapis potwierdzonych informacji'],
  ...
  sync: ['Aktualizujemy widok', 'Wynik operacji dotarł. Pobieramy aktualny profil, aby pokazać spójne informacje.', 'Synchronizacja profilu'],
};
...
export default function InterviewLoading({ operation = 'load', facts, answers, language, template }) {
  ...
  return <section className={classes.loading} aria-label="Przetwarzanie wywiadu">
    ...
    <div className={classes.body}>
      ...
      <dl className={classes.context}>{answers != null && <div><dt>Zapisane odpowiedzi</dt><dd>{answers}</dd></div>}{facts != null && <div><dt>Informacje w profilu</dt><dd>{facts}</dd></div>}{language && <div><dt>Język CV</dt><dd>{language}</dd></div>}{template && <div><dt>Szablon</dt><dd>{template}</dd></div>}</dl>
