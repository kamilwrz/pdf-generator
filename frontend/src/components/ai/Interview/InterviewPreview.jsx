...
export default function InterviewPreview({ preview, source, facts }) {
  ...
  return <section className={classes.preview} aria-label={uiText("interview:interviewPreview.reviewYourNewCv")}>
    <header className={classes.header}><div><span className={classes.eyebrow}>{uiText("interview:interviewPreview.interviewResult")}</span><h3>{uiText("interview:interviewPreview.contentPreviewPages")} {preview.pages}</h3><p>{uiText("interview:interviewPreview.checkTheContentAndThatAchievementsBelong")}</p></div><span className={classes.count}>{uiText("interview:interviewPreview.entries")} {groups.length}</span></header>
    <nav className={classes.views} aria-label={uiText("interview:interviewPreview.previewViews")}>{[['content', uiText("interview:interviewPreview.cvContent")], ['changes', uiText("interview:interviewPreview.changes", { value0: (preview.changes.length) })], ['checks', uiText("interview:interviewPreview.toReview", { value0: (preview.remaining_gaps.length + (preview.review_notes?.length || 0)) })]].map(([key, label]) => <button type="button" key={key} aria-current={view === key ? 'page' : undefined} onClick={() => changeView(key)}>{label}</button>)}</nav>
    {view !== 'checks' && visible.length > 0 && <div className={classes.selector}><label htmlFor={recordId}>{uiText("interview:interviewPreview.cvEntry")}</label><select id={recordId} value={group.key} onChange={(e) => { setSelected(e.target.value); setPage(0); }}>{careerSections.map((section) => visible.some((g) => g.section === section.id) && <optgroup label={section.label} key={section.id}>{visible.filter((g) => g.section === section.id).map((g) => <option key={g.key} value={g.key}>{g.title}{g.subtitle ? ` · ${g.subtitle}` : ''}</option>)}</optgroup>)}</select></div>}
    <div className={classes.body}>
      ...
      {view !== 'content' && rows.length > 5 && <nav className={classes.pagination} aria-label={uiText("interview:interviewPreview.previewPages")}><button type="button" disabled={currentPage === 0} onClick={() => { setPage(currentPage - 1); heading.current?.focus(); }}>{uiText("interview:interviewPreview.previous")}</button><span>{currentPage + 1} / {Math.ceil(rows.length / 5)}</span><button type="button" disabled={(currentPage + 1) * 5 >= rows.length} onClick={() => { setPage(currentPage + 1); heading.current?.focus(); }}>{uiText("interview:interviewPreview.next")}</button></nav>}
    </div>
    {view !== 'checks' && (preview.review_notes?.length > 0 || preview.remaining_gaps.length > 0 || preview.recovered_previous_attempt) && <div className={classes.checkNotice}><p>{uiText("interview:interviewPreview.theResultIncludesClarificationOrGapNotices")}</p><button type="button" onClick={() => changeView('checks')}>{uiText("interview:interviewPreview.reviewCvNotices")}</button></div>}
  </section>;
}
