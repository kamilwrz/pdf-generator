...
import { careerSections, groupCareerFacts, newCareerRecord, careerFieldOptions, isCareerNote } from '../../../utils/careerProfileView';
...
export default function FactEditor({ facts, onChange, disabled = false, onEditingChange, isolated = false, sourceProfile = false, readOnly = false }) {
  useTranslation();
  const id = useId();
  const groups = useMemo(() => groupCareerFacts(facts, { sourceProfile }), [facts, sourceProfile]);
  ...
  const canEditSection = !readOnly && (!sourceProfile || section.id === 'notes');
  const canEditField = (field) => !readOnly && (!sourceProfile || field.ids.every((id) => isCareerNote(facts.find((fact) => fact.id === id))));
  ...
  function openEditor(field, event, additions = []) {
    if (readOnly || sourceProfile && !isCareerNote(field)) return;
  ...
  function applyEdit() {
    ...
    const clean = { id: draft.id, text: draft.text, context: draft.context || '', question: draft.question || '', kind: draft.kind, path: sourceProfile ? '' : draft.path || '', source: draft.source || 'manual' };
  ...
  const options = selected && canEditSection ? careerFieldOptions(selected, facts) : [];
  ...
  return <section className={classes.editor} aria-label={uiText("interview:factEditor.informationToUse")}>
    ...
    <div className={classes.workspace}>
      ...
      <div className={classes.content}>
        ...
        {sourceProfile && section.id !== 'notes' && <p className={classes.navHint}>{uiText("interview:factEditor.sourceReadOnly")}</p>}
        ...
        {editing ? <form className={classes.editForm} onKeyDown={(e) => { if (e.key === 'Escape' && !disabled) { e.preventDefault(); closeEditor(); } }} onSubmit={(e) => { e.preventDefault(); applyEdit(); }}>
          ...
          <details><summary>{uiText("interview:factEditor.contextAndUse")}</summary>
            ...
            {!sourceProfile && (!editing.draft.path || interviewFields[editing.draft.path]) && <label>{uiText("interview:factEditor.purpose")}<select value={editing.draft.path || ''} disabled={disabled} onChange={(e) => setEditing({ ...editing, draft: { ...editing.draft, path: e.target.value } })}><option value="">{uiText("interview:factEditor.informationToDevelopInTheCv")}</option>{Object.entries(interviewFields).map(([path, label]) => <option key={path} value={path}>{label}</option>)}</select></label>}
            {!sourceProfile && editing.draft.path && !interviewFields[editing.draft.path] && <button type="button" disabled={disabled} onClick={() => setEditing({ ...editing, draft: { ...editing.draft, path: '' } })}>{uiText("interview:factEditor.keepAsAdditionalInformation")}</button>}
          </details>
