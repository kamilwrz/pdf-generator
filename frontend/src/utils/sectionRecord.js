/**
 * Append a new record (education / experience style) inside an existing
 * template-mode section.
 *
 * A section is eligible when its body is more than a single content textarea —
 * typically a multi-line record stack tagged with `flowGroup` (wizard fills and
 * custom "cc-edu" / "cc-exp" sections). The new record clones the last record's
 * geometry and styling, replaces copy with field placeholders, and the document
 * is re-packed with `applyFlowSpacing` so pagination and SPACE_* rhythm stay
 * consistent.
 */
