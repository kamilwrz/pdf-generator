...
export function applyMonumentPalette(elements = [], paletteId) {
  ...
  const recolored = elements.map((element) => {
    ...
    if (/\/template-assets\/iconic\/monument(?:-[^/]+)?\//.test(String(next.src || ""))) {
      next = {
        ...next,
    ...
    if (next.contactBand?.id === "monument-contact") {
      next = {
        ...next,
        contactBand: {
          ...next.contactBand,
          text: { ...next.contactBand.text, colorHex: palette.colors.ink },
...
export function applyMonumentTextSize(
  ...
  const resized = elements.map((element) => {
    ...
    if (source.contactBand?.id === "monument-contact") {
