...
for (const lang of ['pl', 'en']) {
  for (const width of [390, 834, 1280, 1920]) {
    test(`interview edits only notes (${lang}, ${width}px)`, async ({ page }) => {
      ...
      await page.getByRole('button', { name: lang === 'pl' ? 'Treść CV' : 'CV content', exact: true }).click();
      ...
      if (width === 390) await page.getByRole('combobox').selectOption('notes');
      else await page.getByRole('button', { name: lang === 'pl' ? 'Z rozmowy i notatki' : 'From conversation and notes', exact: true }).click();
      ...
      await page.getByRole('button', { name: lang === 'pl' ? 'Zastosuj zmianę' : 'Apply change', exact: true }).click();
      await page.getByRole('button', { name: lang === 'pl' ? 'Wróć' : 'Back', exact: true }).click();
      await expect(page.getByRole('button', { name: lang === 'pl' ? 'Treść CV' : 'CV content', exact: true })).toBeEnabled();
