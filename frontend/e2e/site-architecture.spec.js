...
test('template selection is retained and paid creation stays gated', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Otwórz CV w edytorze', exact: true })).toBeDisabled();
  await expect(page.getByRole('radio', { name: /Monument/ })).toBeChecked();
  await expect(page.getByRole('radio', { name: /Monument/ })).toBeDisabled();
  await page.getByRole('button', { name: 'Poznaj Pro', exact: true }).click();
  await expect(page).toHaveURL(/login\?returnTo=/);
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('cvstudio.onboarding.v1')).config.templateId)).toBe('monument');
