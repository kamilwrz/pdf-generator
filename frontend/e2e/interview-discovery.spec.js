...
for (const width of [390, 834, 1280, 1920]) {
  test(`interview can be discovered through pricing and help at ${width}px`, async ({ page }) => {
    ...
    await expect(page.getByText('Asystent CV do opisania doświadczenia i dopasowania CV do ogłoszenia', { exact: true })).toBeVisible();
    ...
    const summary = page.getByText('Jak rozmowa zużywa kredyty AI?', { exact: true });
    await summary.focus();
...
test('Pro users get direct entry points and canonical interview benefits in the picker', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog').getByText('Asystent CV do opisania doświadczenia i dopasowania CV do ogłoszenia', { exact: true })).toBeVisible();
...
for (const width of [390, 1280]) {
  test(`landing, Pro registration and account start explain the interview at ${width}px`, async ({ page }) => {
    ...
    await page.emulateMedia({ reducedMotion: 'reduce' });
    await page.goto('/');
    await page.getByRole('link', { name: 'Zobacz, jak poprawić treść CV' }).click();
    await expect(page.locator('#wywiad')).toBeFocused();
