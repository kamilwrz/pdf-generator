...
4. Page goals are progressive: 3 to 2, then possibly 2 to 1. A failed goal keeps the best measured page count. Remaining pages fill in reading order to the normal 770-point content bottom; no artificial earlier page breaks equalise their heights. `fillInterviewPages` first packs whole records and heading/first-record groups, then compares at most eleven spacing steps up to the shared 1.3 expansion. On finalisation, `expandInterviewLayout` also measures the existing M/L/XL template transactions through `applyTemplateTypography`, rejecting any that reduce current body sizes. Candidates are ranked by first-page fill, then subsequent pages. Expansion cannot move an already packed element to a later page, add a page, or clip flowing text. A shorter last page is expected; large indivisible records can still leave a gap. One-page fits retain their selected layout. This free finishing pass runs after shortening decisions, so expanded spacing never inflates a paid reduction estimate.
...
4. Cele są stopniowe: 3 do 2, następnie ewentualnie 2 do 1. Nieudana próba zachowuje najlepszą zmierzoną liczbę stron. Pozostałe strony zapełniają się w kolejności czytania do normalnej dolnej granicy treści 770 punktów; sztuczne wcześniejsze podziały nie wyrównują ich wysokości. `fillInterviewPages` najpierw układa całe rekordy i grupy nagłówek/pierwszy rekord, a następnie porównuje do jedenastu kroków odstępów aż do wspólnego powiększenia 1,3. Przy finalizacji `expandInterviewLayout` mierzy też istniejące warianty M/L/XL szablonu przez `applyTemplateTypography`, odrzucając warianty zmniejszające bieżącą czcionkę treści. Najważniejsze jest zapełnienie pierwszej strony, potem kolejnych. Powiększenie nie może przenieść już ułożonego elementu na dalszą stronę, dodać strony ani przyciąć tekstu w przepływie. Krótsza ostatnia strona jest oczekiwana; duże niepodzielne rekordy nadal mogą pozostawić odstęp. Wynik jednostronicowy zachowuje dobrany układ. Ten bezpłatny etap następuje po decyzjach o skracaniu, więc większe odstępy nie zawyżają szacunku płatnej redukcji.
