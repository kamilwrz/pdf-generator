...
2. `ai_service.extract_cv_data`:
   ...
   - wysyła zwykły dokument tekstowy do reasoningowej **Gemmy 4 26B**, z niskim poziomem reasoningu i budżetem 32 000 tokenów,
...
Przy ekstrakcji AI dostaje **instrukcję schematu JSON** (experience, education z `school/city/degree/...`, skills, labels PL, `extra_sections` z `kind` / `placement` oraz **rekordowymi** `items` dla projektów/referencji). Temperatura jest niska (`0.1`). Główna Gemma tekstowa używa `max_completion_tokens=32000`, ale produkcyjnie ma wyłączony ukryty tok rozumowania przez udokumentowane przez Cloudflare `chat_template_kwargs.enable_thinking=false`; `CLOUDFLARE_TEXT_ENABLE_THINKING=true` to jawny opt-in eksperymentalny, który przywraca `reasoning_effort`. Gemma nie używa niewspieranego JSON Mode. Jeśli odpowiedź jest pusta, nie jest obiektem JSON, nie przechodzi normalizacji albo Cloudflare zwróci kod braku pojemności `3040`, Llama tekstowa wykonuje jedną próbę z `max_tokens=8000` i oficjalnym `response_format=json_object`. Kod dziennego limitu neuronów `3036` nie jest ponawiany. Klienci SDK mają `max_retries=0`, dlatego warstwa transportowa nie wykonuje ukrytej drugiej inferencji. Qwen Vision nadal używa `max_completion_tokens=8000` i `reasoning_effort=low`, bez JSON Mode. Ścisły prompt i parser backendu wymagają jednego obiektu JSON oraz tolerują wyłącznie opcjonalny Markdown fence lub typowane fragmenty tekstowe. Jawny rollback OpenAI używa `response_format=json_object` i osobnego budżetu JSON 8000 tokenów.
