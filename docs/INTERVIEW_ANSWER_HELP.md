...
3. Review the returned mode. `draft` is a proposed answer grounded in the selected entry's information or the unsaved draft. `options` contains hypothetical activities, with every checkbox initially unchecked. `guidance` offers local coaching and sentence starters tailored to the question angle, with no application action. A short existing duty description may still lead to hypothetical options when the question asks for missing detail. Rejected or declined suggestions always retain this coaching; failed requests show local writing help alongside the error and retry. Reading old saved guidance updates its copy without an AI call or evidence changes.
...
The date check distinguishes a request for a time or period from a narrative clause containing “when” or “kiedy”. For example, “What were the main steps you followed when handling a KYC case?” asks for a process and retains answer help. A blanket match on either word previously removed the entire panel for these questions. Reading a saved session reevaluates eligibility and exposes matching saved help for the active question without generating or charging for it again. The [Python regular-expression reference](https://docs.python.org/3/library/re.html) documents the matching primitives used by the check; the eligibility rule itself belongs to this application.
...
Backend tests exercise validation, ownership, scope, confirmation, retry caching and real ledger settlement with mocked model output. English and Polish narrative questions with contextual “when”/“kiedy” cover generation and free cached reads while direct and indirect exact-time requests remain blocked. The browser fixtures exercise these narrative questions in both languages, including saved-help reload without another generation request. `InterviewAnswerHelp.runtime.test.jsx` covers component state and interactions; the interview runtime tests cover the parent submission boundary. `interview-answer-help.spec.js` uses a synthetic local API to cover both languages and four widths, keyboard use, pending cancellation, draft retention, unchecked choices, overflow errors, cached reload, unknown/skip semantics, source/PDF isolation and credit visibility. Browser/API mocks do not establish live model quality or production-provider availability.
...
3. Przejrzyj wynik. `draft` oznacza propozycję opartą na informacjach o wybranym wpisie lub niezapisanym szkicu. `options` zawiera hipotetyczne czynności, a wszystkie pola wyboru są początkowo niezaznaczone. `guidance` oferuje lokalne wskazówki i początki zdań dopasowane do celu pytania, bez akcji zastosowania. Krótki istniejący opis obowiązków nadal może prowadzić do hipotetycznych opcji, gdy pytanie dotyczy brakujących szczegółów. Odrzucone propozycje i odmowa modelu zawsze pozostawiają tę pomoc; nieudane żądania pokazują lokalne wskazówki obok błędu i ponowienia. Odczyt starej zapisanej wskazówki aktualizuje jej treść bez wywołania AI i zmiany dowodów.
...
Sprawdzanie dat odróżnia prośbę o czas lub okres od zdania opisowego zawierającego „when” lub „kiedy”. Przykładowo pytanie „Jakie kroki wykonywano, kiedy obsługiwano sprawę KYC?” dotyczy procesu i zachowuje pomoc w odpowiedzi. Wcześniej dopasowanie dowolnego wystąpienia tych słów usuwało przy takich pytaniach cały panel. Odczyt zapisanej sesji ponownie ocenia dostępność i udostępnia pasującą zapisaną pomoc przy aktywnym pytaniu bez jej ponownego generowania ani naliczania opłaty. [Dokumentacja wyrażeń regularnych Pythona](https://docs.python.org/3/library/re.html) opisuje mechanizmy dopasowania używane przez sprawdzenie; sama reguła dostępności jest częścią tej aplikacji.
...
Testy backendu obejmują walidację, właściciela, zakres, potwierdzenie, odzyskiwanie i rzeczywiste rozliczanie rejestru z zastąpionym wynikiem modelu. Opisowe pytania PL/EN z kontekstowym „when”/„kiedy” sprawdzają generowanie i bezpłatny odczyt zapisanej pomocy, a bezpośrednie i pośrednie prośby o dokładny czas nadal pozostają zablokowane. Dane testów przeglądarkowych obejmują takie pytania w obu językach, w tym odświeżenie zapisanej pomocy bez kolejnego żądania generowania. `InterviewAnswerHelp.runtime.test.jsx` sprawdza stan komponentu i interakcje, a testy runtime wywiadu obejmują granicę zapisu w komponencie nadrzędnym. `interview-answer-help.spec.js` używa syntetycznego lokalnego API do sprawdzenia obu języków i czterech szerokości, klawiatury, ukrywania podczas pracy, zachowania szkicu, niezaznaczonych opcji, limitu znaków, odświeżenia z zapisanym wynikiem, niepamiętania/pomijania, izolacji źródła/PDF oraz widocznych kredytów. Zastąpione API i przeglądarkowe odpowiedzi nie dowodzą jakości rzeczywistego modelu ani dostępności dostawcy produkcyjnego.
