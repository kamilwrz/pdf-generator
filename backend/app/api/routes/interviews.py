...
def preview_interview(session_id: str, request: GenerateWrite, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    response = service.paid_model(db, user, row, request, "preview", {
        "task": "Przygotuj pełną treść CV jako fields: path/value/evidence_refs. Podstawą są wyłącznie potwierdzone profile facts; offer to kryteria doboru, nie dowody. Zachowaj wszystkie odrębne fakty bazowego CV, wzmacniaj podsumowanie i punkty. Możesz dodać potwierdzone projekty i umiejętności. Po doprecyzowaniu czynności zredaguj cały punkt na nowo. Nie doklejaj kolejnych zdań ani list kryteriów; skondensuj równoważne wyjaśnienia, a odrębne zakresy opisz osobno, jeśli jeden punkt staje się przeciążony. Każdy odrębny fakt opisz raz w obrębie danej roli lub projektu. Nie mieszaj danych różnych ról i projektów. Ogólna znajomość technologii nie potwierdza jej użycia w konkretnym projekcie. Zachowaj dokładnie kolejność działań, kierunek przekazania raportów i granice odpowiedzialności ze źródła; nie dopisuj relacji przed/po ani odbiorców. Każda liczba musi pochodzić z przywołanych faktów. Zwróć pozostałe braki. Nie generuj geometrii. Dane kontaktowe pozostają dosłowne. Używaj języka language dla całej treści."
        + "\nSelekcja: zachowaj odrębne pierwotne fakty, lecz z odpowiedzi wybierz tylko przydatne nowe szczegóły. Nie kopiuj pełnych odpowiedzi ani ogólnych deklaracji ze studiów do kolejnych sekcji. Podsumowanie: 1–2 zwięzłe zdania o głównym profilu. Punkt: jedna czytelna czynność z potwierdzonym kontekstem. Umiejętność to sama nazwa kompetencji lub narzędzia (np. „SQL”, „Transaction Monitoring”, „MS Excel”), bez opisu, dwukropka ani myślnika z wyjaśnieniem; jej zastosowanie opisz w punktach doświadczenia, nie w polu umiejętności. Nigdy fragment zdania zależny od poprzedniego elementu. Liczba punktów nie jest miarą poprawy. Nie potwierdzaj ani nie obniżaj deklarowanej biegłości na podstawie samego opisu podstawowych funkcji. Wszystkie daty, państwa, instytucje i statusy zawodowe muszą być jawnie obecne w cytowanych źródłach. Zachowaj granice odpowiedzialności i negacje."
        # Assembly retains omitted source fields. Explicitly request translated
        # scalar fields as well as prose so a foreign-language source cannot be
