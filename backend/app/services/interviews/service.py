...
from app.services.interviews.sources import has_interview_source
...
from app.services.ai.assistant.service import _gpt, AIServiceError, assistant_reservation_cost_pln
...
from app.services.tailoring.policy import INTERVIEW_JOB_ANALYSIS_TASK
...
def profile_payload(db, owner_id):
    ...
    if profile['source_binding']:
        ...
        from app.services.cv.profile_source import synchronise_source
...
def session_payload(row):
    ...
    from app.services.interviews.answer_help import answer_help_available, public_answer_help
...
def is_fresh_question(question, answers):
    ...
    """
    from app.services.interviews.clarification import question_key
...
def next_question(db, user, row, request):
    ...
    from app.services.interviews.questions import question_guidance
    from app.services.interviews.quality import QUALITY_TASK, apply_discovery_review
    ...
    if state['mode'] == 'tailor' and not state.get('job_analysis_ready'):
        from app.services.interviews.job_analysis import requirement_topics
