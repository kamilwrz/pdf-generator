...
from app.services.interviews.job_analysis import requirement_facts
...
def update_discovery_budget(state, profile):
    ...
    if state.get('mode') == 'tailor':
        ...
        return entries
    from app.services.interviews.quality import adaptive_entries, discovery_history
...
def scoped_question(candidate, selected, entries, answers, is_fresh):
    ...
    """
    from app.services.interviews.questions import fallback_question, is_distinct_question
    from app.services.interviews.quality import follow_up_question
