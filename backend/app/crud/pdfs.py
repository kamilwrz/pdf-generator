...
def update_pdf_elements(db:Session, elements:list, existing_elements:dict, pdf_id:int):
    ...
    for element in elements:
        ...
        if element.element_id in existing_elements and element.deleted == True:
            element_to_delete = db.query(PdfElements).filter(PdfElements.element_id == element.element_id).delete()
            db.flush(element_to_delete)
