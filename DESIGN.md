...
- Show one question in the selected interface language (Polish or British English) with its reason and an honest saved-answer count: tailoring offers two questions per unresolved analysed requirement; creation/enrichment starts at eight questions and grows to cover current entries, capped at 50 saved answers. Model topic names cannot reset per-entry limits. Empty/repeated/wrong-entry proposals use a neutral local fallback without paid retry. Once the queue is complete, hide next-question and extension controls, explain completion and retain information review and preparation. Verification clarifications use their separate counter and cumulative cap above. An explicit additional round permits up to five more while entries remain, without reopening completed entries. Text, confirmed lack of experience, forgotten information and skip are separate actions.
