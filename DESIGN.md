...
- `/app/tailor` uses the same compact CV Assistant shell as content improvement. Its setup progress has exactly three steps: Your CV, Job advert and Conversation. Hide that setup progress after the shared three-step interview begins so only one hierarchy is visible. Existing standalone and embedded interview hosts keep the shared Choose CV, Conversation and Choose template contract.
