...
- Inline language, Skills, contact, masthead, photo, and list-layout actions use the same zoom-growth rate as structural toolbars, with a minimum scale of 1: 24px buttons, 12px icons and 2.4px surface padding at 140%; 32px buttons and 16px icons at 280%. Preserve a 24px minimum target when zooming out, 1px borders, and circular action buttons inside rounded surfaces. This user-requested canvas exception keeps these actions smaller than section/record controls; it does not apply to form inputs or submit buttons.
...
- Contextual editing tools directly on A4 use the shared `canvasTool` radius (`--radius-canvas-tool`, aliased to `pill`): icon buttons are circular, labelled actions and toolbar rows are pill-shaped. This user-requested exception distinguishes transient editing actions from authored CV content. A toolbar radius is capped at half one control row plus padding and border, so wrapping retains usable corners. It covers section/record toolbars and inline language, Skills, contact, masthead, photo, list-layout and settings-cog actions in all states. Menus, tooltips, settings panels, forms and form controls retain standard geometry. Keep existing sizes, palette, focus rings, placement and zoom rules; this styling never enters document layout, persistence or PDF output.
...
- Invite plain-language answers without judging the user's competence by grammar. Explain beside the answer field that the CV will receive automatic style editing while the submitted answer stays unchanged. For creation/enrichment, discovery uses a server-owned entry queue: two main questions and at most one focused follow-up per experience, education or project, and one supplementary question per skill/group or missing language level. Skipped, unknown and no-experience entries close immediately; never chain follow-ups or reopen skipped, unknown or no-experience answers. Name a follow-up and its optional nature without presenting it as an error.
...
- Show one question in the selected interface language (Polish or British English) with its reason and an honest saved-answer count. Also show the current maximum plan as **Question X of up to N** for an active question and **Plan: up to N questions** otherwise. The visible plan comes from actual content scopes, includes optional focused follow-ups, excludes verification clarifications and is recalculated when a declined answer closes a scope or a clarification consumes part of the shared 50-answer ceiling. Tailoring offers two questions per unresolved analysed requirement; before a fresh requirement analysis, explain that the plan is still pending rather than displaying the provisional internal budget. Creation/enrichment retains an internal capacity of at least eight while a sparse CV may truthfully show a smaller plan; capacity grows at session creation to cover current entries. Source refresh must replan the replacement CV, and legacy sessions must not expose clarification-expanded capacity as ordinary-question progress. Model topic names cannot reset per-entry limits. Empty/repeated/wrong-entry proposals use a neutral local fallback without paid retry. Once the queue is complete, hide next-question and extension controls, explain completion and retain information review and preparation. Verification clarifications use their separate counter and cumulative cap above. An explicit additional round permits up to five more while entries remain, without reopening completed entries. Text, confirmed lack of experience, forgotten information and skip are separate actions.
