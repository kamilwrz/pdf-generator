...
- `/app/tailor` uses the same compact CV Assistant shell as content improvement. Its progress has exactly three persistent steps: Your CV, Job advert and Conversation. Keep the rail visible when the shared interview begins and mark Conversation as current through questions, template choice and review; the guided interview hides its own internal rail so only one hierarchy is visible. Existing standalone and embedded interview hosts keep the shared Choose CV, Conversation and Choose template contract.
- Present saved-source choice and PDF upload as parallel options on wide screens and one reading-order column on compact screens. Consolidate Free import limits, Pro pricing, credit use and source-safety copy into one supporting note per stage. Require account access before upload and an advert before starting. Retain text/link drafts, send only the selected option and keep source review visible before the explicit confirmation action. The isolated interview excludes the account career profile and initially uses Linden; preserve its existing factual review and paid-operation boundaries.
...
On the standalone interview route, centre the short page title beside the breadcrumb on wide screens and stack them on compact screens. The focused SiteLayout keeps global navigation; the standalone content-improvement (`/app/interview`) and guided tailoring (`/app/tailor`) routes additionally opt back into the shared persistent footer via `SiteLayout.showFooter`, a user-requested exception to the otherwise footer-less focused layout, so both keep the privacy link and secondary navigation reachable. Use separate counted My CVs / Imports tabs, local name search, an always-visible document-language selector and four complete source rows per page. Preserve server ordering, per-tab search/page state, keyboard arrow/Home/End activation and explicit row activation before starting paid AI. Show one source-selection heading; conversation wording starts only after a session exists. Keep empty tabs, unmatched searches, loading, errors and access restrictions distinct. Fit the ordinary source picker within laptop/desktop height without body clipping or nested scrolling; retain natural page scrolling for small screens, long authored names, expanded content and 200% zoom. Template examples use bounded thumbnails. This scoped workspace exception does not change other site pages, editor chrome or PDF geometry.
