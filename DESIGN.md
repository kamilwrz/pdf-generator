...
- `/app/tailor` uses the same compact CV Assistant shell as content improvement. Its progress has exactly three persistent steps: CV, Job advert and Conversation. Steps before the current one — including a CV chosen upstream in onboarding before this route first renders — are marked as completed with a non-colour-only indicator and an accessible "completed" label; they must never read as pending or skipped. Keep the rail visible when the shared interview begins and mark Conversation as current through questions, template choice and review; the guided interview hides its own internal rail so only one hierarchy is visible. Existing standalone and embedded interview hosts keep the shared CV, Conversation and Choose template contract with the same completed-step marking.
- The CV step uses the shared source picker: one searchable list combines saved CVs and imports (four rows per page, a visible kind badge per row that is also exposed as the row's accessible description) with an inline PDF upload beneath it. Selecting a row only records the choice, shown as a pressed row state; continuing stays behind the explicit stage action, and the selected CV's content is never rendered in this flow — neither automatically on the CV step nor through a content toggle during the conversation. Consolidate Free import limits, Pro pricing, credit use and source-safety copy into one supporting note per stage. Require account access before upload and an advert before starting. Retain text/link drafts and send only the selected option. The isolated interview excludes the account career profile and initially uses Linden; preserve its existing factual review and paid-operation boundaries.
- Autosave intake with visible saving/saved/error states, explicit retry and unsaved-navigation protection. Restore the same owned task after authentication or checkout. Opening or resuming never starts paid AI. Keep price, credit use and relevant recovery controls readable without introducing technical identifiers into product copy.
