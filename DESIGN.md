...
The landing uses continuous white paper across its sections. Restrict warm fields to the illustrative demos; section numbers are plain accent metadata rather than filled chips. Its pricing comparison uses two open light columns with aligned prices, feature boundaries and actions, an accent rule and compact Pro badge. This replaces the landing's former inverse Pro slab; the separate public pricing page retains its existing panel contract. Template previews have a thin paper edge with captions outside, one registry item per link, and a manually scrolled rail with labelled previous/next controls. No automatic marquee or decorative duplicate is mounted. Keyboard focus reveals offscreen links, and reduced motion disables smooth scrolling. Loading and failed images reserve A4 space while names and destinations remain available. Keep standard primary actions, focus rings, the shared type scale and palette, and all document output unchanged.
