...
- Modal backdrops use `modalBackdrop` by default; the user-requested CV import exception uses opaque `paper` through `DialogShell.opaqueBackdrop` in every import state, without a backdrop fade-in. The editor ink scope must not recolor it; modal context is subdued with a consistent translucent white wash on every route.
...
- Contextual section and record toolbars on the A4 canvas are the narrow exception: their buttons grow gently with canvas zoom using scale = max(2/3, (zoom / 1.4)^0.75) below 140% and (2 + zoom / 1.4) / 3 at or above it: about 28px targets at 100%, 24px at 50%; 36px targets, 16px icons and 14px text at 140%; 48px targets at 280%. Labels, icons, padding and menus grow together, with a 12px text minimum and screen-stable focus outlines, while keyboard access and persistent panel alternatives remain available.
...
- Inline language, Skills, contact, masthead, photo, and list-layout actions use the same zoom-growth rate as structural toolbars, with a minimum scale of 2/3: 36px buttons, 16px icons and zero surface padding at 140%; 48px buttons and 21.33px icons at 280%. Preserve a 24px minimum target when zooming out, borderless circular action buttons inside rounded surfaces. This user-requested canvas exception matches these actions to section/record controls; it does not apply to form inputs or submit buttons.
...
- Illustrative product demos on narrative surfaces may use the shared `preview` radius (`--radius-preview`, 12px), one contrasting paper or canvas field and no outer border. Keep their internal steps and quoted content open, with underlines or a single accent rule. This user-requested softer treatment does not apply to authored document paper, working forms or editor panels.
- Interactive controls may use up to `2px` radius.
- Previous/next controls for a narrative gallery may be circular with the shared pill radius and 44px targets. Their accessible labels and disabled boundaries remain explicit.
...
The public landing presents manual creation and one CV Assistant with two modes: content improvement and tailoring to a specific job advert. Both explanatory examples belong to one labelled Assistant region with subordinate headings. Marketing copy leads with the outcome, follows with the concrete workflow and keeps the next action explicit. Position Pro for people preparing several applications: name content improvement, document review, job-specific versions, all templates and unlimited documents before implementation details. Keep the truthful price, 30-day duration, shared 200-credit allowance and lack of automatic renewal close to each Pro decision. Do not promise recruitment results, ATS acceptance or experience the user has not confirmed. Keep one primary hero creation action to shared onboarding, a short PDF outcome and one account note. Group editor demo, PDF import and the two Pro Assistant modes in a compact open tool list; reserve detail for the explanatory sections. Each section has one primary application entry. On wide screens show the Interview example right and the tailoring example left; at 1024px and below stack explanations before examples. All eight landing sections use full-width backgrounds with content aligned to the 1440px header/footer container, token-based gutters (32/24/16px) and section padding (96/64/48px). Use equal columns with a 64px wide-screen gap; top-align copy and examples in both interview sections. Narrative grids stack at 1024px and pricing at 767px. Keep FAQ heading-first, the template gallery inside the content edges and all secondary headings on the global H2 scale. Keep the hero template fan inside its column with enough space from copy. The two illustrative demos share a static source-and-proposal comparison with the complete result visible on arrival. A native details/summary disclosure reveals the supporting question and answer, retains trigger focus and uses a 44px target. Do not use autoplay, typing effects, simulated input fields or timed scene changes; the same immediate interaction applies with reduced motion. Keep the review reminder visible and the source labels distinct (existing CV versus job advert); sample answers never become user data or invoke AI. State Pro access and credit boundaries beside the relevant entry. Preserve existing routes, template selection and guest draft recovery.

Public and account routes use the shared warm `canvas` backdrop with `paper` content and `surface` supporting regions. The shared page hero pairs its title and optional primary actions with a useful contextual note, rather than leaving a full-width empty field. Pricing contrasts a paper Free panel with an `ink` Pro panel; inverse secondary actions retain paper fill and ink text. Help uses a numbered in-page navigation beside reading sections, stacking in DOM order on compact screens. The library keeps compact document rows, visible actions, labelled filters, and contextual empty/retry states. Its compact introduction is followed by one CV Assistant region with a shared heading and description, containing exactly two subordinate modes: Improve CV content and Tailor CV to a job advert. Use one outer paper panel, structural dividers and H3 mode headings instead of two independent feature cards. Align mode actions on wide screens and stack below 768px. The account navigation exposes one CV Assistant link to `/app/assistant`, where the same mode chooser appears; keep it active for both legacy workflow routes and their saved sessions. Both workflows show a breadcrumb back to the assistant and identify their mode; resumed legacy conversations must not be labelled as general improvement without knowing their mode. Keep one tailoring entry in the main content; confirmed AI access opens content improvement, while Free or unresolved access points to the account plan. Free tailoring intake remains available without starting AI. Account usage uses native labelled meters only for known positive finite allowances; unlimited, unavailable, and zero allowances remain explicit text. Settings group sign-in and data controls, with permanent deletion separated by a structural rule. All surfaces reuse global tokens, square geometry and shared site primitives; these treatments never enter CV templates or PDF output.

Landing, onboarding, and explanatory pages may use generous whitespace and asymmetric editorial compositions. Prefer split layouts, staggered content, and strong typographic anchors. Avoid generic three-card feature rows. On narrative surfaces, establish hierarchy with open groups and soft structural rules before enclosing content in rectangles. Use the shared `border-soft` and `ink-soft` aliases for subtle separation and supporting prose.

The landing uses full-width warm canvas bands for CV content improvement, privacy and FAQ, with paper on the hero, job tailoring, Studio/templates, pricing and final action. A 1px shared border separates every adjacent section, including neighbouring paper sections. Preserve content alignment with the header through fluid inline padding rather than viewport-width overflow. Illustrative demos contrast with their host: paper in the warm improvement band and canvas in the paper tailoring section. Section numbers remain plain accent metadata. Its pricing comparison uses two open light columns with aligned prices, feature boundaries and actions, an accent rule and compact Pro badge. This replaces the landing's former inverse Pro slab; the separate public pricing page retains its existing panel contract. Template previews have a thin paper edge with captions outside, one registry item per link, and a manually scrolled rail with labelled previous/next controls. Captions identify the registry Free/Pro tier; each accessible link name includes its template name once. The hero does not ask for a template selection: it presents static decorative CV examples and leaves that decision to onboarding or the template gallery. Explain the shared AI credit pool near the Pro decision and link both Assistant entries to the existing credit explanation; distinguish free answer persistence from paid next-question generation. No automatic marquee or decorative duplicate is mounted. Keyboard focus reveals offscreen links, and reduced motion disables smooth scrolling. Loading and failed images reserve A4 space while names and destinations remain available. Keep standard primary actions, focus rings, the shared type scale and palette, and all document output unchanged.

CV creation uses one `CvOnboarding` for guests and accounts. Every new-CV entry opens it, including navigation, the document library, import, the editor and a selected landing template. Saved documents and browser drafts resume directly. Legacy creation addresses retain their intent and template selection; none automatically creates or clears a document.

Use one question per screen: welcome and existing-CV choice; account-only PDF upload or saved source; work goal for existing content; template/settings for manual work. Blank creation skips source and goal. The semantic ordered step indicator lists the actual branch (two steps for blank, four for manual reuse, three before an Assistant handoff). Back retains answers. Progress does not depend on timers, welcome interstitials or animations. Move focus to the current heading and update the browser title after each step.

A decorative robot icon from the existing React Icons package accompanies the onboarding question, without a name, voice or professional qualification claims. Use the shared `editorInk` colour on white paper: a 96px box to the right of the heading, reduced to 64px on phones or viewports at most 800px high. Keep **Asystent CV / CV Assistant** as an accessible text label and the robot hidden from assistive technology and keyboard focus. A single gentle greeting tilt on entry to each step lasts three `slow` motion intervals (960ms), then rests; ordinary form updates never restart it. This user-requested character animation is a narrow exception to routine motion durations, never a loading indicator. Reduced motion uses the stationary robot. Preserve rectangular controls, 44px targets and all document/PDF boundaries. Do not copy competitor statistics, ATS promises or privacy assurances.

Treat every ordinary onboarding step as a compact working surface: centred progress with clear space above it, then a warm canvas-coloured heading band with left-aligned title/subtitle and a trailing decorative robot, then the current task. The heading copy and welcome actions share an inset reading edge. Use slightly larger vertical intervals between progress, heading and task when viewport height permits; compact and short layouts reclaim that space from outer padding so the standard flows keep fitting without page scroll. The band spans the current content width, including the wider template gallery. On compact screens, place the subtitle across both columns below the title and robot. Fit the default welcome, upload, goal (including AI credit guidance) and template selection without scrolling at 390×844, 834×720, 1280×720 and 1920×720. Place the privacy link in the persistent footer on every step, wrapping below actions on narrow screens. Group file selection and reading on one row where space permits, with any numeric import allowance beside that task. Omit redundant availability announcements, repeated template headings and repeated setup instructions. Keep Free template rows compact and bound the sample preview by viewport height. The shared dialog body remains the single accessible scrolling fallback for enlarged text, shorter viewports, expanded histories/galleries and custom sections; never hide content or shrink controls to fake a fit. These rules are limited to onboarding and do not affect document or PDF geometry.

Onboarding replaces the complete editor shell while the controller retains the active document in memory. Navigation, tool rails, canvas controls and AI actions behind it must be absent from the keyboard order. One replacement decision precedes the final fill. Cancellation, failed preparation and stale responses preserve the active CV. A successful atomic commit hands focus to the new name field; cancellation restores the opener. The shared `DialogShell` owns focus trapping and one scrolling body with persistent actions.

Import accepts PDF up to 10 MB, shows current account allowance, labelled file errors, loading, empty and retry states, and account entry before a guest can choose a file. Existing imports remain reusable without extraction. AI goals explain Pro and credits; unresolved access offers a retry and cannot authorize AI. Improvement and tailoring handoffs carry owned source identifiers into existing flows and never start paid work merely through navigation or refresh. The account page offers resumption after a plan change. The versioned local journal contains configuration and account-bound identifiers only, with CV contents and PDF bytes kept out.

The landing hero shows the three Free template examples (Sterling, Meridian and Linden) as a static A4 fan, with Linden in front. The hero starts with the shared section label “01 · Darmowy kreator CV” (English: “01 · Free CV builder”). The decorative sample fan has no visible caption and is hidden from assistive technology; template selection remains in onboarding. No preview buttons, radios, counters, swipe handling, motion or second compact CTA are mounted. Keep one primary creation action at every width and the browser-draft resume link when applicable. Editor demo and account-aware import are clearly labelled secondary tools; group Improve content and Tailor to a job under CV Assistant with a Pro label. Preview loading and image errors reserve A4 space and never disable creation. Use shared tokens and restrained document elevation; the sample transforms never enter authored CV/PDF output. Template detail and legacy hints still enter onboarding with selection retained; actual creation requires the explicit final editor action.
...
- Public pages, including the landing, keep the shared header and footer outside the main content landmark and expose the shared keyboard skip link to a focusable main destination. Direct help-summary fragments reveal their native disclosure before scroll and focus; this navigation never starts paid work.
...
- The optional `DialogShell.headerAction` belongs to the current task. Onboarding uses it for guest sign-in. Assistant goals live in the source-dependent goal step. Preserve the title, action and close target at compact widths and 200% zoom. `shouldRestoreFocus` may suppress opener restoration after a successful create because the editor name field owns the next focus; cancellation retains ordinary restoration.
- Standard, decision, progress, loading, error, and confirmation modals use the shared translucent white backdrop token, except the CV import workflow, which uses opaque white paper to fully hide the underlying workspace. Fullscreen workflows cover the viewport with their own paper surface and must not introduce a tinted backdrop.
- The template step of `CvOnboarding` uses controlled `CvSetupOptions`. Show Free templates first and additional choices on demand, with Pro locks. Default fields and sections come from `createDefaultStarterConfig`; a valid landing template takes precedence. Blank CVs expose document language plus optional contact, photo and ordered-section settings. Imported CVs retain their content and language. Customize content replaces the gallery with contact or section controls; social links have a disclosure and custom sections have labelled validation. Preserve choices through Back, retries and journal restoration. Use native radios and a labelled A4 example preview; compact layouts show the example on demand. One body scroll handles short viewports, 200% zoom and long section lists. All controls and the guide remain outside the document and PDF tree.
- New-CV replacement confirmation is one compact `decision` state immediately before final creation. It distinguishes the browser draft, a server-saved document and unsaved account work. Returning to choices, Escape or cancellation retains current content; an empty session returns to landing or the library. A recoverable guest draft keeps its direct resume link on landing. In-flight cancellation invalidates callbacks before a fill can commit.
- The built-in demo is product-owned sample content, not an active user document. Leaving, replacing, refreshing or closing the demo never proposes saving or warns about unsaved changes, including after sample edits and for authenticated users. Starting a personal CV from the demo opens onboarding without replacement or unsaved-work confirmation; user-authored active documents retain that protection.
...
- A contact delete surface sits completely outside the right edge of that contact's live hover bounds with a 5-screen-pixel gap and is vertically centred: use the complete field box for wrapped contacts and the visible glyph rectangle for single-line contacts. This user-requested spacing keeps contact text readable. Moving horizontally to the trash must preserve the current contact. If the band-end add surface would overlap the trash, move only that add surface to the right with an 8-screen-pixel gap. The surface claims the shared canvas-toolbar slot and remains mounted while its button has keyboard focus. The visible job-position hide surface stays completely outside the title with an 8-screen-pixel gap. Both rules remain zoom-stable and editor-only.
...
- Slate and Monument masthead names fit within their authored column and keep clear of the portrait. Reduce only the name font down to 14 pt, then wrap complete text, including an unbroken surname. Preserve the authored maximum so shortening restores the intended size; explicitly smaller user settings remain valid. Added lines move the following main-column content and, when needed, repaginate complete records before the footer. Photo geometry and sidebar content remain independent, including Slate's photo-less contact rail. Editing retains the same caret and focus, fitting uses document units at every zoom, and PDF output uses the same fitted geometry without truncating the stored name.
- Vellum, Aurelia and Cadenza preserve their authored display font size and wrap complete long names inside the existing name column. Measure the displayed casing, tracking and loaded font before reserving height. Added name rows move the position line, its hidden-state restoration target, contacts and following body together; shortening removes only the height previously added for the name. Aurelia's enclosing outline grows with the identity block, while Vellum's portrait remains fixed. Preserve the same wrapping and document geometry through editing, title visibility, typography changes, save/reload and PDF export; never truncate identity text or let it overlap the position line or contacts.
...
- The editor assistant uses the compact panel hierarchy throughout tools, configuration, reports, history and embedded interviews: 16px panel/task titles, 14px controls and short working labels, 12px metadata, and 16px reading copy and form inputs. Resolve these sizes through the shared typography tokens. Use 16px body gutters, white paper, editor-brown text and primary actions, beige secondary/hover surfaces, structural borders and rectangular controls. Keep 44px action targets, the 600px desktop reading width, a single scrolling task body, wrapping header metadata and the accessible close action. Shared interview components retain their roomier standalone layout through scoped density variables. No assistant chrome enters CV content or PDF output.
...
- The start surface groups Improve CV content (the enrichment conversation) and Tailor to a job under one CV Assistant heading. Check CV, Quick text corrections, Translation and ATS remain separate supporting tools; quick corrections must not be confused with the conversational improvement mode. Opening the assistant or its history never starts paid AI.
...
- Offer input uses a labelled native radio group, Link or Paste text. Retain both drafts, but bind requests and analysis reuse to the selected source only. Tailoring through the conversation is always the primary action; match-only analysis is a stable secondary action before and after a result. Keep charges and optional notes accessible without repeating them beside both actions.
- “Dopasuj do oferty” replaces the quick-action grid and chat with one full-width, full-height assistant body below its header. Use 16px body/inputs and task heading, 14px control labels, 44px actions, shared tokens and a single vertical scroll container. Back restores the assistant actions and focus; drafts and the current analysis remain available. Analysis results receive heading focus and use plain labels: matched, needs-detail, not-mentioned, plus a fourth **related-experience** (indirect) label for a partial/missing requirement that the candidate's existing evidence supports only transferably. An indirect match shows the quoted related CV text and a one-sentence residual gap; it never claims the requirement is met and never changes the score. Results never expose technical source types, evidence IDs, raw diagnostics or correction cards. Compact/200% layouts wrap without horizontal overflow.
...
- Tailoring discovery is driven by the saved offer analysis, not generic CV records: offer exactly two questions per missing/partial requirement, within the shared 50-answer ceiling, with no third follow-up. Use the selected CV and saved answers as context; only historical sessions may retain their older profile scope. Matched requirements are skipped. No experience, cannot remember and skip close that requirement immediately. When the analysis attached transferable (indirect) support to an unresolved requirement, the first question is a **bridging** question that quotes the candidate's own related experience and asks whether it also covered the requirement, requesting a concrete example — instead of a generic "do you have experience with X?"; this also confirms a soft skill implied by a described situation. The answer decides; a transfer never presents itself as fulfilling the requirement. Stable requirement IDs and saved answers prevent repeated topics on retries or extension. Source refresh invalidates the analysis while retaining answers; no analysis classification becomes a career fact.

### Guided job-tailoring entry

- Saved tailoring rows reuse the shared conversation details: available offer title/company, source CV, candidate, phase, start and last-activity times, saved-answer count and latest question. Use labelled advert excerpts or the selected URL when no parsed title exists; never infer missing metadata or expose the inactive advert draft. Keep explicit resume and available saved-CV actions, owner-scoped reads, distinct loading/error/empty states and wrapping PL/EN layouts. List reads must not invoke AI or rewrite saved work.
- `/app/tailor` is one persistent two-region workspace borrowed from the help-guide layout: a sticky surface-toned rail on the left with the three numbered steps (CV, Job advert, Conversation) and the autosave status, beside one paper working panel on the right. On viewports of at least 1000px the panel keeps the same bounded height on every step — tall enough to work in, short enough that the shared footer stays visible — and its single body owns vertical scrolling; below that width the panel grows naturally and the rail compacts above it. Intake, the conversation, loading states, review and the final download all render inside that same panel: starting the interview shows the shared interview wait surface in place of the offer form (which stays mounted with its draft for failure recovery), and the embedded `InterviewFlow` drops its own border and padding so a stage change never reads as moving to a different window. Steps before the current one — including a CV chosen upstream in onboarding before this route first renders — are marked as completed with a non-colour-only indicator and an accessible "completed" label; they must never read as pending or skipped, and the download stage marks all three as completed. The guided interview hides its own internal rail so only one hierarchy is visible. Existing standalone and embedded interview hosts keep the shared CV, Conversation and Choose template contract with the same completed-step marking.
- The CV step uses the shared source picker: one searchable list combines saved CVs and imports (four rows per page, a visible kind badge per row that is also exposed as the row's accessible description) with an inline PDF upload beneath it. Selecting a row only records the choice, shown as a pressed row state; continuing stays behind the explicit stage action, and the selected CV's content is never rendered in this flow — neither automatically on the CV step nor through a content toggle during the conversation. The former per-stage price/credits/source-safety note was removed on request to reduce cognitive load: pricing stays on the guest gate and in the explicit Pro purchase action's label, and no step repeats it. Require account access before upload and an advert before starting. Retain text/link drafts and send only the selected option. The isolated interview excludes the account career profile and initially uses Linden; preserve its existing factual review and paid-operation boundaries.
- Autosave intake with visible saving/saved/error states in the rail, explicit retry and unsaved-navigation protection. Restore the same owned task after authentication or checkout. Opening or resuming never starts paid AI. Keep the Pro purchase price in its action label and relevant recovery controls readable without introducing technical identifiers into product copy.
- After review, save a separate CV and show Download PDF as primary with the editor as an optional secondary link. Report browser handoff truthfully. Use shared tokens, native labelled controls, 44px targets, step heading focus, keyboard access, PL/EN copy, wrapping compact layouts, 200% zoom and reduced motion. Flow navigation and download controls must not enter document/PDF rendering.

### 5.9 Career interview contract

- The ordinary assistant has exactly three numbered steps: **CV → Conversation → Choose template**. Use a semantic progress list, not four navigation tabs. Steps before the active one are marked completed (non-colour-only indicator plus an accessible "completed" label). A source handed off by onboarding marks the CV step completed and presents the conversation step as current, with the explicit start action as the paid boundary. Source selection creates isolated evidence and immediately generates the first question. Sending an answer persists it before automatically requesting the next; finishing may save the typed answer and enter template selection directly. Opening/resuming remains read-only. No career-profile entry, opt-in, intake notes, compulsory fact review, plan/language/scope status row or fact search appears in this flow.
- **Treść CV / CV content** is optional and opens only on activation. Preserve content/answer drafts and guarded recovery. Template cards are explicit create actions: run verified generation and fitting, then save a separate document and open the editor when the fitted result is a single page. A result that still needs several pages stays on the preview with the queued free one-page template comparison instead of being saved past the user. Stop for factual clarification or failure; never auto-accept an AI factual proposal. Save failure exposes a save-only retry. Historical previews retain manual review and alternative-template controls; single-page direct creation uses the user's chosen template without requiring a comparison step. The guided advert workflow keeps its separate review/download handoff.
- Present one paper working panel with three restrained numbered markers, accent rules and shared typography. Animate panel entry with opacity and a 12px translation for the shared 320ms duration; reduced motion removes it. No stagger, ornamental shadows, gradients or added delays. Source rows and template cards have 44px targets, focus, disabled and pending states, wrap at compact/200% sizes and remain outside PDF output. Show six template samples initially and more on demand.
- Saved conversations live at **/app/conversations**; old **/app/career-profile** links redirect there. Remove account-profile editing from product navigation. Preserve older evidence, session history, export and account erasure; new conversations always exclude shared profile data.


- Saved conversations use separated, wrapping rows with an authored CV or offer title, workflow and phase, candidate/source context, start and last-activity dates with local times, and saved-answer count. Show a labelled advert excerpt when its title is unavailable and the latest question when saved; never invent missing metadata. Resume, open saved CV and delete remain explicit actions. Deletion uses the shared decision dialog with the selected conversation identity and dates, safe initial Cancel focus, fixed action footer, pending lock, inline retryable error and focus restoration. Explain that deletion is permanent while account-profile information and saved CVs remain. After deletion, retain a valid list page and adjust older-history offsets; loading and empty states must remain distinct. Apply the same contract in PL/EN at compact widths, 200% zoom and reduced motion, entirely outside PDF content.



- Clarification always shows the complete proposed description, the detail to check and the explicit confirmation action. A secondary **Popraw opis / Correct description** button opens the full-description editor and focuses its textarea. Closing it or pressing Escape retains its draft and restores the trigger; a retained draft keeps confirmation and alternative answers disabled, with a visible explanation when the editor is closed. Secondary answer meanings live in their own disclosure. Opening or closing either disclosure never saves, advances or starts AI. Keep the correction label, complete-replacement guidance and explicit save action; do not hide the source quotation.

- Answer-help results must always offer useful assistance: a verified draft, unchecked hypothetical activities, or question-angle coaching with concrete sentence starters. A generic refusal is not a completed help result. An existing but underspecified duty description may still support hypothetical choices. Rejected proposals and saved legacy guidance use localised coaching without another paid generation. Provider/transport errors retain their error and retry controls alongside local writing help. Label guidance **Podpowiedź do odpowiedzi / Help with your answer**; never apply coaching as candidate facts. Use the existing shared guidance surface in both interview hosts, preserving responsive reflow, focus and draft retention.
...
- When a verified interview result still spans multiple pages, compare other entitled templates using the same complete content, actual loaded fonts and each template's S preset. Start at compact spacing, then compare the shared supported spacing steps down to the automatic hard floor when needed; retain the loosest measured single-page rhythm for each template and persist that exact spacing with its geometry. This broader range belongs to alternate-template comparison; automatic fitting of the original template retains its compact floor. Announce only measured single-page alternatives and require an explicit template choice. The scan and selection use no AI credits. Keep the current result readable during comparison, with a real completed/total count, cancellation, retry, empty and partial-failure states. Final saving waits for the queued automatic check and active comparison to complete, fail or be explicitly cancelled, so saving cannot navigate away before the user sees a one-page alternative. Explain the wait beside Save and keep Stop checking reachable; cancelling restores saving without changing the current CV. Existing saved previews expose an explicit check; a newly generated or corrected multi-page result starts one free comparison. Template sample thumbnails must be labelled as examples, while page-count claims come from the candidate's actual content. Preserve keyboard selector access, task focus, 44px actions, PL/EN parity, 200% zoom and reduced motion. Changing templates stores matching template identity, spacing and geometry together; restoring the previous version also restores its template. Invalidated or edited previews cannot apply stale recommendations. Comparison controls and samples never enter the PDF.

- The alternative-template selector initially keeps the current template. An explicit selection stages that measured layout for **Save as a new CV**; name the selected template beside Save and explain that the separate Apply action is optional. Save must apply the choice before creating the document, including its measured spacing and geometry. If application fails, retain the choice for retry and do not save the previous template. Rescanning, cancellation and a changed preview discard the staged choice. Preserve the applied template when a later document save fails.
...
- If the CV Assistant result still needs multiple pages, fill them in reading order to the normal printable bottom margin. Never shorten the usable page height to equalise page fills. Keep whole records and headings with their first record; the last page may be shorter. Before finalisation, compare larger template typography and supported spacing expansion using actual browser measurements. Accept expansion only when it improves earlier-page fill, retains the content already packed onto each earlier page, and adds no pages or clipped text. Preserve template identity and persist the chosen geometry and spacing together; this finishing pass uses no AI credits.


- The shared interview workspace shows one static receipt line in both assistant and account entry points: an uppercase **Kredyty / Credits** label with the settled interview total (Zużyte / Used) and, outside the embedded assistant, the known account balance (Dostępne / Available). The former per-request history disclosure and its accounting explanations were removed on request to reduce cognitive load; the ledger read remains the only source of the used total, so replayed stages are never counted twice. Show an explicit pending marker and a dash instead of inventing a price or zero balance. Refresh the receipt after success and failure, preserve the last known amounts with a visible read-failure notice, and provide a read-only retry. Use shared tokens, compact wrapping typography, PL/EN parity and a polite status region without moving focus; receipt data never enters PDF content.

- Interviews enrich existing CVs. Require a saved CV, a successful nondeleted import, or the active editor CV with a nonblank candidate name; empty templates, notes and account facts alone never unlock the workflow. Do not offer blank-candidate or unbound-profile starts. The standalone source list offers saved CVs, successful imports and an inline PDF upload; uploading only adds a selectable import row and never selects a source or starts a conversation. Every new source choice uses isolated session evidence. The server supplies eligible owned source choices and enforces the same rule before session creation, source refresh and profile writes. At `/app/interview`, show the shared source-required surface with import/manual-creation actions instead of an empty fact editor. The assistant offers return to its current CV. Loading/error/retry states must not claim no sources until reads succeed. Preserve saved histories and personal-data deletion; historical sessions without source data require a new source-bound interview. Opening these surfaces never starts AI. Keep native labelled selection, 44px actions, keyboard access, PL/EN parity and responsive reflow; all chrome remains outside PDF output.
...
- Every newly generated preview runs drafting, separate professional style editing and independent factual verification against original evidence. Disclose all three paid stages before generation, without simulated stage progress. Readability findings are optional section-level advice in the existing review notice: they never discard a fact-checked preview, block saving or trigger another editing/verification pair. Keep them distinct from rejected factual claims. Credits remain charged for each completed paid stage, including when wording advice remains. Style editing is automatic within the preview request, not on answer save, navigation or session read. A failed editing stage preserves saved work and exposes recovery; never publish unverified output. Existing previews stay readable. Resume completed stages only within the unchanged versioned attempt; claim free recovery only if all three paid stages were reused. Application guidance and diagnostics never enter the PDF.

- User-authored interview answers are confirmed by submission and persist immediately to the selected evidence store; never ask the user to approve the same text again in **Treść CV**. Text answers and explicit lack-of-experience choices update evidence atomically with answer history, while unknown and skipped answers create no fact. Selecting or refreshing a source confirms its existing facts directly; optional manual edits retain explicit persistence. Keep optional content edits behind **Treść CV**, with a guarded return action. Unchanged navigation makes no request and preserves the preview. Keep full fact review optional after answers and clarifications. Do not start paid AI from navigation. Open field edits prevent transitions, failed saves retain the current stage/draft, and a synchronous lock prevents duplicate submissions. Historical profile data retains API compatibility without a dedicated application editor.
...
- Long operations show an operation-specific wait surface inside the same working panel as the task they belong to — loading never replaces the conversation with a separate window. Answer persistence, question requests, confirmation and synchronization keep the existing form mounted and visible, with its duplicate actions disabled and a compact status (named indeterminate indicator plus elapsed time) above it; heavier operations may temporarily hide the interactive content below that status while the panel itself stays. Do not add decorative document illustrations. Distinguish answer persistence, AI questions, generation and profile synchronization using actual request boundaries. Never simulate server stages, percentages, countdowns or success. After 30 seconds, explain the continuing wait without automatically retrying. Omit unavailable counts; announce operation changes politely, not every timer tick. Disable duplicate actions, retain mounted drafts, restore task focus on completion and stop animation for reduced motion. The surface is shared by the standalone page and embedded assistant, and stays outside PDF content.

- Discoverability uses the existing site hierarchy: public pricing and landing explain the interview and link to bookmarkable help topics for the interview, tailoring and saved conversations. Canonical Pro copy is shared with registration and plan selection. State the shared credit pool and free history access after Pro expires; never imply unlimited AI or a fixed number of interviews.
- Library and account invitations claim active Pro access only after the server AI entitlement resolves true. Keep manual starts, document navigation and conversation history reachable. Invitations are links and never invoke paid AI automatically. Help topics use native disclosures for optional explanations, meaningful headings, fragment focus after client-side navigation, and the shared responsive guide layout. In-progress interview help opens in a labelled new tab to preserve the current form.
...
### Compact assistant workspace

The assistant uses the shared dense page heading with its Pro label, without a narrative introduction. Keep the stage heading and utility links in one wrapping header. Use 24px outer panel padding, reduced to 16px in embedded, compact and short laptop contexts; fact editors retain 16px padding. Working headings use 18px, with the current question at 20px in standalone flows and the existing 16px title token in the editor. Retain 44px controls, readable input text and focus targets. Account profile spacing remains unchanged. Credit totals are one static line: an uppercase label with used and available numeric amounts always visible; unknown is a dash and pending settlement is explicit. There is no history disclosure; read failures and retry remain visible. The three-step progress list is informational; CV content is shown only on demand. Inline operation time sits beside the title. Preserve all existing drafts, permissions, billing, announcements, narrow-container navigation and PDF isolation.

### Assistant source selection

Across the ordinary flow, keep CV content access in the task header. The conversation is one reading column at every width: the compact requirement context sits directly above the question and answer form, never in a second pane beside it. At working-panel widths of at least 900px, six template samples share one row. Group the answer actions, retain the distinction between saved answers and generated suggestions, and allow long questions, factual clarification, optional assistance and expanded template galleries to grow naturally. Do not clip content to meet a viewport target.

On the standalone interview route, centre the short page title beside the breadcrumb on wide screens and stack them on compact screens. The focused SiteLayout keeps global navigation; the standalone content-improvement (`/app/interview`) and guided tailoring (`/app/tailor`) routes additionally opt back into the shared persistent footer via `SiteLayout.showFooter`, a user-requested exception to the otherwise footer-less focused layout, so both keep the privacy link and secondary navigation reachable. Use one searchable list that combines saved CVs and imports with a per-row kind badge (also exposed as the row's accessible description), an inline PDF upload beneath the list, an always-visible document-language selector and four complete source rows per page. The same shared picker serves the guided tailoring CV step, where row activation only records a selection. Preserve server ordering (documents before imports), one shared search/page state and explicit row activation before starting paid AI. Show one source-selection heading; conversation wording starts only after a session exists. Keep empty accounts, unmatched searches, loading, errors and access restrictions distinct. Fit the ordinary source picker within laptop/desktop height without body clipping or nested scrolling; retain natural page scrolling for small screens, long authored names, expanded content and 200% zoom. Template examples use bounded thumbnails. This scoped workspace exception does not change other site pages, editor chrome or PDF geometry.

### Interview task hierarchy

Keep the three-step progress informational and visually secondary: 12px numbers, 14px labels, a fine separator and a stronger current-step rule. The working heading and utility links form one row; links use compact secondary text. Credit totals remain immediately available in one warm neutral static line. Errors and retry stay visible beside it.

The source picker places search and document language on one toolbar row above the combined list. Preserve four complete rows, the shared search/page state and the explicit paid start boundary. Questions and their quieter reference context stack in one readable column in DOM order at every width. The answer field has no visible caption; its accessible name (Twoja odpowiedź / Your answer) is a deliberate exception to the visible-label rule, because the question heading directly above serves as the visual prompt. Put exactly three actions directly after the answer field, before optional answer assistance: primary Send, a Skip action (Pomiń) that is disabled while a typed draft exists so text is never discarded silently, and the tertiary Finish-and-choose-a-template action with a full 44px target. The former "other answers" disclosure (no experience / cannot remember) was removed from the ordinary question form; those explicit meanings remain available in the clarification step, and skipping still closes the current scope without inventing experience. Answer assistance remains available below those actions without a per-use credit explanation note. Task-option checkboxes synchronise their text with the answer immediately: checking inserts the task and unchecking removes only that inserted task, without a separate apply button. Preserve the explicit confirmation wording after adopting AI suggestions. No form or question content may be hidden to make a viewport fit.

Use neutral paper template choices with bounded example thumbnails, shared hover/focus/disabled states and low-emphasis gallery/back controls. Loading without a current question shows the operation description and elapsed time beside an indeterminate circular indicator; answer persistence remains compact above the mounted form. Hide the unavailable fact-review action while its save is pending. The indicator never suggests a percentage and is stationary under reduced motion. These rules apply to the shared InterviewFlow hosts without changing provider calls, entitlements, credit accounting, saved evidence or PDF output.
