...
On the standalone interview route, centre the short page title beside the breadcrumb on wide screens and stack them on compact screens. The focused SiteLayout keeps global navigation and omits the narrative footer to prioritise the task. Use separate counted My CVs / Imports tabs, local name search, an always-visible document-language selector and four complete source rows per page. Preserve server ordering, per-tab search/page state, keyboard arrow/Home/End activation and explicit row activation before starting paid AI. Show one source-selection heading; conversation wording starts only after a session exists. Keep empty tabs, unmatched searches, loading, errors and access restrictions distinct. Fit the ordinary source picker within laptop/desktop height without body clipping or nested scrolling; retain natural page scrolling for small screens, long authored names, expanded content and 200% zoom. Template examples use bounded thumbnails. This scoped workspace exception does not change other site pages, editor chrome or PDF geometry.
