...
- `/app/tailor` uses one five-step progress list: Your CV, Job advert, Questions, Review CV, Download. In this host only, hide the shared interview's four-stage navigation and profile/history shortcut to avoid duplicate task hierarchies. Existing standalone and embedded interview hosts keep their contracts.
...
- Profile and interview information review show only populated CV sections plus the always-available notes section. Offer a direct notes shortcut, retain search across all records, and omit decorative section numbering and repeated overview instructions. Keep source selection and recovery visible. Profile deletion belongs under a labelled Profile options disclosure with its existing explicit confirmation; restore focus only after the request finishes and the target is enabled. Intake shows the current CV language in a collapsed, keyboard-operable Change disclosure and preserves the choice when closed. During an open question or clarification, omit the duplicate information-review action so answering remains the task; stage guards, factual confirmation, credits and error recovery remain visible and unchanged. Apply these rules in both interview hosts and both languages, outside document/PDF content.

- Keep one primary next action in each conversation state: request the next question while the current round has capacity, save the current answer while a question is open, and prepare the CV when the round ends. Early preparation remains secondary; optional extension is inside a labelled disclosure. Navigation and answer saving never request paid AI automatically. Keep the four existing stages and their shared desktop/compact guards. Intake notes are optional and collapsed when empty; retain source-specific drafts. Routine source refresh belongs in information review, while a changed-source warning exposes recovery immediately. Preparation shows a concise, always-visible credit explanation with the complete process under a disclosure. Receipts show the latest settled charge, total and currently known balance in a compact wrapping row. Retain the latest settled charge and total during work or failed refresh; mark an unavailable balance explicitly. Show history only when it has entries. Use direct operation names and shared tokens in both hosts and languages.
