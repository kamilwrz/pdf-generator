...
Account onboarding is shown only to authenticated users and is the canonical destination of generic **Create CV** entry points, including shared navigation. Explicit manual-creation actions in the document library and interview/profile source recovery open A4 setup directly through `getEditorPath({ start: "new" })`, preserving the existing replacement guard. Guests enter A4 setup/editor directly, while contextual template actions keep the selected template and open setup without an extra choice; refresh restores a guest's browser draft. Guest import keeps its Topbar trigger but opens the shared account gate with explicit registration/login copy and a preserved import intent, without mounting the upload UI. Cancel/Escape restores focus and preserves the document.
