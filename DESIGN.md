...
- **Treść CV / CV content** is optional and opens only on activation. Preserve content/answer drafts and guarded recovery. Template cards are explicit create actions: run verified generation and fitting, save a separate document, and open the editor. Stop for factual clarification or failure; never auto-accept an AI factual proposal. Save failure exposes a save-only retry. Historical previews retain manual review and alternative-template controls; direct creation uses the user's chosen template without requiring a comparison step. The guided advert workflow keeps its separate review/download handoff.
