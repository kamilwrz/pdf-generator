...
- Populated single-line text uses its visible glyph bounds for the edit-focus outline, including when an authored full-column frame centres or right-aligns it. Keep outline padding screen-stable during zoom; empty fields retain a visible editing fallback. These measurements must never change the alignment frame, caret, saved geometry or PDF output.
