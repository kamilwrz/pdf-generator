...
The landing hero shows the three Free template examples (Sterling, Meridian and Linden) as a static A4 fan, with Linden in front. A figure caption identifies samples and explains that selection happens in onboarding. No preview buttons, radios, counters, swipe handling, motion or second compact CTA are mounted. Keep one primary creation action at every width and the browser-draft resume link when applicable. Editor demo and account-aware import are clearly labelled secondary tools; group Improve content and Tailor to a job under CV Assistant with a Pro label. Preview loading and image errors reserve A4 space and never disable creation. Use shared tokens and restrained document elevation; the sample transforms never enter authored CV/PDF output. Template detail and legacy hints still enter onboarding with selection retained; actual creation requires the explicit final editor action.
