...
- New CV setup is an opaque, edge-to-edge fullscreen workflow with three steps: template, header/contact, and ordered sections. Preserve choices between steps, show a labelled sample preview and configuration summary, use a single scrolling body with persistent actions, and focus each new step heading. Compact layouts keep the active form primary and move the summary below it. Template choices use native radios with arrow-key support; reorder actions have 44px targets. Only the short replacement confirmation remains a floating modal. New CV setup and its replacement confirmation use predominantly white paper surfaces. Keep beige to selected-template and hover accents, with brown headings, selections, and primary actions. This surface treatment must not change the backdrop.
