...
- `/app/tailor` uses one five-step progress list: Your CV, Job advert, Questions, Review CV, Download. In this host only, hide the shared interview's three-step progress and profile/history shortcut to avoid duplicate task hierarchies. Existing standalone and embedded interview hosts keep their contracts.
- Explain Free import limits and Pro pricing before personal-data intake. Require account access before upload and an advert before starting. Retain text/link drafts, send only the selected option and keep source review visible before the explicit confirmation action. The isolated interview excludes the account career profile and initially uses Linden; preserve its existing factual review and paid-operation boundaries.
