...
The assistant uses the shared dense page heading with its Pro label, without a narrative introduction. Keep the stage heading and utility links in one wrapping header. Use 24px outer panel padding, reduced to 16px in embedded, compact and short laptop contexts; fact editors retain 16px padding. Working headings use 18px, with the current question at 20px in standalone flows and the existing 16px title token in the editor. Retain 44px controls, readable input text and focus targets. Account profile spacing remains unchanged. Credit totals are a compact native disclosure: used and available numeric amounts remain visible; unknown is a dash and pending settlement is explicit. History and accounting explanations expand on request; read failures and retry remain visible. The three-step progress list is informational; CV content is shown only on demand. Inline operation time sits beside the title. Preserve all existing drafts, permissions, billing, announcements, narrow-container navigation and PDF isolation.

### Assistant source selection

Across the ordinary flow, keep CV content access in the task header. At working-panel widths of at least 900px, place question context beside the answer form and six template samples in one row. Group Send and Finish actions, retain the distinction between saved answers and generated suggestions, and allow long questions, factual clarification, optional assistance and expanded template galleries to grow naturally. Compact/embedded panels retain their stacked layout. Do not clip content to meet a viewport target.
...
Keep the three-step progress informational and visually secondary: 12px numbers, 14px labels, a fine separator and a stronger current-step rule. The working heading and utility links form one row; links use compact secondary text. Credit totals remain immediately available in one warm neutral disclosure without stacking divider lines. Errors and retry stay visible outside the disclosure.

The source picker places search and document language on one toolbar row above the combined list. Preserve four complete rows, the shared search/page state and the explicit paid start boundary. Questions use a readable answer column and a quieter reference context on wide screens, stacked in DOM order on compact screens. Put Send and Finish directly after the answer field, before optional answer assistance; keep Finish a tertiary labelled action with a full 44px target. Assistance and its credit disclosure remain available below those actions. Task-option checkboxes synchronise their text with the answer immediately: checking inserts the task and unchecking removes only that inserted task, without a separate apply button. Preserve the explicit confirmation wording after adopting AI suggestions. No form or question content may be hidden to make a viewport fit.
